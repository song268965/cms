...
Class Ask_Show_List
        Private classid,cid,topicmode,child,parentclassname,classname,parentstr,param,csetting
		...
		Sub ShowMain()
			 ...
			 FCls.RefreshFolderID = KS.ChkClng(KS.S("id"))   '设置当前刷新目录ID 为"0" 以取得通用标签
		...
		Sub GetListParam()
		   ...
		   If Not Catelist Is Nothing Then
			...
			if Node.SelectSingleNode("@parentid").text<>0 then
			 dim pnode:set pnode=Catelist.documentElement.selectSingleNode("row[@classid="&Node.SelectSingleNode("@parentid").text&"]")
			 parentclassname=pNode.SelectSingleNode("@classname").text
			end if
            csetting=split(Node.selectSingleNode("@c_setting").text&"№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№","№§№")
		...
		Sub EchoClassItem(sTokenName)
		     ...
		     Select Case lcase(sTokenName)
				...
				case "seokeyword" Echo csetting(0)
				case "seodescription" Echo csetting(1)
				case "parentclassname" Echo parentclassname
				case "classid" Echo classid
				case "questionnum" echo conn.execute("select count(1) from ks_asktopic " & param)(0)
				case "childclasslist"
