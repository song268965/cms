...
%>
<h3>以下是和您的问题类似并已得到解决的问题，希望能帮到您！</h3>
<ul>
<%
...
searchmode = 0
Keyword = Request("q")
If Len(Keyword) < 2 Then Keyword = request("word")
If Len(Keyword) < 2 Then Keyword = ""
If Keyword = "请输入关键字" Then Keyword = ""
KeyWord=KS.DelSQL(UnEscape(keyword))
showmain()
CloseConn()
Set KS=Nothing
...
Sub showmain()
			...
			If Len(KeywordArray(i)) > 1 Then
				If n = 0 Then
					If DataBaseType=1 Then
						FoundSQL = "title like " & KS.WithKorean() &"'%"&KeywordArray(i)&"%'"
					Else
						FoundSQL = "InStr(1,LCase(Title),LCase('"&KeywordArray(i)&"'),0)>0"
					End If
				Else
					If DataBaseType=1 Then
						FoundSQL = FoundSQL & " Or title like " & KS.WithKorean() &"'%"&KeywordArray(i)&"%'"
					Else
	...
	Else
		If ws.CheckKeyword(Keyword) Then
			If DataBaseType=1 Then
				FoundSQL = "title like " & KS.WithKorean() &"'%"&Keyword&"%'"
			Else
