...
Class Ask_A
		...
		Private AskTopic,classid,PclassName,classname,Quserid,PostUsername
		...
		Sub Kesion()
			...
			Sub saveanswer()
				Dim Rs,SQL,TextContent,UserNowPoint,TextLength,LockTopic
				If allowAnswers = 0 Then
					KS.Die "<script>alert('友情提示!\n\n本站暂时禁止回答问题!');</script>"
				End If
				If KS.ASetting(8)="1" And lcase(Trim(Request.Form("Verifycode")))<>lcase(Trim(Session("Verifycode"))) Then
			   	    KS.Die "<script>alert('友情提示!\n\n您输入的验证码不正确,请重输!');</script>"
			    End If
				...
				If Len(TextContent) < 2 Then
					KS.Die "<script>alert('友情提示!\n\n请填写正确的答案内容!');</script>"
				End If
				TextContent = KS.FilterIllegalChar(KS.ClearBadChr(TextContent))
				...
				If KS.ChkClng(KS.ASetting(18))=2 or KS.ChkClng(KS.ASetting(18))=3 Then
				 LockTopic=1
				Else
				 LockTopic=0
				End If
				...
				Set Rs = Server.CreateObject("ADODB.Recordset")
				SQL = "SELECT top 1 * FROM " & TopicUseTable & " WHERE (postsid is null)"
				Rs.Open SQL,Conn,1,3
				Rs.Addnew
					Rs("classid") = classid
					Rs("TopicID") = TopicID
				...
				Rs.Update
				Rs.MoveLast
				Dim postsid:postsid=Rs("postsid")
				Rs.Close
				
				'判断是专家的话给他增加问题回答数
				RS.Open "select top 1 * From KS_AskZJ Where UserName='" & KSUser.UserName &"'",conn,1,3
				If Not RS.Eof Then
				   RS("AskDoneNum")=KS.ChkClng(RS("AskDoneNum"))+1
				   RS.Update
				   Conn.Execute("Update KS_AskPosts1 Set IsZJ=1 Where postsid=" & KS.ChkClng(postsid))
				End If
				RS.Close
				Set Rs = Nothing
			...
			Sub LoadTopicInfo(iMode)
				Dim SQL,Rs
				SQL = "SELECT TopicID,classid,classname,pclassname,title,Username,Expired,Closed,PostTable,DateAndTime,Reward,PostNum,CommentNum,TopicMode,supplement FROM KS_AskTopic WHERE TopicID="&TopicID&" And TopicMode="&iMode&" And LockTopic=0"
