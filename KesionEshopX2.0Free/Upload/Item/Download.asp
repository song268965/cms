...
Class DownLoad
		...
		Private FileContent,RSObj,SqlStr,ShowInfoStr,InfoPurview,ReadPoint,ChargeType,PitchTime,ReadTimes,Title
		...
		Sub downloadFile(strFile,FileOldName)
			...
			response.AddHeader "Content-Disposition","attachment;filename=" & replace(DownFileName," ","")
		...
		Public Sub Kesion()
			...
			End If
			 ...
			 If RSObj("Verific")<>1 And KS.C("UserName")<>RSObj("Inputer") And KS.C("AdminName")="" then
			      TitleStr="下载错误提示"
				  ShowInfoStr = ShowInfoStr & "<li>" & KS.C_S(ChannelID,3) & "未审核，不能下载！</li>"
				  FoundErr=True:Call ShowInfo :Exit Sub
			 end if
		    ...
		    If InfoPurview=2 or ReadPoint>0 Then
			   IF UserLoginTF=false Then
				 Call GetNoLoginInfo
			   Else
					 IF Not KS.IsNul(RSObj("ArrGroupID")) and KS.FoundInArr(RSObj("ArrGroupID"),KSUser.GroupID,",")=false Then
		 ...
		 ElseIF InfoPurview=0 And (ClassPurview=1 Or ClassPurview=2) Then 
			  ...
			  Else
				 ...
				 If ClassPurview=2 Then
					 IF Not KS.IsNul(RSObj("DefaultArrGroupID")) and KS.FoundInArr(RSObj("DefaultArrGroupID"),KSUser.GroupID,",")=false Then
		 ...
		 End If 
		   ...
		   If DownUrlTF=true Then
			   ...
			   Else
			        dim PitchTime_j,Score,DownloadPath,unionId
					Set Rs = Server.CreateObject("ADODB.Recordset")
					SQL = "SELECT top 1 * FROM KS_DownSer WHERE downid="& KS.ChkClng(KS.S("Sid"))
					...
					If Not(Rs.BOF And Rs.EOF) Then
						...
						Score=KS.ChkClng(RS("Down_s"))
						PitchTime_j=KS.ChkClng(RS("Down_day"))
						DownloadPath=RS("DownloadPath")
						unionId=RS("unionId")
					
						if UserLoginTF=true  and score>0 then
						 call CheckAddScore(PitchTime,Score,rs("IsOuter"))
						end if
					  Dim Url:url=DownArr(2)
					  if left(lcase(url),4)<>"http" then url=DownloadPath & URL
					  Select Case rs("IsOuter")
					   Case 0
						   Response.Redirect(URL)
					   Case 2
						 Call ThunderDownloadUrl(ThunderEncode(URL),unionId)
					   Case 3
						 Call FlashGetDownloadUrl(URL,unionId)
					  End Select
	   ...
	   End Sub
	    ...
	    '检查增加积分
	   Sub CheckAddScore(PitchTime,Score,flag)
	    Dim NewID:NewID=flag&""&id
	    Dim SqlStr:SqlStr="Select top 1 Times From KS_LogScore Where ChannelID=" & ChannelID & " And InfoID=" & NewID & " And  InOrOutFlag=1 and UserName='" & KSUser.UserName & "' And datediff(" & DataPart_H &",AddDate," & SqlNowString & ")<" & PitchTime
		...
		IF RS.Eof And RS.Bof Then
			...
			Elseif KS.ChkClng(KS.G("flag"))=2 then 
			 ...
			 d_str="下载"
			End If
			Call KS.ScoreInOrOut(KSUser.UserName,1,Score,"系统",d_str&"["+Title+"]赠送积分",ChannelID,NewID)
		Else
		       RS.Movelast
			   RS(0)=RS(0)+1
			   RS.Update
					...
					If PayTF="yes" Then
						...
						Select Case ModelChargeType
						   ...
						   case 2
							...
							If Cbool(KS.ScoreInOrOut(KSUser.UserName,2,KS.ChkClng(ReadPoint),"系统",Descript,ChannelID,ID))=True Then
								 '支付投稿者提成
...
Function FlashGetDownloadUrl(url,unionid)
	...
	Response.Write "<script>window.setInterval(""window.opener= null;window.open('','_self');window.close()"",1000);</script>" & vbCrLf
End Function
