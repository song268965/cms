...
Class SearchCls
        ...
        Private KS,ChannelID,ModelTable,Param,XML,Node,StartTime,leavetime,I,FieldMenu,QueryParam,SearchType
		Private CurrPage,MaxPerPage,TotalPut,PageNum,Key,KeyWordArr,SqlStr,OrderStr,currclassid,ClassXML,autoid
		...
		Session("SearchTime")=Now()
		  ...
		  If KS.ChkClng(KS.M_C(ChannelID,29))<>1 Then
					Call KS.AlertDoFun("对不起，该频道没有启用搜索功能!","history.back()")
					KS.Die ""
		  End If
				
				
		  SearchType=KS.ChkClng(Request("t"))   '搜索类型
		  if SearchType=0 then SearchType=KS.ChkClng(GetParam("t"))
	   ...
	   Sub ParseArea(sTokenName, sTemplate)
			...
			Select Case lcase(sTokenType)
				...
				case "search" 
				          select case sTokenName
							...
							case "queryparam" 
							 dim newqp,ii,qp:qp=split(Replace(GetFieldLink("","","key,p,t"),".html",""),",")
							 for ii=0 to ubound(qp)
							   if left(lcase(qp(ii)),3)<>"ks_" then
							     if newqp="" then
								   newqp=qp(ii)
								 else
								   newqp=newqp & "," & qp(ii)
		  ...
		  Select Case sTokenName
		   case "autoid" echo autoid
			...
			case "linkurl"    
			 ...
			 Else 
			  echo KS.GetItemURL(ChannelID,GetNodeText("tid"),GetNodeText("id"),GetNodeText("fname"),GetNodeText("adddate"))
			 End If
			...
			case "price","price_member"
			   if ChannelID=5 then 
			    echo KS.GetPrice(GetNodeText(sTokenName))
			   else
			...
			case else
			  ...
			  Dim TArr,FieldName,FieldParamArr,Length,FieldType,FieldValue
			   If Instr(sTokenName,"(")<>0 Then
				   Tarr=Split(sTokenName,"(")
				   FieldName=lcase(Tarr(0))
				   FieldValue=GetNodeText(FieldName)
				   If Not KS.IsNul(FieldValue) Then 
		...
		Sub InitialSearch()
		  ...
		  If Key="" Then Key=KS.S("Key")
		  Key=KS.CheckXSS(KS.R(KEY))
		  ...
		  For Each OrderNode in FieldNode
			...
			Else 
			 ssel=""
			End If
			If Not KS.IsNUL(OrderNode.SelectSingleNode("uptitle").text) Then
			...
			ssel=""
			End If
			If Not KS.IsNUL(OrderNode.SelectSingleNode("downtitle").text) Then
			  ...
			  Else 
				  ...
				  For Each Node In Application(KS.SiteSN&"_class").DocumentElement.SelectNodes("class[@ks12=" & ChannelID &" and @ks10=1 and @ks25=1 and @ks14=1]")
					 If KS.ChkClng(Node.SelectSingleNode("@ks9").text)=currk Then
					   ...
					   FieldMenu=FieldMenu & "<strong>" & Node.SelectSingleNode("@ks1").text & "(<span class=""num"">" &conn.execute("select count(1) from " & KS.C_S(ChannelID,2)& " where deltf=0 and verific=1 and tid in(select id from ks_class where ts like '%" & Node.SelectSingleNode("@ks0").text &",%')")(0) &"</span>)</strong>"
					 Else
					   FieldMenu=FieldMenu & "<a href=""" & GetFieldLink("tid",Node.SelectSingleNode("@ks9").text,"") & """ title=""" &Node.SelectSingleNode("@ks1").text&""">" & Node.SelectSingleNode("@ks1").text & "(<span class=""num"">" &conn.execute("select count(1) from " & KS.C_S(ChannelID,2)& " where deltf=0 and verific=1 and tid in(select id from ks_class where ts like '%" & Node.SelectSingleNode("@ks0").text &",%')")(0) &"</span>)</a>"
					 End If
					 k=K+1
				  Next
				   FieldMenu=FieldMenu & "</div>"
			...
			Case Else
				...
				End If
					...
					For K=0 To Ubound(OptionArr)
					 ...
					 Else
					   if CurrNode.SelectSingleNode("fieldtype").text="11" then
					...
					Next
			    FieldMenu=FieldMenu & "</div>"
				if CurrNode.SelectSingleNode("fieldtype").text="11" and KS.ChkClng(GetParam("v"&i))>0 then
					Call GetSubldMenu(CurrNode.SelectSingleNode("@name").text,OptionArr(KS.ChkClng(GetParam("v"&i))-1))
				end if
				FieldMenu=FieldMenu &"<div class=""clear""></div>"
			End Select
		  Next
			 ...
			 ' SET FieldNode=FieldXML.DocumentElement.SelectNodes("item[@enabled='true']")
			  For K=0 To FieldNode.length-1
				  ...
				  ElseIf CurrNode.SelectSingleNode("showvalue").text="0" Then   '自定义字段读数据库的值
				    Set ONode=Application(KS.SiteSN&"_field").documentElement.selectsinglenode("fielditem[@fieldname='" & FieldName &"']")
					If Not ONode Is Nothing Then
						 OptionArr=Split(ONode.SelectSingleNode("options").text,"\n")
						  if qv-1<=Ubound(OptionArr) and qv-1>=0 Then
						   v=OptionArr(qv-1)
						   if instr(v,"|")<>0 then v=split(v,"|")(0)
							if CurrNode.SelectSingleNode("fieldtype").text="7" then
							 Param=Param & " and " & FieldName & " like " & KS.WithKorean() &"'%" & v &"%'"
							else
							 Param=Param & " and " & FieldName & "=" & KS.WithKorean() &"'" & v &"'"
							end if
						  End If
						  
						  if CurrNode.SelectSingleNode("fieldtype").text="11" and KS.ChkClng(GetParam("v"&k))>0 then
						    Call GetSubldParam(CurrNode.SelectSingleNode("@name").text,OptionArr(KS.ChkClng(GetParam("v"&k))-1))
						  end if
				 ...
				 ElseIf searchvalue<>"" and searchvalue<>"0" Then   '按设置的选项值搜索
					...
					if v<>"" then
						select case lcase(CurrNode.SelectSingleNode("condition").text)
							case "dy" Param=Param & " and " & FieldName & "=" & KS.WithKorean() &"'" & v &"'"
							case "dys" Param=Param & " and " & FieldName & "=" & v &""
							case "like" Param=Param & " and " & FieldName & " like " & KS.WithKorean() &"'%" & v &"%'"
							case "fw" 
							   FWValue=split(v&"-","-")
			  ...
			  case 1
			       If (FuzzySearch=1) Then
					   For II=0 To Ubound(KeyWordArr)
						   If II=0 Then
						   Param=Param & " And (Title Like " & KS.WithKorean() &"'%" & KeyWordArr(II) & "%'"
						   Else
						   Param = Param & " or Title Like " & KS.WithKorean() &"'%" & KeyWordArr(II) & "%'"
						   End If
					  Next
				   Else
				    Param=Param & " And (Title Like " & KS.WithKorean() &"'%" & Key & "%'"
				   End If
				   Param=Param & ")"
			   Case 7
			    if KS.ChkClng(KS.C_S(ChannelID,6))=1 then Param=Param & " And intro Like " & KS.WithKorean() &"'%" & Key & "%'"
			   case 8 '按日期搜索
			     if (isnumeric(key)) then
				   Param=Param & " and year(adddate)=" & key
				 elseif instr(key,"~")<>0 and instr(key,"-")<>0 then
				   dim datearr:datearr=split(key,"~")
				   dim begindate:begindate=""
				   dim enddate:enddate=""
				   if isdate(datearr(0)) and isdate(datearr(1)) then
				      begindate=datearr(0)
					  enddate=datearr(1)
					  if Ubound(split(datearr(0),"-"))<2 then begindate=begindate&"-1"
					  if Ubound(split(datearr(1),"-"))<2 then 
					   if isdate(enddate&"-31") then 
					   enddate=enddate&"-31"
					   elseif isdate(enddate&"-30") then 
					     enddate=enddate&"-30"
					   elseif isdate(enddate&"-29") then 
					     enddate=enddate&"-29"
					   elseif isdate(enddate&"-28") then 
					     enddate=enddate&"-28"
					   end if
					  end if
				      if DataBaseType=1 Then
					    Param=Param & " and (adddate between '" & begindate &"' and '" & enddate &"')"
					  Else
					    Param=Param & " and (adddate between #" & datearr(0) &"# and #" & datearr(1) &"#)"
					  End If
				   end if
				 elseif isdate(key) then
				    datearr=split(key,"-")
					if ubound(datearr)=2 then
				    Param=Param & " and year(adddate)=" & datearr(0) &" and month(adddate)=" & datearr(1) &" and day(adddate)=" & datearr(2) 
					else
					Param=Param & " and year(adddate)=" & datearr(0) &" and month(adddate)=" & datearr(1)
					end if
				 end if
			   Case 2
			    Select Case KS.ChkClng(KS.C_S(ChannelID,6))
				 Case 1:Param=Param & " And ArticleContent Like " & KS.WithKorean() &"'%" & Key & "%'"
				 Case 2:Param=Param & " And PictureContent Like " & KS.WithKorean() &"'%" & Key & "%'"
				 Case 3:Param=Param & " And DownContent Like " & KS.WithKorean() &"'%" & Key & "%'"
				 Case 4:Param=Param & " And FlashContent Like " & KS.WithKorean() &"'%" & Key & "%'"
				 Case 5:Param=Param & " And ProIntro Like " & KS.WithKorean() &"'%" & Key & "%'"
				 Case 7:Param=Param & " And MovieContent Like " & KS.WithKorean() &"'%" & Key & "%'"
				 Case 8:Param=Param & " And GQContent Like " & KS.WithKorean() &"'%" & Key & "%'"
				End Select
			   Case 3
			    If KS.ChkClng(KS.C_S(ChannelID,6))<=5 Then
			    Param=Param & " And Author Like " & KS.WithKorean() &"'%" & Key & "%'"
				ElseIf KS.ChkClng(KS.C_S(ChannelID,6))=7 Then
			    Param=Param & " And MovieAct Like " & KS.WithKorean() &"'%" & Key & "%'"
				End If
			   Case 4:Param=Param & " And Inputer Like " & KS.WithKorean() &"'%" & Key & "%'"
			   Case 5:Param=Param & " And KeyWords Like " & KS.WithKorean() &"'%" & Key & "%'"
			   Case 6
			     If KS.ChkClng(KS.C_S(ChannelID,6))=5 Then
			       Param=Param & " And ProID Like " & KS.WithKorean() &"'%" & Key & "%'"
				 ElseIf KS.ChkClng(KS.C_S(ChannelID,6))=7 Then
			       Param=Param & " And MovieDY Like " & KS.WithKorean() &"'%" & Key & "%'"
			     End If
			  case else
				if (FuzzySearch=1) Then
				  For I=0 To Ubound(KeyWordArr)
				   If I=0 Then
				   Param=Param & " And (Title Like " & KS.WithKorean() &"'%" & KeyWordArr(i) & "%'"
				   Else
				   Param = Param & " or Title Like " & KS.WithKorean() &"'%" & KeyWordArr(i) & "%'"
				   End If
				  Next
				Else
				 Param=Param & " And (Title Like " & KS.WithKorean() &"'%" & Key & "%'"
		 ...
		 if Not KS.IsNUL(TopParam) Then
			...
			End If
		 End If
		 If KS.ChkClng(KS.C_S(ChannelID,6))=1 or  KS.ChkClng(KS.C_S(ChannelID,6))=8 Then
			 If Not KS.IsNul(Province) Then
				 Param=Param & " And Province=" & KS.WithKorean() &"'" & KS.DelSQL(Province) &"'"
			 End If
			 If Not KS.IsNul(City) Then
				 Param=Param & " And City=" & KS.WithKorean() &"'" & KS.DelSQL(City) &"'"
			 End If
		 ...
		 If Channelid=1 Then  '定义要增加搜索条件的模型
		 
		   ' Dim KS_test:KS_test=GetValue("KS_test")       '自定义字段一
			'If Not KS.IsNUL(KS_test) Then  Param= Param & " and KS_test='" & KS_test & "'"
			
		'	Dim KS_radio:KS_radio=GetValue("KS_radio")    '自定义字段二
		'	If Not KS.IsNUL(KS_radio) Then  Param= Param & " and KS_radio='" & KS_radio & "'"
		 ...
		 '==================自定义字段搜索条件结束================================
		   ...
		   Select Case KS.C_S(ChannelID,6)           rem 查询的字段,方便调用这里把一张表的都查询出来了,你可以只列出要查询的字段
			...
			Case 9 FieldStr="*,sj As Intro,date as AddDate"
		   End Select
			  ...
			  if Nodes.length>0 Then
				  ...
				  For Each Node In Application(KS.SiteSN&"_class").DocumentElement.SelectNodes("class[@ks12=" & ChannelID &" and @ks13='" & parentid & "' and @ks25=1 and @ks14=1]")
					 ...
					 If KS.ChkClng(currclassid)=KS.ChkClng(Node.SelectSingleNode("@ks9").text) Then
					    ...
					    FieldMenu=FieldMenu & "<strong>" & Node.SelectSingleNode("@ks1").text & "(<span class=""num"">" &conn.execute("select count(1) from " & KS.C_S(ChannelID,2)& " where deltf=0 and verific=1 and tid in(select id from ks_class where ts like '%" & Node.SelectSingleNode("@ks0").text &",%')")(0) &"</span>)</strong>"
					 Else
					   FieldMenu=FieldMenu & "<a href=""" & GetFieldLink("tid",param,"") & """ title=""" &Node.SelectSingleNode("@ks1").text&""">" & Node.SelectSingleNode("@ks1").text & "(<span class=""num"">" &conn.execute("select count(1) from " & KS.C_S(ChannelID,2)& " where deltf=0 and verific=1 and tid in(select id from ks_class where ts like '%" & Node.SelectSingleNode("@ks0").text &",%')")(0) &"</span>)</a>"
					 End If
				Next
				FieldMenu=FieldMenu & "</div>"
	  ...
	  Sub GetSubSJTidMenu(parentid)
		...
		'联动条件
		sub GetSubldParam(parentfieldname,parentld)
		       Dim Options,currfieldname,currfieldvalue
	           Dim ONode:Set ONode=Application(KS.SiteSN&"_field").documentElement.selectsinglenode("fielditem[parentfieldname='" & parentfieldname &"']")
				If Not ONode Is Nothing Then
				  	Options=ONode.selectsinglenode("options").text
					currfieldname=ONode.selectsinglenode("@fieldname").text
				else
					exit sub
				end if
				currareaid=KS.ChkClng(GetParam("v"&parentfieldname))
				If Not KS.IsNul(parentld) and Not KS.IsNul(Options) Then	
              Dim Node,currareaid,k,nodes,tj,ts,nn,pparam,OptionArr,ptionArr_x,i
			   OptionArr=Split(Options,"\n")
			   if Ubound(OptionArr)>0 then
				   for i=0 to Ubound(OptionArr)
						if  trim(parentld) = trim(Split(OptionArr(i),"|")(0)) then
							If KS.ChkClng(currareaid)=KS.ChkClng(nn+1) Then
							    currfieldvalue=Split(OptionArr(i),"|")(1)
								param=param & " and " & currfieldname & "='" & currfieldvalue & "'"
							End If
							nn=nn+1
						else
							nn=0	
				   ...
				   next
				   if (not ks.isnul(currfieldvalue)) then
				     call GetSubldParam(currfieldname,currfieldvalue)
				   end if
			   end if
			End If
	  ...
	  '联动
	  Sub GetSubldMenu(parentfieldname,parentld)
	          Dim Options,currfieldname,currfieldvalue
	          Dim ONode:Set ONode=Application(KS.SiteSN&"_field").documentElement.selectsinglenode("fielditem[parentfieldname='" & parentfieldname &"']")
				If Not ONode Is Nothing Then
				  	Options=ONode.selectsinglenode("options").text
             ...
             Dim Node,currareaid,k,nodes,tj,ts,nn,pparam,OptionArr,ptionArr_x,i
			   ...
			   if Ubound(OptionArr)>0 then
						...
						if  trim(parentld) = trim(Split(OptionArr(i),"|")(0)) then
						    if nn=0 then
							  If ks.chkclng(currareaid)=0 Then
								 FieldMenu=FieldMenu & "<strong>不限</strong>"
							  Else
								 FieldMenu=FieldMenu & "<a href=""" & GetFieldLink("v"&parentfieldname,"0","") & """>不限</a>"
							  End If
							end if
							If KS.ChkClng(currareaid)=KS.ChkClng(nn+1) Then
							    currfieldvalue=Split(OptionArr(i),"|")(1)
								FieldMenu=FieldMenu & "<strong>" & currfieldvalue & "</strong>"
								
							Else
							 	FieldMenu=FieldMenu & "<a href=""" & GetFieldLink("v"&parentfieldname,nn+1,"") & """ title=""" &Split(OptionArr(i),"|")(1)&""">" & Split(OptionArr(i),"|")(1) & "</a>"
							End If
							nn=nn+1
						else
							nn=0	
				   ...
				   if (not ks.isnul(currfieldvalue)) then
				     call GetSubldMenu(currfieldname,currfieldvalue)
				   end if
			   end if
			End If		 	
	  End Sub
	   ...
	   End Sub
		...
		Function GetParam(key)
		  ...
		  Dim PArr:Parr=Split(Replace(GetQuery,".html",""),",")
		  Dim Pkey,i,j
		  Dim Pval:Pval=""
		  For I=0 To Ubound(Parr)
		     if instr(Parr(i),"-")<>0 Then
		       ...
		       if lcase(key)=lcase(Pkey) Then
			     dim pparr:pparr=split(Parr(i),"-")
			     for j=1 to ubound(pparr)
				  if pval="" then
				   Pval=pparr(j)
				  else
			       Pval=Pval&"-" & pparr(j)
		 ...
		 If Not KS.IsNul(QP) Then
		   ...
		   For i=0 To Ubound(QPArr)
		     If instr(QParr(i),"-")<>0 Then
			   ...
			   Pval=""
			     dim j,pparr1:pparr1=split(QParr(i),"-")
			     for j=1 to ubound(pparr1)
				  if pval="" then
				   Pval=pparr1(j)
				  else
			       Pval=Pval&"-" & pparr1(j)
