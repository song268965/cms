...
Class SiteIndex
		...
		Public Sub Kesion()
		 ...
		 Dim ClassID:ClassID=KS.ChkClng(KS.S("ID"))
		 CurrPage=KS.ChkClng(KS.S("Page"))
		 If CurrPage<=0 Then CurrPage=CurrPage+1
		 IF ClassID=0 Then Exit Sub
		 Set RSObj=Server.CreateObject("ADODB.RECORDSET")
		  RSObj.Open "Select top 1 * From KS_SpecialClass Where ClassID=" & ClassID,Conn,1,1
		  ...
		  if Instr(FileContent,"[KS:PageStyle]")<>0 Then
			FileContent=Replace(FileContent,"[KS:PageStyle]",KS.ReplacePage(FCls.PageStyle,-1,classid,CurrPage,FCls.TotalPut,FCls.PerPageNum))
			End If
