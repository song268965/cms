...
Class User_Friend
       ...
       Public Sub loadMain()
		...
		<div class="tabs">	
			<ul>
				<li<%If action="" then KS.Echo " class='puton'"%>>
				<span class="rl bl" onMouseover="$('#f').show();"><a href="?">我的好友 <img src="images/menu_icon.gif" align="absmiddle"/></a>
				 <div id="f" class='abs' onMouseOut="$('#f').hide();">
				 <dl><a href='?listtype=1'> 好 朋 友 </a>&nbsp;&nbsp;</dl>
				 <dl><a href='?action=friendrequest'> 陌 生 人 </a>&nbsp;&nbsp;</dl>
				 <dl><a href='?listtype=3'> 黑 名 单 </a>&nbsp;&nbsp;</dl>
				 </div>
				</span>
				</li>
				<li<%If action="addF" then KS.Echo " class='puton'"%>><a href="?action=addF">寻找好友</a></li>
				<li<%If action="online" then KS.Echo " class='puton'"%>><a href="user_friend.asp?action=online">在线用户</a></li>
				<li<%if action="add" then KS.Echo " class='puton'"%>><a href="?Action=add">添加好友</a></li>
				<li<%if action="mail" then KS.Echo " class='puton'"%>><a href="?Action=mail">邮件邀请好友</a></li>
			</ul>
	 </div>
	  ...
	  End Sub
		...
		<form action="?action=addsave" method="post" name="myform">
			<tr height="23" class="title">
				<td>添加好友<span style='font-weight:normal'>（通过对方验证才能成为好友）</span></td>
			</tr>
			<tr> 
			  <td>
			    ...
			    <table border="0" cellpadding="0" cellspacing="1" width="100%">
				 <tr class='tdbg'>
				  <td width="344"><b>您的姓名</b><br>
			       显示在对方邮箱的发件人中。</td>
				  <td width="607"><input type="text" class="textbox" name="realname" id="realname" value="<%=KSUser.GetUserInfo("RealName")%>" size=20 style="width:150"> <font color=red>*</font></td>
				 </tr>
				 <tr class='tdbg'>
				  <td><b>好友的邮箱</b><br></td>
		...
		<div style="width:95%; margin:0 auto"><font color=green>奖励说明：成功推荐一个好友注册,您还可以增加 <font color=red><%=KS.Setting(144)%></font> 个积分。赶快行动吧！</font></div>
		<%
		end if
		End Sub
		...
		Sub mailsave()
		 ...
		 for i=0 to ubound(email)
		   dim user_face:user_face=KSUser.GetUserInfo("UserFace")
		   ...
		   content=content & "<tr><td width=""150"" align=""center""><a style=""border:1px solid #ccc;padding:1px"" href=""" & KS.GetDomain & "space?" & KSUser.UserName & """ target=""_blank""><img width=""82"" height=""82"" src=""" & user_face & """ border=""0"" /></a><br/>" & KSUser.GetUserInfo("RealName") & "</td>"
		   content=content & "<td><p>Hi,最近又在做些什么活动,我发现一个叫做" & KS.Setting(1) & "的网站,这里有很多好玩的东西,在这里我发现了好多相识的老朋友和刚认识的新朋友,我们都有共同的兴趣爱好,你也一起加入吧!</p><p>点击这个链接，把我加为好友吧！<br /><a href=""" & KS.GetDomain & "user/reg/?uid=" & KSUser.GetUserInfo("UserID") & "&f=r&u=" & server.urlencode(email(i)) &""" target=""_blank"">" & KS.GetDomain & "user/reg/?uid=" & KSUser.GetUserInfo("UserID") & "&f=r&u=" & server.urlencode(email(i)) &"</a></p>"
		   content=content & "注明：本邮件是您的好友<a href=""" & KS.GetDomain & "space/?" & KSUser.GetUserInfo("UserID") & """ target=""_blank"">" & KSUser.GetUserInfo("RealName") & "</a>通过" & KS.Setting(1) & "发送，无需回复。"
		...
		%>
		<style type="text/css">
		 .friendlist{ color:#555;font-size:12px;}
		 .friendlist .t a{font-size:14px;color:#ff6600;height:30px;line-height:30px}
		 .friendlist li{float:left;width:48%; margin-right:2%;}
		 .friendlist li div{height:22px;line-height:22px}
		 .friendlist .l{height:100px;width:80px;float:left;margin-right:15px; margin-top:10px; text-align:left;}
		 .friendlist .l a{display:block}
		 .friendlist .l a img{}
		 .rriendlist .r{float:left}
		 .friendlist .r .zl span{color:#999999}
		</style>
		<form action="User_Friend.asp?action=move" method="post" name="inbox" class="border" style="color:#555;">
		...
		<%  dim param,i
			...
			sql="select userid as id,userid,RealName,Username,userface,sex,birthday,province,city,isonline from KS_user where isonline=1 order by userid desc"
			else
			sql="select f.id,u.userid,U.RealName,U.Username,u.userface,u.sex,u.birthday,u.province,u.city,u.isonline,f.flag,f.ShieldDT from KS_Friend F inner join KS_User U on F.Friend=U.UserName where F.accepted=1 and F.Username='"&KSUser.UserName&"' " & param & " order by F.addtime desc"
			end if
			...
			%>
					<li>
						<div class='l'>
								...
								response.write "<a href='" & KS.GetSpaceUrl(rs("userid")) & "' target='_blank'><img src='" & user_face & "' width='60' height='60'/></a>"
								...
								<div class='r'>
								 <div class='t'>
								 <a href="<%=KS.GetSpaceUrl(rs("userid"))%>" target="_blank"><%=rs("username")%></a>(<%=rs("realname")%>)
	 ...
	 <div class="clear"></div>	
		  ...
		  <select class="select" name="grouptype" id="grouptype">
		   <option value="1">好朋友</option>
		   <option value="3">黑名单</option>
		  </select>&nbsp;
		   <button id="b1" type="submit" class="pn pnc"><strong>确定操作</strong></button>
	 ...
	 <%if action="" then%>
     <div style="color:#555; padding:0 25px;">
	<strong>提示:</strong>
...
<Select id="birth_y" name="birth_y" style="width:50px"  class="select"> 
...
<Select id="birth_m" name="birth_m" style="width:50px" class="select"> 
...
<Select id="birth_d" name="birth_d" style="width:50px" class="select"> 
  ...
  %>
            ...
            <Input id="realname" size="12" name="realname" class="textbox"> </td>
				  
				  <tr class="tdbg"> 
					<td colspan=2 height="50" valign=middle> 
					 <button id="b1" type="submit" class="pn"><strong> 找 朋 友</strong></button>
					</td>
				  </tr>
		  </form>
</table><br>
		...
		MaxPerPage=12
		Response.Write "<table width=""98%"" class='border' align=center cellpadding=2 cellspacing=1 border=0><tr class=title><td>帮您推荐<span style='font-weight:normal'>（以下是在本站最活跃会员）</span></td></tr></table>"
		Response.Write "<table width=""98%"" class='border' align=center cellpadding=2 cellspacing=1 border=0><tr class='tdbg'>"
		dim user_face,user_info,sex,i,n
		sql="select top 500 UserName,Sex,qq,Email,userface,realname,province,city,isonline,userid from KS_User where GroupID<>1 and username<>'" & KSUser.UserName & "' order by Logintimes desc,UserID"
		set rs=Server.CreateObject("adodb.recordSet")
		rs.Open sql,Conn,1,1
		i=0:n=0:TotalPut=0
		if not (rs.bof and rs.eof) then
			...
			user_face=rs("userface")
								If user_face="" or isnull(user_face) then 
								if rs("sex")="男" then  user_face="images/face/boy.jpg" else user_face="images/face/girl.jpg"
							    End If
			If lcase(left(user_face,4))<>"http" then user_face=KS.Setting(2) & "/" & user_face
			
				Response.Write "<td class=""splittd"" style=""padding:15px 5px"" height=20 align=""center"" width=""14%"">"
				response.write " <table border='0' width='100%' cellspacing='0' cellpading='0'>"
				response.write " <tr><td width='65' align='center'>"
				Response.Write "<div class=""mem_left_photo""><img  onerror=""this.src='../images/face/boy.jpg'"" width=""62"" height=""58"" src=""" & user_face&"""></div></td>"
				response.write "<td style='text-align:left'>用户:<a target='_blank' href='"&KS.GetSpaceUrl(rs("userid"))&"' title="""& user_info &""">"&rs(0)&"</a>"
