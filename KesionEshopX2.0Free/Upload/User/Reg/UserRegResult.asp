...
Class User_RegPost
		...
		Public Sub Kesion()
		 ...
		 Dim NewRegUserPoint:NewRegUserPoint=KS.Setting(40) : If Not IsNumeric(NewRegUserPoint) Then NewRegUserPoint=0
		  ...
		  If Instr(Lcase(Request.ServerVariables("HTTP_REFERER")),"user/reg/")=0 Then Call KS.Alert("请不要非法提交!","../") : Response.End
		 ...
		 Dim UserName:UserName=Trim(KS.R(KS.S("UserName")))
        ...
        End If
		 ...
		 End If
		 If KS.Setting(129)="1" and KS.Setting(149)="1" Then
		 ...
		 if IsArray(SQL) Then
		  For K=0 To UBound(SQL,2)
			  If SQL(0,K)="Province&City" Then
			           regDetailStr=regDetailStr & "<tr><td width=""120""><strong>" & SQL(2,K) & "：</strong></td><td>" &KS.S("Province")&KS.S("City")&KS.S("County") & "</td></tr>"
			  Else
			           regDetailStr=regDetailStr & "<tr><td width=""120""><strong>" & SQL(2,K) & "：</strong></td><td>" & KS.S(SQL(0,K)) & "</td></tr>"
			  End If
		  Next
		 End If
		...
		RSG.Close:Set RSG=Nothing
		 ...
		 Conn.Execute("Update KS_User Set ChargeType=" & RS("ChargeType") & ",UserType=" & UserType &" Where UserID=" & UserID)
		 end if
		...
		Else
		    ReturnInfo="<li>注册成功!您的用户名:<font color=red>" & UserName & "</font>,您已成为了本站的正式会员!<br></li>"
			...
			If AllianceUser<>"" and AllianceUser<>UserName  Then
			 If Not Conn.Execute("Select Top 1 UserID From KS_User Where UserName='" & AllianceUser & "'").eof Then
			   ...
			   Conn.Execute("Insert InTo KS_PromotedPlan(UserName,UserIP,AddDate,ComeUrl,Score,AllianceUser) values('" & AllianceUser & "','" & KS.GetIP & "'," & SqlNowString & ",'" & CheckInSQL(KS.URLDecode(Request.ServerVariables("HTTP_REFERER"))) & "'," & KS.ChkClng(KS.Setting(144)) & ",'" & UserName & "')")
			   End If
		...
		RS.Close
			...
			If Not KS.IsNul(SmsContent) And Not KS.IsNul(Mobile) Then
			   ...
			   SmsContent=Replace(SmsContent,"{$password}",NoMD5_Pass)
			...
			'==================注册成功发邮件通知======================
			 If KS.ChkClng(KS.Setting(146))<>0 and Not KS.IsNul(KS.Setting(147)) Then
				MailBodyStr = Replace(KS.Setting(147), "{$UserName}", UserName)
				MailBodyStr = Replace(MailBodyStr, "{$PassWord}", NoMD5_Pass)
				MailBodyStr = Replace(MailBodyStr, "{$SiteName}", KS.Setting(0))
				MailBodyStr = Replace(MailBodyStr, "{$Mobile}", Mobile)
				MailBodyStr = Replace(MailBodyStr, "{$Email}", Email)
				...
				'发给会员
				if KS.ChkClng(KS.Setting(146))<>2 then
				...
				ReturnInfo=KS.SendMail(KS.Setting(12), KS.Setting(13), KS.Setting(14), "会员[" & username &"]注册成功", KS.Setting(11),UserName, MailBodyStr,KS.Setting(11))
				end if
			...
			if KS.SSetting(0)=1 And KS.SSetting(1)=1 then
			 ...
			 RS.AddNew
			  ...
			  End If
			  RS("TemplateID")=KS.ChkClng(KS.U_S(GroupID,29))
			  RS("Announce")="暂无公告!"
			  RS("ContentLen")=500
			  RS("Recommend")=0
    ...
    End If	 
		...
		Sub ShowRegResult(ReturnInfo)
		   ReturnInfo="<table border='0' align='center' width='100%' cellspacing='1' cellpadding='3'><tr><td>" & ReturnInfo & "<li class=""go""><span id=""countdown""> <font color='#ff6600'><strong>10</strong></font> </span>秒钟后自动转到"
		   If Gcls.ComeUrl="" Then ReturnInfo=ReturnInfo & "会员中心" Else ReturnInfo=ReturnInfo & Gcls.ComeUrl
		   ReturnInfo=ReturnInfo & "</li><li id=""do""><h3><a href=""../index.asp"">进入会员中心</a>  <a href=""" &  KS.Setting(3)& """>返回网站首页</a></h3></li></td></tr></table>" & vbcrlf
		   Dim FileContent
		   If KS.Setting(119)="" Then Response.Write "请先到""基本信息设置->模板绑定""进行模板绑定操作!":response.end
            ...
            FileContent= Replace(FileContent,"{$UserName}",KS.S("username"))
		...
		If Gcls.ComeUrl<>"" and instr(Gcls.ComeUrl,"user/reg")=0 Then
		GetTurnMember=GetTurnMember & "	top.location.href='" & Gcls.ComeUrl & "';" & vbcrlf
		Else
