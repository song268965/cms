...
Class User_LogMoney
       ...
       Public Sub loadMain()
		...
		<div class="tabs">	
			<ul>
				<li class="puton"><a href="user_logmoney.asp">资金明细</a></li>
			...
			<div class="writeblog"> <a href='User_LogMoney.asp'>所有记录</a><a href='?IncomeOrPayOut=1'>收入记录(<%=conn.execute("select count(id) from ks_logmoney where IncomeOrPayOut=1 and username='" & KSUser.UserName & "'")(0)%>)</a><a href='?IncomeOrPayOut=2'>支出记录(<%=conn.execute("select count(id) from ks_logmoney where IncomeOrPayOut=2 and username='" & KSUser.UserName & "'")(0)%>)</a>
		   </div>
  ...
  Sub ShowContent()
    ...
    <tr class="tdbg" >
     ...
     <tr class="tdbg" >
      ...
      <td class="splittd" align=right><%=formatnumber(intotalmoney,2,-1)%></td>
      <td class="splittd" align=right><%=formatnumber(outtotalmoney,2,-1)%></td>
      <td class="splittd" align=middle colSpan=3>资金余额：<%=formatnumber(KSUser.GetUserInfo("Money"),2,-1)%></td>
