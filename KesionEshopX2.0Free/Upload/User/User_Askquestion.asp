...
Class User_myask
   ...
   Sub Tw()
	   %>
	    <table width="100%"  border="0" align="center" cellpadding="1" cellspacing="1" class="border">
		  ...
		  <% 
		   dim rs
			...
			sql="select b.classname,a.* from KS_asktopic a inner join KS_AskClass b on a.classid=b.classid where a.locktopic<>1 and a.askuserid="&KSUser.getuserinfo("userid")&"  order by a.topicid desc"
			rs.open sql,Conn,1,1
			if rs.eof and rs.bof then
		%>
			<tr>
				<td height="26" colspan="6" align="center" valign=middle  class='tdbg' >没有记录！</td>
			</tr>
		<%else
		...
		%>
						<tr class='tdbg' >
							...
							·[<%=rs(0)%>]<a href="../<%=KS.ASetting(1)%><%if KS.ASetting(16)="1" then KS.Echo "show-" & rs("topicid") & KS.ASetting(17) Else KS.Echo "q.asp?id=" & rs("topicid")%>" target="_blank"><%=KS.GotTopic(rs("title"),60)%></a> 
							</td>
							...
							<td class="splittd" align=center><%=rs("dateandtime")%></td>
							<td class="splittd" align=center><%
							if conn.execute("select top 1 postsid from KS_AskPosts1 where topicid=" & rs("topicid") & " and username='" & KSUser.UserName & "'").eof then
							  response.write "<font color=red>未解答</font>"
							else
							  response.write "<font color=green>已解答</font>"
							end if
							%></td>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
			<tr>
				<td colspan="10" height="45" align="center"><%=KSUser.UserName%>  等级头衔:<%=KSUser.GetUserInfo("GradeTitle")%></td>
			</tr>
		...
		<div class="tabs">	
			<ul>
				<li<%If action="" then KS.Echo " class='puton'"%>><a href="?">我的问题</a></li>
				<li<%If action="answer" Then KS.Echo " class='puton'"%>><a href="?action=answer">我的回答</a></li>
				<li<%If action="tw" Then KS.Echo " class='puton'"%>><a href="?action=tw">谁向我提问 </a></li>
				<li<%If action="fav" Then KS.Echo " class='puton'"%>><a href="?action=fav">我的收藏</a></li>
				<li><a href="user_askzj.asp">申请问答专家</a></li>
			</ul>
		</div>
   ...
   %>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
		...
		%>
			<tr>
				<td height="26" colspan=3 align=center valign=middle  class='tdbg' >您没有提问题！</td>
			</tr>
		<%else
		          totalPut = RS.RecordCount
				  If CurrentPage < 1 Then	CurrentPage = 1
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
		...
		%>
			<tr>
				<td height="26" colspan=3 align=center valign=middle  class='tdbg' >您没有回答过问题！</td>
			</tr>
		<%else
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
		...
		<% 
			set rs=server.createobject("adodb.recordset")
			sql="select b.classname,b.username,a.*,b.title,b.topicmode,b.reward from KS_askfavorite a inner join KS_AskTopic b on a.topicid=b.topicid where a.typeflag=0 and a.Username='"&KSUser.UserName&"' order by a.favorid desc"
			rs.open sql,Conn,1,1
			if rs.eof and rs.bof then
		%>
			<tr>
				<td height="26" colspan=3 align=center valign=middle  class='tdbg' >您没有收藏问题！</td>
			</tr>
		<%else
