       ...
       Public Sub loadMain()
		...
		<div class="tabs">	
			<ul>
				...
				<li class="puton"><a href="user_logpoint.asp">点券明细</a></li>
				<li><a href="user_logedays.asp">有效期明细</a></li>
				<li><a href="user_logscore.asp">积分明细</a></li>
				<li><a href="user_LogConsum.asp">使用记录</a></li>
			</ul>
		</div>
			<div class="writeblog"> <a href='User_LogPoint.asp'>所有记录</a><a href='?InOrOutFlag=1'>收入记录(<%=conn.execute("select count(id) from ks_logPoint where InOrOutFlag=1 and username='" & KSUser.UserName & "'")(0)%>)</a><a href='?InOrOutFlag=2'>支出记录(<%=conn.execute("select count(id) from ks_logPoint where InOrOutFlag=2 and username='" & KSUser.UserName & "'")(0)%>)</a>
			</div>
...
For i=0 To Ubound(SQL,2)
  ...
  <tr height="25" class='tdbg' onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
    ...
    <td class="splittd" align="right"><%if SQL(5,I)=1 Then Response.Write SQL(4,I) & KS.Setting(46):InPoint=InPoint+SQL(4,I) ELSE Response.Write "-"%></td>
    <td class="splittd" align="right"><%if SQL(5,I)=2 Then Response.Write SQL(4,I) & KS.Setting(46):OutPoint=OutPoint+SQL(4,I) ELSE Response.Write "-"%></td>
  ...
  <%Next%>
  <tr class='tdbg' onMouseOut="this.className='tdbg'" onMouseOver="this.className='tdbgmouseover'">    <td class="splittd" colspan='3' align='right'>本页合计：</td>    <td class="splittd" align='right'><%=InPoint%><%=KS.Setting(46)%></td>    <td class="splittd" align='right'><%=KS.ChkClng(OutPoint)%><%=KS.Setting(46)%></td>    <td class="splittd" colspan='4'>&nbsp;</td>  </tr> 
  ...
  %>
    <tr class='tdbg' onMouseOut="this.className='tdbg'" onMouseOver="this.className='tdbgmouseover'">    <td class="splittd" colspan='3' align='right'>所有合计：</td>    <td class="splittd" align='right'><%=KS.ChkClng(totalInPoint)%><%=KS.Setting(46)%></td>    <td class="splittd" align='right'><%=KS.ChkClng(totalOutPoint)%><%=KS.Setting(46)%></td>    <td class="splittd" colspan='4' align='center'>累计还剩：<%=totalInPoint-totalOutPoint%><%=KS.Setting(46)%></td>  </tr> 
