...
Class MyComment
		...
		<div class="tabs">						  
			<ul>
				<li<%If flag=0 and request("action")<>"My" then response.write " class='puton'"%>><a href="User_MyComment.asp">我参与的评论(<span class="red"><%=Conn.Execute("Select count(id) from KS_Comment" & Param & "")(0)%></span>)</a></li>
				<li<%If flag=1 then response.write " class='puton'"%>><a href="User_MyComment.asp?flag=1">已审核的评论(<span class="red"><%=Conn.Execute("Select count(id) from KS_Comment" & Param & " and verific=1")(0)%></span>)</a></li>
				<li<%If flag=2 then response.write " class='puton'"%>><a href="User_MyComment.asp?flag=2">未审核的评论(<span class="red"><%=Conn.Execute("Select count(id) from KS_Comment" & Param & " and verific=0")(0)%></span>)</a></li>
				<li<%If request("action")="My" then response.write " class='puton'"%>><a href="User_MyComment.asp?action=My">用户对我的评论(<span class="red"><%=Conn.Execute("Select count(id) from KS_Comment" & Param & "")(0)%></span>)</a></li>
			
			</ul>
          </div>
					 <table width="98%" align="center" border="0" cellspacing="1" cellpadding="1" class="border">
                              <%
  ...
  Sub ShowContent()
   ...
   Do While Not RS.Eof
		%>
            <tr>
			  <td style="width:30px"><img src="images/comment.gif"></td>
              <td height="65" class="splittd"> 
			...
			SqlStr="Select ID,Title,Tid,Fname,AddDate From " & KS.C_S(RS(5),2) & " Where ID=" & RS(6)
			 Dim RSI:Set RSI=Conn.Execute(SqlStr)
			 If NoT RSI.Eof Then
			  Response.Write "&nbsp;&nbsp;&nbsp;信息：<a href='" & KS.GetItemUrl(RS(5),RSI(2),RSI(0),RSI(3),RST(4)) & "' target='_blank'>" & RSI(1) & "</a>"
			 End If
...
%>
 ...
 Sub GetWriteComment(ID,Page)
		...
		With KS
		 ...
		 If From="My" Then
			 .echo "<tr class=""title"">"
			 .echo "  <td height=""26"" colspan=""3"">查看回复评论</td></tr>"
			 .echo "<tr class=""tdbg"">"
			 .echo "  <td height=""30"" colspan=""3"">评论人：&nbsp;&nbsp;" & RS("UserName")
			 .echo "   </td>"
			 .echo "  </tr>"
			 .echo "<tr class=""tdbg"">"
			 ...
			 .echo "  </tr>"
			 .echo "<tr class=""tdbg"">"
			 .echo "  <td height=""30"" colspan=""3"">回复内容：&nbsp;&nbsp;<textarea name='ReplyContent' rows='6' id='ReplyContent' class=""textbox"" style=""width:500px;height:100px;"">" & RS("ReplyContent") & "</textarea>"
			 .echo "   </td>"
			 .echo "  </tr>"
		 ...
		 Else
			 .echo "<tr class=""title"">"
			 .echo "  <td height=""26"" colspan=""3"">修改评论</td></tr>"
			 .echo "<tr class=""tdbg"">"
			 .echo "  <td height=""30""> 评 论 人：</td>"
			 .echo "   <td  colspan=""2""><input name=""AnounName"" type=""text"" readonly  class=""textbox"" id=""AnounName"" value=""" & RS("UserName") & """ style=""width:55%""/>"
			 .echo "   </td>"
			 .echo "  </tr>"
			 .echo "  <tr class=""tdbg"">"
			 .echo "    <td width=""80"">评论内容：</td>"
			 .echo "    <td height=""25""><textarea name=""Content"" rows=""6"" id=""Content"" class=""textbox"" style=""width:500px;height:100px;"">" & RS("Content") & "</textarea></td>"
