...
Class User_ExChange
		...
		If Mid(KS.Setting(170),1,1)="1" or Mid(KS.Setting(170),2,1)="1" Then
			...
			Response.Write " <li class='puton'><a href=""user_exchange.asp?Action=Point"">兑换" & KS.Setting(45) & "</a></li>"
			Else
		...
		If Mid(KS.Setting(170),3,1)="1" or Mid(KS.Setting(170),4,1)="1" Then
			...
			Response.Write " <li class='puton'><a href=""user_exchange.asp?Action=Edays"">兑换有效期</a></li>"
			Else
		...
		If Mid(KS.Setting(170),5,1)="1" Then
			...
			Response.Write " <li class='puton'><a href=""user_exchange.asp?Action=Money"">" & KS.Setting(45) & "兑换账户资金</a></li>"
			Else
	   ...
	   <script>
		...
		<FORM name=myform action="User_Exchange.asp" method="post">
		  <table class=border cellSpacing=1 cellPadding=2 width="100%" align=center border=0>
			<tr class=title>
			  <td  colSpan=2 height=22>兑换<%=KS.Setting(45)%></td>
			</tr> 
			<tr class=tdbg>
			...
			<tr class=tdbg>
			  ...
			  <td><input type='hidden' value='<%=KSUser.GetUserInfo("Score")%>' name='PreScore'><%=KSUser.GetUserInfo("Score")%> 分</td>
			</tr>
			<tr class=tdbg>
			...
			<tr class=tdbg>
			  <td align=right width=120>兑换<%=KS.Setting(45)%>：</td>
			  <td>
			  <%if Mid(KS.Setting(170),1,1)="1" then%>
		  <Input type=radio CHECKED value="1" name="ChangeType">使用资金余额： 将 
		  <Input style="text-align: center; width:100px;" class="textbox"  maxLength=8 size=6 value=100 name="Money"> 元兑换成<%=KS.Setting(45)%> &nbsp;&nbsp;&nbsp;&nbsp;<Font color=red>兑换比率：<%=KS.Setting(43)%>元:1<%=KS.Setting(46)%><%=KS.Setting(45)%></Font> 
		     <br><br>
			 <%End If%>
		     <%if Mid(KS.Setting(170),2,1)="1" then%>
		  <Input type=radio value="2" name="ChangeType"<%IF Mid(KS.Setting(170),1,1)="0" then Response.Write " checked"%>>使用经验积分： 将 
				<Input style="text-align: center; width:100px;"" class="textbox" maxLength=8 size=6 value=100 name="Score"> 分兑换成<%=KS.Setting(45)%> &nbsp;&nbsp;&nbsp;&nbsp;<Font color=red>兑换比率：<%=KS.Setting(41)%>分:1<%=KS.Setting(46)%><%=KS.Setting(45)%> </Font>
	   ...
	   Sub SaveExchangePoint()
	     Dim ChangeType:ChangeType=KS.ChkCLng(KS.S("ChangeType"))
		 
		  If ChangeType=1 And Mid(KS.Setting(170),1,1)="0" Then
			   Response.Write "<script>alert('对不起,没有开启资金换" & KS.Setting(45) &"服务!');history.back();</script>"
			   Exit Sub
		  ElseIf ChangeType=2 And Mid(KS.Setting(170),2,1)="0" Then
			   Response.Write "<script>alert('对不起,没有开启经验积分换" & KS.Setting(45) &"服务!');history.back();</script>"
			   Exit Sub
		  End If
		 ...
		 If ChangeType=1 Then
			...
			If KS.ChkClng(Money)<KS.ChkClng(KS.Setting(43)) Then
			   ...
			   Exit Sub
			End If
		   IF Round(RS("Money"),2)<Round(Money,2) Then
			   Rs.Close:Set RS=Nothing
		   ...
		   End If
			...
			Call KS.MoneyInOrOut(rs("UserName"),rs("RealName"),Money,4,2,now,0,"System","用于兑换点券",0,0,0)
	   ...
	   %>
		...
		<FORM name=myforms action="User_Exchange.asp" method="post">
		  <table class=border cellSpacing=1 cellPadding=2 width="100%" align=center border=0>
			...
			<tr class=tdbg>
			  <td align=right width=120>资金余额：</td>
			  <td><%=Formatnumber(KSUser.GetUserInfo("Money"),2,-1,0,-1)%> 元</td>
			</tr>
			<tr class=tdbg>
			  <td align=right width=120>可用积分：</td>
			  <td><%=KSUser.GetUserInfo("Score")%> 分</td>
			</tr>
			<tr class=tdbg>
			  <td align=right width=120>可用<%=KS.Setting(45)%>：</td>
			  <td><%=formatnumber(KSUser.GetUserInfo("Point"),2,-1,0,-1)%>&nbsp;<%=KS.Setting(46)%><input type='hidden' value='<%=KSUser.GetUserInfo("Point")%>' name='Prepoint'></td>
			</tr>
			<tr class=tdbg>
			  <td align=right width=120>兑换资金：</td>
			  <td>
		   将
		  <Input style="TEXT-ALIGN: center" maxLength=8 size=6 value=<%=KS.ChkClng(KSUser.GetUserInfo("Point"))%> name="Point"> <%=KS.Setting(46)%><%=KS.Setting(45)%>兑换成账户资金 &nbsp;&nbsp;&nbsp;&nbsp;<Font color=red>兑换比率：1<%=KS.Setting(46)%>:<%=KS.Setting(43)%>元</Font> <br>
		  </td>
			</tr>
			<tr class=tdbg>
			  <td align=middle colSpan=2 height=40>
	   ...
	   Sub SaveExchangeMoney()
		 Dim Point:Point=KS.S("Point")
		    ...
		    If Round(Point,2)<=0 Then
			   Response.Write "<script>alert('你输入的" & KS.Setting(45) & "不正确,必须大于0!');history.back();</script>"
			   Exit Sub
			End iF
		  If Mid(KS.Setting(170),5,1)="0" Then
			   Response.Write "<script>alert('对不起,没有开启" & KS.Setting(45) & "换换资金服务!');history.back();</script>"
			   Exit Sub
		  End If
		...
		DIM RS:Set RS=Server.CreateObject("ADODB.RECORDSET")
		 ...
		 If RS.Eof  Then
		   ...
		   Exit Sub
		 End If
		   IF Round(RS("Point"),2)<Round(Point,2) Then
			   Rs.Close:Set RS=Nothing
		   ...
		   End If
			'  ChannelID,InfoID,UserName,InOrOutFlag,Point,User,Descript
			 ...
			 Call KS.MoneyInOrOut(rs("UserName"),rs("RealName"),(point*KS.Setting(43)),4,1,now,0,"System","点券兑换所得",0,0,0)
		...
		<FORM name=myform action="User_Exchange.asp" method="post">
		  <table class=border cellSpacing=1 cellPadding=2 width="100%" align=center border=0>
			...
			<tr class=tdbg>
			  ...
			  <td>
			  <%If Mid(KS.Setting(170),3,1)="1" Then%>
		  <Input type=radio CHECKED value="1" name="ChangeType">使用资金余额： 将 
		  ...
		  <%End If%>
		  <%If Mid(KS.Setting(170),4,1)="1" Then%>
		  <Input type=radio value="2" name="ChangeType"<%If Mid(KS.Setting(170),3,1)="0" Then response.write " checked"%>>使用经验积分： 将 
				<Input style="TEXT-ALIGN: center" maxLength=8 size=6 value=100 name="Score"> 分兑换成有效天数 &nbsp;&nbsp;&nbsp;&nbsp;<Font color=red>兑换比率：<%=KS.Setting(42)%>分:1天 </Font>
		  <%End If%>		
				</td>
			</tr>
			<tr class=tdbg>
	   ...
	   Sub SaveExchangeEdays()
	   	 Dim ChangeType:ChangeType=KS.ChkClng(KS.S("ChangeType"))
		  If ChangeType=1 And Mid(KS.Setting(170),3,1)="0" Then
			   Response.Write "<script>alert('对不起,没有开启资金换有效天数服务!');history.back();</script>"
			   Exit Sub
		  ElseIf ChangeType=1 And Mid(KS.Setting(170),4,1)="0" Then
			   Response.Write "<script>alert('对不起,没有开启经验积分换有效天数服务!');history.back();</script>"
			   Exit Sub
		  End If
		 ...
		 If ChangeType=1 Then
		   ...
		   End If
			...
			Call KS.MoneyInOrOut(rs("UserName"),rs("RealName"),Money,4,2,now,0,"System","用于兑换有效天数",0,0,0)
