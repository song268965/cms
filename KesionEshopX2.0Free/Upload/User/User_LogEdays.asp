       ...
       Public Sub loadMain()		
		...
		<div class="tabs">	
			<ul>
				...
				<li class="puton"><a href="user_logedays.asp">有效期明细</a></li>
				<li><a href="user_logscore.asp">积分明细</a></li>
				<li><a href="user_LogConsum.asp">使用记录</a></li>
			</ul>
		</div>
			  <div class="writeblog"> <a href='User_LogEdays.asp'>所有记录</a><a href='?InOrOutFlag=1'>收入记录(<%=conn.execute("select count(id) from ks_logEdays where InOrOutFlag=1 and username='" & KSUser.UserName & "'")(0)%>)</a><a href='?InOrOutFlag=2'>支出记录(<%=conn.execute("select count(id) from ks_logEdays where InOrOutFlag=2 and username='" & KSUser.UserName & "'")(0)%>)</a>
			 </div>
