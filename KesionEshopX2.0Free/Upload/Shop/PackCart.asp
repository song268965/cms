			 ...
			 If Not RSB.Eof Then
					  ...
					  If Conn.Execute("Select Sum(Amount) From KS_ShopPackageSelect Where Packid=" & GNode.SelectSingleNode("@packid").text & " and username='" & GetUserID & "'")(0)=RSB("num")  or rsb("PackType")=1 Then   '商品件数是否一致
						...
						RSS.Open "Select a.title,a.weight,a.Price_Member,a.Price,b.* From KS_Product A inner join KS_ShopPackageSelect b on a.id=b.proid Where b.packid=" & GNode.SelectSingleNode("@packid").text & " and  b.UserName='" & GetUserID & "'",Conn,1,1
						  str=str & "<tr class='tdbg' height=""25"" style='text-align:center'><td></td><td  style='text-align:left'><strong>礼包【<a href='pack.asp?id=" & RSB("ID") & "' target='_blank'>" & RSB("PackName") & "</a>】</strong></td>"
						  n=1
						...
						str=str &"<td>1</td><td>￥" & TotalPackPrice & "</td><td>" & rsb("discount") & "折</td><td style='color:#ff3300;font-size:14px;font-weight:bold'>￥" & formatnumber((TotalPackPrice*rsb("discount")/10),2,-1) & "</td><td>￥<span name='totalmyprice'>" & formatnumber((TotalPackPrice*rsb("discount")/10),2,-1) & "</span></td>"
					    if showdel then
						str=str & "<td><a href='?action=delpack&id=" & rsb("id") & "' onclick=""return(confirm('确定删除该礼包吗?'))"">删除</a></td>"
						end if
						str=str & "</tr><tr><td></td><td align='left' colspan=9>您选择的套装详细如下:<br/>" & tempstr & "</td></tr>" 
...
Function GetBundleSalePro(ProID,ShowAction)   '获得捆绑促销的商品
  ...
  If IsObject(XML) Then
     ...
     For Each Node In Xml.DocumentElement.SelectNodes("row")
	  ...
	  str=str &"<td>---</td> " & vbcrlf
	  str=str &"<td style='color:#ff3300;font-size:14px;font-weight:bold'>￥" & KS.GetPrice(Node.SelectSingleNode("@price").text) &"</span></td> " & vbcrlf
	  
	  str=str&"	<td>￥<span name='totalmyprice' id='myprice" & Node.SelectSingleNode("@selid").text &"'>" & KS.GetPrice(Node.SelectSingleNode("@price").text*Node.SelectSingleNode("@amount").text)  & "</span><span id='hidmyprice" & Node.SelectSingleNode("@selid").text &"' style='display:none'>" & Node.SelectSingleNode("@price").text  & "</span></td>" & vbcrlf

	  str=str &"<td style='display:none' ><span id='hidmyscore" & Node.SelectSingleNode("@selid").text &"'>0</span><span name='totalmyscore' id='myscore" & Node.SelectSingleNode("@selid").text &"'>0</span></td> " & vbcrlf
	  If ShowAction=true Then
...
If MemberNum<>0 Then
   HasMemberBuyNum=KS.ChkClng(Conn.Execute("select sum(Amount) From KS_OrderItem Where UserIP='"& KS.GetIP &"' and ProID=" & RS("ID") & " AND IsMember=1 and datediff(" & DataPart_D & ",begindate," & SQLNowString & ")<=0")(0))
End If
If visitornum<>0 Then
   HasVisitorBuyNum=KS.ChkClng(Conn.Execute("select sum(Amount) From KS_OrderItem Where UserIP='"& KS.GetIP &"' and ProID=" & RS("ID") & " AND IsMember=0 and datediff(" & DataPart_D & ",begindate," & SQLNowString & ")<=0")(0))
End If
