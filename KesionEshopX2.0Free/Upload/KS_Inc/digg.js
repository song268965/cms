...
{
 try{
  ajaxLoadPage(installdir+'plus/digmood/digg.asp','digtype='+type+'&action=hits&ChannelID='+channelid+'&infoid=' +infoid,'post','digbacks');
 }catch(e){
	 var head = document.getElementsByTagName("head")[0];        
	 var js = document.createElement("script"); 
	 js.src = installdir+'plus/digmood/digg.asp?printout=js&digtype='+type+'&action=hits&ChannelID='+channelid+'&infoid=' +infoid; 
	 head.appendChild(js);   
  }
}
function show_digg(channelid,infoid,installdir)
{ 
 var url=installdir+"plus/digmood/digg.asp?channelid="+channelid+"&infoid="+infoid+"&action=show";
 try
 {
