myFocus.pattern.extend({//*********************pithy******************
	'mF_pithy_tb':function(settings,$){
		...
		$thumbBox.find('img').each(function(){$(this).css({height:thuHeight-12,width:p.thumbWidth-21})});//鍑忓幓padding+margin+border
		$prevBtn[0].style.height=$nextBtn[0].style.height=p.thumbBtnHeight+'px';//鏆傛椂涓岖敤
		...
		//Control
		p.trigger='click';//trigger闄愬畾涓篶lick
...
myFocus.config.extend({
	'mF_pithy_tb':{//鍙€変釜镐у弬鏁?
		seamless:true,//鏄惁镞犵绅锛屽彲阃夛细true(鏄? | false(鍚?
		txtHeight:38,//镙囬楂桦害
		thumbShowNum:4,//鐣ョ缉锲炬樉绀烘暟鐩?
		thumbBtnHeight:0,//鐣ョ缉锲惧鑸?prev/next)鎸夐挳镄勯佩搴?
		thumbWidth:112//鐣ョ缉锲炬€诲搴?
	}
});