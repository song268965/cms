...
Dim del:del=KS.Checkxss(request("del"))
if PhotoUrl="" Then
  response.write "<script>alert('您没有选择图片!');top.frames['MainFrame'].box.close();</script>"
end if
...
<body>
<iframe src="imgcut.asp?action=main&del=<%=del%>&photourl=<%=photourl%>&fieldid=<%=request("fieldid")%>" scrolling="yes" width="100%" height="540"></iframe>
</body>
</html>
<%
end if
...
<table border="0" width="99%" bgcolor="#666666" align="center" cellspacing="0" cellpadding="0">
  ...
  裁剪高度：<input value="200" name="drag_h" id="drag_h" type="text" style="width:30px;"/> px 
    ...
    <input name="" type="button" value="放弃使用原图" onClick="top.frames['MainFrame'].box.close();"/>
        </form>
	   </td>
	  </tr>
	  </table>
...
function Create(){
	...
	$("#myform").attr("action","ImgCutSave.asp?fieldid=<%=request("fieldid")%>&p=" + p + "&x=" + x + "&y=" + y + "&w=" + w + "&h=" + h + "&pw=" + pw + "&ph=" + ph + "&" + Math.random());
	$("#myform").submit();
}
