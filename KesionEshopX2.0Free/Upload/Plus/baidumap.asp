...
mapKey=KS.Setting(22)
MapCenterPoint=KS.CheckXSS(Request("MapMark"))
...
<style type="text/css">
 body{font-size:12px;}
.button{height:23px;background:#f1f1f1;border:1px solid #cccccc;}
</style>
...
<style>
.textbox{padding: 0 5px; height:28px;border: 1px solid;border-color: #e0e6e6;background: #F9F9F9;color: #333;resize: none;margin-left: 5px;}
.button{background: #247ec0;color: #fff;border: 1px solid #247ec0;padding: 0 10px;height: 30px;cursor: pointer;}
</style>
</head>
<body onLoad="KesionDotccMap();" onkeydown="if(event.keyCode==13)KesionDotccMap()">
<%if request("action")<>"getcenter" then%>
 <div style="margin:13px">
 搜索：<input type="text" value="北京市" name="keyword" id="keyword" class="textbox"/> <input class="button" onClick="searchMap($('#keyword').val())" type="button" value="搜索经纬坐标" class="button"/><br><br>
 <span id="info"></span>


<div style="margin-top:4px;width:540px;height:420px;border:1px solid #ede7e7; float:left" id="KesionMap"></div>
<div style="margin-top:10px; margin-left:10px; float:left">
	<div style="margin-top:10px; margin-left:20px;"><strong>使用方法：</strong><br/><br/>拖动需要查看地点并点击即可标注</div>
	...
	<div style="margin-top:10px;text-align:center"><input type='button' value='确定保存以上标志' onclick='setOk()' class="button" /></div>
</div>
<%else%>
 <div style="margin:13px">
 搜索：<input type="text" value="北京市" name="keyword" id="keyword" class="textbox"/> <input class="button" onClick="searchMap($('#keyword').val())" type="button" value="搜索经纬坐标"/>
 <span id="info"></span>

</div>
<div style="margin-top:4px;width:680px;height:360px;border:1px solid gray; float:left" id="KesionMap"></div>
<%end if%>
</body>
</html>
