	   ...
	   Sub ParseArea(sTokenName, sTemplate)
		...
		Sub InitialSearch()
		  ...
		  Else
		    ...
		    If ChannelID=0 Then
			 '找出不允许搜索的模型ID
			  Dim NotAllowSearchChannelID:NotAllowSearchChannelID=""
			  For Each Node In Application(KS.SiteSN&"_ChannelConfig").DocumentElement.SelectNodes("channel[@ks21=1 and @ks0!=6 and @ks6<9]")
				 If KS.ChkClng(KS.M_C(Node.SelectSingleNode("@ks0").text,29))<>1 Then
				   If NotAllowSearchChannelID="" Then
					 NotAllowSearchChannelID=Node.SelectSingleNode("@ks0").text
				   Else
					 NotAllowSearchChannelID=NotAllowSearchChannelID &"," & Node.SelectSingleNode("@ks0").text
				   End If
				 End If
			  Next
			  If NotAllowSearchChannelID<>"" Then Param=Param & " and ChannelID Not In("& NotAllowSearchChannelID &")"
		    ElseIf ChannelID>0 And KS.ChkClng(KS.C_S(ChannelID,6))<10 Then
		  ...
		  If Not KS.IsNul(Key) Then
				select case stype
				 ...
				 case 3 
				   If ChannelID=102 Then
				     Param=Param & " And UserName Like " & KS.WithKorean() &"'%" & Key & "%'"
				   Else
				     Param=Param & " And inputer Like " & KS.WithKorean() &"'%" & Key & "%'"
				   End If
				 case else 
		 ...
		 if request("otid")<>"" and request("otid")<>"0" then
		     If ChannelID<>102 Then
		       Param=Param & " And oTid In(" & KS.GetFolderTid(KS.S("oTid")) & ")"
		   end if
		 end if
		...
		Sub KeyToDataBase()
		  ...
		  If len(key)>15 or KS.HasChinese(Key)=false Then Exit Sub
		  If IsValidStr(key) Then Exit Sub
		  Dim RS:Set RS=Server.CreateObject("adodb.recordset")
		  RS.Open "Select top 1 * From KS_KeyWords Where IsSearch=1 and KeyText=" & KS.WithKorean() &"'" & Key &"'",conn,1,3
		  If RS.Eof Then
		    RS.AddNew
			RS("AddDate")=Now
			RS("IsSearch")=1
		...
		Sub SearchRecords()
		  ...
		  Key=KS.R(Key)
		  If Not KS.IsNul(Key) and len(key)<15  and IsValidStr(key)=false Then
		...
		Function AutoKey(ByVal strKey,FieldName) 
			...
			If lngLenKey <=1 Then AutoKey="(" & FieldName & " like " & KS.WithKorean() &"'%" & strKey & "%')": Exit Function
			'若长度大于1，则从字符串首字符开始，循环取长度为2的子字符串作为查询条件 
			For i=1 To lngLenKey-(lngSubKey-1) 
			  strSubKey=Mid(strKey,i,lngSubKey) 
			  If Param="" Then
			   Param = "(" & FieldName & " like " & KS.WithKorean() &"'%" & strSubKey & "%'" 
			  Else
			   Param=Param & " or " & FieldName & " like " & KS.WithKorean() &"'%" & strSubKey & "%'"
			  End If
			Next
		...
		For Each Node In ModelXML.documentElement.SelectNodes("channel")
		 if Node.SelectSingleNode("@ks21").text="1" and Node.SelectSingleNode("@ks0").text<>"6" and Node.SelectSingleNode("@ks0").text<>"9" and Node.SelectSingleNode("@ks0").text<>"10" and Node.SelectSingleNode("@ks0").text<>"11"  and Node.SelectSingleNode("@ks0").text<>"12"  and Node.SelectSingleNode("@ks0").text<>"13" Then
