					...
					Do While Not RS.Eof
					 ...
					 str=str & "	<td><h1>"
					  if not ks.isnul(rs("photourl")) then
					   str=str &"<img style='border:1px solid #f1f1f1;padding:1px;margin:4px' src='" & rs("photourl") &"' align='left' width='70'/>"
					 end if
					 str=str &"<span class='PKdate'>" & rs("adddate") & "</span>  <a href='" & url & "' target='_blank'>" & rs("title")
					 if not ks.isnul(rs("title2")) then
					  str=str & " <font class=""pk"">PK</font> " & rs("title2")
					 end if
					 str=str & "</a></h1>" &vbcrlf
