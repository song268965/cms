...
Sub Show()
   With KS
	 ...
	 If IsObject(DigXml) Then
	  ...
	  If PrintOut="js" Then
	   .echo "$('#s" & infoid & "').html('" & DiggNum & "');" & vbcrlf
	   .echo "try{" & vbcrlf
	   .echo "   $('#c" & infoid & "').html(" & CDiggNum & ");" & vbcrlf
	   .echo "   var znum=$('#s" & infoid & "').html();" & vbcrlf
	   .echo " 	 var cnum=$('#c" & infoid & "').html();" & vbcrlf
	   .echo "	 var totalnum=parseInt(znum)+parseInt(cnum);" & vbcrlf
	   .echo " 	$('#perz" & infoid & "').html(((znum*100)/totalnum).toFixed(2)+'%');" & vbcrlf
	   .echo "	$('#perc" & infoid & "').html(((cnum*100)/totalnum).toFixed(2)+'%');" & vbcrlf
	   .echo "	$('#digzcimg').width(parseInt((znum/totalnum)*55));" & vbcrlf
	   .echo "	$('#digcimg').width(parseInt((cnum/totalnum)*55));" & vbcrlf
	   .echo "}catch(e){" & vbcrlf
	   .echo "}" & vbcrlf
	  Else
