  ...
  Else
	...
	Else
		...
		End If
		 ...
		 RSI("Title")=Conn.Execute("Select top 1 Title From " & KS.C_S(ChannelID,2) & " Where ID=" & InfoID)(0)
	...
	For I=0 To Ubound(VoteArr)
	 If trim(VoteArr(I))<>"" Then
	   ...
	   If Ubound(VoteItemArr)>=1 And VoteItemArr(0)<>"" And VoteItemArr(1)<>"" Then
		 ...
		 VoteStr=VoteStr & "<td valign=""bottom""  style=""text-align:center"">" & ItemVoteNum(I) & "<br/><a href=""javascript:;"" onclick=""MoodPosition(" & I &");""><img alt=""" & Percentage & "%"" src=""" & KS.Setting(2) & KS.Setting(3) & "images/default/post.gif"" height=""" & PerVote*50 & """ width=""20""><div class=""mymoodbox""><img alt=""" & VoteItemArr(0) & """ src=""" & ImgSrc & """ border=""0""/></a><br/><a href=""javascript:;"" onclick=""MoodPosition(" & I &");"">" & VoteItemArr(0) & "</a></div></td>"
	   End If
	 End If
	Next
	VoteStr=VoteStr & "</tr>"
...
ajaxReadText('<%=KS.Setting(2)%><%=KS.Setting(3)%>plus/digmood/mood.asp?action=hits&itemid='+itemid+'&id=<%=MoodID%>&m_ID=<%=ChannelID%>&c_id=<%=InfoID%>','MoodPositionBack(obj)');
}
