...
Const IsWithKorean=False           '是否有日韩文

'========数据库设置开始========
 If DataBaseType=0 then
'如果是ACCESS数据库，请认真修改好下面的数据库的文件名
	DBPath       = "/KS_Data/KesionCMSx20.mdb"     'ACCESS数据库的文件名，请使用相对于网站根目录的的绝对路径
Else
		 '如果是SQL数据库，请认真修改好以下数据库选项
	 DataServer   = "(local)"                                  '数据库服务器IP
	 DataUser     = "sa"                                       '访问数据库用户名
	 DataBaseName = "kesioncmsX20"                                '数据库名称
...
Sub OpenConn()
  '  On Error Resume Next
    ...
    conn.open ConnStr
    If Err Then Set conn = Nothing:Response.Write "数据库连接出错，请检查Conn.asp文件中的数据库参数设置。<br/>出错原因:" & Err.Description:Err.Clear:Response.End
...
Const RootDomain = "kesion.com"       rem 网站主域名根,如果有多个子域名,必须设置
'=============================================二级域名配置结束========================================================
