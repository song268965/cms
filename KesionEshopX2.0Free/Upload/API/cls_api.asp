...
Dim API_Deliveryapi,API_Deliveryapi_Key
Dim API_WeiXinEnable,API_WeiXinAppId,API_WeiXinAppKey,API_WeiXinCallBack 
...
function get_user_info(ApiType,access_token,openid)
	...
	elseif apitype="4" then '微信
	    url = "https://api.weixin.qq.com/sns/userinfo"
        get_user_info = file_get_contents(url,"get","access_token="&access_token&"&openid="&openid)
...
Sub DoBind(tips,username,userface,sex,openid)
	...
	dim newuser:newuser=replace(replace(KS.R(username),"~","")," ","")
	do while not conn.execute("select top 1 userid from ks_user where username='" & newuser & "'").eof
		newuser=KS.R(username) &  KS.MakeRandom(5)
	loop
	username=newuser
...
'保存注册
'参数 flag 1表示qq绑定注册 2表示新浪微博 3支付宝 4微信登录
Sub DoRegSave(Flag)
...
Sub SaveReg(IsApi,UserName,PassWord,GroupID,Sex,Email,UserFace)		 
		...
		' RS("UserFace")=UserFace
		 ...
		 RS("WeiXinOpenId")=ks.c("weixinopenid")
		 ...
		 '===================写入个人空间================
			if KS.SSetting(0)=1 And KS.SSetting(1)=1 then
			 ...
			 RS.AddNew
			  ...
			  RS("TemplateID")=KS.U_S(GroupID,29)
			  RS("Announce")="暂无公告!"
			  RS("ContentLen")=500
			  RS("Recommend")=0
...
Sub DoLogin(userName,Password)
  Dim UserRS:Set UserRS=Server.CreateObject("Adodb.RecordSet")
	 UserRS.Open "Select top 1 * From KS_User Where UserName='" &UserName & "' And PassWord='" & PassWord & "'",Conn,1,1
	 ...
	 Else
		...
		End if
			...
			'更新登录信息	
			Conn.Execute("Update KS_User Set LastLoginIP='" & KS.GetIP &"',	LastLoginTime=" & SQLNowString &",	LoginTimes=LoginTimes+1,RndPassWord='" & RndPassWord &"' Where UserID=" & UserRS("UserID"))
...
Sub LoadXslConfig()
	...
	Else
		...
		API_WeiXinEnable        = XslNode.getAttribute("api_weixinenable")
		API_WeiXinAppId         = XslNode.getAttribute("api_weixinappid")
		API_WeiXinAppKey        = XslNode.getAttribute("api_weixinappkey")
		API_WeiXinCallBack      = XslNode.getAttribute("api_weixincallback")
