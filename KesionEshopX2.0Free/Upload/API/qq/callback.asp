...
function qq_callback()
    if(lcase(Request("state")) = lcase(Session("state"))) Then
        ...
        result = file_get_contents(token_url,"get","grant_type=authorization_code&client_id="&appid&"&redirect_uri="&server.URLEncode(callback) & "&client_secret="&appkey &"&code="&KS.CheckXSS(REQUEST("code")))
