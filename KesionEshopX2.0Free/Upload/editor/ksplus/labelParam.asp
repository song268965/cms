...
function OK() {
    ...
    Val = '{=GetLimitBuy('+document.myform.taskid.value+','+document.myform.num.value+')}';  
...
<FIELDSET align=center>
 <LEGEND align=left>限时限量调用参数设置</LEGEND>
 <table  width='100%' border='0'  align='center' cellpadding='2' cellspacing='1'>
  <tr >
    <td align="right"><div align="center">调用项目</div></td>
    <td>
	 <select name="taskid" id="taskid">
	   <option value='0'>--调用最新一条抢购任务--</option>
	   <%
	   dim rs:set rs=conn.execute("select id,taskname from KS_ShopLimitBuy order by id desc")
	   do while not rs.eof
  ...
  <tr >
    <td width="24%" align="right"><div align="center">调用商品数</div></td>
    <td width="76%" ><input name="num" type="text" id="num" value="1" size="8" onBlur="CheckNumber(this,'调用商品数');">
    件</td>
  </tr>
</table>
</FIELDSET>
...
function OK() {
    var Val;
    Val = '{=GetTags('+document.myform.sorts.value+','+document.myform.num.value+','+$("#from").val()+')}';  
    window.returnValue = Val;
    window.close();
}
</script>
...
<FIELDSET align=center>
 ...
 <table  width='100%' border='0'  align='center' cellpadding='2' cellspacing='1'>
  <tr >
    <td width="40%" align="right"><div align="center">Tags来源：</div></td>
    <td width="60%" >
	 <select name="from" id="from">
	    <option value="0">系统Tags</option>
