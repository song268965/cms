...
Class CommandStatus
		...
		Public Sub Kesion()
			...
			Select Case KS.C_S(ChannelID,6)
			 Case 1:Url="System/KS.ItemInfo.asp"
			 Case 2:Url="System/KS.ItemInfo.asp"
			 Case 3:Url="System/KS.ItemInfo.asp"
			 Case 4:Url="System/KS.ItemInfo.asp"
			 Case 5:Url="System/KS.ItemInfo.asp"
			 Case 6:ItemName="歌曲"
			 Case 7:Url="System/KS.ItemInfo.asp"
			 Case 8:Url="System/KS.ItemInfo.asp"
			 Case else
			  ItemName="文章"
			 End Select
			...
			With KS
				...
				.echo"<script src=""Include/SetFocus.js""></script>"
				.echo"<body>"
				.echo"  <ul id='split'>"
				...
				.echo("   function ClickButton1(){ "& vbcrlf)
				   Select Case (UCase(ButtonSymbol))
							...
							Case "VIEWFOLDER", "ARTICLESEARCH"
								 .echo("$(parent.document).find('#MainFrame')[0].contentWindow.CreateNews(1);" & vbcrlf)
							Case "ADDINFO"
				...
				.echo(" {")
				 Select Case (UCase(ButtonSymbol))
						 Case "VIEWFOLDER", "ARTICLESEARCH"
							.echo("$(parent.document).find('#MainFrame')[0].contentWindow.Edit(1);")
						 Case "PICTURESEARCH"
							.echo("$(parent.document).find('#MainFrame')[0].contentWindow.Edit(1);")
						 Case "DOWNSEARCH"
							.echo("$(parent.document).find('#MainFrame')[0].contentWindow.Edit(1);")
						 ...
						 Case "ADDINFO"
							...
							.echo("location.href='Post.Asp?ButtonSymBol=ViewFolder&FolderID=" & FolderID & "&ChannelID=" & ChannelID & "';")
						Case "ADDGQ"
							.echo("$(parent.document).find('#MainFrame')[0].contentWindow.location.href='KS.Supply.asp?ID=" & FolderID & "&DisplayMode=" & DisplayMode &"';")
							.echo("location.href='Post.Asp?ButtonSymBol=ViewGQFolder&FolderID=" & FolderID & "';")
