...
Class Admin_PhotoVote
		...
		.echo "<ul id='mt' class='menu_top_fixed'> <div id='mtl'>"
		
		.echo "<strong>查看详细的投票记录:</strong>"
		.echo "<select"
        ...
        .echo("<div class='pageCont2'><form name=""myform"" method=""Post"" action=""KS.PhotoVote.asp?Action=Del&ChannelID=" & ChannelID & """>")
		.echo "<table width='100%'border='0' cellpadding='0' cellspacing='0'>"
		.echo "    <tr class='sort'>"
		.echo "    <td width='60' align='center'>选中</td>"
		.echo "    <td width='78' align='center'>参与用户</td>"
		.echo "    <td align='center'>投票时间</td>"
		.echo "    <td width='100' align='center'>参与用户IP</td>"
		...
		.echo "  </tr>"
		 ...
		 .echo ("</td>")
	     .echo ("<td class='pt10'><input type=""submit"" value=""删除选中的记录"" onclick=""return confirm('确定要执行删除操作吗,删除后相应的票数将减少？');"" class=""button""></td>")
	     .echo ("<td align='right'>")
	Call KS.ShowPage(totalput, MaxPerPage, "", CurrentPage,true,true)
    	...
    	.echo (" <div style=""position:absolute;top:4px;right:8px; overflow:hidden;"" >")
		.echo "  <select style=""white-space:nowrap"" OnChange=""location.href='KS.PhotoVote.asp?ChannelID=" & ChannelID & "&id='+this.options[this.options.selectedIndex].value+'';"" style='width:120px' name='id'>"
		.echo "<option value=''>按栏目查看投票记录...</option>"
		.echo Replace(KS.LoadClassOption(ChannelID,false),"value='" & KS.S("ID") & "'","value='" & KS.S("ID") &"' selected") & "</select>&nbsp;&nbsp;"
		.echo ("</div>")
...
%>
			 ...
			 <br/>
			 <strong>说明：</strong><br>
			 &nbsp;清零操作将所选栏目的得票数设为“0”，一般在开始投票前操作，否则请不要进行此操作</div>
<%
		  End With
		 End Sub
