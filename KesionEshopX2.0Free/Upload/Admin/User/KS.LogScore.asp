...
Class User_ScoreDetail
       ...
       Sub Kesion()
	      ...
	      If Not KS.ReturnPowerResult(0, "KMUA10017") Then
			  response.Write ("<script>$(parent.document).find('#BottomFrame')[0].src='javascript:history.back();';</script>")
			  Call KS.ReturnErr(1, "")
			End If
			Response.Write"<div class='topdashed quickLink' style='text-align:left'>会员积分明细: <a href='?'>所有积分明细</a> | <a href='?channelid=1000'>点广告积分明细</a> | <a href='?channelid=1001'>点友情链接积分明细</a></div>"
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0">
  <tr class="sort">
    ...
    <td width="65" align="center"><strong> 可用积分</strong></td>
  ...
  Dim FieldStr,SQLStr
  FieldStr="ID,UserName,AddDate,IP,Score,InOrOutFlag,CurrScore,[User],Descript,AvailableScore"
...
For i=0 To Ubound(SQL,2)
  ...
  <tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
    ...
    <td class="splittd" align="center"><%=SQL(9,i)%> 分</td>
    <td class="splittd" align="center"><%=SQL(7,i)%></td>
	<td class="splittd"><%=SQL(8,i)%></td>
  </tr>
  <%Next%>
  <tr class='list' onmouseout="this.className='list'" onmouseover="this.className='listmouseover'">    <td class="splittd" colspan='3' align='right'>本页合计：</td>    <td class="splittd" align='right'><%=InScore%>分</td>    <td align='right'><%=OutScore%>分</td>    <td class="splittd" colspan='5'>&nbsp;</td>  </tr> 
  ...
  %>
    <tr class='list' onmouseout="this.className='list'" onmouseover="this.className='listmouseover'">    <td class="splittd" colspan='3' align='right'>所有合计：</td>    <td class="splittd" align='right'><%=totalInScore%>分</td>    <td class="splittd" align='right'><%=totalOutScore%>分</td>    <td class="splittd" colspan='5'>&nbsp;</td>  </tr> 
  <%  
  Response.Write "<tr><td colspan=10 align=right class='list' onMouseOver=""this.className='listmouseover'"" onMouseOut=""this.className='list'"">"
