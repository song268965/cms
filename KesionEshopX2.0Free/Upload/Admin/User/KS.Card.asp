...
Class Admin_Card
       ...
       Sub Kesion()
          ...
          Response.Write "<!DOCTYPE html><html>"
			...
			Response.Write"	<ul id='mt' class='quickLink'> "
			Response.Write " <span id='mtl'>操作导航:</span><span>&nbsp;<a href=""?cardtype=" & cardtype & """>所有充值卡</a> | "
			If CardType=1 Then
			Response.Write "<a href=""?action=AddMore&cardtype=1"">生成在线充值卡</a></li>"
			Else
			Response.Write "<a href=""?status=1&cardtype=0"">未使用充值卡</a> | <a href=""?status=2&cardtype=0"">已使用充值卡</a> | <a href=""?status=3&cardtype=0"">已失效充值卡</a> | <a href=""?status=4&cardtype=0"">未失效充值卡</a> | <a href=""?action=Add&cardtype=0"">添加充值卡</a> | <a href=""?action=AddMore&cardtype=0"">批量生成充值卡</a></span>"
			End If
			Response.Write	" </ul>"
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0">
  ...
  Set RS=Server.CreateObject("ADODB.RecordSet")
	...
	If RS.Eof And RS.Bof Then
	 Response.Write "<tr><td colspan=11 align=center height=25 class='splittd'>没有充值卡！</td></tr><tr><td colspan=13 background='images/line.gif'></td></tr>"
	Else
...
<div><b>快速搜索=></b>:
<%
	           Response.Write " &nbsp;<select name='groupname' class='textbox'>"
...
For i=0 To Ubound(SQL,2)
  ...
  <tr height="22" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	<a href="?action=Edit&ID=<%=SQL(0,i)%>&cardtype=<%=cardtype%>" class='setA'>修改</a>|<a href="?action=Del&cardtype=<%=cardtype%>&ID=<%=SQL(0,i)%>" class='setA'>删除</a>
	<%end if%>&nbsp;
	</td>
  </tr>
  <%Next
  
  Response.Write "<tr class='list'><td height='30' colspan=4 class='operatingBox'>"
  ...
  Response.Write "</td></tr>  <tr><td colspan=13 background='images/line.gif'></td></tr>"
End Sub
 ...
 End Sub
  ...
  <table width='100%' border='0' align='center' cellpadding='2' cellspacing='1'>
    ...
    <tr class='tdbg'> 
      ...
      <td width='60%'><input name='GroupName' class='textbox' type='text' size='20' maxlength='100'> 如:“10元购100点充值卡”等</td>
    </tr>
    <tr class='tdbg'> 
   ...
   <tbody id="showother" style="display:<%if cardtype=0 then response.write "" else response.write "none"%>">
    <tr class='tdbg'> 
      ...
      <td width='60%'><input name='Nums' type='text' class='textbox' value='100' size='10' maxlength='10'>
        张</td>
    </tr>
    <tr class='tdbg'>
      <td width='40%' class="clefttitle"><strong>充值卡号码前缀：</strong><br>
        例如：<%=year(now)%>,KS<%=year(now)%>等固定不变的字母或数字</td>
      <td width='60%'><input name='CardNumPrefix' class='textbox' type='text' id='CardNumPrefix' value='KS<%=year(now)%>' size='10' maxlength='10'></td>
    </tr>
    <tr class='tdbg'>
      <td width='40%' class="clefttitle"><strong>充值卡号码位数：</strong><br>请输入包含前缀字符在内的总位数</td>
      <td width='60%'><input name='CardNumLen' class='textbox' type='text' id='CardNumLen' value='12' size='10' maxlength='10'>
        <font color='#0000FF'>建议设为10--15位</font></td>
    </tr>
    <tr class='tdbg'>
      <td width='40%' class="clefttitle"><strong>充值卡密码位数：</strong></td>
      <td width='60%'><input name='PasswordLen' class='textbox' type='text' id='PasswordLen' value='6' size='10' maxlength='10'>
    ...
    <tr class='tdbg'>
      ...
      即购买人需要花费的实际金额</td>
      <td width='60%'><input name='Money' class='textbox' type='text' id='Money' value='50' size='10'>
      元</td>
    </tr>
    <tr class='tdbg'> 
      <td width='40%' class="clefttitle"><strong>充值卡点数、资金或有效期：</strong><br>
        购买人可以得到的点数、资金、有效期和积分      </td>
      <td width='60%'><input name='ValidNum' type='text' class='textbox' id='ValidNum' value='50' size='10' maxlength='10'>
        <select name='ValidUnit' id='ValidUnit' class='textbox'>
    ...
    <tr class='tdbg'>
      ...
      <td width='60%'><select name="GroupID" id="GroupID" class="textbox">
	  <option value='0'>---保持原有用户组---</option>
	<%=KS.GetUserGroup_Option(0)%>
	 </select></td>
    </tr>
    <tr class='tdbg'>
      <td width='40%' class="clefttitle"><strong>到期后自动归入的用户组：</strong>
     </td>
      <td width='60%'><select name="ExpireGroupID" id="ExpireGroupID" class="textbox">
	  <option value='0'>---保持原有用户组---</option>
	<%=KS.GetUserGroup_Option(0)%>
	 </select>
	 <br>
    ...
    <tr class='tdbg'>
      ...
      <td width='60%' class='tdbg'><input name='EndDate' class='textbox' type='text' id='EndDate' value='<%=dateadd("yyyy",2,now)%>' size='20'></td>
    </tr>
    <tr class='tdbg'> 
...
<div class="pageCont2">    
	<div class="tabTitle">
	  <%IF KS.g("Action")="Edit" then
	   response.write "修改充值卡"
	    Else
		Response.Write "添加充值卡"
	    End If
		%></div>
  <table width='100%' border='0' align='center' cellpadding='2' cellspacing='1'>
    ...
    <tr class='tdbg'> 
      <td width='20%' class="clefttitle"><strong>充值卡名称：</strong></td>
      <td width='60%'><input name='GroupName' class='textbox' type='text' size='20' value="<%=GroupName%>" maxlength='100'> 如:“10元购100点充值卡”等</td>
    </tr>
    <tr class='tdbg'> 
      <td width='20%' class="clefttitle"><strong>充值方式：</strong></td>
      <td width='60%'>
	  <input type="radio" name="cardtype" value="1"  onclick="$('#showother').hide()"<%if cardtype=0 then response.write " disabled" else response.write " checked"%>>在线充值购买(会员中心由会员自己购买充值）<br/>
	  <input type="radio" name="cardtype" value="0" onclick="$('#showother').show()"<%if cardtype=1 then response.write " disabled" else response.write " checked"%>>线下销售
	  </td>
   ...
   <tbody id="showother" style="display:<%if cardtype=0 then response.write "" else response.write "none"%>">
    ...
    <tr class='tdbg'<%if KS.g("action")="Edit" Then response.write " style='display:none'"%>> 
      <td width='20%' class="clefttitle"><strong>添加方式：</strong></td>
    ...
    <tr class='tdbg' id='trSingle1'>
      <td width='20%' class="clefttitle"><b>充值卡卡号：</b></td>
      <td><input name='CardNum' type='text' class='textbox' id='CardNum' size='20' value="<%=CardNum%>" maxlength='30'>
        <font color='#0000FF'>建议设为10--15位</font></td>
    </tr>
    <tr class='tdbg' id='trSingle2'>
      <td width='20%' class="clefttitle"><b>充值卡密码：</b></td>
      <td><input name='Password' type='text' class='textbox' id='Password' size='20' value="<%=PassWord%>" maxlength='30'>
        <font color='#0000FF'>建议设为6--10位 </font></td>
    </tr>
    <tr class='tdbg' id='trBatch' style='display:none'>
      <td width='20%' class="clefttitle"><b>格式文本：</b><br><font color='red'>请按照每行一张卡，每张卡按“卡号＋分隔符＋密码”的格式录入</font><br>例：734534759|kSo94Sf4Xs（以“|”作为分隔符）</td>
      <td><textarea name='CardList' rows='10' class='textbox' cols='50'></textarea></td>
    </tr>
	</tbody>
    <tr class='tdbg'>
      <td width='20%' class="clefttitle"><strong>充值卡面值：</strong><br>
      即购买人需要花费的实际金额</td>
      <td width='60%'><input name='Money' class='textbox' type='text' id='Money' value='<%=formatnumber(Money,2,-1)%>' size='10'>
      元</td>
    </tr>
    <tr class='tdbg'> 
      <td width='20%' class="clefttitle"><strong>充值卡点数、资金或有效期：</strong><br>
        购买人可以得到的点数、资金、有效期和积分      </td>
      <td width='60%'><input name='ValidNum' value="<%=ValidNum%>" class="textbox" type='text' id='ValidNum' size='10' maxlength='10'>
        <select name='ValidUnit' id='ValidUnit' class='textbox'>
    ...
    <tr class='tdbg'>
      <td width='20%' class="clefttitle"><strong>允许使用此充值卡的用户组：</strong><br>
	  不限制请留空或全部选中。
     </td>
      <td width='60%'><%=KS.GetUserGroup_CheckBox("AllowGroupID",allowgroupid,5)%></td>
    </tr>
    <tr class='tdbg'>
      <td width='20%' class="clefttitle"><strong>充值后自动归入的用户组：</strong><br>
     </td>
      <td width='60%'><select name="GroupID" id="GroupID" class='textbox'>
	  <option value='0'>---保持原有用户组---</option>
    ...
    <tr class='tdbg'>
      <td width='20%' class="clefttitle"><strong>到期后自动归入的用户组：</strong>
     </td>
      <td width='60%'><select name="ExpireGroupID" id="ExpireGroupID" class='textbox'>
	  <option value='0'>---保持原有用户组---</option>
    ...
    <tr class='tdbg'>
      <td width='20%' class="clefttitle"><strong>充值截止期限：</strong><br>
      购买人必须在此日期前进行充值，否则自动失效</td>
      <td width='60%' class='tdbg'><input name='EndDate' class='textbox' type='text' id='EndDate' value='<%=EndDate%>' size='20'></td>
    </tr>
	<tr class='tdbg'<%if cardtype=1 then response.write " style='display:none'"%>>
      <td width='20%' class="clefttitle"><strong>是否出售：</strong><br>
      添加新充值卡，请选项未出售</td>
	...
	<tr class='tdbg'<%if cardtype=1 then response.write " style='display:none'"%>>
      <td width='20%' class="clefttitle"><strong>是否使用：</strong><br>
      添加新充值卡，请选项未使用</td>
