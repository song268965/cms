...
Class Admin_User
		...
		Private UserID,UserSearch,Keyword,strField,Page,sql,FoundErr,RS,TotalPut,I
       ...
       Sub Kesion()
			...
			Response.Write"	<div id='mt' class=''>"
			Response.Write "<span style='padding-right:20px;line-height:35px;float:right'>共有<font color=red>" & Conn.Execute("select count(1) From KS_User")(0) & "</font>位会员</span>"
			Response.Write "<span id='mtl'>快速查找用户：</span><span><a href=""KS.User.asp?Action=Search"">搜索用户</a></span>&nbsp;|&nbsp;<a href=""?UserSearch=12"">所有用户</a>&nbsp;|&nbsp;<a href=""?UserSearch=13"" style='color:red'>在线用户</a>&nbsp;|&nbsp;<a href=""?UserSearch=1"">被锁住</a>&nbsp;|&nbsp;<a href=""?UserSearch=2"">管理员</a>&nbsp;|&nbsp;<a href=""?UserSearch=3"">待审批</a>&nbsp;|&nbsp;<a href=""?UserSearch=4"">待邮件激活</a>&nbsp;|&nbsp;<a href=""?UserSearch=5"">24小时内登录</a>&nbsp;|&nbsp;<a href=""?UserSearch=6"">24小时内注册</a>&nbsp;|&nbsp;<a href=""?UserSearch=15"">已过期</a>&nbsp;|&nbsp;<a href=""?UserSearch=16"">未过期</a>"
			Response.Write "&nbsp;|&nbsp;<a href='?action=updateGroup' onclick=""return(confirm('此操作将通过在线充值卡获得的VIP用户组到期后更新为免费用户名,并且此操作不可逆，确认操作吗？'))"" title="""">更新过期的用户组</a>"
			Response.write "&nbsp;|&nbsp;<a href=""?action=countarea"">按省份统计</a>"
			Response.Write	" </div>"
		...
		Case "countarea"
		    Call countarea()
		Case "Add"         
		 If Not KS.ReturnPowerResult(0, "KMUA100027") Then
			 Call KS.ReturnErr(1, "")
	     Else
		   call AddUser()
		 end if
		...
		Case "Modify"        
			If Not KS.ReturnPowerResult(0, "KMUA100022") Then
			 Call KS.ReturnErr(1, "")
			Else
			 call Modify()
			End If
		Case "SaveModify"    
		 ...
		 End If
		Case "Del"   
			If Not KS.ReturnPowerResult(0, "KMUA100024") Then
				 Call KS.ReturnErr(1, "")
			Else
		   call DelUser()
		    End If
		Case "Lock"	       
		    If Not KS.ReturnPowerResult(0, "KMUA100023") Then
				 Call KS.ReturnErr(1, "")
			Else
		   call locked()
		   End If
		...
		Case "Move"	       
		  If Not KS.ReturnPowerResult(0, "KMUA100022") Then
		   ...
		   call MoveUser()
		    End If
		Case "AddMoney"	     
		 If Not KS.ReturnPowerResult(0, "KMUA100025") Then
			 Call KS.ReturnErr(1, "")
		 else
		 call AddMoney()
		  End If
		Case "AddScore"    
		 If Not KS.ReturnPowerResult(0, "KMUA100026") Then
			 Call KS.ReturnErr(1, "")
		 else
		   call AddScore()
		  End If
		...
		Case "ShowDetail"	
		  If Not KS.ReturnPowerResult(0, "KMUA100021") Then
			 Call KS.ReturnErr(1, "")
		  else
		  Call ShowDetail()
		  end if
		...
		Response.Write "<div class='footerTable'><div style=""clear:both;text-align:center;color:#003300"">-----------------------------------------------------------------------------------------------------------</div>"
		...
		Sub Main()
		    Dim O,OrderArray:OrderArray=array("默认排序|userid|1","用户UserId↑|userid|0","可用资金↓|money|1","可用" & KS.Setting(45) &"↓|point|1","总积分↓|score|1","最新登录时间↓|LastLoginTime|1","总登录次数↓|LoginTimes|1")
			 O=KS.ChkClng(KS.S("o"))

		   %>
           <div class="tabTitle">用户管理</div>
           <div class="tabs_header mt0">
                <span style="float:right">
				 ...
				 <select onchange="location.href=this.value">
				  <%
				   dim nn
				   for nn=0 to ubound(OrderArray)
					dim orderarr:orderarr=split(OrderArray(nn),"|")
					 if o=nn then
					  ks.echo "<option selected value='KS.User.asp?o=" & nn &"&" & KS.QueryParam("o") &"'>" & orderarr(0) & "</option>"
					 else
					   ks.echo "<option value='KS.User.asp?o=" & nn &"&" & KS.QueryParam("o") &"'>" & orderarr(0) & "</option>"
					 end if
				  next
				  %>
                  </select>
                ...
                <ul class="tabs">
                <li <%if ks.isnul(ks.s("GroupID")) then response.write " class='active'"%>><a href="KS.User.asp"><span>所有会员</span></a></li>
                <%
				KS.LoadUserGroup()
				Dim Node
		        For Each Node In Application(KS.SiteSN&"_UserGroup").DocumentElement.SelectNodes("row[@tj=1]")
				 Response.Write "<li"
				 If KS.S("GroupID")=Node.SelectSingleNode("@id").text then Response.Write " class='active'"
				 Response.Write "><a href='?GroupID=" & Node.SelectSingleNode("@id").text &"'><span>" & Node.SelectSingleNode("@groupname").text &"</span></a></li>" &vbcrlf
				Next
				%>
				<form action="KS.User.asp" method="post" name="form1">
				 查找会员：<input type="text" name="keyword" class="textbox" value="<%=KS.S("Keyword")%>" /> 条件：<select name="tj">
				 <option value="0"<%if KS.S("Tj")="0" then response.write " selected"%>>会员名称</option>
				 <option value="1"<%if KS.S("Tj")="1" then response.write " selected"%>>真实姓名</option>
				 <option value="2"<%if KS.S("Tj")="2" then response.write " selected"%>>手机号码</option>
				 <option value="3"<%if KS.S("Tj")="3" then response.write " selected"%>>电子邮箱</option>
           ...
           <%
		    Dim GroupID:GroupID=KS.ChkClng(KS.G("GroupID"))
				...
				strGuide="<div class='pageCont'><table width='100%' align='center' border='0' cellpadding='0' cellspacing='1'><tr class='list'><td align='center' height='25'>&nbsp;"
				...
				Case 11
					...
					if UserID>0 then
						...
						if KS.ChkClng(request("GroupID"))>0 then
							if strsql="" then
						...
						if request("mobile")<>"" then
							if strsql="" then
								strsql=strsql & " mobile like '%"&request("mobile")&"%'"
							else
								strsql=strsql & " and mobile like '%"&request("mobile")&"%'"
							end if
						end if
		            '======特殊搜索=======
						dim Tsqlstr
				...
				If GroupID<>0 Then Param=Param & " and groupid=" & GroupID
				If KS.C("SuperTF")<>"1" then Param=Param & " and (groupid<>1 or username='" & KS.C("AdminName") & "')"
				If KS.S("keyword")<>"" Then 
				  select case KS.ChkClng(request("tj"))
				    case 0
				     Param=Param & " and username like '%" & KS.S("keyword") &"%'"
				    case 1
				     Param=Param & " and realname like '%" & KS.S("keyword") &"%'"
				    case 2
				     Param=Param & " and mobile like '%" & KS.S("keyword") &"%'"
				    case 3
				     Param=Param & " and email like '%" & KS.S("keyword") &"%'"
				  end select
				End If
			   ...
			   End If
				 ...
				 If DataBaseType=1 Then
					...
					Set Cmd=Nothing
				 Else
						SQL=KS.GetPageSQL("KS_User",OrderField,MaxPerPage,Page,OrderType,Param,sSQL)
				...
				if rs.eof and rs.bof then
					TotalPut=0
					Response.Write strGuide & "共找到 <font color=#ff6600>0</font> 个用户&nbsp;&nbsp;&nbsp;&nbsp;</td></tr><tr class='list'><td colspan=10 align='center' height='50'>没有找到符合条件的用户！</td></tr></table></div>"
        ...
        <div class="pageCont">
		  ...
		  <table width="100%" style="" border="0" align="center" cellspacing="0" cellpadding="0">
				  <tr class='sort'>
					...
					<td align="center" width="140"><%=KS.Setting(45)%>/天数</td>
			 ...
			 %>
				...
				select case SQL(9,i)
				   ...
				   case else
					...
					<td class='splittd' align="center"><%
				If KS.C("SuperTF")<>1 and SQL(2,i)=1 and  SQL(1,i)<>KS.C("AdminName") then
					 response.write "---"
				else	
				  If KS.ReturnPowerResult(0, "KMUA100022") Then
					Response.Write "<a href='KS.User.asp?Action=Modify&UserID=" & SQL(0,i) & "' class='setA'>改</a>&nbsp;"
				  End If
					if SQL(2,i)<>1 then  '管理员判断
					   If KS.ReturnPowerResult(0, "KMUA100023") Then
						if SQL(9,i)=0 then
							Response.Write "<a href='KS.User.asp?Action=Lock&UserID=" & SQL(0,i) & "' class='setA'>锁</a>&nbsp;"
						else
							Response.Write "<a href='KS.User.asp?Action=UnLock&UserID=" & SQL(0,i) & "' class='setA'>解</a>&nbsp;"
						end if
					  End If
					  If KS.ReturnPowerResult(0, "KMUA100024") Then
						Response.Write "<a href='KS.User.asp?Action=Del&UserID=" & SQL(0,i) & "' onClick='return confirm(""确定要删除此用户吗？"");' class='setA'>删</a>|"
					  End If
					  If KS.ReturnPowerResult(0, "KMUA100025") Then
						 If SQL(3,i)=1 Then
						   Response.Write "<a href='KS.User.asp?Action=AddMoney&UserID=" & SQL(0,i) & "' class='setA'>续" & KS.Setting(45) & "</a>"
						 ElseIf SQL(3,I)=2 Then
						   Response.Write "<a href='KS.User.asp?Action=AddMoney&UserID=" & SQL(0,i) & "' class='setA'>续天数</a>"
						 End IF
					  End If
					end if
					If KS.ReturnPowerResult(0, "KMUA100025") Then
					%> |<a href='KS.User.asp?Action=AddZJ&UserID=<%=SQL(0,I)%>' class="setA">续费</a>
					<%end if
					If KS.ReturnPowerResult(0, "KMUA100026") Then
					%>
					|<a href='KS.User.asp?Action=AddScore&UserID=<%=SQL(0,I)%>' class="setA">加积分</a>
					|<a href='KS.User.asp?Action=UserLogin&UserID=<%=SQL(0,I)%>' title="登录该用户的会员中心" target="_blank" class="setA">进入</a>
				<%end if
		  ...
		  <tr class='list' onMouseOver="this.className='list'" onMouseOut="this.className='list'">
			<td colspan=10 height="30" class="operatingBox">&nbsp;<label><input name="chkAll" type="checkbox" id="chkAll" onclick=CheckAll(this.form) value="checkbox">
					  ...
					  &nbsp;<input type="submit" name="Submit" class='button' value=" 执 行 " >&nbsp;<input class='button' type="button" name="Submit" value="发送邮件" onclick="this.form.action='KS.UserMail.asp?InceptType=2';this.form.submit()" >&nbsp;<input class='button' type="button" name="Submit" value="发送站内消息" onclick="this.form.action='KS.UserMessage.asp?action=new';this.form.submit()" >&nbsp;<input class='button' type="button" name="Submit" value="发送手机短信" onclick="this.form.action='KS.UserMessage.asp?action=sms';this.form.submit()" > </td>
		  </tr></form>
		<tr valign=middle class='list' onMouseOver="this.className='list'" onMouseOut="this.className='list'">
		...
		Sub UserLogin()
		  ...
		  If Not RS.Eof Then
		     ...
		     If EnabledSubDomain Then
			  ...
			  End If
				 Response.Cookies(KS.SiteSn)("UserID") = RS("UserID")
				 Response.Cookies(KS.SiteSn)("UserName") = RS("UserName")
				 Response.Cookies(KS.SiteSn)("Password") = RS("Password")
				 Response.Cookies(KS.SiteSN)("RndPassword")= RS("RndPassword")&""
				 Response.Cookies(KS.SiteSN)("GroupID")= RS("GroupID")
		  End If
		  RS.Close
		  Set RS=Nothing
		...
		Sub ShowSearch()
		%>
        <div class="pageCont2 mb0" style="border-radius:5px 5px 0 0;">
		<div class="tabTitle">高级查询</div>
		...
		<tr class="tdbg">
			<td class="clefttitle" align="right"><strong>手机号包含:</strong></td>
			<td><input class="textbox" size="45" name="Mobile" type=text></td>
		</tr>
		<tr class="tdbg">
		...
		<!--特殊搜索结束-->
		<tr class="tdbg">
		  <td width="100%" colspan="2" ><input name="submit" class='button' type=submit value="   搜  索   "></td>
		</tr>
		...
		<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
			...
			<TR class="tdbg"> 
			  ...
			  <TD height="25"><select name="GroupID" id="GroupID" class='textbox' onchange="location.href='KS.User.asp?action=Add&amp;GroupID='+this.value;">
                <%=KS.GetUserGroup_Option(GroupID)%>
              </select>
			   论坛头衔
			  <select name="clubgradeid" class="textbox">
			  <%KS.LoadAskGrade
			  ...
			  问吧头衔
			  <select name="gradeid" class="textbox">
			  <%
			   set xml=Application(KS.SiteSN&"_AskGrade")
			   If IsObject(XML) Then
...
锁定</TD>
			...
			<TR  class="tdbg"> 
				...
				<TD><Select id=Question name=Question class="textbox">
...
无限期</TD>
						   ...
						   For K=0 TO Ubound(SQL,2)
							 If KS.FoundInArr(FieldsList,SQL(0,k),",") Then
							  ...
							  Case 10
								   ...
								   if KS.ChkClng(SQL(10,K))<>0 then
									 InputStr=InputStr & "<script>setTimeout(""var editor" & SQL(2,K) &" = " & GetEditorTag() &".getEditor('" & SQL(2,K) &"',{toolbars:[" & GetEditorToolBar(SQL(7,K)) &"],maximumWords:" &KS.ChkClng(SQL(10,K)) & "});"",10);</script>"
									Else
									 InputStr=InputStr & "<script>setTimeout(""var editor" & SQL(2,K) &" = " & GetEditorTag() &".getEditor('" & SQL(2,K) &"',{toolbars:[" & GetEditorToolBar(SQL(7,K)) &"],wordCount:false});"",10);</script>"
									End If
		...
		sub SaveAdd()
		    dim UserID,UserName,RealName,Password,PwdConfirm,Question,Answer,Sex,Email,HomePage,QQ,MSN,GroupID,locked,DataCount,ChargeType,Point,Money,BeginDate,Edays,province,county,clubgradeid,gradeid,GradeTitle
			dim rsUser,sqlUser
       ...
       End If
		...
		<div class="tabTitle">修改注册用户信息</div>
...
无限期</TD>
						   ...
						   For K=0 TO Ubound(SQL,2)
							 If KS.FoundInArr(FieldsList,SQL(0,k),",") Then
							  ...
							  If lcase(replace(SQL(2,K),"&",""))="provincecity" Then
							  	 InputStr="<script>try{setCookie(""pid"",'" & rsuser("province") & "');setCookie(""cid"",'" & rsuser("City") & "');}catch(e){}</script>" & vbcrlf
						         ...
						         If rsUser("City")<>"" And Not ISNull(rsUser("City")) Then
								  ...
								  If rsUser("County")<>"" And Not ISNull(rsUser("County")) Then
								  InputStr=InputStr & "$('#County').val('" & rsUser("County") & "');" &Vbcrlf
						         end if
							  ...
							  Case 10
									...
									InputStr=InputStr & "<script id=""" & SQL(2,K) &""" name=""" & SQL(2,K) &""" type=""text/plain"" style=""width:" & SQL(4,K) &"px;height:" & SQL(5,K) & "px;"">" &H_Value & "</script>"
								   if KS.ChkClng(SQL(10,K))<>0 then
									 InputStr=InputStr & "<script>setTimeout(""var editor" & SQL(2,K) &" = " & GetEditorTag() &".getEditor('" & SQL(2,K) &"',{toolbars:[" & GetEditorToolBar(SQL(7,K)) &"],maximumWords:" &KS.ChkClng(SQL(10,K)) & "});"",10);</script>"
									Else
									 InputStr=InputStr & "<script>setTimeout(""var editor" & SQL(2,K) &" = " & GetEditorTag() &".getEditor('" & SQL(2,K) &"',{toolbars:[" & GetEditorToolBar(SQL(7,K)) &"],wordCount:false});"",10);</script>"
		...
		<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
			...
			<TR class="tdbg"> 
				...
				<strong>用户名称：</strong><%=rsUser("UserName")%>&nbsp;<strong>用户组:</strong><%=KS.U_G(rsUser("groupid"),"groupname")%>&nbsp;<strong>注册时间：</strong><%=rsUser("RegDate")%>&nbsp;&nbsp;<strong>推荐人：</strong><%=rsUser("AllianceUser")%>&nbsp;&nbsp;<strong>可用资金：</strong><%=rsUser("money")%>元&nbsp;&nbsp;<strong>可用点券：</strong><%=rsUser("point")%>点&nbsp;&nbsp;<strong>总积分：</strong><%=rsUser("score")%>分&nbsp;<strong>已消费积分：</strong><%=rsUser("scorehasuse")%>分&nbsp;<strong>可用积分:</strong><%=rsuser("score")-rsUser("scorehasuse")%>分
				</TD>
			</TR>
	
			<tr class="tdbg">
			  <td colspan="4" class="userlist">
			  <style>
			  .userlist{}
			  .userlist dd{padding:0px;margin:0px;margin-bottom:10px;}
			  .userlist dd img{max-width:250px;}
			  .userlist div.left{font-weight:bold;}
			  </style>
			  <%
			            
					Dim Template:Template=LFCls.GetSingleFieldValue("Select top 1 Template From KS_UserForm Where ID=" & KS.ChkClng(KS.U_G(GroupID,"formid")))
						   Dim FieldsList:FieldsList=LFCls.GetSingleFieldValue("Select FormField From KS_UserForm Where ID=" & KS.ChkClng(KS.U_G(GroupID,"formid")))
						   Dim RS:Set RS=Server.CreateObject("ADODB.RECORDSET")
						   ...
						   For K=0 TO Ubound(SQL,2)
							 If KS.FoundInArr(FieldsList,SQL(0,k),",") Then
							  ...
							  if right(lcase(InputStr),4)=".png" or right(lcase(InputStr),4)=".jpg" or right(lcase(InputStr),4)=".gif" then
							   InputStr="<a href='" & InputStr &"' target='_blank' title='文件名：" & InputStr &"'><img src='" & InputStr &"'></a>"
							  elseif SQL(1,K)=9 and len(inputstr)>10 then
							   InputStr="<a href='" & InputStr &"' target='_blank' title='文件名：" & InputStr &"'>" & InputStr &"</a>"
							  end if 
						   ...
						   Next
							...
							Template=Replace(Template,"<font color=red> * </font>","")
	...
	sub AddScore()
		...
		%>
        <div class="pageCont pt10" style="border-radius:5px 5px 0 0;">
		<table width="100%" style="margin-top:10px" border="0" align="center" cellpadding="3" cellspacing="1" class="ctable">
		<FORM name="myform" action="KS.User.asp?ComeUrl=<%=Server.URLEncode(Request.ServerVariables("HTTP_REFERER"))%>" method="post">
			...
			<TR  class='tdbg'>
			  ...
			  <TD> <input name="Score" class="textbox" type="text" id="Score" value="100" size="10" maxlength="10">
			  分</TD>
			</TR>
			<TR class='tdbg'>
			  <TD height="28" align="right" class="clefttitle"><strong>请输入原因：</strong></TD>
			  <TD> <input name="Reason" class="textbox" type="text" id="Reason" value="积分奖励" size="55"></TD>
			</TR>
			<TR  class='tdbg'>
	 ...
	 sub AddMoney()
		...
		<FORM name="myform" action="KS.User.asp?ComeUrl=<%=Server.URLEncode(Request.ServerVariables("HTTP_REFERER"))%>" method="post">
			...
			<TR  class='tdbg'>
			  ...
			  <TD> <input name="Point"  class="textbox" type="text" id="Point" value="100" size="10" maxlength="10">
			  <%=KS.Setting(46)%></TD>
			</TR>
			<%else%>
			<TR class='tdbg'>
			...
			<tr class='tdbg'>
			  ...
			  <td>
			  <input name="Edays" class="textbox" type="text" id="Edays" value="100" size="10" maxlength="10">
			  天<br />
...
若目前用户已经过了有效期，则有效期从续费之日起重新计数。</td>
			...
			<tr class='tdbg'>
			  ...
			  <td>
			  <input name="Money" class="textbox" type="text" id="Money" value="100" size="10" maxlength="10"> 元人民币
			  <font color=red>
			  <%if rsUser("ChargeType")=1 then %>
			...
			<TR class='tdbg'>
			  <TD height="28" align="right" class="clefttitle"><strong>请输入原因：</strong></TD>
			  <TD> <input name="Reason" class="textbox" type="text" id="Reason" value="<%If rsUser("ChargeType")=1 Then Response.Write "续" & KS.Setting(45) & "操作" Else Response.Write "续有效天数操作"%>" size="55"></TD>
			</TR>
			<TR class='tdbg'> 
		...
		sub SaveModify()
			dim UserID,UserName,RealName,Password,PwdConfirm,Question,Answer,Sex,Email,HomePage,QQ,MSN,GroupID,locked,DataCount,ChargeType,Point,Money,BeginDate,Edays,province,county,fax,UserFace,clubgradeid,gradeid,GradeTitle
			dim rsUser,sqlUser
			dim OfficeTel,Address,Sign
			dim IDCard,BirthDay,City,Zip,ICQ,UC
			Dim SchoolAge,UserWorking,HomeTel,Mobile
	...
	End If	
	   ...
	   End If
			...
			'-----------------------------------------------------------------
			If API_Enable Then
				call uc_user_edit(rsUser("UserName") ,"" ,"",Email,1,"","")
			End If
		...
		sub SaveAddMoney()
			...
			Else
			Call KS.Alert("操作成功!","user/KS.User.asp")
			End IF
		end sub
		...
		<FORM name="myform" action="KS.User.asp?ComeUrl=<%=Server.URLEncode(Request.ServerVariables("HTTP_REFERER"))%>" method="post">
			...
			<TR class='tdbg'>
			  ...
			  <TD> <input name="Remark" class="textbox" type="text" id="Remark" value="银行汇款" size="55"></TD>
			</TR>
			<TR class='tdbg'> 
		...
		Sub updateGroup()
		   Dim Param:Param="chargetype=2 and datediff(" & DataPart_d & ",beginDate," & SqlNowString & ")>edays"
		   Dim RS:Set RS=Server.CreateObject("adodb.recordset")
		   rs.open "select * From KS_User Where " & Param,conn,1,1
		   Do While Not RS.Eof
		   
		             '判断上一次是不是通过充值卡充值
					 Dim UpdateField:UpdateField=""
						If RS("UserCardID")<>0 Then
						  Dim RSCard,ValidUnit,ExpireGroupID
						  Set RSCard=Conn.Execute("Select top 1 * From KS_UserCard Where ID=" & RS("UserCardID"))
						  If Not RSCard.Eof Then
						     ValidUnit=RSCard("ValidUnit")
							 ExpireGroupID=RSCard("ExpireGroupID")
							 If ValidUnit=1 Then                      '点券
							   If RS("Point")<=0 And ExpireGroupID<>0 Then
							     UpdateField="GroupID=" & ExpireGroupID & ",UserCardID=0,ChargeType=" & KS.ChkClng(KS.U_G(ExpireGroupID,"chargetype"))
							   End If
							 ElseIf ValidUnit=2 Then                   '有效天数
							   If RS("Edays")-DateDiff("D",RS("BeginDate"),now())<=0 And ExpireGroupID<>0 Then
							     UpdateField="GroupID=" & ExpireGroupID & ",UserCardID=0,ChargeType=" & KS.ChkClng(KS.U_G(ExpireGroupID,"chargetype"))
							   End If 
							 ElseIf ValidUnit=3 Then                  '资金
							   If RS("Money")<=0 And ExpireGroupID<>0 Then
							     UpdateField="GroupID=" & ExpireGroupID & ",UserCardID=0,ChargeType=" & KS.ChkClng(KS.U_G(ExpireGroupID,"chargetype"))
							   End If
							 End If
						  End If
						...
						End If
		       If UpdateField<>"" Then
			     Conn.Execute("Update KS_User Set " & UpdateField & " Where UserId=" & RS("USERID"))
			   End If
		   RS.MoveNext
		   Loop
		   RS.Close
		   Set RS=Nothing
		   KS.Alert "恭喜，更新完毕!","user/ks.user.asp"
		End Sub
		...
		<div class="tabTitle">	
			<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0">
					  <tr>
						<td  align="center">各省份网站注册会员分布情况</td>
    ...
    <div class="votemain">
        <div class="votemainbox">
        
        <div id="container" style="margin:0 auto;width:95%;height:700px"></div>
        
        <script type="text/javascript">
       (function basic_pie(container) {
      var graph;
      graph = Flotr.draw(container, [
        <%dim rs:set rs=conn.execute("select id,city from KS_Province Where Parentid=0")
        Do While NOt RS.EOF 
         If Conn.Execute("select count(1) From KS_User Where Province='" & RS(1) &"'")(0)>0 Then%>
        { data : [[0, <%=Conn.Execute("select count(1) From KS_User Where Province='" & RS(1) &"'")(0)%>]], label : '<%=RS(1)%>' },
        <%
         End If
         RS.MoveNext
        Loop
