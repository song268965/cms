...
Class Admin_UserForm
		...
		Sub createtemplate()
				...
				f=KS.ChkClng(KS.G("f"))
				Call KS.CreateFieldXML(101," and FieldID in(" & FieldID & ")")
		...
		Sub UserFormList()			
			...
			<script language="javascript">
				...
				function UserFormAdd()
				{
					...
					window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=会员系统 >> 会员表单管理 >> <font color=red>新增会员表单</font>&ButtonSymbol=GO';
				}
				function EditUserForm(id)
				{
					location="KS.UserForm.asp?Action=Edit&ID="+id;
					window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=会员系统 >> 会员表单管理 >> <font color=red>编辑会员表单</font>&ButtonSymbol=GOSave';
				}
			...
			.Write "<li class='parent' onClick=""UserFormAdd();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加会员表单</span></li>"
			.Write "<li class='parent' onClick=""UserFormControl(1);""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>编辑会员表单</span></li>"
			.Write "<li class='parent' onClick=""UserFormControl(2);""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除会员表单</span></li>"
			.Write "</ul>"
			.Write "<div class='pageCont2'>"
			.Write "<div class='tabTitle'>会员表单管理</div>"
			.Write ("<form name='myform' method='Post' action='?'>")
	        .Write ("<input type='hidden' name='action' id='action' value='" & Action & "'>")
			.Write "<table width='100%' border='0' cellspacing='0' cellpadding='0'>"
			.Write "        <tr>"
			.Write "          <td class=""sort"" width='35' style=""text-align:center"">选择</td>"
	        ...
	        .Write ("<tr><td width='180' class='operatingBox'><div><b>选择：</b><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a> </div>")
	        .Write ("</td>")
	        .Write ("<td class='operatingBox'><select onchange='set(this.value)' name='setattribute' class='h30'><option value=0>快速选项...</option><option value='1'>编辑会员表单</option><option value='2'>执行删除</option></select></td>")
			...
			Sub showContent()
			   With Response
					Do While Not RS.EOF
			          ...
			          .Write "<td class='splittd' style=""text-align:center""><input name='id'  onclick=""chk_iddiv('" &RS("ID") & "')"" type='checkbox' id='c"& RS("ID") & "' value='" &RS("ID") & "'></td>"
					  ...
					  .Write "  <td class='splittd' align='center'><a href='javascript:EditUserForm(" & RS("ID") & ")' class='setA'>修改</a>|<a href='javascript:DelUserForm(" & RS("id") & ")' class='setA'>删除</a></td>"
					  .Write "</tr>"
					  I = I + 1
			...
			.Write "      <td height='30' nowrap><input name='FormName' type='text' size='50' id='FormName' value='" & FormName & "' class='textbox'></b>*(如：企业注册填写表单) </td>" & vbCrLf
			.Write "    </tr>"
			.Write "    <tr class='tdbg'>" & vbCrLf
			.Write "      <td class='clefttitle' align='right'><strong>选择字段：</strong></td>" & vbCrLf
			.Write "      <td height='30' nowrap>"
			.Write "      <table border='0' wdith='100%' cellspacing='0' cellpadding='0'>"
			.Write "        <tr>"
			.Write "         <td align='center' class='sort'>选 择</td>"
			...
			.Write "         <td align='center' class='sort'>注册显示</td>"
			...
			Do While Not RSF.Eof
			 .Write "<tr>"
			 .Write "<td class='splittd' style=""text-align:center"">"
			 ...
			 .Write "<td class='splittd' style=""text-align:center"">" & RSF("orderid") & "</td>"
			 .Write "</tr>"
			...
			.Write "    <tr class='tdbg'>" & vbCrLf
			.Write "      <td width='160' class='clefttitle' align='right'><strong>3G表单模板：</strong><br/><font color=blue>(<label><input type='checkbox' name='autoform' id='autoform2' value='1' onclick='LoadTemplate(2)'/>自动生成3G表单模板</label>)</font></td>" & vbCrLf
			.Write "      <td height='30' nowrap><textarea name='WapTemplate' id='WapTemplate' cols='80' rows='10'>" & Server.htmlencode(WapTemplate) & "</textarea></td>" & vbCrLf
			.Write "    </tr>"
			End If
			.Write "    <tr class='tdbg'>" & vbCrLf
			...
			Sub UserFormAddSave()
			    Dim FormName:FormName = KS.G("FormName")
				 ...
				 Response.Write ("<script> if (confirm('会员表单增加成功,继续添加吗?')) { location.href='?Action=Add';} else{location.href='?Page=" & Page &"';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=会员系统 >> 会员表单管理&ButtonSymbol=Disabled';}</script>")
				 Else
				 Response.Write ("<Script>alert('恭喜，表单修改成功!');location.href='?Page=" & Page &"';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=会员系统 >> 会员表单管理&ButtonSymbol=Disabled';</script>")
				 End If
