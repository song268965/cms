...
Class Admin_UserMail
		...
		Public Sub Kesion()
			...
			Response.Write"<div class=""topdashed quickLink"" style=""text-align:left"">管理导航:<a href='KS.UserMail.asp?action=MailSub'>订阅邮件管理</a> |  <a href='KS.UserMail.asp'>发送邮件</a> | <a href='?action=MailOut'>导出邮件</a>"
			Response.Write	"</div>"
		...
		Sub MailSub()
		 ...
		 MaxPerPage=20
		 With KS
		 .echo "<div class='tabTitle'>订阅邮件管理</div>"
		...
		.echo "  </tr>"
		 ...
		 .echo ("<tr><td width='170'><div class='operatingBox'><b>选择：</b><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a> </div>")
		 .echo ("</td>")
	     .echo ("<td colspan=6  class='operatingBox'><input type=""submit"" onclick=""$('#action').val('DelMail');return(confirm('此操作不可逆,确定删除选中的记录吗？'))"" value=""删除选中的记录""  class=""button""> <input type=""submit"" onclick=""$('#action').val('active');$('#v').val(1)"" value=""批量激活""  class=""button""> <input type=""submit"" onclick=""$('#action').val('active');$('#v').val(0)"" value=""批量锁定""  class=""button""></td>")
...
function CheckForm(){
  if (document.myform.title.value==''){
     top.$.dialog.alert('邮件主题不能为空！',function(){
     document.myform.title.focus();
	 });
     return false;
  }
  if (parseInt($("input[name=sendtype]:checked").val())==1){
	   if (<%=GetEditorContent("Content")%>==false )
		{
		  top.$.dialog.alert("邮件内容不能为空！",function(){
...
<div class="tabTitle">发送邮件</div>
  <table class="ctable" cellSpacing=1 cellPadding=2 width="100%" align=center border=0>
        ...
        <table>
          <tr>
            <td colspan="2" style="color:red;font-weight:bold">
              <Input onClick="$('#showgroup').hide();" type=radio<%if InceptType="-1" then response.write " CHECKED"%> value="-1" name=InceptType> 所有订阅邮箱，共有 <font color=green><%=Conn.Execute("select count(1) from ks_usermail where activetf=1")(0)%></font> 个有效订阅<span style='color:#999999'>（通过<a href='../plus/mailsub.asp' target='_blank'>/plus/mailsub.asp</a>订阅的用户）</span></td>
          </tr>
          <tr>
            <td>
              <Input onClick="$('#showgroup').hide();"  type=radio<%if InceptType="0" then response.write " CHECKED"%> value="0" name=InceptType> 所有会员</td>
            <td></td>
          </tr>
          <tr>
            <td vAlign=top>
              <Input type=radio value="1"  onclick="$('#showgroup').show();" name=InceptType<%if InceptType="1" then response.write " CHECKED"%>> 指定会员组</td>
            <td>
            <div id="showgroup" style="display:none">
			<%=KS.GetUserGroup_CheckBox("GroupID",0,4)%>
            </div>
            </td>
          </tr>
          <tr>
            <td vAlign=top>
              <Input type=radio onClick="$('#showgroup').hide();"  value="2" name=InceptType<%if InceptType="2" then response.write " CHECKED"%>> 指定用户名</td>
            <td>
          ...
          <tr>
            <td vAlign=top>
              <Input type=radio value="3" onClick="$('#showgroup').hide();"  name=InceptType<%if InceptType="3" then response.write " CHECKED"%>>              指定Email</td>
            <td>
    ...
    <tr class=tdbg>
      ...
      <td width="85%">
        <Input size="70" name=title id="title" class="textbox" value="邮件订阅服务内容"> 
    ...
    <tr class=tdbg>
	  ...
	  <div id="c0">
	    <table>
		 <tr>
		  <td colspan="2">
	    发送篇数：<input type="text" class="textbox" name="sendnum" value="10" style="width:50px;text-align:center"/> 篇  从所有允许订阅的栏目中选择指定的篇数发送。<br/><br/>
	    限定天数：<input type="text" class="textbox" name="sendday" value="3" style="width:50px;text-align:center"/> 天  不限制请输入0，否则只从指定天数内选择文章发送。
		  </td>
		 </tr>
		 <tr>
		   ...
		   <td>
			...
			<%
				 ...
				 Dim mailtemplate:mailtemplate="<strong>您好，感谢您参与【{$GetSiteName}】网站的订阅服务！</strong><br/>以下是本期根据您感兴趣的栏目为您发送的订阅内容:<br/><br/>{$MailContent}<br/><div style=""text-align:right"">{$GetSiteName}<br/>发送时间：{$SendDate}</div><a href=""{$GetSiteUrl}"" target=""_blank"">点此访问本站</a> | <a onClick=""return(confirm('您确认取消该订阅服务吗？'))"" href=""{$GetCancelUrl}"" target=""_blank"">取消订阅服务</a><br/>"
				 Response.Write EchoEditor("mailtemplate",mailtemplate,"Basic","96%","350px")
	  ...
	  <div id="c1" style="display:none">
	            ...
	            <%
				 ...
				 Response.Write EchoEditor("Content","","Basic","96%","350px")
				%>
    ...
    <tr class=tdbg>
      ...
      <td>附件一.<Input class="textbox" size=30 type="text" id="file1" name="sendfile"> <input class="button"  type='button' name='Submit' value='选择...' onClick="OpenThenSetValue('Include/SelectPic.asp?ChannelID=1&CurrPath=<%=KS.GetUpFilesDir()%>',550,290,window,$('#file1')[0]);"><br /><br/>
	  附件二.<Input class="textbox" size=30 type="text" id="file2" name="sendfile"> <input class="button"  type='button' name='Submit' value='选择...' onClick="OpenThenSetValue('Include/SelectPic.asp?ChannelID=1&CurrPath=<%=KS.GetUpFilesDir()%>',550,290,window,$('#file2')[0]);"><br /><br/>
	  附件三.<Input class="textbox" size=30 type="text" id="file3" name="sendfile"> <input class="button"  type='button' name='Submit' value='选择...' onClick="OpenThenSetValue('Include/SelectPic.asp?ChannelID=1&CurrPath=<%=KS.GetUpFilesDir()%>',550,290,window,$('#file3')[0]);"><br /><br/>
  ...
  <Input id=Submit class="button" type=submit value=" 发 送 " name=Submit>&nbsp; 
		...
		Sub Send()
			...
			End Select
			Call KS.Die("<script>top.$.dialog.alert('操作成功！本次发送"&Numc&"个用户。请继续别的操作。',function(){location.href='" & KS.Setting(3) & KS.Setting(89) &"user/KS.UserMail.asp';});</script>")
		End Sub
...
<Select id=GroupID name=GroupID class="textbox">
  <Option value=0 selected>全部会员</Option>
<%=KS.GetUserGroup_Option(0)%>
</Select> &nbsp;<font color=blue>到</font>&nbsp; 
  <Input id=ExportFileName maxLength=200  class="textbox" size=30 value=<%=KS.Setting(3)%>usermail.mdb name=ExportFileName> 
        <Input type=submit value=开始导出 class="button" name=Submit> </td>
    </tr>
</FORM>
  </table>
<br>
...
<%=KS.GetUserGroup_Option(0)%>
  ...
  <Input id=ExportFileName maxLength=200 size=30 class="textbox" value=<%=KS.Setting(3)%>usermail.txt name=ExportFileName> 
        <Input type=submit value=开始导出 class="button" name=Submit2> </td>
    </tr>
</FORM>
  </table>
<br><br>
