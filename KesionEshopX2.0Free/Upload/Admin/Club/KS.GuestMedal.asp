...
Class Admin_Ask_Class
		...
		<body>
		<div class='topdashed sort menu_top_fixed' style="padding-left:20px;text-align:left"><a href="KS.GuestMedal.asp">论坛勋章设置</a> | <a href="KS.Guestmedal.asp?action=pub">颁发勋章</a> | <a href="KS.GuestMedal.asp?action=clear" onClick="return(confirm('此操作直接清除所有用户的勋章，且不可恢复！确认操作吗?'))">一键重置用户勋章</a></div>
		...
		KS.Die "<script>$.dialog.alert('恭喜，所有用户的勋章已被清除!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
		End Sub
		Sub Pub()
		%>
		...
		<div class="pageCont2">
		<table width="100%" border="0" style="margin-top:3px" align=center cellpadding="3" cellspacing="1" class="otable">
		 <form action="KS.GuestMedal.asp" method=post name="myform" id="myform">
		   <input type="hidden" value="DoPub" name="action">
			...
			<tr class="tdbg">
				<td height="25" width="120" align="right" class="clefttitle pt10">用户类别：</td>
				<td class="pt10">
        ...
        Sub DoPub()
            ...
            If medalid="" Then KS.Die "<script>$.dialog.alert('对不起，您没有选择勋章!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
			...
			End Select
			KS.Die "<script>$.dialog.alert('操作成功！本次颁发"&Numc+1&"个用户!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
		End Sub
		'按所有用户发送
		Sub SaveMsg_0()
			Dim Rs,Sql,i
		...
		Sub Modify()
		 ...
		 If ID=0 Then KS.Die "<script>$.dialog.alert('出错啦!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
		 Set RS=Server.CreateObject("ADODB.RECORDSET")
		 RS.Open "Select * From KS_GuestMedal Where MedalID=" & ID,conn,1,1
		 If RS.Eof And RS.Bof Then
		   RS.Close : Set RS=Nothing
		   KS.Die "<script>$.dialog.alert('勋章不存在!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
		 End If
		%>
		...
		Response.Write ("<script>alert('恭喜，勋章详情修改成功!');location.href='KS.GuestMedal.asp';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr=论坛系统 >> <font color=red>勋章管理</font>';</script>")
		End Sub
		...
		<tr align="center">
			...
			<td class="splittd">
			 <a href="javascript:window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=论坛系统 >> <font color=red>勋章详情设置</font>&ButtonSymbol=GoSave';location.href='?action=modify&id=<%=DataArry(0,i)%>'" class='setA'>详情</a>|<a href="?x=c&id=<%=DataArry(0,i)%>" onClick="return(confirm('确定删除吗?'))" class='setA'>删除</a>
			</td>
		</tr>
		<%
				Next
		...
		<tr align="center">
		   <td class="splittd"></td>
			<td class="splittd" colspan="6" style="text-align:left;padding-left:100px;">
		 ...
		 Select case request("x")
		   case "b"
		       If KS.G("MedalName")="" Then Response.Write "<script>alert('请输入勋章名称!');history.back();</script>":response.end
			    ...
			    If DataBaseType=1 Then  CONN.execute("SET IDENTITY_INSERT [KS_GuestMedal] ON")
				...
				If DataBaseType=1 Then  CONN.execute("SET IDENTITY_INSERT [KS_GuestMedal] Off")
				
				KS.Die "<script>$.dialog.alert('恭喜,勋章添加成功!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
		   case "c"
				conn.execute("Delete from KS_GuestMedal where MedalID="& KS.ChkClng(KS.G("id")))
				KS.Die "<script>$.dialog.alert('恭喜,勋章删除成功!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
		End Select
		...
		Sub saveScore()
			...
			Dim MedalID,MedalName,Score,Ico,clubpostnum,Status
                ...
                For I=0 To Ubound(MedalID)
				 ...
				 End If
			   Next
			   KS.Die "<script>$.dialog.alert('恭喜您！批量更新勋章成功!',function(){location.replace('club/KS.GuestMedal.asp');});</script>"
		End Sub
End Class
%>