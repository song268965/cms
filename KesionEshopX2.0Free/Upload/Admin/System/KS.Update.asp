...
'官方远程文件版本地址
const Kesion_Version_XmlUrl="http://vip.kesion.com/update.aspx" 

'官方远程文件更新列表地址,必须/结束   
const Kesion_Update_FileUrl="http://vip.kesion.com/patch/"   
...
sub LoadRemoteXML()
	  ...
	  xmlObj.load(server.mappath("../../config/version.xml"))
...
<body class="tdbg">
	...
	if xmlObj.readystate=4 and xmlObj.parseError.errorCode=0 Then 
	  ...
	  For Each Node In XMLNode
	    If lcase(Node.SelectSingleNode("authorize").text)="false" Then
			...
			response.write "<br/>2、如需要手工升级，请登录官方<a style='color:red' href='http://vip.kesion.com/user/' target='_blank'><u>商业用户自动中心</u></a>下载补丁包;"
			response.write "<br/>3、对授权有任何疑问，请登录官方网站<a style='color:red' href='http://www.kesion.com/sysq/sqcx.html' target='_blank'><u>授权查询中心</u></a>查询;"
			response.write "</font></td></tr>"
			exit for
	    ElseIf lcase(Node.SelectSingleNode("authorize").text)="expire" Then
		    showupdatebutton=false
			response.write "<tr class='tdbg'>" & vbcrlf
			response.write " <td class='splittd' style='color:blue;font-size:14px;height:60px;line-height:32px' colspan=6><b>温馨提示:</b><br/><font color=#ff6600>1、系统检测到有新的可升级补丁，但发现您已过了升级服务年限，如需继续享受升级服务请联系<a href='http://www.kesion.com' style='color:red' target='_blank'><u>官方</u></a>升级;"
			response.write "<br/>2、若您不需要升级服务且不想再看到此提示，请打开文件【" & KS.Setting(89) & "KS.Update.asp】，将系统检测最新版本的开关设置为false即可，即找到EnabledAutoUpdate=true 将true改为false;"
			response.write "<br/>3、对授权有任何疑问，请登录官方<a style='color:red' href='http://vip.kesion.com/user/' target='_blank'><u>商业用户自动中心</u></a>查询;"
			response.write "</font></td></tr>"
			exit for
		Elseif KS.ChkClng(split(Node.SelectSingleNode("version").text,".")(0))>KS.ChkCLng(split(LocalVersion,".")(0)) then
			...
			response.write "<tr class='tdbg'>" & vbcrlf
			response.write " <td class='splittd' style='color:blue;font-size:14px;height:60px;line-height:22px' colspan=6><strong>温馨提示，官方最新版本为<font color=red>V" & Node.SelectSingleNode("version").text &"</font>，您当前版本为<font color=red>V" & LocalVersion & "</font>,此版本不支持在线升级，建议您及时到<a href='http://www.kesion.com' target='_blank' style='color:blue;'>官方网站</a>下载最新程序并根据升级提示手工升级到此版本。</strong>"
			...
			response.write " <td class='splittd'>补丁号：<font color=red>" & Node.SelectSingleNode("version").text &"</font> 适合版本：<font color=red>" & Node.SelectSingleNode("forversion").text & "</font></td>"& vbcrlf
			...
			response.write " <td class='splittd'>" 
			 if num=1 then
			  ...
			  response.write "<label style='color:#ff6600'>该补丁不支持在线升级，<br/>请到 <a style='color:#ff6600' href='http://www.kesion.com' target='_blank'>http://www.kesion.com</a> 下载补丁包手工升级。</label>"
			  end if
			 else
 ...
 <tr><td style='height:40px;text-align:center'>
   <input type='submit' onClick="return(check())" value=" 开始下载升级文件 " class="button"/> <input type="button" value=" 暂不升级 " class="button" onClick="top.box.close();"/></td></tr>
 <%if num-1>1 then%>
...
<%else%>
 <tr><td style='height:40px;text-align:center'>
   <input type='submit' onClick="top.box.close();" value=" 关闭提示窗口 " class="button"/> </td></tr>
<%end if%>
</table>
</body>
</html>
...
<%
  ...
  Next
  If ErrNum>0 Then
   response.write ("<div style='margin:2px'><font color=blue>发现有文件没有成功下载，下载失败的文件:" &ErrListFile &",建议直接到<a href='http://www.kesion.com' target='_blank'><font color=blue><u>官方网站</u></font></a>下载升级包覆盖!</font></div>")
  Else
    response.write ("<div style='margin:2px'>共成功下载了 <font color=red>" & SuccNum & "</font>个更新文件到临时目录，请按“立即更新”按钮覆盖。</div>")
  End If
  %> 
   
   <div style='height:40px;text-align:center'><input type='submit'  value=" 立即更新 " class="button"/> <input type="button" value=" 暂不升级 " class="button" onClick="top.box.close()"/></div>
</body>
</html>
 <%
end sub
	...
	Next
	 ...
	 if xmlObj.readystate=4 and xmlObj.parseError.errorCode=0 Then 
	   ...
	   If KS.ChkClng(RemoteVersion)<>0 Then
		 ...
		 doc.save(Server.MapPath("../../config/version.xml"))
	   End If
	end if
	 Call InnerHtml("<b><font color=red>恭喜，您已成功的升级到版本 " &RemoteVersion & "！</font></b>")
