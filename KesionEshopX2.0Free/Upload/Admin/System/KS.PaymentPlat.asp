...
Class PaymentPlatCls
             ...
             With KS
		         ...
		         .echo "<script language=""JavaScript"" src=""../../KS_Inc/common.js""></script>" & vbCrLf
	         	 .echo "<script language=""JavaScript"" src=""../../KS_Inc/jQuery.js""></script>" & vbCrLf
					If Not KS.ReturnPowerResult(0, "KMST10003") Then          '检查是否有基本信息设置的权限
		...
		<%
		 ...
		 .echo "<div class='tabTitle'>在线支付平台管理</div>"
         ...
         SqlStr = "SELECT ID,OrderID,PlatName,AccountID,Note,MD5Key,Rate,RateByUser,IsDisabled,IsDefault FROM [KS_PaymentPlat] Where IsShow=1 order by orderid"
		 ...
		 Else
			 For K=0 To Ubound(SQL,2)
			   ...
			   .echo "<td class='splittd' align='center'><input type='hidden' value='" & SQL(0,K) & "' name='id'><input class='textbox' type='text' name='orderid' value='" &SQL(1,K) & "' style='width:36px;text-align:center'></td>"
			   ...
			   End If
			    .echo "</td>"
			    .echo " <td class='splittd' align='center'><a href='?Action=Modify&ID=" & SQL(0,K) &"' class='setA'>修改</a>|"
			   If SQL(8,K)=1 Then
			   	 .echo "<a href='?V=0&Action=Disabled&id=" & SQL(0,K) & "' class='setA'>关闭</a>"
			   Else
			     .echo "<a href='?V=1&Action=Disabled&id=" & SQL(0,K) & "' class='setA'>启用</a>"
			   End If
			    .echo " </td>"
				...
				case 16  .echo "<a href='https://mp.weixin.qq.com/' target='_blank'>申请商户</a>"
			   End Select 
                .echo "</td>"
			    .echo "</tr>"
			Next
		 ...
		 End If
          .echo "<tr>"
		  .echo " <td colspan='8' height='40' class='operatingBox'>&nbsp;&nbsp;"
		...
		<li class=parent onClick="location.href='?ChannelID=1';"><SPAN class=child onMouseOver="this.parentNode.className='parent_border'" onMouseOut="this.parentNode.className='parent'"><i class="icon back"></i>取消返回</SPAN></li></ul>
		<div class="pageCont2">
		<FORM name=myform onsubmit=return(CheckForm()) action="?action=DoModifySave&ID=<%=rs("ID")%>" method=post>
		  <dl class="dtable">
			<dd>
			  <div>平台名称：</div>
			  <Input value="<%=rs("PlatName")%>" Class="textbox" size="50" name="PlatName"> 
			</dd>
			<dd><div>备注说明：</div>
		           <textarea name="Note" cols="70" rows="5" class='textbox'><%=rs("note")%></textarea>
		   </dd>
			<dd>
				 <div>
				 <%
				 if rs("id")=16 then
				   response.write "微信支付分配的商户号mch_id"
				 else
				   response.write "支付编号"
				 end if
				 %>
				 ：</div>
				<Input size=50 id="AccountID" class="textbox" name="AccountID" maxlength="50" value="<%=rs("AccountID")%>"> <span>请填入您在线支付平台申请的商户编号</span>
			</dd>
			<dd>
			<%if rs("id")=16 then%>
			   <div>微信分配的公众账号 AppId：</div>
			   <Input size=50 class="textbox" name="appid" maxlength="30" value="<%=split(rs("MD5Key")&"|||","|")(0)%>">
			   <span class="tips">在微信公众平台中 “开发者中心”栏目可以查看到 </span>
			   <div>微信分配的公众账号 AppSecret：</div>
			   <Input size=50 class="textbox" name="appsecret" maxlength="40" value="<%=split(rs("MD5Key")&"|||","|")(1)%>">
			   <span class="tips">在微信公众平台中 “开发者中心”栏目可以查看到 </span>
			   <div>API密钥（PaySignKey）：</div>
			   <Input size=50 class="textbox" name="PaySignKey" maxlength="32" value="<%=split(rs("MD5Key")&"|||","|")(2)%>">
			   <span class="tips">微信商户平台“账户中心”--“API安全”--”设置API密钥“，只能修改不能查看 </span>
			<%else%>
					  <div>支付密钥：</div>

						<Input size=50 class="textbox" name="MD5Key" value="<%=rs("MD5Key")%>">
						
						 <span>请填入您在上述在线支付平台中设置的MD5私钥,部分在线支付平台不需要此项
						<%if rs("id")="7" or rs("id")="9" or rs("id")="15" then%>
						<div style="color:green">格式：交易安全校验码（key）|合作者身份(partnered) </div>
                        <%end if%>
						</span>
			<%end if %>
					</dd>
		...
		Sub DoModifySave()
		  ...
		  If Not RS.Eof Then
			...
			if id=16 then
			 RS("MD5Key")   = KS.G("Appid")&"|" & KS.G("appsecret") &"|" & KS.G("PaySignKey")
			else
			 RS("MD5Key")   = MD5Key
			end if
			RS("Rate")     = Rate
		  ...
		  End If
		  RS.Close:Set RS=Nothing
		  KS.Alert "恭喜，修改成功！",KS.Setting(3) & KS.Setting(89) & "system/KS.PaymentPlat.asp" 
		End Sub
		...
		Sub DoBatch()
			...
			For K=0 TO Ubound(ID_Arr)
			 ...
			 End If
			Next
			KS.Alert "恭喜，批量设置成功！" , KS.Setting(3) & KS.Setting(89) & "system/KS.PaymentPlat.asp"
		 End Sub
		Sub DoDisabled()
