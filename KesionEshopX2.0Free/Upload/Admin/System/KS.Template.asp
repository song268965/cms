...
Class Template
		...
		if instr(lcase(replace(currpath&"","/","")),lcase(replace(KS.Setting(90)&"","/","")))=0  and instr(lcase(replace(currpath&"","/","")),lcase(replace(KS.Setting(178)&"","/","")))=0 then ks.die "error template dir!"
		...
		{
		  top.openWin('添加新的模板文件','System/KS.Template.asp?t=<%=request("flag")%>&Action=AddTemplateFile&currpath='+escape(CurrPath),true,500,120);
		}
		...
		$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape("模板管理中心 >> 编辑模板")+'&ButtonSymbol=TemplateAdd';
		}
		function TextEdit(Flag)
		{
		 ...
		 if (SelectedFile!='')
			if (SelectedFile.indexOf(',')==-1) 
			{
			 ...
			 $(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape("模板管理中心 >> 编辑模板")+'&ButtonSymbol=Gosave';
			}
		...
		.Write "<li class='parent' onclick=""AddTemplateFile();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add'></i>"
		If Request("flag")="skin" then .write "新建CSS" Else .Write"新建模板"
		.Write"</span></li>"
		.Write "<li class='parent' onclick=""location.href='KS.Template.asp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon merge'></i>模板管理</span></li>"
		.Write "<li class='parent' onclick=""location.href='KS.Template.asp?flag=skin';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon set'></i>CSS管理</span></li>"
		.Write "<li class='parent' "
		If Lcase(CurrPath & "/")=lcase(InstallDir & KS.Setting(90)) Then .Write " Disabled"
	    .Write " onclick=""GoBack();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon back'></i>回上一级</span></li>"
		.Write "</ul>"	
		.Write "<div class='pageCont2'>"
		.Write "<div class='tabTitle'>模板管理</div>"
    ...
    If request("flag")<>"skin" Then
		For Each FsoItem In SubFolderObj
		  ...
		  .Write ("<td align='center'  class='splittd'><a href=""javascript:EditFolder('" & FsoItem.name & "')"" class='setA'>重命名</a>|<a href='KS.Template.asp?Action=Del&FileName=" & Server.URLEncode(FsoItem.name)&"&CurrPath=" & Server.URLEncode(CurrPath) & "' onclick=""return(confirm('此操作不可逆，确定删除吗？'))"" class='setA'>删除</a></td>")
		  .Write "</tr>"
		  Next
	 End If	  
		 For Each FsoItem In FileObj
		  ...
		  .Write "         <a href='KS.Template.asp?t=" & request("flag") & "&Action=Modify&FileName=" &Server.URLEncode(FsoItem.name)&"&Flag=text&CurrPath=" & Server.URLEncode(CurrPath) & "' onclick=""$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=" & server.URLEncode("模板管理中心 >> 编辑模板") &"&ButtonSymbol=GoSave'"" title=""模板路径：" & CurrPath &"/" & FsoItem.name &""">" & FsoItem.name & "</a></td>"
		  .Write "      </tr>"
		  .Write "    </table></td>"
		  ...
		  .Write ("<td align='center'  class='splittd'>" & FsoItem.DateLastModified & " </td>")
		  .Write ("<td align='center'  class='splittd'> <a href='KS.Template.asp?t=" & request("flag") & "&Action=Modify&FileName=" &Server.URLEncode(FsoItem.name)&"&Flag=text&CurrPath=" & Server.URLEncode(CurrPath) & "' onclick=""$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=" & server.URLEncode("模板管理中心 >> 编辑模板") &"&ButtonSymbol=GoSave'"" class='setA'>编辑</a>")
		  
		  .Write ("|<a href=""javascript:EditFile('" & FsoItem.name & "')"" class='setA'>重命名</a>|<a href='KS.Template.asp?Action=Del&FileName=" & Server.URLEncode(FsoItem.name)&"&CurrPath=" & Server.URLEncode(CurrPath) & "' onclick=""return(confirm('此操作不可逆，确定删除吗？'))"" class='setA'>删除</a></td>")
		  .Write "</tr>"
		  End If
		  Next	
		  .write "</table>"
	   ...
	   If Request.Form("Flag") = "Add" Then
		 ...
		 if request("t")<>"skin" then
	      NewTemplateStr = "<!DOCTYPE html>" & vbnewline &"<html>" & vbnewline &"<head>" & vbnewline & "<meta http-equiv=""Content-Type"" content=""text/html; charset=utf-8"">" & vbnewline
		  ...
		  PhysicalPath=Server.Mappath(TemplateFile)
			...
			if FsoObj.FileExists(PhysicalPath) = False then
			   Call KS.WriteTOFile(TemplateFile, NewTemplateStr)
				
			else
				Call KS.AlertHistory("文件名已经存在,请另取一个名称!",-1)
			end if
		if request("t")<>"skin" then
			Response.Write "<script>if (confirm('新建模板成功，继续添加吗?')){location.href='KS.Template.asp?Action=AddTemplateFile&ChannelID=" & ChannelID & "&TemplateType=" & TemplateType & "&CurrPath=" & Request("CurrPath") & "';}else{ top.box.close();}</script>"
		Else
			Response.Write "<script>if (confirm('增加CSS文件成功，继续添加吗?')){location.href='KS.Template.asp?t=skin&Action=AddTemplateFile&ChannelID=" & ChannelID & "&TemplateType=" & TemplateType & "&CurrPath=" & Request("CurrPath") & "';}else{ top.box.close();}</script>"
		End If
	   End If
	   End Sub
	  ...
	  Sub AddTemplate()
		...
		Response.Write " <div class=""topdashed sort"">"
		If Request("t")="skin" Then
		...
		Response.Write "<div class='pageCont2'>"
		 Response.Write "<table width='100%' height=""350"" style=""width:100%;height:100%;padding-right: 2px;padding-left: 2px;padding-bottom: 0px;"" border='0' align='center' cellpadding='0' cellspacing='0' class='ctable'  height='100%'>"
		 ...
		 Else
		  Response.Write "模板地址"
		 End If
		 Response.Write "：</b><input class='textbox' name=""TemplateFileName"" type=""text"" id=""TemplateFileName"" size=""50"" Value=""" & TemplateFileName & """ readonly>"
		 ...
		 Response.Write ">"
		 Response.Write "	<td valign=""top"" style='height:30px;text-align:left'><strong>插入标签：</strong>"
		 Response.Write "<select id=""mylabel"" style=""width:160px"" class='textbox'>"
		...
		if KS.G("flag")="text" then 
		 ...
		 Response.Write "   <td>"
		 Response.Write "     <table border='0' style='width:100%;height:100%' cellspacing='0' cellspadding='0'>"
		 Response.Write "	  <tr>"
		 Response.Write "       <td valign=""top"" width=""50"">"
		 %>
		  <textarea name="txt_ln" id="txt_ln" cols="6" style="width:50px;height:95%;overflow:hidden;min-height:383px;background-color:highlight;border-right:0px;text-align:right;font-family: tahoma;font-size:12px;font-weight:bold;color:highlighttext;cursor:default;line-height:16px;" readonly>
<% Dim N
For N=1 To 3000
...
%>
					...
					{ if (document.all){
					 ...
					 <textarea onclick='setPos()' onkeyup='setPos()' name="Content" rows="2" cols="30" id="Content" onscroll="show_ln('txt_ln','Content')" onKeyDown="editTab()" onChange="TemplateContent.SetContentIni();" style="line-height:16px;height:95%;min-height:382px;width:99%;"><%=Server.HTMLEncode(TemplateFromFileContent)%></textarea>
</td>
		             </tr>
					 </table>
		</td>
	  ...
	  Sub TemplateSave()
		    ...
		    End IF
			  ...
			  If KS.WriteTOFile(TemplateFileName, TemplateContent) = True Then
			   If right(lcase(TemplateFileName),4) =".css" Then
			    Response.Write ("<script> alert('恭喜，CSS在线修改成功!');top.frames['BottomFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?OpStr=模板管理&ButtonSymbol=Disabled'; location.href='KS.Template.asp?flag=skin&currpath=" & request.Form("currpath") &"';</script>")
			   Else
			    Response.Write ("<script> alert('成功提示:模板修改成功!');top.frames['BottomFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?OpStr=模板管理&ButtonSymbol=Disabled'; location.href='KS.Template.asp?currpath=" & request.Form("currpath") &"';</script>")
			   End If
			  Else
	   ...
	   Sub SelectTemplate()
		...
		Response.Write "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>"
		Response.Write "<title>选择模板文件</title><link href='../Include/Admin_Style.CSS' rel='stylesheet' type='text/css'>"
		%>
		<script src="../../ks_inc/jquery.js"></script>
		<script>
		 $(document).ready(function(){
		   $("#FolderList").height(390);
		 });
		</script>
		<%
		Response.Write "</head><body leftmargin='0' topmargin='0' marginwidth='0' marginheight='0'>"
		...
		Response.Write "  </tr>"
		Response.Write "  <tr>"
		Response.Write "    <td style=""width:100%;height:100%""><iframe id=""FolderList"" name=""FolderList"" width='100%' height='100%' frameborder='1' src='../Include/FolderList.asp?CurrPath=" & CurrPath & "'></iframe></td>"
		Response.Write "  </tr>"
		Response.Write "  <tr>"
		...
		Response.Write "    &nbsp;&nbsp;<input class='button' onClick='closeWin()' type='button' name='Submit3' value=' 取 消 '>"
		Response.Write "      </td>"
		Response.Write "  </tr>"
		Response.Write "</table>"
		Response.Write "</body>"
		...
		Response.Write "function closeWin(){" &vbcrlf
		Response.Write " try{ top.frames['MainFrame'].frames[0].box.close();}catch(e){ try { top.frames['MainFrame'].box.close();}catch(e){top.close();}}" &vbcrlf
		Response.Write "}" &vbcrlf
		...
		Response.Write "{ "
		Response.Write " if (file==null) file=$(window.frames[""FolderList""].document).find("".FolderSelectItem"").attr(""File"");"&vbcrlf
		Response.Write "    if (file!='' && file!=undefined)"&vbcrlf
		Response.Write "    {"&vbcrlf
		...
		if request("fieldid")<>"" then
		 Response.Write " try{ top.frames['MainFrame'].frames[0].document.getElementById('" &KS.S("fieldID")&"').value=templatedir;}catch(e){" &vbcrlf
		 Response.Write "top.frames['MainFrame'].document.getElementById('" &KS.S("fieldID")&"').value=templatedir;}"
		Else
		Response.Write "    if (document.all){window.returnValue=templatedir;}else{ top.opener.setVal(templatedir);}"&vbcrlf
		End If
		Response.Write "       closeWin();"&vbcrlf
		Response.Write "    }"&vbcrlf
		Response.Write "    else{"&vbcrlf
