...
{
    top.openWin("查看产品","space/KS.EnterPrisePro.asp?action=View&ProID="+id,false);
}
</script>
<div class="pageCont2 pt0" style="border-radius:0 0 5px 5px;">
...
<tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	%></td>
	<td class="splittd" align="center"><a href="#" onclick="ShowIframe(<%=rs("id")%>)" class="setA">浏览</a>|<a href="?Action=Del&ID=<%=rs("id")%>" onclick="return(confirm('确定删除吗？'));" class="setA">删除</a>|<a href="?Action=verific&id=<%=rs("id")%>" class="setA">审核</a></td>
</tr>
<%
		Rs.movenext
			i = i + 1
...
'审核
Sub ShowNews()
        Response.Write "<div class=""productbox"">"
	    Dim RS:Set RS=Server.CreateObject("ADODB.RECORDSET")
		RS.Open "Select * From KS_Product where id=" &KS.ChkClng(KS.S("ProID")),conn,1,1
		If Not RS.Eof Then
		   Response.WRITE "<div class='proinbox'><div><strong>产品名称：</strong>" & rs("title") & "</div>"
	      ...
	      response.write "</div>"
		   ...
		   Response.Write "<div style=""text-align:left"">产品图片：<img src='" & RS("photourl") & "' class='productimg'></div>"
		   End If
		   Response.Write "<div style='line-height:35px;'>产品介绍：" & KS.HTMLCode(rs("prointro")) & "</div>"
		   Response.Write "</div>"
		End If
		RS.Close:Set RS=Nothing
		Response.Write "</div>"
