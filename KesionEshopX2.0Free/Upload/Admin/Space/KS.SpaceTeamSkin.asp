...
Class BlogUserSkin
		...
		flag=3        '圈子模板
		 With Response
			  ...
			  .Write "<li class='parent' onclick=""location.href='KS.SpaceTeam.asp?action=topic';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon num'></i>帖子管理</span></li>"
			  ...
			  .Write "<li class='parent' onclick=""location.href='KS.SpaceTeam.asp?action=class';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon folder'></i>圈子分类</span></li>"
			  .Write	" </ul>"
		End With
...
%> 
    <tr class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'"> 
      ...
      <td width="40%" class="splittd"> <div align="center"><a href="../space/showtemplate.asp?templateid=<%=rs("id")%>" target="_blank" class="setA">预览</a>|<a onclick="$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户 >> <font color=red>修改模板</font>')+'&ButtonSymbol=GOSave';location.href='KS.SpaceTeamSkin.asp?action=modifytext&id=<%=rs("id")%>&flag=<%=KS.g("flag")%>'" href="#" class="setA">修改模版</a>|<a href="KS.SpaceTeamSkin.asp?action=delconfig&id=<%=rs("id")%>&flag=<%=KS.g("flag")%>" onclick=return(confirm("确定要删除这个模版吗？")) class="setA">删除模版</a></div>
	  </td>
    </tr>
      <%
rs.movenext
...
<form method="POST" action="KS.SpaceTeamSkin.asp?ID=<%=KS.G("id")%>&flag=<%=KS.g("flag")%>&action=<%=Action%>" id="myform" name="myform">
    <tr class="tdbg">
	  ...
	  <td><input name="TemplateName" class="textbox" type="text" id="TemplateName" value="<%=templatename%>"></td>
	</tr>
	<tr class="tdbg">
	  <td align="right" class="clefttitle"><strong>模板作者：</strong></td>
	  <td><input name="TemplateAuthor" class="textbox" type="text" id="TemplateAuthor" value="<%=templateauthor%>"> 如科汛,kesion.com等</td>
	</tr>
	<tr class="tdbg">
	   <td align="right" class="clefttitle"><strong>预 览 图：</strong></td>
	   <td><input type="text" class="textbox" name="TemplatePic" value="<%=templatepic%>">&nbsp;<input class='button' type='button' name='Submit' value='选择预览图...' onClick="OpenThenSetValue('Include/SelectPic.asp?Currpath=<%=CurrPath%>',550,290,window,document.all.TemplatePic);">
		</td>
    </tr>
	 <tr class="tdbg"> 
	  <td height="25" class="clefttitle" width="120" align="right"><strong>主 模 板：</strong></td>
      <td height="25" class="tdbg">
	  <input type="text" class="textbox" name="TemplateMain" id='TemplateMain' size='25' value="<%=templateMain%>"> <%=KSCls.Get_KS_T_C("$('#TemplateMain')[0]")%> 主模板必须放{$GroupMain}标签
      </td>
    </tr>
    <tr class="tdbg"> 
