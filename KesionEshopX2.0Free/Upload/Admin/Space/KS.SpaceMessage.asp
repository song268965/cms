...
Class Admin_BlogMessage
		...
		.Write"</head><body>"
		.Write "<ul id='mt' class='topdashed sort' style='padding-top:0;'>"
		.Write "用户留言管理"
		.Write ""
		.Write ""
		.Write ""
...
%>
<div class="pageCont2" style="border-radius:5px !important;">
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="padding-top:20px;">
...
<%
	...
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr class='list'><td height=""25"" align=center colspan=8 class='splittd noborder'>没有人发表留言！</td></tr>"
	Else
...
<tr height="22" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	<td class="splittd" align="center">
	 <%if rs("status")="1" then
	   ...
	   dim userid:userid=0
	   dim rsu:set rsu=conn.execute("select top 1 userid from ks_user where username='" & rs("username") &"'")
	   if not rsu.eof then
	    userid=rsu(0)
	   end if
	   rsu.close
	...
	<td class="splittd" align="center"><a href="../../space/?<%=userid%>/message/#<%=rs("id")%>" target="_blank">浏览</a> <a href="?Action=Del&ID=<%=RS("ID")%>" onclick="return(confirm('确定删除该留言吗？'));">删除</a> </td>
</tr>
<%
		Rs.movenext
			i = i + 1
...
%>
<tr onMouseOver="this.className=''" onMouseOut="this.className='list'">
...
$("#MainFrame").load(function(){
	$(this).contents().find("body").css("background","#edf1f2");
});
