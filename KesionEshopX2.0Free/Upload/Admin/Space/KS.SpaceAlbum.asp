...
Class Admin_Photoxc
		...
		With Response
					If Not KS.ReturnPowerResult(0, "KSMS10003") Then          '检查是权限
					 ...
					 .End
					 End If
			.Write "<!DOCTYPE html><html><meta http-equiv='Content-Type' content='text/html; charset=utf-8'><head>"
		    .Write "<script src='../../KS_Inc/jquery.js'></script>"
		    .Write "<script src='../../KS_Inc/common.js'></script>"
		    .Write "<link href='../Include/Admin_Style.CSS' rel='stylesheet' type='text/css'></head><body>"
			.Write "<ul id='menu_top'>"
			.Write "<li class='parent' onclick=""location.href='KS.SpaceAlbum.asp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon photo'></i>相册管理</span></li>"
			.Write "<li class='parent' onclick=""location.href='?action=showzp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon photo'></i>照片管理</span></li>"
			.Write "<li class='parent' onclick=""location.href='?action=comment';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add'></i>相片评论</span></li>"
			.Write "<li class='parent' onclick=""location.href='?action=photoclass';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon folder'></i>相册分类</span></li>"
			.Write	" </ul>"
		End With	
...
'博文评论管理
Sub commentshow()
		totalPut = Conn.Execute("Select Count(ID) from KS_PhotoComment")(0)
...
<%
	...
	SQL = "select * from KS_PhotoComment order by id desc"
	Rs.Open SQL, Conn, 1, 1
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr class='list'><td height=""25"" align=center colspan=7 class='splittd'>没有人发表评论！</td></tr>"
	Else
...
<tr height="22" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	<td class="splittd">
	<strong>标题:</strong><a href="../../space/?<%=rs("username")%>/showalbum/<%=rs("xcid")%>/<%=rs("photoid")%>" target="_blank"><%=Rs("title")%></a>
	...
	<td class="splittd" align="center"><a href="../../space/?<%=rs("username")%>/showalbum/<%=rs("xcid")%>/<%=rs("photoid")%>" target="_blank">浏览</a> <a href="?Action=commentdel&ID=<%=RS("ID")%>" onclick="return(confirm('确定删除该评论吗？'));">删除</a> </td>
</tr>
<%
		Rs.movenext
			i = i + 1
...
Sub CommentDel()
 ...
 Conn.execute("Delete From KS_PhotoComment Where ID In("& id & ")")
...
<%
	...
	Rs.Open SQL, Conn, 1, 1
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr class='list'><td height=""25"" align=center colspan=7 class='splittd'>没有用户创建相册！</td></tr>"
	Else
		If TotalPageNum > 1 then Rs.Move (CurrentPage - 1) * maxperpage
		i = 0
%>
...
<tr height="22" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	%></font></td>
	<td class="splittd" align="center">
	<a href="?action=modify&id=<%=rs("id")%>" onclick="window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户管理 >> <font color=red>修改相册信息</font>')+'&ButtonSymbol=GOSave';">编辑</a>
...
%>
<tr>
	<td  class='pt10' onMouseOver="this.className='pt10'" onMouseOut="this.className='pt10'" height='25' colspan=8>
	&nbsp;&nbsp;<input id="chkAll" onClick="CheckAll(this.form)" type="checkbox" value="checkbox"  name="chkAll">全选
...
<tr class="tdbg">
			...
			<tr class="tdbg"> 
                  ...
                  <td><select style="width:160px" onChange="if(this.options[selectedIndex].value=='3'){document.myform.all.mmtt.style.display='block';}else{document.myform.all.mmtt.style.display='none';}"  name="flag" class="textbox">
            ...
            <tr class="tdbg">
              ...
              <td><textarea class="textbox textarea" name="Descript" id="Descript" cols=50 rows=10><%=RS("Descript")%></textarea>              <Br/><span class="msgtips" style="margin: 5px;">关于此相册的简要文字说明。</span>
				  </td>
            </tr>
            <tr class="tdbg">
            ...
            <tr class="tdbg">
              <td class="clefttitle">状  态：</td>
              <td>
			  <input name="status" type="radio" value="0" <%If rs("status")="0" then response.write " checked"%>/>未审核
			  <input name="status" type="radio" value="1" <%If rs("status")="1" then response.write " checked"%>/>已审核
			  <input name="status" type="radio" value="2" <%If rs("status")="2" then response.write " checked"%>/>锁定
			 </td>
            </tr>
	</table>
	</form>
...
Sub ModifySave()
		 ...
		 End If
			...
			RS("status")=KS.ChkClng(KS.S("status"))
		  RS.Update
		  RS.MoveLast
		  ID=rs("id")
		  RS.Close:Set RS=Nothing
		   Call KS.FileAssociation(1028,ID,PhotoUrl,1)
		   Response.Write "<script>top.$.dialog.alert('相册修改成功!',function(){ location.href='space/KS.SpaceAlbum.asp';});</script>"
End Sub
...
<%
	...
	Rs.Open SQL, Conn, 1, 1
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr class='list'><td height=""25"" align=center colspan=7 class='splittd'>没有用户创建照片！</td></tr>"
	Else
		If TotalPageNum > 1 then Rs.Move (CurrentPage - 1) * maxperpage
		i = 0
%>
