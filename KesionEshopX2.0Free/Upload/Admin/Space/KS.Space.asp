...
Class Admin_Space
		...
		Public Sub Kesion()
		 With Response
			  ...
			  .Write "<li class='parent' onclick=""location.href='KS.Space.asp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon set'></i>空间管理</span></li>"
			  .Write "<li class='parent' onclick=""location.href='KS.SpaceSkin.asp?flag=2';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon merge'></i>模板管理</span></li>"
			  .Write "<li class='parent' onclick=""location.href='?action=class';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon folder'></i>空间分类</span></li>"
			  .Write "<li></li>"
			  if request("showtype")<>"1" then
			  .Write "<div><select name='classid' onchange=""location.href='?classid='+this.value;"" class=""h30"">"
...
<div class="tabs_header">
    <ul class="tabs">
    <li <%if ks.isnul(ks.s("id")) then response.write " class='active'"%>><a href="?showtype=1"><span>所有空间</span></a></li>
	<%
	set rs=conn.execute("select * From KS_UserGroup Where usertype=0 and [type]<2 order by id")
	do while not rs.eof
	  %>
...
<%
	...
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr><td height=""25"" align=center bgcolor=""#ffffff"" colspan=7 class='splittd'>没有用户申请个人空间！</td></tr>"
	Else
...
<tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	%></td>
	<td class="splittd" align="center"><a href="../../space/?<%=rs("userid")%>" target="_blank" class="setA">浏览</a>|<a href="?action=modify&id=<%=rs("blogid")%>" onclick="window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户管理 >> <font color=red>修改空间信息</font>')+'&ButtonSymbol=GOSave';" class="setA">编辑</a>|<a href="?Action=Del&ID=<%=rs("blogid")%>" onclick="return(confirm('删除站点将删除站点下的所有日志，确定删除吗？'));" class="setA">删除</a>|<%IF rs("recommend")="1" then %><a href="?Action=Cancelrecommend&id=<%=rs("blogid")%>" class="setA"><font color=red>取消推荐</font></a>|<%else%><a href="?Action=recommend&id=<%=rs("blogid")%>" class="setA">设为推荐</a>|<%end if%>&nbsp;<%if rs("status")=0 then%><a href="?Action=verific&id=<%=rs("blogid")%>" class="setA">审核</a>|<%elseif rs("status")=1 then%><a href="?Action=lock&id=<%=rs("blogid")%>" class="setA">锁定</a>|<%elseif rs("status")=2 then%><a href="?Action=unlock&id=<%=rs("blogid")%>" class="setA">解锁</a>|<%end if%>
	<a onclick="window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户管理 >> <font color=red>升级为企业空间</font>')+'&ButtonSymbol=GOSave';" href="KS.EnterPrise.asp?action=Edit&flag=update&username=<%=rs("username")%>" title="升级为企业空间">升级</a>
	</td>
</tr>
<%
		Rs.movenext
...
<tr class='list' onMouseOver="this.className=''" onMouseOut="this.className='list'">
	...
	<%if request("showtype")="1" then%>
	<input type="button" value="开通个人空间" onclick="window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户管理 >> <font color=red>开通个人空间</font>')+'&ButtonSymbol=GO';location.href='KS.Space.asp?action=add';" class="button">
	<%end if%>
	</td>
</tr>
</form>
   ...
   <input type="hidden" value="<%=request.servervariables("http_referer")%>" name="comeurl">
      <%if request("action")="add" then%>
          <tr class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">
           ...
           <td height='28'>&nbsp;<input type='text'  class="textbox" name='username' id='username'/> <font color=red>*</font>请输入要开通个人空间的用户名</td>
          </tr>
	  <%else%>
          <tr class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">
	  ...
	  <%end if%>  
          <tr class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">
           ...
           <td height='28'>&nbsp;<input type='text' name='BlogName' class="textbox" value='<%=BlogName%>' size="40"> <font color=red>*</font></td>
          </tr> 
		  
		 <tr class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>绑定模板：</strong></td>
           <td height='28'>&nbsp;<select name="templateid" id='templateid' class="textbox">
          ...
          <tr class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">
           ...
           <td height='28'>
		          <div class="md10"><label style="margin-left:5px;"><input type="radio" <%if instr(domain,".")=0 then response.write " checked"%> name="domaintype" onclick="$('#domain0').show();$('#domain1').hide();" value="0" class="textbox"/>二级域名</label>
			      <label><input type="radio" <%if instr(domain,".")<>0 then response.write " checked"%> name="domaintype" onclick="$('#domain0').hide();$('#domain1').show();" value="1" class="textbox"/>顶级独立域名</label></div>
          ...
          <tr class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">
           ...
           <td height='28'>&nbsp;<input type='text' class="textbox" name='logo' value='<%=logo%>' size="40"></td>
          </tr>  
          <tr class="tdbg" onMouseOver="this.className=''" onMouseOut="this.className='tdbg'">
...
Sub ModifySave()
 ...
 Set RS=Nothing
  ...
  if id=0 then
 Response.Write "<script>alert('恭喜，空间添加成功！');$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr='+escape('空间门户管理 >> <font color=red>空间站点管理</font>');location.href='KS.Space.asp?showtype=1';</script>"
  else
 Response.Write "<script>alert('恭喜，空间修改成功！');$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr='+escape('空间门户管理 >> <font color=red>空间站点管理</font>');location.href='"& Request.Form("ComeUrl") & "';</script>"
 end if
End Sub
...
<div class="pageCont2">
		...
		rs.close%>
			...
			<tr valign="middle"> 
			  <td height="25" class="splittd"></td>
			  <td height="25" class="splittd" align="center"><input name="ClassName" type="text" class="textbox" id="ClassName" size="25"></td>
			  <td height="25" class="splittd" align="center"><input style="text-align:center" name="orderid" type="text" value="<%=orderid+1%>" class="textbox" id="orderid" size="8">
			  <td height="25" class="splittd" align="center"><input name="Submit3" class="button" type="submit" value="OK,提交"></td>
			</tr>
		</form>
</table>
</div>
