...
{
   top.openWin('<b>查看行业关键词广告管理</b>',"space/KS.EnterpriseAD.asp?action=View&ProID="+id,false);
}
</script>
<div class="pageCont2">
...
<%
	...
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr><td height=""25"" align=center colspan=10 class='splittd'>找不到行业关键词广告！</td></tr>"
	Else
...
<tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	%></td>
	<td class="splittd" align="center"><a href="#" onclick="ShowIframe(<%=rs("id")%>)" class="setA">浏览</a>|<a href="?type=<%=typeflag%>&Action=Edit&ID=<%=rs("id")%>" class="setA">修改</a>|<a href="?type=<%=typeflag%>&Action=Del&ID=<%=rs("id")%>" onclick="return(confirm('确定删除吗？'));" class="setA">删除</a>|<a href="?type=<%=typeflag%>&Action=verific&id=<%=rs("id")%>" class="setA">审核</a></td>
</tr>
<%
		Rs.movenext
			i = i + 1
...
<tr class='list' onMouseOver="this.className=''" onMouseOut="this.className='list'">
	<td  class="operatingBox" height='25' colspan=8>
...
Sub AddAd()
     if KS.S("Action")="Edit" Then
		...
		function changelocation(locationid)
		  ...
		  <select class="textbox" name="ClassID" onChange="changelocation(document.myform.ClassID.options[document.myform.ClassID.selectedIndex].value)" size="1">
        ...
        rsb.close
			%>
                  ...
                  <select class="textbox" name="SmallClassID"<%if adtype="1" then response.write " disabled"%>>
					...
					%>
                      ...
                      <tr class="tdbg" style="display:none">
                              ...
                              <tr class="tdbg">
                                ...
                                <td height="25">　
                            <select name="datatimed" id="datatimed" class="textbox">
		        ...
		        <br>
		        <div class="attention">
                  <font color="red"><STRONG>注意事项：</STRONG>
				  <br /><IMG height="8" src="images/expand.gif" width="8">请确保您的广告健康，不含黄色信息。确定真实性，合法性，否则后果自负，<%=KS.Setting(1)%>不承担任何责任。<br /><IMG height="8" src="images/expand.gif" width="8">提交的行业广告必须经过管理员审核后才能生效。生效时间以审核时间为准。</font>
                </div>
			</div>
<%
End Sub
...
'审核
Sub ShowNews()
         response.write "<div style='padding:6px'>"
