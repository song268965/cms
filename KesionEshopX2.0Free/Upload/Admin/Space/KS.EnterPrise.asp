...
Class Admin_EnterPrise
		...
		Public Sub Kesion()
		 With Response
			  ...
			  .Write "<li class='parent' onclick=""location.href='KS.EnterPrisePro.asp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>企业新闻</span></li>"
		...
		if ks.chkclng(ks.s("id"))=0 then
		 ...
		 Param=" inner join ks_user c on a.username=c.username where c.groupid=" & ks.chkclng(ks.s("id"))
		end if
...
<div class="tabs_header">
    <ul class="tabs">
    <li <%if ks.isnul(ks.s("id")) then response.write " class='active'"%>><a href="KS.Enterprise.asp"><span>所有企业</span></a></li>
	<%
	set rs=conn.execute("select * From KS_UserGroup Where usertype=1 and [type]<2 order by id")
	do while not rs.eof
	  %>
	  <li<%if ks.chkclng(ks.s("id"))=ks.chkclng(rs("id")) then response.write " class='active'"%>><a href="?id=<%=rs("id")%>"><span><%=rs("groupname")%></span></a></li>
	 <%
	rs.movenext
	loop
	rs.close
...
<%
	...
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr><td height=""25"" align=center bgcolor=""#ffffff"" colspan=7 class='splittd'>没有用户申请开通企业空间！</td></tr>"
	Else
...
<tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	%></td>
	<td class="splittd" align="center"><a href="../../space/?<%=rs("username")%>" target="_blank" class="setA">浏览</a>|<a href="?action=Edit&username=<%=RS("username")%>"  onclick="window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('企业&产品库 >> <font color=red>修改企业信息</font>')+'&ButtonSymbol=GOSave';" class="setA">修改</a>|<a href="?Action=Del&ID=<%=rs("id")%>" onclick="return(confirm('确定删除该企业吗？'));" class="setA">删除</a>|<%IF rs("recommend")="1" then %><a href="?Action=Cancelrecommend&id=<%=rs("id")%>" class="setA"><font color=red>取消推荐</font></a>|<%else%><a href="?Action=recommend&id=<%=rs("id")%>" class="setA">设为推荐</a>|<%end if%>&nbsp;<%if rs("status")=0 then%><a href="?Action=verific&id=<%=rs("id")%>" class="setA">审核</a>|<%elseif rs("status")=1 then%><a href="?Action=lock&id=<%=rs("id")%>" class="setA">锁定</a><%elseif rs("status")=2 then%><a href="?Action=unlock&id=<%=rs("id")%>" class="setA">解锁</a><%end if%></td>
</tr>
<%
		Rs.movenext
			i = i + 1
...
<tr class='list' onMouseOver="this.className=''" onMouseOut="this.className='list'">
	...
	<input type="button" class="button" value="开通企业空间" onclick="window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('企业&产品库 >> <font color=red>开通企业空间</font>')+'&ButtonSymbol=GO';location.href='?action=Add';"/>
	</td>
</tr>
</form>
<tr>
...
Sub EnterPriseEdit()
Dim companyname,ClassID,BusinessLicense,LegalPeople,CompanyScale,RegisteredCapital,Province,City,County,ContactMan,Address
   ...
   <input type="hidden" value="<%=request.servervariables("http_referer")%>" name="comeurl">
      <%if request("action")="Add" then%>
          ...
          <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
           ...
           <td height='28'>&nbsp;<input class="textbox" type='text' id='username' name='username' size="20"> <font color=red>*</font> 输入您要为其开通的会员名称</td>
          </tr>  
          <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>空间LOGO：</strong></td>
           <td height='28' class='tips'>&nbsp;<input class="textbox" type='text' id='logo' name='logo' size="40"> <input class="button"  type='button' name='Submit' value='选择Logo...' onClick="OpenThenSetValue('Include/SelectPic.asp?ChannelID=1&CurrPath=<%=KS.GetUpFilesDir%>',550,290,window,document.myform.logo);"> 支持jpg gif 或png格式，如/images/logo.gif  </td>
          </tr>  
	  <%else%>
   ...
   <input type="hidden" value="<%=username%>" name="username">
	  <%end if%>
          <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
           ...
           <td height='28'>&nbsp;<input type='text' class="textbox" id='companyname' name='companyname' value='<%=companyname%>' size="40"> <font color=red>*</font></td>
          </tr>  
          <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
		...
		</script><select class="face" name="ClassID" onChange="changelocation(document.myform.ClassID.options[document.myform.ClassID.selectedIndex].value)" size="1">
        ...
        rsb.close
           ...
           <td height='28'>&nbsp;<input type='text' class="textbox" name='BusinessLicense' value='<%=BusinessLicense%>' size="40"> <font color=red>*</font></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>企业法人：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='LegalPeople' value='<%=LegalPeople%>' size="40"> <font color=red>*</font></td>
          </tr>  
		  <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
	 ...
	 <script type="text/javascript">
		  ...
		  top.openWin('电子地图标注','../plus/baidumap.asp?obj=parent.frames["MainFrame"].document.getElementById("MapMark")&MapMark='+escape($("#MapMark").val()),false,860,430);
           ...
           <td height='28'>&nbsp;<input type='text' class="textbox" name='ContactMan' value='<%=ContactMan%>' size="40"> </td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>公司地址：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='Address' value='<%=Address%>' size="40"></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>邮政编码：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='zipcode' value='<%=zipcode%>' size="40"></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>联系电话：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='Telphone' value='<%=telphone%>' size="40"></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>传真号码：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='Fax' value='<%=fax%>' size="40"></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>公司网站：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='weburl' value='<%=weburl%>' size="40"></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>开户银行：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='BankAccount' value='<%=BankAccount%>' size="40"></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>银行账号：</strong></td>
           <td height='28'>&nbsp;<input type='text' class="textbox" name='AccountNumber' value='<%=AccountNumber%>' size="40"></td>
          </tr>  
		   <tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
            <td  width='160' height='30' align='right' class='clefttitle'><strong>公司介绍：</strong></td>
           <td height='28'>
		   <%
		   Response.Write EchoEditor("Intro",Intro,"Basic","96%","260px")
			%>
		   </td>
          </tr>  
      </table>
		...
		RS.Open "Select top 1 * From KS_Enterprise Where ID=" & ID,Conn,1,3
			  ...
			  End If
				...
				If IsObject(FieldsXml) Then
				 ...
				 if KS.G("Flag")<>"" then
				 Response.Write "<script>alert('空间升级成功！');$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr='+escape('空间门户管理 >> <font color=red>企业空间管理</font>');location.href='KS.EnterPrise.asp';</script>"
				 ElseIf ID=0 Then
				 Response.Write "<script>alert('恭喜，企业空间创建成功！');$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr='+escape('空间门户管理 >> <font color=red>企业空间管理</font>');location.href='KS.EnterPrise.asp';</script>"
				 Else
				 Response.Write "<script>alert('企业基本信息资料更新成功！');$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr='+escape('空间门户管理 >> <font color=red>企业空间管理</font>');location.href='"& Request.Form("ComeUrl") & "';</script>"
				 End If
