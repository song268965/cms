...
Class BlogUserSkin
		...
		flag=KS.chkclng(KS.g("flag"))
		 ...
		 With Response
			  ...
			  .Write "<li class='parent' onclick=""location.href='KS.SpaceSkin.asp?flag=" & flag & "';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon merge'></i>模板管理</span></li>"
			  .Write "<li class='parent' onclick=""location.href='KS.Space.asp?action=class';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon folder'></i>空间分类</span></li>"
			  .Write "</ul>"
		End With
...
%> 
    <tr class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'"> 
      ...
      <td class="splittd" width="40%"> <div align="center"><a href="../../space/showtemplate.asp?templateid=<%=rs("id")%>" target="_blank" class="setA">预览</a>|<a onclick="$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户 >> <font color=red>修改模板</font>')+'&ButtonSymbol=GOSave';location.href='KS.SpaceSkin.asp?action=modifytext&id=<%=rs("id")%>&flag=<%=KS.g("flag")%>'" href="#" class="setA">修改模版</a>|<a href="KS.SpaceSkin.asp?action=delconfig&id=<%=rs("id")%>&flag=<%=KS.g("flag")%>" onclick=return(confirm("确定要删除这个模版吗？")) class="setA">删除模版</a></div>
	  </td>
    </tr>
      <%
rs.movenext
wend
%>
    <td height="40" colspan="5" align="center" class="operatingBox">  
        <div align="center">
          <input type="submit" name="Submit" class="button" value="保存设置">&nbsp;&nbsp;
		  <input type="button" name="Submit1" class="button" value="添加新模板" onClick="$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape('空间门户 >> <font color=red>添加模板</font>')+'&ButtonSymbol=GO';location.href='?Action=newtemplate&flag=<%=KS.g("flag")%>';">
      </div></td>
  </tr>
</form>  
</table>
...
<table width="100%" border="0" align="center" class="ctable" cellpadding="2" cellspacing="1">
    <tr class="tdbg">
      <td width="20%" height="25" align="right" class="clefttitle"><strong>模版名称：</strong></td>
	  <td width="80%"><input name="TemplateName" size="50" class="textbox" type="text" id="TemplateName" value="<%=templatename%>"></td>
	</tr>
	<tr class="tdbg">
	  <td align="right" class="clefttitle"><strong>模板作者：</strong></td>
	  <td><input name="TemplateAuthor" size="50" class="textbox" type="text" id="TemplateAuthor" value="<%=templateauthor%>"> 如科汛,kesion.com等</td>
	</tr>
	<tr class="tdbg">
	   <td align="right" class="clefttitle"><strong>预 览 图：</strong></td>
	   <td><input type="text" size="50" class="textbox" name="TemplatePic" id="TemplatePic" value="<%=templatepic%>">&nbsp;<input class='button' type='button' name='Submit' value='选择预览图...' onClick="OpenThenSetValue('Include/SelectPic.asp?Currpath=<%=CurrPath%>',550,290,window,document.myform.TemplatePic);">
		</td>
    </tr>
	 ...
	 <tr> 
	  ...
	  <input type="text" size="50" class="textbox" name="TemplateMain" id='TemplateMain' value="<%=templateMain%>"> <%=KSCls.Get_KS_T_C("$('#TemplateMain')[0]")%>
      </td>
    </tr>
    ...
    <tr class="tdbg"> 
      ...
      <td height="25">
	  <input type="text" class="textbox" name="TemplateSub" id='TemplateSub' size="50" value="<%=templateSub%>"> <%=KSCls.Get_KS_T_C("$('#TemplateSub')[0]")%> 
      </td>
    </tr>
    <tr class="tdbg"> 
...
<div class="attention">	     
	     ...
	     <table width="100%" id="gallery-table"  align="center">
				...
				if KS.g("action")="modifytext" then
				  ...
				  pnum=0
				   do while not rss.eof
					...
					<a href="javascript:;" onclick="removeImg(this)">[-]</a>
					 <%end if%>
				  图片名称 <input type="text"  class="textbox" name="photonameUrl<%=pnum%>" value="<%=rss("photoname")%>" size="20"/> <span style='color:red'>*必填，如顶部banner,左边广告图等</span> <Br/>
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;图片地址 <input class="textbox" type="text" value="<%=rss("photourl")%>" name="imgUrl<%=pnum%>" id="Url<%=pnum%>" size="20" /> <button class='button' type='button' name='Submit' onClick="OpenThenSetValue('Include/SelectPic.asp?Currpath=<%=CurrPath%>',550,290,window,$('#Url<%=pnum%>')[0]);">选择</button>
				  ...
				  链接Url <input type="text" class="textbox" name="LinkUrl<%=pnum%>" value="<%=rss("linkurl")%>" size="20" /> <label><input type="checkbox" name="ModifyLinkUrl<%=pnum%>" value="1"<%if rss("modifylink")="1" then response.write " checked"%>>允许修链接</label></span>
				  宽<input style="text-align:center" class="textbox" type="text" name="widthUrl<%=pnum%>" value="<%=rss("width")%>" size="5"> 高<input class="textbox" type="text" name="heightUrl<%=pnum%>" style="text-align:center" value="<%=rss("height")%>" size="5">
				 
				  
				  <br/>
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;图片备注 <input  class="textbox" type="text" name="NoteUrl<%=pnum%>" value="<%=rss("descript")%>" size="20" /><br/>
				   </td>
				  </tr>
					<%
				     rss.movenext
			...
			<tr>
				<td>
				  ...
				  <a href="javascript:;" onclick="addImg(this)">[+]</a>
				  图片名称 <input  class="textbox" type="text" name="photonameUrl1" size="20"/> <span style='color:red'>*必填，如顶部banner,左边广告图等</span> <Br/>
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;图片地址 <input  class="textbox" type="text" name="imgUrl1" id="Url1" size="20" /> <input class='button' type='button' name='Submit' value='选择' onClick="OpenThenSetValue('Include/SelectPic.asp?Currpath=<%=CurrPath%>',550,290,window,$('#Url1')[0]);">
				  <label><input type="checkbox" onclick="if (this.checked){$('#LUrl1').hide()}else{$('#LUrl1').show()}" name="TagUrl1" value="1">背景图</label>
				  <span id="LUrl1">
				  链接Url <input  class="textbox" type="text" name="LinkUrl1"  size="20" /> <label><input type="checkbox" name="ModifyLinkUrl1" value="1">允许修链接</label></span> 宽<input name="widthUrl1"  class="textbox" style="text-align:center" type="text" size="5"> 高<input  class="textbox" type="text" style="text-align:center" name="heightUrl1" size="5">
				  <br/>
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;图片备注 <input  class="textbox" type="text" name="NoteUrl1" size="20" />
				</td>
			  </tr>
				<%end if %>  
		   </table>
