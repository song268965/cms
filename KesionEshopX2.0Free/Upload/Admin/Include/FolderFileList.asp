...
Class FolderFileList
		...
		function OpenParentFolder()
		{
			if (CanBackFlag==0) return;
			   location.href='FolderFileList.asp?channelid=<%=channelid%>&CurrPath='+ParentPath;
		...
		Response.Write "  <tr>"
		Response.Write "    <td class=""sort""> <div align=""center"">文件/文件夹名</div></td>"
		Response.Write "    <td class=""sort""> <div align=""right"">大小</div></td>"
		Response.Write "    <td class=""sort"" width=""70""> <div align=""center"">操作</div></td>"
		Response.Write "  </tr>"
		...
		Response.Write "        <tr>"
		Response.Write "          <td width=""24""><font color=""#FFFFFF""><i class=""icon back""></i></font></td>"
		Response.Write "          <td><a href='javascript:OpenParentFolder();'>返回上级目录</a></td>"
		Response.Write "        </tr>"
		...
		Response.Write "        <tr title=""点击鼠标进入此目录"">"
		Response.Write "         <td><i class=""icon folder""></i><a href=""javascript:OpenFolder('" & FsoItem.name & "');""><span class=""FolderItem"" Path=""" & FsoItem.name & """ onDblClick=""OpenFolder('" & FsoItem.name & "');"">"
		...
		Response.Write "    <td align=""center""><a href=""javascript:ModifyFolder('" & FsoItem.name & "')""><img border='0' src='../images/icon04.png'/></a> <a href=""javascript:DelFolder('" & FsoItem.name & "')""><img border='0' src='../images/del.gif'/></a></td>"
		Response.Write "   </tr>"
		...
		Response.Write "   <tr title=""点击鼠标选择该文件"">"
		Response.Write "   <td><img src='"&InstallDir&"Editor/KSPlus/FileIcon/"&GetFileIcon(FsoItem.name)&"' border=0 width=""16"" height=""16"" align=""absmiddle"" alt='"& FsoItem.Type&"'> <span class=""FolderItem"" File=""" & FsoItem.name & """ onDblClick=""SetFile('" & replace(FsoItem.name,"'","\'") &"');"" onClick=""SelectFile(this,'" & replace(FsoItem.name,"'","\'") &"');"">"
		Response.Write FsoItem.name
		Response.Write "            </span> </td>"
		Response.Write "    <td><div align=""Right"">"
		Response.Write FsoItem.size
		Response.Write "        &nbsp;字节 </div></td>"
		Response.Write "    <td align=""center""><a href=""javascript:ModifyFile('" & FsoItem.name & "')""><img border='0' src='../images/icon04.png'/></a> <a href=""javascript:DelFile('" & FsoItem.name & "')""><img border='0' src='../images/del.gif'/></a></td>"
		Response.Write "  </tr>"
		...
		If Err Then
		 Response.Write ("<Script> alert('系统检测到网站信息配置有误,请检查!');parent.closeWin()</Script>")
		 Err.Clear
		End If
		Set FsoObj = Nothing
		...
		function DelFolder(v){
			if (v!='')
			{ if (confirm('此操作不可逆，确定删除吗？'))
				window.location.href='?Type=DelFolder&Path='+CurrPath+'/'+v+'&CurrPath='+CurrPath;
			}
			else alert('请选择要删除的目录');
		}
		function ModifyFolder(v)
		{
			var ReturnValue='';
			if (v!='')
			{
			 ReturnValue=prompt('修改的名称：',v);
			 if ((ReturnValue!='') && (ReturnValue!=null)) window.location.href='?Type=FolderReName&Path='+CurrPath+'&CurrPath='+CurrPath+'&OldPathName='+v+'&NewPathName='+ReturnValue;
			}
		...
		function DelFile(v){
			if (v!='')
			{ if (confirm('此操作不可逆，确定删除吗？'))
				window.location.href='?Type=DelFile&Path='+CurrPath+'&FileName='+v+'&CurrPath='+CurrPath;
			}
			else alert('请选择要删除的目录');
		}
		function ModifyFile(v)
		{
			var ReturnValue='';
			if (v!='')
			{
			 ReturnValue=prompt('修改的名称：',v);
			if ((ReturnValue!='') && (ReturnValue!=null)) window.location.href='?Type=FileReName&Path='+CurrPath+'&CurrPath='+CurrPath+'&OldFileName='+v+'&NewFileName='+ReturnValue;
			}
			else alert('请填写要更名的文件名称');
		}
