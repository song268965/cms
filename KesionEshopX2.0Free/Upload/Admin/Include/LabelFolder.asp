...
Class LabelFolder
		...
		Select Case (Request.Form("Action"))
		 Case "Submit"
			If Request.Form("FolderName") = "" Then
				Call KS.alert("错误提示:\n\n请填写目录名称", "include/LabelFolder.asp?LabelType=" & LabelType & "&FolderID=" & FolderID)
				Response.End
			End If
			If KS.strLength(Trim(Request("FolderName"))) > 50 Then
			   Call KS.alert("目录名称不能超过25个汉字(50个英文字符)!", "include/LabelFolder.asp?LabelType=" & LabelType & "&FolderID=" & FolderID)
			   Response.End
			 End If
			 If KS.strLength(Trim(Request.Form("Descript"))) > 255 Then
			   Call KS.alert("目录描述不能超过个125汉字(255个英文字符)!", "include/LabelFolder.asp?LabelType=" & LabelType & "&FolderID=" & FolderID)
			...
			LabelFolderRS.Open "SELECT * FROM KS_LabelFolder WHERE FolderType=" & LabelType & " and FolderName='" & Request.Form("FolderName") & "'", Conn, 1, 1
			 If Not LabelFolderRS.EOF Then
			  LabelFolderRS.Close
			  Call KS.alert("错误提示:\n\n该目录已存在!", "include/LabelFolder.asp?LabelType=" & LabelType & "&FolderID=" & FolderID)
			 ...
			 Else
			   ...
			   LabelFolderRS.Close
				...
				Response.Write ("<script>if (confirm('成功提示:\n\n添加目录分类成功,继续添加目录分类吗?')){location.href='LabelFolder.asp?LabelType=" & LabelType & "&Folderid=" & FolderID & "';}else{top.box.close();}</script>")
			End If
		Case "EditOk"
			  If Request.Form("FolderName") = "" Then
				Call KS.alert("错误提示:\n\n请填写目录名称", "include/LabelFolder.asp?Action=EditFolder&LabelType=" & LabelType & "&FolderID=" & FolderID)
				
				Response.End
			  End If
			  If KS.strLength(Trim(Request("FolderName"))) > 50 Then
			   Call KS.alert("目录名称不能超过25个汉字(50个英文字符)!", "include/LabelFolder.asp?Action=EditFolder&LabelType=" & LabelType & "&FolderID=" & FolderID)
			   
			   Response.End
			 End If
			 If KS.strLength(Trim(Request.Form("Descript"))) > 255 Then
			   Call KS.alert("目录描述不能超过个125汉字(255个英文字符)!", "include/LabelFolder.asp?Action=EditFolder&LabelType=" & LabelType & "&FolderID=" & FolderID)
			 ...
			 End If
			   ...
			   If Not LabelFolderRS.EOF Then
				Call KS.alert("错误提示:\n\n目录名称已存在", "include/LabelFolder.asp?Action=EditFolder&LabelType=" & LabelType & "&FolderID=" & FolderID)
		...
		Response.Write "</head>"
		Response.Write "<body style=""background: #fff;"" scroll=no topmargin=""0"" leftmargin=""0"">"
		Response.Write "<br>"
