 ...
 Case "Add" 
	...
	ELSE
		...
		IF JSType=0 Then
		 ...
		 Set JSRS=Nothing
		 Set KSRObj=Nothing
     	Response.Write("<script>if (confirm('成功提示:\n\n添加JS成功,继续添加JS吗?')){location.href='" & FileUrl & "?Action=Add&FolderID=" & FolderID & "';}else{top.frames['BottomFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & FolderID &TempSymbol &"';top.frames['MainFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "include/JS_Main.asp?FolderID=" & FolderID &"&JSType=" & JSType & "';}</script>") 
	END IF
Case "Edit"
    Dim Page
		...
		JSRS.Open "Select * From [KS_JSFile] Where JSID='" & JSID &"'",Conn,1,3
		 ...
		 IF KeyWord="" Then
		    ...
		    ELSE
		        TempSymbol="&OpStr=JS管理  >> 自由JS &ButtonSymbol=FreeJSList"
		    END IF
     	   Response.Write("<script>alert('成功提示:\n\nJS修改成功!');top.frames['BottomFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & FolderID &TempSymbol &"';top.frames['MainFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "include/JS_Main.asp?FolderID="& FolderID & "&Page=" & Page & "&JSType=" & JSType & "';</script>") 
		 ELSE
		    IF JSType=0 Then
		    ...
		    END IF
     	   Response.Write("<script>alert('成功提示:\n\nJS修改成功!');top.frames['BottomFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?" &TempSymbol &"';top.frames['MainFrame'].location.href='" & KS.Setting(3) & KS.Setting(89) & "include/JS_Main.asp?KeyWord="& KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate &"&EndDate=" & EndDate &"&Page=" & Page & "&JSType=" & JSType & "';</script>") 
		 END IF
	END IF
		 JSRS.Close
		 Set JSRS=Nothing
