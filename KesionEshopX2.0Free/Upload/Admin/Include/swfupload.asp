...
Class UpFileSaveBySwfUpload
        ...
        Private Sub Class_Terminate()
		 ...
		 Set KS=Nothing
		End Sub
		Function CheckIsLogin(AdminID,Pass)
		     If AdminID="" Or Pass="" Then Check=false: Exit Function
			 ...
			 Else
			   	 ...
			   	 Response.Cookies(KS.SiteSn)("AdminID")=AdminID
			   	 Response.Cookies(KS.SiteSn)("AdminName")=ChkRS("UserName")
			   	 Response.Cookies(KS.SiteSn)("AdminPass")=Pass
                 CheckIsLogin=true
			 End If
		...
		Sub Kesion()
			...
			If KSLoginCls.Check=false Then
			   If CheckIsLogin(UpFileObj.Form("AdminID"),UpFileObj.Form("AdminPass")) =false Then
		...
		if channelid=0 then 
		 MaxFileSize=KS.ChkClng(KS.Setting(6))
		 AllowFileExtStr =KS.Setting(7)
		End If
		...
		if ReturnValue <> "" then
			 ...
			 Else
				 Select Case BasicType
					  ...
					  Case 2,5  '图片
					       if UpType="pic" or UpType="ProImage" then
							  if ThumbPathFileName="" then ThumbPathFileName=replace(TempFileStr,"|","")
							  KS.Die replace(TempFileStr,"|","") &  "@" & ThumbPathFileName & "@" & escape(FileTitles)
						   Else
							  KS.Die replace(TempFileStr,"|","") 
						   End If
					 Case Else KS.Die replace(TempFileStr&"","|","")
				 End Select
			End If
		  End iF
