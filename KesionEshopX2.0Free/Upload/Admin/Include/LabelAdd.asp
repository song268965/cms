...
Class LabelAdd
		...
		Case "EditSubmit"
			...
			Else
				...
				LabelRS.Open "Select top 1 * From [KS_Label] Where ID='" & LabelID & "'", Conn, 1, 3
				 ...
				 If KeyWord = "" Then
				   	KS.Echo "<script>top.$.dialog.alert('<br/>恭喜，标签修改成功!',function(){$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & ParentID & "&OpStr=标签管理  >> 自定义静态标签&ButtonSymbol=FreeLabel';location.href='" & KS.Setting(3) & KS.Setting(89) & "include/Label_main.asp?Page=" & Page & "&LabelType=1&FolderID=" & ParentID & "';});</script>"
				 Else
				   	KS.Echo "<script>top.$.dialog.alert('<br/>恭喜，标签修改成功!',function(){$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?OpStr=标签管理 >> <font color=red>搜索自定义静态标签结果</font>&ButtonSymbol=FreeLabelSearch';location.href='" & KS.Setting(3) & KS.Setting(89) & "include/Label_main.asp?Page=" & Page & "&LabelType=1&KeyWord=" & KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate & "&EndDate=" & EndDate & "';});</script>"
		...
		.Write "  <form name=""LabelForm"" id=""LabelForm"" method=post action=""LabelAdd.asp"" onSubmit=""return(CheckForm())"">"
		...
		.Write "<div class='pageCont2'><table width=""100%"" height=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"">"
		...
		.Write "      <td><textarea name=""Description"" rows=""3"" style=""width:100%;"" class='textbox'>" & Descript & "</textarea></td>"
		.Write "    </tr>"
		.Write "    <tr><td colspan=""2"" align=""center"" height=""25"" class=""tabTitle"">自 定 义 静 态 标 签 内 容</td></tr>"
