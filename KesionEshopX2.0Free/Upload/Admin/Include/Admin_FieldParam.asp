...
function submitdate(){
    ...
    top.frames["MainFrame"].InsertValue(dbname);
	top.box.close();
}
</script>
</head>
...
<form method='post' action='' name='myform'>
    <tr class="tdbg"><td><strong>字段名称：</strong><input name='FieldName' class="textbox" type='text' id='FieldName' size='20' value="<% =fieldname %>" readonly></td></tr>
...
End If
%>
    <tr class="tdbg"><td><strong>输出长度：</strong><input name='CatNum' class="textbox"  type='text' id='gotopic' size='6' value=0>
    &nbsp;&nbsp;&nbsp;<font color='#FF0000'>为0则不截断</font></td></tr>
	<tr class="tdbg"><td><strong>截断显示：</strong><Input name='CatType' class="textbox"  type='text' value='...' size="6">
    ...
    <tr class="tdbg"><td><strong>过滤处理：</strong><select name='OutSplit'><option value='0' selected>解析HTML标记</option><option value='1'>不解析HTML标记</option><option value='2'>过滤HTML标记</option></select></td></tr>
	    <tr class="tdbg"><td><strong>字段值为空时输出：</strong><input class="textbox"  title='(如图片值为空，则输出一张默认的图片 "/Images/defaule.gif")' name='NullChar' type='text' id='NullChar' size='20' value=""></td></tr>
...
%>
    ...
    <tbody id='input22' style='display:none'><tr class="tdbg"><td><strong>小数位数：</strong><input class="textbox"  name='XiaoShu' type='text' id='XiaoShu' size='4' value=2></td></tr></tbody>
</tbody>
...
%>
    ...
    <tr class="tdbg">
      <td><strong>输出格式：</strong>
        <input class="textbox"  name='Datemb' type='text' id='Datemb' size='28' value="YYYY-MM-DD">
		<br>
...
对象Url(PC) 
<Input type='radio' name='outype' value='-1'>
对象Url(手机) 
...
栏目名称
<Input type='radio' name='outype' value='4'> 
顶级栏目名称
<Input type='radio' name='outype' value='5'> 
顶级栏目Url
<%end if%>
 </td>
</tr>
...
<tr class="tdbg"><td  style="text-align:center"><input class="button" type='button' value="插入" onClick="submitdate();">&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' class="button" value="取消" onClick="top.box.close();"></td></tr>
