...
'主体部分
Public Sub Kesion()
Dim InstallDir, CurrPath, FolderID, LabelContent, L_C_A, Action, LabelID, Str, Descript,LabelStyle,CallFrom
Dim TypeFlag, OpenType, NavType, Navi, TitleCss, ColNumber, SplitPic, ChannelID,PrintType,DivID,DivClass,UlID,UlClass,LiID,LiClass,Page
...
Else
  ...
  End If
	...
	If  Not Node Is Nothing Then
			...
			CallFrom =  Node.getAttribute("callfrom")
...
If KS.IsNul(CallFrom) Then CallFrom=0
If Navi = "" Then Navi = " | "
If ColNumber = "" Then ColNumber = 10
If LabelStyle="" Then LabelStyle="[loop={@num}] " & vbcrlf & "<li{@currclass}><a href=""{@classurl}""{@target}>{@foldername}</a></li>" & vbcrlf & "[/loop]"
...
<script type="text/javascript">
   ...
   function InsertLabel(label)
		...
		function CheckForm()
		{
			...
			var tagVal='{Tag:GetNavigation labelid="0" callfrom="'+$("input[name=CallFrom]:checked").val()+'" channelid="'+ChannelID+'" navtype="'+NavType+'" nav="'+Nav+'" splitpic="'+SplitPic+'" col="'+ColNumber+'" opentype="'+OpenType+'" titlecss="'+TitleCss+'" printtype="'+PrintType+'" divid="'+divid+'" divclass="'+divclass+'" ulid="'+ulid+'" ulclass="'+ulclass+'" liid="'+liid+'" liclass="'+liclass+'"}'+$("#LabelStyle").val()+'{/Tag}'
...
<%
		...
		.echo "            <tr class=tdbg>"
		.echo "              <td height=""24"">调用地方 "
		  If KS.ChkClng(CallFrom) = 0 Then
					.echo ("&nbsp;&nbsp;&nbsp;<input name=""CallFrom"" type=""radio"" value=""0"" checked>PC版")
					.echo ("&nbsp;&nbsp;&nbsp;<input name=""CallFrom"" type=""radio"" value=""1"">手机版")
		 Else
					  .echo ("&nbsp;&nbsp;&nbsp;<input type=""radio"" value=""true"" name=""0"">PC版")
					  .echo ("&nbsp;&nbsp;&nbsp;<input type=""radio"" value=""false"" name=""1"" checked>手机版")
		 End If
		.echo "              </td>"
        .echo "</tr>"
		...
		.echo "            <tr class=tdbg>"
		.echo "              <td colspan='2' id='ShowFieldArea' class='field'><li onclick=""InsertLabel('{@autoid}')"">行 号</li><li onclick=""InsertLabel('{@classurl}')"">栏目URL(PC)</li><li onclick=""InsertLabel('{@class3gurl}')"">栏目URL(手机)</li> <li onclick=""InsertLabel('{@id}')"">栏目ID</li><li onclick=""InsertLabel('{@classid}')"" title='栏目的ClassID'>栏目小ID</li><li onclick=""InsertLabel('{@foldername}')"">栏目名称</li><li onclick=""InsertLabel('{@classename}')"">栏目英文名称</li><li onclick=""InsertLabel('{@classimg}')"">栏目图片地址</li><li onclick=""InsertLabel('{@classintro}')"">栏目介绍</li><li onclick=""InsertLabel('{@currclass}')"">当前栏目CSS</li><li onclick=""InsertLabel('{@target}')"">栏目打开方式</li></td>"
		.echo "            </tr>"
		.echo "            <tr class=tdbg>"
...
.echo "                <input  class='button' name=""SubmitPic"" onClick=""OpenThenSetValue('include/SelectPic.asp?CurrPath=" & CurrPath & "&ShowVirtualPath=true',550,290,window,document.myform.SplitPic);"" type=""button"" id=""SubmitPic2"" value=""选择图片..."">"
