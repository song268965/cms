...
Class GetLocation
		...
		Dim Bold, StartTag, OpenType, NavType, Navi, MoreLinkType, TitleCss,ShowTitle,Page,ShowHomePage
		...
		Else
		  ...
		  End If
			...
			If  Not Node Is Nothing Then
				...
				ShowHomePage=Node.getAttribute("showhomepage")
			End If
			Set Node=Nothing
			XMLDoc=Empty
		End If
		...
		function CheckForm()
		{
			...
			var Bold=false; 
			if ($("#Bold").prop("checked")==true){Bold=true}
			var ShowTitle=false; 
			if ($("#ShowTitle").prop("checked")==true){ShowTitle=true}
			...
			if ($("#ShowHomePage").prop("checked")==true){ShowHomePage=true}
			 var tagVal='{Tag:GetLocation labelid="0" bold="'+Bold+'" starttag="'+StartTag+'" navtype="'+NavType+'" nav="'+Nav+'" opentype="'+OpenType+'" titlecss="'+TitleCss+'" showtitle="'+ShowTitle+'" showhomepage="'+ShowHomePage+'"}{/Tag}';
		...
		.echo "            </tr>"
		.echo "            <tr class='tdbg'>"
		.echo "              <td height=""30"" colspan='2'>最终页显示标题"
		...
		.echo ">"
		.echo "   &nbsp;从网站首页开始导航"
		.echo "                &nbsp;&nbsp;&nbsp;<input name=""ShowHomePage"" type=""checkbox"" id=""ShowHomePage"" value=""true"""
		If Cbool(ShowHomePage)=true Then .echo " checked"
		.echo "> <span class='tips'>Tips:不勾选将从频道开始导航。</span></td>"
		.echo "        </tr>"
