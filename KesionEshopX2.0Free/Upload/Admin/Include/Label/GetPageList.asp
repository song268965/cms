...
Class GetPageList
		...
		Dim ChannelID,ClassID, IncludeSubClass, ShowClassName, OpenType, DocProperty, Num, RowHeight, TitleLen, OrderStr, ColNumber, ShowPicFlag, NavType, Navi, SplitPic, DateRule, DateAlign, TitleCss, DateCss,ShowNewFlag,ShowHotFlag, PrintType,AjaxOut,LabelStyle,IntroLen,Province,City,County,IsPic
		Dim PicWidth,PicHeight,PicStyle,PicBorderColor,PicSpacing,Page
		...
		Else
		  ...
		  End If
			...
			LabelContent       = Replace(Replace(LabelContent, "{Tag:GetPageList", ""),"}" & LabelStyle&"{/Tag}", "")
			 LabelContent=replace(replace( LabelContent,"<","&lt;"),">","&gt;")
            'ks.die LabelContent
			Dim XMLDoc,Node
		...
		If LabelStyle="" Then LabelStyle="<ul>" & vbcrlf & "[loop={@num}] " & vbcrlf & " <li><a href=""{@linkurl}"" target=""_blank"" title=""{@fulltitle}"">{@title}</a></li>" & vbcrlf & "[/loop]" &vbcrlf & "</ul>" & vbcrlf & "[KS:PageStyle]"
		...
		{  
		   switch (parseInt(channelid)){
		    ...
		    case 7:
		     ...
		     break;
		    case 8:
		     $("#ShowFieldArea").html(TempFieldStr+"<li onclick=\"InsertLabel('{@validdate}')\" title=\"有效期\">有 效 期</li><li onclick=\"InsertLabel('{@typeid}')\" title=\"交易类别\">交易类别</li><li title=\"联系人\" onclick=\"InsertLabel('{@contactman}')\">联 系 人</li><li title=\"公司名称\" onclick=\"InsertLabel('{@companyname}')\">公司名称</li><li title=\"所在省份\" onclick=\"InsertLabel('{@province}')\">所在省份</li><li title=\"所在城市\" onclick=\"InsertLabel('{@city}')\">所在城市</li><li title=\"所在城镇\" onclick=\"InsertLabel('{@county}')\">所在城镇</li><li title=\"详细地址\" onclick=\"InsertLabel('{@address}')\">详细地址<li title=\"联系电话\" onclick=\"InsertLabel('{@tel}')\">联系电话</li></li>");
		     break;
      ...
      function FieldInsertCode(fieldname,dbtype,dbname)
		...
		.echo "            <tr class=tdbg>"
		.echo "              <td colspan='2' id='ShowFieldArea' class='field'><li onclick=""InsertLabel('{@autoid}')"">行 号</li><li onclick=""InsertLabel('{@linkurl}')"">链接URL</li>"
		If KS.Wsetting(0)="1" Then
		 .echo "<li onclick=""InsertLabel('{@link3gurl}')"">链接URL(手机)</li>"
		End If
		.echo  " <li onclick=""InsertLabel('{@id}')"">文档ID</li><li onclick=""InsertLabel('{@title}')"">标 题</li><li onclick=""InsertLabel('{@fulltitle}')"" style='color:red'>不截断标题</li> <li onclick=""InsertLabel('{@intro}')"">简要介绍</li><li onclick=""InsertLabel('{@photourl}')"">图片地址</li><li onclick=""InsertLabel('{@adddate}')"">添加时间</li><li onclick=""InsertLabel('{@inputer}')"">录入员</li><li onclick=""InsertLabel('{@hits}')"">点击数</li><li onclick=""InsertLabel('{@newimg}')"" title='显示新信息图片标志' style='color:red;'>最新图标志</li><li onclick=""InsertLabel('{@hotimg}')"" title='显示热门信息图片标志' style='color:red;'>热门图标志</li><li onclick=""InsertLabel('{@classname}')"">当前栏目名称</li><li onclick=""InsertLabel('{@classurl}')"">当前栏目URL</li><li onclick=""InsertLabel('{@topclassname}')"">一级栏目名称</li><li onclick=""InsertLabel('{@topclassurl}')"">一级栏目URL</li>"
		If KS.Wsetting(0)="1" Then
		  .echo "<li onclick=""InsertLabel('{@class3gurl}')"">当前栏目URL(手机)</li>"
		End If
		.echo "<li onclick=""InsertLabel('{@oclassname}')"">附属栏目名称</li><li onclick=""InsertLabel('{@oclassurl}')"">附属栏目URL</li><li onclick=""InsertLabel('{@otopclassname}')"">附属一级栏目名称</li><li onclick=""InsertLabel('{@otopclassurl}')"">附属一级栏目URL</li><li onclick=""InsertLabel('{@keywords}')"">关键字</li><li onclick=""InsertLabel('{@keywordsbylink}')"">关键字带链接</li></td>"
		.echo "            </tr>"
		.echo "            <tr class=tdbg>"
		...
		.echo "            </tr>"
		.echo "            <tr class=tdbg>"
		.echo "              <td colspan='2' class='attention'><strong><font color=red>使用说明 :</font></strong><br />1、循环标签[loop=n][/loop]对可以省略,也可以平行出现多对；<br />2、自定义输出样式时，在需要显示分页的地方放入标签：[KS:PageStyle]<br/>3、输出格式选择不带自定义字段输出的运行效率高于带自定义字段,如果列表没有用到自定义字段请选择不带自定义字段输出</font><br/>4、支持条件判断语句，格式：<font color=blue>{$IF 条件}</font><font color=red>{成立执行的代码}</font><font color=green>{不成立执行的代码}</font><font color=blue>{/$IF}</font></td>"
		.echo "            </tr>"
		.echo "           </tbody>"
		...
		.echo "           <tbody id='ModelParamArea1'><tr class='tdbg'><td>指定地区 "
		 Response.Write "<script type='text/javascript'>"
							...
							<%
		.echo "</td><td><label style='font-weight:bold;color:brown'><input type='checkbox' value='1' name='ispic' id='ispic'"
