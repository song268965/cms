...
Class GetGenericList
		...
		End Sub
		Public Sub Kesion()
		Dim TempClassList, Page,InstallDir, CurrPath, FolderID, LabelContent, L_C_A, Action, LabelID, Str, Descript, LabelFlag
		...
		Dim PicWidth,PicHeight,PicStyle,PicBorderColor,PicSpacing,CallBySpace,oTid,IncludeoTid
		Dim ButtonType,PriceType,SQLParam
		Dim TypeID,ShowGQType,Province,City,County
		...
		If LabelStyle="" Then LabelStyle="[loop={@num}] " & vbcrlf & "<li><a href=""{@linkurl}"" target=""_blank"" title=""{@fulltitle}"">{@title}</a></li>" & vbcrlf & "[/loop]"
		...
		$(document).ready(function(){
		  ...
		  $("#ChannelID").change(function(){
			...
			$.get('../../../plus/ajaxs.asp',{from:'label',action:'GetClassOption',channelid:$(this).val()},function(data){
			  $("#ClassList").empty();
			  $("#ClassList").append("<option value='-1' style='color:red'>-当前栏目(通用)-</option>").append("<option value='-2' style='color:green'>-同顶级栏目-</option>").append("<option value='0'>-不指定栏目-</option>").append(unescape(data));
		...
		function SetField(channelid){  
		   switch (parseInt(channelid)){
		    ...
		    case 7:
		     ...
		     break;
		    case 8:
		     $("#ShowFieldArea").html(TempFieldStr+"<li onclick=\"InsertLabel('{@validdate}')\" title=\"有效期\">有 效 期</li><li onclick=\"InsertLabel('{@typeid}')\" title=\"交易类别名称\">交易类别名称</li><li onclick=\"InsertLabel('{@gqtypeid}')\" title=\"交易类别ID\">交易类别ID</li><li onclick=\"InsertLabel('{@price}')\" title=\"交易价格\">交易价格</li><li title=\"联系人\" onclick=\"InsertLabel('{@contactman}')\">联 系 人</li><li title=\"公司名称\" onclick=\"InsertLabel('{@companyname}')\">公司名称</li><li title=\"所在省份\" onclick=\"InsertLabel('{@province}')\">所在省份</li><li title=\"所在城市\" onclick=\"InsertLabel('{@city}')\">所在城市</li><li title=\"所在城镇\" onclick=\"InsertLabel('{@county}')\">所在城镇</li><li title=\"详细地址\" onclick=\"InsertLabel('{@address}')\">详细地址<li title=\"联系电话\" onclick=\"InsertLabel('{@tel}')\">联系电话</li></li>");
		     break;
			
		   default:
		     $("#ShowFieldArea").html(TempFieldStr);
		   }
		   if (channelid!=0)  $("#ShowFieldArea").html($("#ShowFieldArea").html()+'<li onclick="InsertLabel(\'{@cmtnum}\')">评论数</li><li onclick="InsertLabel(\'{@avgscore}\')">点评平均分</li>')
		 
		   $("#OrderStr option[style]").remove();
		   $(parent.document).find('#ajaxmsg').toggle();
		  	  $.get('../../../plus/ajaxs.asp',{action:'GetOrderOption',channelid:channelid},function(data){
			  $("#OrderStr").append(data)
			  $("#OrderStr option[value='<%=OrderStr%>']").attr("selected",true);
			  $(parent.document).find('#ajaxmsg').toggle();
			 });
		...
		function SetModelParam(channelid)
		{
		  ...
		  $.get('../../../plus/ajaxs.asp',{action:'getModelType',channelid:channelid},function(basictype){
		     if (basictype==1 ||basictype==2 || basictype==8){
		...
		{ 
		   ...
		   var link="include/FieldParam.asp?fieldname=" + fieldname + "&dbtype="+ dbtype + "&dbname=" + dbname+"&datasourcetype=0";
		  top.openWin('插入字段标签',link,false,350,240);
		}
		...
		function CheckForm(){
			...
			if ($("#ShowClassName").prop("checked")==true) ShowClassName = true;
			var ShowPicFlag=false;
			if ($("#ShowPicFlag").prop("checked")==true)  ShowPicFlag= true;
			var ShowHotFlag=false;
			if ($("#ShowHotFlag").prop("checked")==true)  ShowHotFlag= true;
			var ShowNewFlag=false;
			if ($("#ShowNewFlag").prop("checked")==true)  ShowNewFlag= true;
			var CallBySpace=false;
			if ($("#CallBySpace").prop("checked")==true) CallBySpace=true;
			var ispic=false;
			if ($("#ispic").prop("checked")==true) ispic=true;
		   ...
		   $("input[name=attr]").each(function(){
			...
			if (ChannelID==5){
			 ...
			 if($("#ShowGQType").prop("checked")==true) ShowGQType=true;
		...
		.echo "                 <option value=""-2"" style=""color:green"">- 同顶级栏目-</option>"
		...
		.echo "&nbsp;&nbsp;<input type='checkbox' name='CallBySpace' onclick=""if (this.checked){alert('提示：勾选后该标签只能放在个人/企业空间模板');}"" id='CallBySpace' value='1'"
		...
		.echo "                <label><input name=""DocProperty""  id=""DocProperty"" type=""checkbox""  value=""1"""
		If mid(DocProperty,1,1) = 1 Then .echo (" Checked")
		.echo ">推荐</label>"
		.echo "<label><input name=""DocProperty"" id=""DocProperty"" type=""checkbox""  value=""2"""
		If mid(DocProperty,2,1) = 1 Then .echo (" Checked")
		  .echo ">滚动</label>"
		.echo "<label><input name=""DocProperty"" id=""DocProperty"" type=""checkbox"" value=""3"""
		If mid(DocProperty,3,1) = 1 Then .echo (" Checked")
		  .echo ">头条</label>"
		.echo "<label><input name=""DocProperty"" id=""DocProperty"" type=""checkbox"" value=""4"""
		If mid(DocProperty,4,1) = 1 Then .echo (" Checked")
		  .echo ">热门</label>"
		.echo "<label><input name=""DocProperty"" id=""DocProperty"" type=""checkbox"" value=""5"""
		If mid(DocProperty,5,1) = 1 Then .echo (" Checked")
		  .echo ">幻灯</label>"
		.echo "<label><input onclick=""if(this.checked)alert('温馨提示:标签只有指定到文章类模型此属性才有效')"" id=""DocProperty"" name=""DocProperty"" type=""checkbox"" value=""6"""
		...
		.echo "                <select style=""width:70%;"" class='textbox' name=""OrderStr"" id=""OrderStr"">"
					...
					.echo ("<option value='Rnd' selected>随机显示</option>")
					Else
					.echo ("<option value='Rnd'>随机显示</option>")
					End If
		...
		.echo "           <tbody id='ModelParamArea1'><tr class='tdbg'><td>指定地区"
							...
							<%
		.echo" </script></td><td><label style='font-weight:bold;color:brown'><input type='checkbox' value='1' name='ispic' id='ispic'"
		if cbool(ispic)=true then .echo " checked"
		.echo "/>仅显示有图片的文档</label></td></tr></tbody>"
		
		.echo "            <tbody id=""DiyArea"">"
		.echo "            <tr class=tdbg>"
		.echo "              <td colspan='2' id='ShowFieldArea' class='field'><li onclick=""InsertLabel('{@autoid}')"">行 号</li><li onclick=""InsertLabel('{@linkurl}')"">链接URL(PC)</li>"
		If KS.Wsetting(0)="1" Then
		 .echo "<li onclick=""InsertLabel('{@link3gurl}')"">链接URL(手机)</li>"
		End If
		.echo "<li onclick=""InsertLabel('{@id}')"">文档ID</li><li onclick=""InsertLabel('{@title}')"">标 题</li><li onclick=""InsertLabel('{@fulltitle}')"" style='color:red'>不截断标题</li> <li onclick=""InsertLabel('{@intro}')"">简要介绍</li><li onclick=""InsertLabel('{@photourl}')"">图片地址</li><li onclick=""InsertLabel('{@adddate}')"">添加时间</li><li onclick=""InsertLabel('{@inputer}')"">录入员</li><li onclick=""InsertLabel('{@hits}')"">点击数</li><li onclick=""InsertLabel('{@newimg}')"" title='显示新信息图片标志' style='color:red;'>最新图标志</li><li onclick=""InsertLabel('{@hotimg}')"" title='显示热门信息图片标志' style='color:red;'>热门图标志</li><li onclick=""InsertLabel('{@classname}')"">当前栏目名称</li><li onclick=""InsertLabel('{@classurl}')"">当前栏目URL(PC)</li>"
		If KS.Wsetting(0)="1" Then
		  .echo "<li onclick=""InsertLabel('{@class3gurl}')"">当前栏目URL(手机)</li>"
		End If
		.echo "<li onclick=""InsertLabel('{@topclassname}')"">一级栏目名称</li><li onclick=""InsertLabel('{@topclassurl}')"">一级栏目URL</li><li onclick=""InsertLabel('{@oclassname}')"">附属栏目名称</li><li onclick=""InsertLabel('{@oclassurl}')"">附属栏目URL</li><li onclick=""InsertLabel('{@otopclassname}')"">附属一级栏目名称</li><li onclick=""InsertLabel('{@otopclassurl}')"">附属一级栏目URL</li><li onclick=""InsertLabel('{@otopclassurl}')"">附属一级栏目URL</li><li onclick=""InsertLabel('{@keywords}')"">关键字</li><li onclick=""InsertLabel('{@keywordsbylink}')"">关键字带链接</li></td>"
		.echo "            </tr>"
		.echo "            <tr class=tdbg>"
		...
		.echo "                <select name=""NavType"" style=""width:70%;"" class='textbox' onchange=""SetNavStatus()"">"
		 ...
		 .echo "             <td width=""50%"" height=""24"">"
			   If LabelID = "" Or CStr(NavType) = "0" Then
				  ...
				  .echo ("<input class='button' type=""button"" onClick=""OpenThenSetValue('include/SelectPic.asp?CurrPath=" & CurrPath & "&ShowVirtualPath=true',550,290,window,document.myform.NaviPic);"" name=""Submit3"" value=""选择图片..."">")
		...
		.echo "              <td width=""50%"" height=""24"">"
				If LabelID = "" Or CStr(MoreLinkType) = "0" Then
					...
					.echo ("<input class='button' type=""button"" onClick=""OpenThenSetValue('include/SelectPic.asp?CurrPath=" & CurrPath & "&ShowVirtualPath=true',550,290,window,document.myform.MoreLinkPic);"" name=""Submit3"" value=""选择图片..."">")
		...
		.echo "                <input class='button' name=""SubmitPic"" class='button' onClick=""OpenThenSetValue('include/SelectPic.asp?CurrPath=" & CurrPath & "&ShowVirtualPath=true',550,290,window,document.myform.SplitPic);"" type=""button"" id=""SubmitPic2"" value=""选择图片..."">"
		...
		.echo "            </tr>"
		.echo "            <tr class=tdbg>"
		.echo "              <td height=""24"">边框颜色 <input type=""text"" id=""PicBorderColor"" class=""textbox"" name=""PicBorderColor"" style=""width:120;"" value=""" & PicBorderColor & """><img border=0 id=""MarkFontColorShow"" src=""../../images/rect.gif"" style=""cursor:pointer;background-Color:" & PicBorderColor & ";"" onClick=""Getcolor('MarkFontColorShow','../../../editor/ksplus/selectcolor.asp?MarkFontColorShow|PicBorderColor');"" title=""选取颜色""> 可留空</td>"
		.echo "            </tr>"
		.echo "            <tr class=tdbg>"
