...
'主体部分
Public Sub Kesion()
Dim TypeID,InstallDir, CurrPath, FolderID, LabelContent, L_C_A, Action, LabelID, Str, Descript,DateRule,DateAlign,UserName,Page
Dim TypeFlag, OpenType, NavType, Navi, TitleCss, Num, TitleLen,SplitPic, ChannelID,PrintType,isbest,morestr,ShowType,AjaxOut,LabelStyle,OrderStr,RowHeight,isslide,recommend
  ...
  End If
    ...
    Set LabelRS = Nothing
			...
			If  Not Node Is Nothing Then
				...
				isslide         = Node.getAttribute("isslide")
...
%>
   ...
   function CheckForm()
   {
	...
	if (document.myform.isbest.checked)
	   ...
	   isbest=false;
	if (document.myform.isslide.checked)
	   isslide= true
	else
	   isslide=false;
...
<%
		...
		.echo "                   <option value=""0"">-不指定类别-</option>"
		 ...
		 dim i,rs:Set Rs = Conn.Execute("SELECT typeid,depth,typeName FROM KS_blogtype ORDER BY rootid,orderid")
							Do While Not Rs.EOF
								Response.Write "<option value=""" & Rs("typeid") & """ "
								If trim(TypeID)=trim(RS("TypeID")) Then Response.Write "selected"
								Response.Write ">"
...
.echo "              <td height=""30"">"
    If cbool(isbest) = True Then
		 .echo ("&nbsp;&nbsp;&nbsp;<input type=""checkbox"" value=""true"" name=""isbest"" checked>精华")
	Else
		 .echo ("&nbsp;&nbsp;&nbsp;<input type=""checkbox"" value=""true"" name=""isbest"">精华")
	End If
    If cbool(recommend) = True Then
		 .echo ("&nbsp;&nbsp;&nbsp;<input type=""checkbox"" value=""true"" name=""recommend"" checked>推荐")
	Else
		 .echo ("&nbsp;&nbsp;&nbsp;<input type=""checkbox"" value=""true"" name=""recommend"">推荐")
	End If
    If cbool(isslide) = True Then
		 .echo ("&nbsp;&nbsp;&nbsp;<input type=""checkbox"" value=""true"" name=""isslide"" checked>幻灯")
	Else
		 .echo ("&nbsp;&nbsp;&nbsp;<input type=""checkbox"" value=""true"" name=""isslide"">幻灯")
	End If
