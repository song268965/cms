...
Class GetMarqueeArticle
		...
		Dim ChannelID,ClassID, IncludeSubClass, MarqueeWidth, MarqueeHeight, DateRule, OpenType, Num, MarqueeBgcolor, MarqueeDirection, TitleLen, MarqueeStyle, OrderStr, MarqueeSpeed, TitleCss, DateCss,SpecialID,DocProperty,NavType,Navi,CurrPath,InstallDir,Attr,Page,oTid,IncludeoTid
		...
		{  
		   ...
		   $("input[name=attr]").each(function(){
			...
			var tagVal='{Tag:GetMarquee labelid="0" marqueetype="text" modelid="'+ChannelID+'" classid="'+ClassList+'" otid="'+$("#oTid").val()+'" includeotid="'+IncludeoTid+'" specialid="'+SpecialID+'" includesubclass="'+IncludeSubClass+'" attr="'+av+'" docproperty="'+DocProperty+'" orderstr="'+OrderStr+'" marqueewidth="'+MarqueeWidth+'" marqueeheight="'+MarqueeHeight+'" opentype="'+OpenType+'" num="'+Num+'" titlelen="'+TitleLen+'" titlecss="'+TitleCss+'" datecss="'+DateCss+'" daterule="'+DateRule+'" marqueedirection="'+MarqueeDirection+'" marqueespeed="'+MarqueeSpeed+'" marqueebgcolor="'+MarqueeBgcolor+'" marqueestyle="'+MarqueeStyle+'" navtype="'+NavType+'" nav="'+Nav+'"}{/Tag}';
