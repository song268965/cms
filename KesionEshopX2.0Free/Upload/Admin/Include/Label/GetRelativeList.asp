...
Class GetRelativeList
		...
		if KS.IsNul(IncludeSubClass) then IncludeSubClass=false
		If Num = "" Then Num = 10
		...
		$(document).ready(function(){
		  $("#ChannelID").change(function(){
				...
				$.get('../../../plus/ajaxs.asp',{from:'label',action:'GetClassOption',channelid:$(this).val()},function(data){
				  $("#ClassList").empty();
				  $("#ClassList").append("<option value='0'>-不指定栏目-</option>").append(unescape(data));
				  $(parent.document).find('#ajaxmsg').toggle();
				 });
      ...
      function FieldInsertCode(fieldname,dbtype,dbname){ 
		...
		function CheckForm()
		{
			...
			var tagVal='{Tag:GetRelativeList labelid="0" printtype="'+PrintType+'" ajaxout="'+AjaxOut+'" modelid="'+ChannelID+'" classid="'+ClassList+'" includesubclass="'+IncludeSubClass+'" docproperty="'+DocProperty+'" relativetext="'+$("#relativeText option:selected").val()+'" relativetype="'+RelativeType+'" orderstr="'+OrderStr+'" opentype="'+OpenType+'" num="'+Num+'" titlelen="'+TitleLen+'" introlen="'+IntroLen+'" rowheight="'+RowHeight+'" col="'+ColNumber+'" showclassname="'+ShowClassName+'" showpicflag="'+ShowPicFlag+'" shownewflag="'+ShowNewFlag+'" showhotflag="'+ShowHotFlag+'" navtype="'+NavType+'" nav="'+Nav+'"  splitpic="'+SplitPic+'" daterule="'+DateRule+'" datealign="'+DateAlign+'" titlecss="'+TitleCss+'" datecss="'+DateCss+'" picwidth="'+PicWidth+'" picheight="'+PicHeight+'" picstyle="'+PicStyle+'" picbordercolor="'+PicBorderColor+'" picspacing="'+PicSpacing+'"';
			if (ChannelID==5){
...
.echo "                <select class=""textbox"" style=""width:170px"" name=""ClassList"" id=""ClassList"" onChange=""SetLabelFlag(this)"">"
		            ...
		            If lcase(IncludeSubClass) = "true" Or LabelID = "" Then
					  Str = " Checked"
					Else
					  Str = ""
					End If
		...
		.echo "         <span style='color:green'>不区分模型和栏目可以关联非本模型或栏目下的信息,如文章模型的文章可以关联出相关的商品</span>"			
