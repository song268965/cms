...
Class GetRolls
		...
		Dim ClassID, IncludeSubClass, MarqueeDirection, MarqueeWidth, MarqueeHeight, PicWidth, PicHeight, PicStyle, OpenType, Num, TitleLen, ShowTitle, OrderStr, MarqueeSpeed, TitleCss,SpecialID,DocProperty,Attr,oTid,IncludeoTid
		Dim CurrPath, InstallDir,Page
		...
		{  
		   ...
		   $("input[name=attr]").each(function(){
			...
			var tagVal='{Tag:GetRolls labelid="0" modelid="'+ChannelID+'" classid="'+ClassList+'" otid="'+$("#oTid").val()+'" includeotid="'+IncludeoTid+'" specialid="'+SpecialID+'" includesubclass="'+IncludeSubClass+'" attr="'+av+'" docproperty="'+DocProperty+'" orderstr="'+OrderStr+'" marqueewidth="'+MarqueeWidth+'" marqueeheight="'+MarqueeHeight+'" opentype="'+OpenType+'" showtitle="'+ShowTitle+'" picwidth="'+PicWidth+'" picheight="'+PicHeight+'" num="'+Num+'" titlelen="'+TitleLen+'" titlecss="'+TitleCss+'" marqueedirection="'+MarqueeDirection+'" marqueespeed="'+MarqueeSpeed+'" picbordercolor="'+PicBorderColor+'"}{/Tag}';
		...
		.echo "                <select name=""ChannelID"" id=""ChannelID"" class='textbox'>"
		...
		.echo "            </tr>"
		  ...
		  .echo (" <input type=""text"" class=""textbox"" name=""PicBorderColor"" id=""PicBorderColor"" style=""width:120px;"" value=""" & PicBorderColor & """>")
		  .echo (" <img border=0 id=""PicBorderColorShow"" src=""../../images/rect.gif"" style=""cursor:pointer;background-Color:" & PicBorderColor & ";"" onClick=""Getcolor('PicBorderColorShow','../../../editor/ksplus/selectcolor.asp?PicBorderColorShow|PicBorderColor');"" title=""选取颜色""> 可留空")
