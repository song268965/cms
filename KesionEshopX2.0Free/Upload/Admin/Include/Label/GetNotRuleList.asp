...
Class GetNotRuleList
		...
		Dim ChannelID,ClassID, IncludeSubClass,  OpenType, ArticleProperty, RowNumber, RowHeight, ShowNumPerRow, OrderStr,ShowPicFlag, NavType, Navi, MoreLinkType, MoreLink, SplitPic,  TitleCss, PrintType,DocProperty,SpecialID,AjaxOut,Attr,Page,oTid,IncludeoTid
		...
		function CheckForm()
		{
		   ...
		   $("input[name=attr]").each(function(){
			...
			var tagVal='{Tag:GetNotRuleList labelid="0" ajaxout="'+AjaxOut+'" modelid="'+ChannelID+'" classid="'+ClassList+'" otid="'+$("#oTid").val()+'" includeotid="'+IncludeoTid+'" specialid="'+SpecialID+'" includesubclass="'+IncludeSubClass+'" attr="'+av+'" docproperty="'+DocProperty+'" orderstr="'+OrderStr+'" opentype="'+OpenType+'" rownumber="'+RowNumber+'" shownumperrow="'+ShowNumPerRow+'" rowheight="'+RowHeight+'" titlecss="'+TitleCss+'" navtype="'+NavType+'" nav="'+Nav+'" splitpic="'+SplitPic+'" printtype="'+PrintType+'" morelinktype="'+MoreLinkType+'" morelink="'+MoreLink+'"}{/Tag}';
		...
		.echo "                 <option selected value=""-1"" style=""color:red"">- 当前栏目(通用)-</option>"
					...
					End If
					  ...
					  .echo ("&nbsp;&nbsp;&nbsp;<input name=""IncludeSubClass"" type=""checkbox"" id=""IncludeSubClass"" value=""true""" & Str & ">调用子栏目</div>")
