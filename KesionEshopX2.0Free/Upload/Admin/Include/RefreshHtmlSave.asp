...
Class RefreshHtmlSave
		...
		Private Types,From,RefreshTag,FromStr
		...
		Public Sub Kesion()
			...
			from=KS.S("From")
			RefreshTag=KS.C_S(ChannelID,7)
			FromStr="PC版"
			if From="3g" Then RefreshTag=KS.ChkClng(KS.M_C(ChannelID,28)) : FromStr="手机版" :  Fcls.CallFrom3g="true"
			
			If From="3g" and IsBusiness=false Then
			  ' KS.AlertHintScript "对不起，免费体验版不支持手机版生成HTML。"
			End If
			...
			Select Case Types
			 Case "Content"
			            If RefreshTag<>1 and RefreshTag<>2 Then Call KS.AlertHistory("KesionCMS系统提醒您：<br/><br/>此模型" & FromStr &"内容页没有启用生成静态HTML功能<br/><br/>如需启用请到模型管理->模型信息设置->生成选项里启用生成静态Html功能",-1):Exit Sub
						Call RefreshContent
			 Case "Folder"
			          
			          If ChannelID<>0 and KS.C_S(ChannelID,7)<>1 Then Call KS.AlertHistory("KesionCMS系统提醒您：<br/><br/>此模型栏目页没有启用生成静态HTML功能<br/><br/>如需启用请到模型管理->模型信息设置->生成选项里启用生成静态Html功能",-1):Exit Sub
		...
		Sub Main()
		  ...
		  .echo ("var box='';"&vbcrlf)
		  .echo ("var input;"&vbcrlf)
		  .echo (" function show(){"&vbcrlf)
		  .echo ("  box=$.dialog({id:'msg',title:false,content:""<div id='fsotips'>正在整理数据,请稍候!</div>"",init: function(){"&vbcrlf)
		  .echo ("	          input = this.DOM.content[0].getElementsByTagName('div')[0];"&vbcrlf)
	      .echo ("			},min:false,max:false});"&vbcrlf)
		  .echo ("}"&vbcrlf)
		  .echo ("</script>")
		  .echo ("</head>")
		  .echo ("<link rel=""stylesheet"" href=""Admin_Style.css"">")
		  If RefreshFlag<>"ID" Then
		  .echo ("<body oncontextmenu=""return false;"" scroll=no><div class='pageCont2'>")
		  Else
		 ...
		 Else
				.echo "<div class='pageCont2'><br><br><br><table id=""BarShowArea"" width=""400"" border=""0"" align=""center"" cellspacing=""1"" cellpadding=""1"" style='margin:0 auto 20px;'>"
		 End iF
				.echo "<tr> "
				...
				.echo "</td></tr></table>"
				.echo "<table width=""100%"" border=""0"" align=""center"" cellspacing=""1"" cellpadding=""1""><tr> "
...
'                                                     以下为本模块相应处理的函数		'================================================================================================================================
		 ...
		 If NewsNo = "" Then NewsNo = 0
		  Select Case RefreshFlag
			...
			Case "IDS"
			    ...
			    R_Sql = "Select * from KS_Class where ID IN('" & FolderID & "') and ClassType<>2 and DelTF=0"
			Case "Folder"
				FolderID = Trim(Request("FolderID"))
				R_Sql = "Select * from KS_Class where ChannelID=" & ChannelID &" and  DelTF=0 And ID IN (" & FolderID & ") and classType<>2 Order By FolderOrder ASC"
		   Case "All"
		...
		Else
				...
				.echo "</script>" & vbCrLf
		End If
		  .echo "<script>try{box.close();}catch(e){}</script>"&vbcrlf
		End With
		End Sub
		...
		If R_Sql <> "" Then
		  If DataBaseType=1 Then
			...
			If R_RS.EOF And R_RS.BOF Then
				.echo "<script>img2.width=""0"";" & vbCrLf
				.echo "txt2.innerHTML=""没有可生成的内容页！<br><br><input name='button1' type='button' onclick=javascript:location='RefreshHtml.asp?from=" & KS.S("from") &"&Action=ref&ChannelID=" & ChannelID  &"'; class='button' value=' 返 回 '>"";" & vbCrLf
				.echo "txt3.innerHTML="""";" & vbCrLf
				.echo "txt4.innerHTML="""";" & vbCrLf
				.echo "document.all.BarShowArea.style.display='none';" & vbCrLf
				.echo "</script>" & vbCrLf
			...
			Else
				...
				For NowNum=CurrNowNum To RefreshTotalNum
					...
					If RefreshTag=0 Then
					      FsoHtmlList=GetRefreshErr(KSRObj.Node,ItemName)
					Else
					
					      '附属文档
						   IF KS.ChkClng(KSRObj.Node.SelectSingleNode("@oid").text)<>0 Then
							  If KS.ChkClng(KS.C_C(KSRObj.Node.SelectSingleNode("@otid").text,12))>0 Then
								Dim RSO:Set RSO=Conn.Execute("select top 1 * From "& KS.C_S(KS.C_C(KSRObj.Node.SelectSingleNode("@otid").text,12),2) & " Where ID=" & KS.ChkClng(KSRObj.Node.SelectSingleNode("@oid").text))
								If Not RSO.Eof Then
								   Dim DocOXML:Set DocOXML=KS.RsToXml(RSO,"row","root")
								   Set KSRObj.ONode=DocOXML.DocumentElement.SelectSingleNode("row")
								End If
								RSO.Close
								Set RSO=Nothing
							  End If
				...
				If PauseNum>0 Then
					If RefreshTotalNum>1 and NowNum Mod PauseNum=0 Then
					    R_RS.Close:Set R_RS=Nothing
						 ...
						 Session("R_Sql")=R_Sql
						 .echo "<script>"
						 .echo "fsohtml.innerHTML='<div style=""text-align:cdenter""><div style=""margin:10px;height:80px;padding:8px;border:1px dashed #cccccc;text-align:left;""><img src=""../images/succeed.gif"" align=""left""><br>&nbsp;&nbsp;&nbsp;&nbsp;<b>温馨提示：</b><br>&nbsp;&nbsp;&nbsp;&nbsp;以免过度占用服务器资源，系统暂停" & PauseTime & "秒后继续<img src=""../../images/default/wait.gif""><br>&nbsp;&nbsp;&nbsp;&nbsp;如果" & PauseTime & "秒后没有继续，请点此<a href=""RefreshHtmlSave.asp?from=" & KS.S("from") &"&CurrNowNum=" & NowNum+1 & "&ChannelID=" & ChannelID & "&RefreshFlag=Pause&Types=" & Types & "&StartRefreshTime=" & StartRefreshTime & "&RefreshTotalNum=" & RefreshTotalNum & """><font color=red>继续</font></a>或点此<a href=""refreshhtml.asp?Action=ref&channelid=" & channelid & """><font color=red>停止</font></a>!</div></div>';" & vbCrLf
						 .echo "</script>" &vbcrlf
						 .die "<meta http-equiv=""refresh"" content=""" & PauseTime & ";url=RefreshHtmlSave.asp?from=" & KS.S("from") &"&f=" & f & "&CurrNowNum=" & NowNum+1 & "&ChannelID=" & ChannelID & "&RefreshFlag=Pause&Types=" & Types & "&StartRefreshTime=" & StartRefreshTime &  "&RefreshTotalNum=" & RefreshTotalNum & """>"
					End If
			   End If
			Next
				...
				.echo "txt3.innerHTML=""总共生成了 <font color=red><b>" & RefreshTotalNum & "</b></font> 条,总费时:<font color=red>" & Left((Timer() - StartRefreshTime), 4) & "</font> 秒<br><br><input name='button1' type='button' onclick=javascript:location='RefreshHtml.asp?from=" & KS.S("from") &"&Action=ref&ChannelID=" & ChannelID & "'; class='button' value=' 返 回 '>"";" & vbCrLf
		...
		Function GetRefreshSucc(Node,ItemName)
		 ...
		 If Fcls.CallFrom3g="true" Then
		   FsoHtmlPath = KS.Get3GItemURL(ChannelID,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@id").text&KS.WSetting(9))
		 Else
		   FsoHtmlPath = KS.GetItemURL(ChannelID,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,node.SelectSingleNode("@fname").text,node.SelectSingleNode("@adddate").text)
		 End If
		 str=""
		 if RefreshFlag<>"ID" Then str="<img src=""../images/succeed.gif"" align=""left""><table border=""0"">"
		 
		 if Types="Content" and Not KS.IsNul(KS.Setting(189)) and Node.SelectSingleNode("@refreshtf").text="0" Then
		     dim baiduts
	         baiduts=PostHTTPPage(KS.Setting(189),FsoHtmlPath)
			 GetRefreshSucc=str & "<table border=""0"">"_
									...
									& "<tr><td><li><strong>生成路径：</strong></li></td><td><a href=""" & FsoHtmlPath & """ target=""_blank"">" & FsoHtmlPath & "</a></li></td><tr>" _
									& "<tr><td><li><strong>百度返回码：</strong></li></td><td>"&baiduts&"</li></td><tr>" _
									& "</table>"
				  conn.execute("update " & Table & " set refreshtf=1 where id=" & Node.SelectSingleNode("@id").text)	 
		 else
		 
		 GetRefreshSucc=str & "<table border=""0"">"_
									& "<tr><td><li><strong>ID 号为：</strong></li></td><td> <font color=red>"  & Node.SelectSingleNode("@id").text & "</font> 的" & ItemName & "(" & FromStr &")已生成</td></tr>"_
									& "<tr><td><li><strong>" & ItemName & "标题：</strong></li></td><td><font color=red>" & Node.SelectSingleNode("@title").text & "</font></li></td><tr>" _
									& "<tr><td><li><strong>生成路径：</strong></li></td><td><a href=""" & FsoHtmlPath & """ target=""_blank"">" & FsoHtmlPath & "</a></li></td><tr>" _
									& "</table>"
		End if		  
