...
Class DIYFunction
	  ...
	  Sub Step0()
	    With KS
		 .echo "<body>"
	 	 .echo " <table width='100%' height='25' border='0' cellpadding='0' cellspacing='1' class='sort' style='background:none;'>"
 ...
 %>
		...
		<div class="pageCont2">
	     <table border='0' cellspacing='1' cellpadding='1' width='95%' align='center' class='ctable'>
		  ...
		  <tr class='tdbg'>
		    ...
		    <td><input name="LabelName" class="textbox" id="LabelName" value="<%=LabelName%>" onblur='testlabelname()' style="width:200px;"> <font color=red>*</font> <span class="tips">例如百度新闻XML,幻灯片调用XML等。</span><span id='labelmessage'></span></td>
		  </tr>
		  <tr class='tdbg'>
		    <td class='clefttitle' align='right'><strong>生成XML文件名:</strong></td>
		    <td class="tips"><%=KS.Setting(127)%><input class="textbox" name="FileName" id="FileName" value="<%=FileName%>" onblur='testfilename()' style="width:200px;"> <font color=red>*</font> 例如news.xml,config.xml等。</font><span id='filenamemessage'></span></td>
		  </tr>
		  <tr class='tdbg'>
		  ...
		  <tr class='tdbg'>
		   ...
		   <td>
		     <select name="datasourcetype" id="datasourcetype" style="width:290px" onChange="changeconnstr()" class="textbox">
		  ...
		  <tr class='tdbg'>
		   ...
		   <td><textarea <%if datasourcetype=0 then .echo  " disabled"%> name="datasourcestr" id="datasourcestr" cols="70" rows="3" class="textbox"><%=Datasourcestr%></textarea>
		  ...
		  <tr class='tdbg'>
		   ...
		   <td><textarea name="LabelIntro" cols="80" style="width:98%" rows="4" class="textbox"><%=LabelIntro%></textarea>
		   </td>
		  </tr>
		  <%End If%>
		  ...
		  <tr class='tdbg'>
		   ...
		   <td><textarea name="note" cols="80" style="width:98%" rows="9" class="textbox"><%=note%></textarea>
		   </td>
		  </tr>
	...
	End Function
	 ...
	 Sub Step2()
		...
		.echo "      <td class=""tips""><input type=""hidden"" name=""FileName"" id=""FileName"" value=""" & FileName &""" ><input class=""textbox"" type=""text"" name=""FileName1"" id=""FileName1"" value=""" & FileName &""" disabled> 不可改</td>"
		.echo "    </tr>"
        .echo "    <tr class=""tableBorder1"">"
...
Sub AddLabelSave()
			...
			Else
				...
				LabelRS.AddNew
				 ...
				 KS.echo ("<script>if (confirm('成功提示:\n\n添加生成XML文档参数成功,继续添加吗?')){location.href='LabelXML.asp?Action=AddNew&LabelType=7&FolderID=" & FolderID & "';}else{$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & FolderID & "&OpStr=标签管理 >> 自义定生成XML文档&ButtonSymbol=DIYFunctionLabel';parent.frames['MainFrame'].location.href='Label_Main.asp?LabelType=7&FolderID=" & FolderID & "';}</script>")
			End If
	End Sub
	...
	Sub EditLabelSave()
			...
			Else
				...
				LabelRS.Open "Select top 1 * From [KS_Label] Where ID='" & LabelID & "'", Conn, 1, 3
				 ...
				 If KeyWord = "" Then
					KS.Echo ("<script>alert('成功提示:\n\nXML文档参数修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & FolderID & "&OpStr=标签管理  >> 自定义函数标签&ButtonSymbol=DIYFunctionLabel';location.href='Label_main.asp?Page=" & Page & "&LabelType=7&FolderID=" & FolderID & "';</script>")
				 Else
					KS.Echo ("<script>alert('成功提示:\n\nXML文档参数修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?OpStr=标签管理 >> <font color=red>搜索自定义生成XML文档结果</font>&ButtonSymbol=DIYFunctionSearch';location.href='Label_main.asp?Page=" & Page & "&LabelType=7&KeyWord=" & KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate & "&EndDate=" & EndDate & "';</script>")
				 End If
			End If
	End Sub
