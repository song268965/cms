...
Class RefreshHtmlSave
		...
		Public Sub Kesion()
			...
			Select Case Action
			 ...
			 Case Else
				  ...
				  ReturnInfo=ReturnInfo & "<form name='myform' action='?action=settingsave' method='post' class='mt20'><strong>生成配置：</strong>每个模型只生成最新添加的 <input type='text' name='num' value='" & KS.ChkClng(KS.ReadSetting(1)) & "' class='textbox' style='text-align:center;height:23px;line-height:23px;width:50px'/> 篇文档 <input type='submit' value=' 保存设置 ' class='button'/><br/><br/><font color=red>tips:如果不限制请输入0,如果您的网站数据量很大，旧的数据一般不需要重新生成，所以可以在此处设置一个值。</font></form>"
			      Call Main()
			End Select
			End With
		End Sub
		...
		Sub Initial()
		  Dim XMLStr,i,RS
			...
			RS.Open "select * from KS_Channel Where ChannelID not in(6,10) And BasicType<=8 And FsoHtmlTF<>0 And ChannelStatus=1 order by ChannelID",conn,1,1
			    i=0
				Do While Not RS.EOf
			...
			If KS.WSetting(0)="1" Then
				RS.Close
                RS.Open "select * from KS_Channel Where BasicType<>4 and BasicType<>6 And BasicType<=8 And ChannelStatus=1 order by ChannelID",conn,1,1
				Do While Not RS.Eof
				  If KS.ChkClng(KS.M_C(RS("ChannelID"),28))>=1 Then
				        i=I+1
					    XMLStr=XMLStr & "  <fsoitem isfinish=""0"" id=""" & i &""" channelid=""" & rs("channelid") & """>"&vbcrlf
						XMLStr=XMLStr & "    <name><![CDATA[" & rs("channelname") &"]]></name>" &vbcrlf
						XMLStr=XMLStr & "    <fsotype>-2</fsotype>" &vbcrlf
		...
		Sub Start()
		    call main()
			response.write "<div class='pageCont2'><table border='0' id='mytable' width='100%' height='100%' cellspacing='0' cellpadding='0'>"
			response.write "<tr><td width='280'>"
		   response.write "<iframe src='refreshquick.asp?action=startleft' frameborder='0' name='myframeleft' id='myframeleft' width='100%' height='100%'></iframe>"
           response.write "</td><td>"
		   response.write "<iframe src='refreshquick.asp?action=startright' frameborder='0' name='myframeright' id='myframeright' width='100%' height='100%'></iframe>"
		...
		Sub StartLeft()
			...
			response.write ("<link rel=""stylesheet"" href=""Admin_Style.css""><style>li{padding-left:4px;height:26px;line-height:26px;}body{background: #fff;}</style>")
			...
			For Each Node In TaskXML.DocumentElement.SelectNodes("fsoitem[@isfinish=1]")
			  select case KS.ChkClng(node.selectsinglenode("fsotype").text)
			     case 0 
                 ...
                 case 2
				  	response.write "<li><font color=green><b>√</b></font> [" & node.selectsinglenode("name").text & "]内容页(PC版)生成完毕</li>"
                 case -2
				  	response.write "<li><font color=green><b>√</b></font> [" & node.selectsinglenode("name").text & "]内容页(手机版)生成完毕</li>"
			     case 90 
            ...
            For Each Node In TaskXML.DocumentElement.SelectNodes("fsoitem[@isfinish=0]")
			   ...
			   select case KS.ChkClng(node.selectsinglenode("fsotype").text)
				 ...
				 case 1
					   ...
					   response.end
				 case 2
				       response.write "<li style='color:red;font-weight:bold'>正在生成[" & Node.SelectSingleNode("name").text & "]的内容页(PC版)...</li>"
					  ...
					  response.end
				 case -2
				       response.write "<li style='color:red;font-weight:bold'>正在生成[" & Node.SelectSingleNode("name").text & "]的内容页(手机版)...</li>"
					   response.write "<script>parent.myframeright.location.href='refreshquick.asp?action=startright&channelid=" & ChannelID & "&fsotype=-2';</script>"
					  response.end
				 case 90
		...
		Sub StartRight()
		 ...
		 Select Case FsoType
		  ...
		  case -1
				...
				FsoHtmlList="<div style='text-align:center'><div style='margin:0 auto;text-align:center;font-weight:bold;color:green;width:400px;'><img src='../images/succeed.gif' align='left'><br/>恭喜，所有生成任务执行完毕,共执行了 <font color=red>" & TotalNum & "</font> 个生成项目！<br/><br/><input type='button' value='返回一键发布首页' onclick=""parent.location.href='refreshquick.asp';"" class='button'/></div></div>" 
				Call Main
		  case else
		...
		Sub Main()
		  ...
		  <script>
		   $(function(){
		    $("#mytable").height($(window).height());
		   });
		  </script>
		  <%
		  .echo ("<body oncontextmenus=""return false;"" scroll=no style='background: #fff;'>")
		if action<>"startright" Then
		 .echo "<div class='pageCont2' style='margin-top:20px;'><div class='topdashed sort'> 一键快速生成HTML管理</div>"
		End If
