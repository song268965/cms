...
Class LabelSQLCls
	  ...
	  Sub Step0()
	    ...
	    <script type="text/javascript">
		  function CheckForm()
		  {
		   ...
		   if ($('#lbtf').val()=='false')
		   {
		      top.$.dialog.alert('标签名称不可用，请重输!',function(){
			  $('#LabelName').focus();});
			  return;
		   }
		   $('#myform').submit();
		  ...
		  function testsource()
		  {
			  ...
			  if (str=='')
			  {
				top.$.dialog.alert('请输入连接字符串!',function(){
				$('#datasourcestr').focus();});
				return false;
			  }
			  var url = 'LabelSQL.asp';
			  $.get(url,{action:"testsource",datatype:datatype,str:str},function(d){
				if (d=='true')
				 top.$.dialog.alert('恭喜，测试通过!')
				else
				 top.$.dialog.alert('对不起，字符串连接有误!');
			  });
		 } 
		...
		<div class="pageCont2">
	     <table border='0' cellspacing='1' cellpadding='1' width='95%' align='center' class='ctable'>
		  ...
		  <tr class='tdbg'>
		    ...
		    <td><input name="LabelName" class="textbox" id="LabelName" value="<%=LabelName%>" onblur='testlabelname()' style="width:200px;"> <font color=red>*</font><span id='labelmessage'></span><br>例如标签名称：&quot;推荐文章列表&quot;，则在模板中调用：<font color="#FF0000">&quot;{SQL_推荐文章列表(参数1,参数2...)}&quot;</font>。</td>
		  </tr>
		  <tr class='tdbg'>
		  ...
		  <tr class='tdbg' id='lbt'>
		   ...
		   <td>
			...
			<table border='0' id='pagearea' <%if sqltype=0 then .echo  " style=display:none"%>>
			 <tr><td>分页项目单位：<input type="text" value="<%=itemname%>" class="textbox" name="ItemName" size="6"> 如：篇、组、个、部等</td><td nowrap="nowrap">&nbsp;&nbsp;&nbsp;<%=ReturnDiyPageStyle(PageStyle)%></td>
			 </tr>
			 </table>
			</td>
	  ...
	  function checkfield(){
		var strtmpp='' ;
		strtmpp= "<table border='1' cellpadding='2' cellspacing='1'  width='98%' class='border'>";
		<%if datasourcetype=0 then%>
		strtmpp = strtmpp + "<tr align='center'><td title='通用标签'><font color=red>通用标签=></font></td>";
		...
		if (document.myform.FieldParam.value!=''){
	  ...
	  function CheckForm()
		{ var form=document.myform; 
		  ...
		  {  top.$.dialog.alert('请输入标签名称!',function(){
			  ...
			  form.submit(); 
			  $(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=标签管理 >> <font color=red>自定义SQL标签</font>&ButtonSymbol=LabelAdd';
			  return true;
		}
		 function changedb()
		 {
		...
		<script type="text/javascript">
		function ShowIframe()
		{  top.openWin("查看栏目<=>ID对照表","include/LabelXML.asp?action=ShowClassID",false);
		}
		</script>
	  <%
	   ...
	   With KS
		...
		<Select class="textbox" style="WIDTH: 250px; HEIGHT: 210px" onChange='addfield()' multiple size=2 name="field2">
	      ...
	      <tr class="tdbg">
            ...
            <td colspan=3><div id="ParamList"></div>
			<script>
			checkfield();
			</script>
			<textarea name='LabelIntro' onClick="setPos()" class="textbox" cols='97' rows='5' style='width:98%;height:80px' id='LabelIntro'>select top 10 * from KS_Article</textarea>
			<br>
			<font color=red>特别提示：</font>
			 ...
			 <li>2.循环体支持使用IF条件语句,详看<a href="http://www.kesion.com/tech/jc/79.html" target="_blank">http://www.kesion.com/tech/jc/79.html</a></li>
			 ...
			 <li>4.标签使用ajax输出或是您的网站不启用生成时,可以用<font color=red>"{$CurrUserName}"</font>取得当前登录的用户名
			 <br><font color=blue>如:select top 1 username,realname from ks_user where username='{$CurrUserName}'</font></li>
			</td>
	   </tr>
	...
	End Function
	 ...
	 Sub Step2()
		...
		{ 
		   ...
		   var link="include/Admin_FieldParam.asp?fieldname=" + fieldname + "&dbtype="+ dbtype + "&dbname=" + dbname+"&datasourcetype=<%=datasourcetype%>";
		   top.openWin('插入字段标签',link,false,350,230);
		}
		...
		.echo "      <td>"
		 ...
		 RSField.Open ClearParam(LabelIntro),Conn,1,3
		  ...
		  .echo "<table style=""table-layout:fixed"" border=1 bordercolordark=""#999999"" bordercolorlight=""#FFFFFF"" width=""99%"" cellpadding='0' cellspacing='0'>"
		  ...
		  For I=0 To RSField.Fields.count-1
			 ...
			 If j=5 Then j=0:.echo "</tr><tr class='tdbg' height='20'>"
			  ...
			  .echo "<td  width=""20%"" style=""text-align:center;cursor:pointer;"" onMouseOut=""this.className='tdbg'"" onMouseOver=""this.className='tdbgmouseover'"" onClick=""" & ClickStr & """>" & GetFieldName(trim(FieldName),"") & "</td>"
			 else
			  .echo "<td  width=""20%"" astyle=""text-align:center;cursor:pointer;"" onMouseOut=""this.className='tdbg'"" onMouseOver=""this.className='tdbgmouseover'"" onClick=""" & ClickStr & """>" & split(FieldName,".")(0)&"." & GetFieldName(trim(split(FieldName,".")(1)),"") & "</td>"
			 end if
		 Next
		 For I=J+1 to 5
		...
		End If
		 ...
		 .echo "	<td align='center'><strong>循 环 体</strong><br/>正序：{$AutoID}<br/>倒序：{$DaoXiID}</td><td height='230' valign=""top""><textarea id='txt_ln' name='rollContent' cols='6' style='line-height:16px;width:35px;overflow:hidden;height:220px;background-color:highlight;border-right:0px;text-align:right;font-family: tahoma;font-size:12px;font-weight:bold;color:highlighttext;cursor:default;' readonly>"
		 Dim N
		 For N=1 To 3000
			.echo N & "&#13;&#10;"
		 Next
		 .echo"</textarea>"
		 .echo "<textarea name='LabelContent'  onclick='setPos()' onkeyup='setPos()' id='LabelContent' style='line-height:16px;min-width:600px;width:92%;height:220px;' rows='15' id='txt_main' onscroll=""show_ln('txt_ln','LabelContent')"" wrap='on'>" & LabelContent & "</textarea>" & vbNewLine
		 .echo "	<script>for(var i=3000; i<=3000; i++) document.getElementById('txt_ln').value += i + '\n';</script>"
		 .echo "   </td></tr>"
		 
		 .echo "   <tr class=""tableBorder1"">"
 
		 .echo "	<td><strong>简要描述</strong></td><td><br/><font color=red>1、SQL标签定义规则</font><br>循环体格式：[loop=n]循环标签的内容[/loop]<br>其中n表示循环次数，且n满足n>=0。loop为循环关键字，此循环体可以重复使用,但是不能嵌套。<font color=red><br>2、SQL标签字段规则</font><br>字段格式：{$Field(FieldName,OutType,Param,...)}<br>FieldName&nbsp;&nbsp;--数据库表的字段名称<br>OutType&nbsp;&nbsp;&nbsp;&nbsp;--输出类型 支持：文本(Text)、日期(Date)、数据(Num)、对象URL(GetInfoUrl)，栏目URL(GetClassUrl) 5种类型<br><font color=red>3、支持使用{ReqNum(字符串)}或{ReqStr(字符串)}来取得Url的参数值</font><br>如：http://www.kesion.com/index.asp?ClassID=100,那么{ReqNum(ClassID)} 将得到100<br/><font color=red>4.当个人/企业空间要使用sql标签时,可以用<font color=red>""{$GetUserName}""</font>取得当前空间的用户名 </font><br>如:select top 10 id,title from ks_article where inputer='{$GetUserName}' order by id desc<br/><font color=red>5.查询KS_ItemInfo表时,允许使用{$GetItemUrl}得到文档的URL链接</font><br/>如:当有查询到KS_ItemInfo表时,可以使用{$GetItemUrl}来得到文档的URL,但要在查询的SQL语句中包含ChannelID,InfoID,Tid,Fname,AddDate五个字段。<br/>举例：select top 10 <span style='color:green'>i.channelid,i.infoid,i.tid,i.fname,i.adddate</span>,title,diggnum from KS_digglist d inner join ks_iteminfo i on d.infoid=i.infoid where d.channelid=i.channelid order by diggnum desc<br/>循环体:[loop=10]&lt;a href=""<span style='color:green'>{$GetItemUrl}</span>"">{$Field(title,Text,0,...,0,)}&lt;/a>[/loop]"
...
Sub AddLabelSave()
			...
			Else
				...
				LabelRS.AddNew
				 ...
				 LabelRS("Description") = Descript &"@@@"&FieldParam&"@@@"&SQLType&"@@@"&ItemName&"@@@"&PageStyle&"@@@"&Ajax&"@@@"& datasourcetype &"@@@" &datasourcestr & "@@@" & note & "@@@" & Request("NoRecord")
				...
				KS.echo "<script>top.$.dialog.confirm('恭喜，添加标签成功,继续添加标签吗?',function(){location.href='"&KS.Setting(3)& KS.Setting(89) & "Include/LabelSQL.asp?Action=AddNew&LabelType=5&FolderID=" & FolderID & "';},function(){$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & FolderID & "&OpStr=标签管理 >> 自义定函数标签&ButtonSymbol=DIYFunctionLabel';parent.frames['MainFrame'].location.href='"&KS.Setting(3) & KS.Setting(89) & "Include/Label_Main.asp?LabelType=5&FolderID=" & FolderID & "';});</script>"
	...
	Sub EditLabelSave()
			...
			Else
				...
				LabelRS.Open "Select top 1 * From [KS_Label] Where ID='" & LabelID & "'", Conn, 1, 3
				 ...
				 If KeyWord = "" Then
				   	KS.Echo "<script>top.$.dialog.alert('<br/>恭喜，标签修改成功!',function(){$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?LabelFolderID=" & FolderID & "&OpStr=" & server.URLEncode("标签管理  >> 自定义函数标签") &"&ButtonSymbol=DIYFunctionLabel';location.href='" & KS.Setting(3) & KS.Setting(89) & "include/Label_main.asp?Page=" & Page & "&LabelType=5&FolderID=" & FolderID & "';});</script>"

				 Else
				   	KS.Echo "<script>top.$.dialog.alert('恭喜，标签修改成功!',function(){$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "Post.Asp?OpStr=" & server.URLEncode("标签管理 >> <font color=red>搜索自定义函数标签结果</font>") &"&ButtonSymbol=DIYFunctionSearch';location.href='" & KS.Setting(3) & KS.Setting(89) & "include/Label_main.asp?Page=" & Page & "&LabelType=5&KeyWord=" & KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate & "&EndDate=" & EndDate & "';});</script>"
				 End If
			End If
	End Sub
...
'替换request的值,支持ReqNum和ReqStr两个标签
		Function ReplaceRequest(Content)
			 ...
			 For Each Match In Matches
				...
				Else
				 Content=Replace(Content,TempStr,"2000-1-1")
				End If
			Next
