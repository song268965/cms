...
.echo "<li class='parent' onclick='addserver()'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加服务器</span></li>"
.echo "<li class='parent' onclick=""location.href='KS.DownServer.asp?action=serverorders&ChannelID=" & ChannelID & "';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon back'></i>路径排序</span></li>"
.echo "<li class='parent' onclick=""location.href='KS.DownServer.asp?ChannelID=" & ChannelID & "';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon mainer'></i>返回首页</span></li>"
			.echo "<li></li><div class='mx'><strong>按模型设置:</strong><select id='channelid' name='channelid' onchange=""if (this.value!=0){location.href='?channelid='+this.value;}"">"
			...
			For Each Node In ModelXML.documentElement.SelectNodes("channel[@ks21=1][@ks6=3||@ks6=7]")
...
function addserver(editid)
{
 ...
 $(parent.document).find('#BottomFrame')[0].src='Post.Asp?ChannelID=<%=ChannelID%>&OpStr=下载服务器管理 >> <font color=red>添加下载服务器</font>&ButtonSymbol=GO';
}
function editserver(editid)
{
 location.href='?action=edit&editid='+editid+'&channelid=<%=channelid%>';
 $(parent.document).find('#BottomFrame')[0].src='Post.Asp?ChannelID=<%=ChannelID%>&OpStr=下载服务器管理 >> <font color=red>编辑下载服务器</font>&ButtonSymbol=GOSave';
}
...
Sub ShowMain()
	Dim DownloadName
	Response.Write "<script>$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ChannelID=" & ChannelID & "&OpStr=" & KS.C_S(ChannelID,1)&" >> <font color=red>下载服务器管理</font>&ButtonSymbol=Disabled';</script>"

	Response.Write " <div class=""pageCont2""><table width=""100%"" cellspacing=""0"" cellpadding=""0"" align=center>" & vbcrlf
	...
	Do While Not Rs.EOF
		selfont = Rs("selfont") & ""
		Response.Write " <tr height='30' class='list' onmouseout=""this.className='list'"" onmouseover=""this.className='listmouseover'"">" & vbcrlf
		...
		If Rs("isLock") = 1 Then
			Response.Write " <span style='color:red'><i>锁</i></span>"
		End If
		...
		If Rs("depth") = 0 Then
			Response.Write "<a href=""javascript:addserver(" &Rs("downid") &");"" class='setA'>添加服务器</a>" & vbcrlf
		Else
		    If Rs("isLock") <> 1 Then
			Response.Write "<a href=""KS.DownServer.asp?action=lock&editid="&Rs("downid")&"&ChannelID=" & ChannelID & """ class='setA'>锁定</a>"
			Else
			Response.Write "<a href=""KS.DownServer.asp?action=free&editid="&Rs("downid")&"&ChannelID=" & ChannelID & """ class='setA'>解除</a>"
			End If
		End If
		Response.Write " | <a href=""javascript:editserver(" & Rs("downid") & ");"" class='setA'>修改</a>" & vbcrlf
		Response.Write " |" & vbcrlf
		Response.Write " "
		If Rs("child") = 0 Then
			Response.Write " <a href=""KS.DownServer.asp?action=del&editid="
			Response.Write Rs("downid")
			Response.Write "&amp;ChannelID=" & ChannelID & """ onclick=""{if(confirm('删除将包括该服务器的所有信息，确定删除吗?')){return true;}return false;}"" class='setA'>删除" & vbcrlf
			Response.Write " "
		Else
			Response.Write "<a href=""#"" onclick=""{if(confirm('该服务器含有下载路径，必须先删除其下载路径方能删除本服务器！')){return true;}return false;}"" class='setA'>" & vbcrlf
...
Sub sAdd()
	...
	Rs.Close
	Response.Write "<div class='pageCont2'><table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"" align=center>" & vbcrlf
	...
	Response.Write " <dl class=""dtable"">" & vbcrlf
	Response.Write " <dd><div>所属类别：</div>" & vbcrlf
	...
	Response.Write " <dd><div>服务器名称：</div>" & vbcrlf
	Response.Write " <input type=""text"" class=""textbox"" name=""DownloadName"" size=""60"">" & vbcrlf
	Response.Write "</dd>" & vbcrlf
	Response.Write "<span id=""s1"">" & vbcrlf
	Response.Write " <dd  style=""display:none""><div>服务器名称样式：</div>" & vbcrlf
	Response.Write "样式:<input type=""hidden"" name=""selfont"" id=""selfont"" size=""1"" value="""">&nbsp;"
	Response.Write "</dd>" & vbcrlf
	'-------
	
	Response.Write " <dd><div>下载得分：</div>" & vbcrlf
	Response.Write " <input type=""text"" class=""textbox"" name=""Down_s""  onBlur=""CheckNumber(this,'下载得分');"" id=""Down_s"" size=""5"" value="""
	Response.Write Rs_e("Down_s")
	Response.Write """> 分" & vbcrlf
	Response.Write " </dd>" & vbcrlf
	
	Response.Write " <dd><div>得分间隔时间：</div>" & vbcrlf
	Response.Write " <input type=""text"" class=""textbox"" onBlur=""CheckNumber(this,'得分间隔时间');""  name=""Down_day"" id=""Down_day"" size=""5"" value="""
	Response.Write Rs_e("Down_day")
	Response.Write """>小时 格式:1 表示1小时" & vbcrlf
	Response.Write " </dd>" & vbcrlf
	
	
	Response.Write " <dd><div>服务器路径：</div>" & vbcrlf
	Response.Write " <input type=""text"" class=""textbox"" name=""DownloadPath"" id=""DownloadPath"" size=""60"">" & vbcrlf
	Response.Write "</dd>" & vbcrlf

	Response.Write " <dd><div>是否直接显示下载地址：</div>" & vbcrlf
	Response.Write " <input type=radio name=isDisp value=""0"" checked> 否&nbsp;&nbsp;"
	Response.Write " <input type=radio name=isDisp value=""1""> 是"
	Response.Write " </dd>" & vbcrlf
	Response.Write " <dd><div>是否外部连接：</div>" & vbcrlf
	...
	Response.Write "<div class=""clear""></div><font id='unionarea' style='display:none'>"
	Response.Write "联盟ID:<input type='text' class=""textbox"" name='unionid' size=12>"
...
Sub sEdit()
	...
	On Error Resume Next
	Set Rs = CreateObject("ADODB.Recordset")
	SQL = "SELECT top 1 * FROM KS_DownSer WHERE downid=" & CLng(Request("editid"))
	Set Rs_e = Conn.Execute(SQL)
	Response.Write "<div class='pageCont2'><table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"" align=centers>" & vbcrlf
	...
	Response.Write "</td>" & vbcrlf
	Response.Write " </tr></table>" & vbcrlf
	Response.Write " <dl class=""dtable""><dd><div class='firstd'>所属类别：</div>" & vbcrlf
	Response.Write " <select name=""servers"" id=""servers"" onchange='setdisabled(this.value)'>" & vbcrlf
	...
	Response.Write " </dd>" & vbcrlf
	Response.Write " <dd><div class='firstd'>服务器名称：</div>" & vbcrlf
	Response.Write " <input type=""text"" class=""textbox"" name=""DownloadName"" size=""60"" value="""
	Response.Write Rs_e("DownloadName")
	Response.Write """>" & vbcrlf
	Response.Write " </dd>" & vbcrlf
	'Response.Write " <dd id='s1' style=""display:none"">" & vbcrlf
	'Response.Write " <div>服务器名称样式：</div>" & vbcrlf
	'Response.Write " 样式:<input type=""hidden"" class=""textbox"" name=""selfont"" id=""selfont"" size=""1"" value="""& Server.HTMLEncode(Rs_e("selfont") & "") &""">&nbsp;"
	...
	Response.Write " <dd id='s2'>" & vbcrlf
	Response.Write " <div class='firstd'>下载得分：</div>" & vbcrlf
	Response.Write " <input type=""text"" name=""Down_s"" class=""textbox"" onBlur=""CheckNumber(this,'下载得分');"" id=""Down_s"" size=""5"" value="""
	Response.Write Rs_e("Down_s")
	Response.Write """> 分" & vbcrlf
	Response.Write " </dd>" & vbcrlf
	
	Response.Write " <dd id='s2'>" & vbcrlf
	Response.Write " <div class='firstd'>得分间隔时间：</div>" & vbcrlf
	Response.Write " <input type=""text"" name=""Down_day"" onBlur=""CheckNumber(this,'得分间隔时间');"" class=""textbox"" id=""Down_day"" size=""5"" value="""
	Response.Write Rs_e("Down_day")
	Response.Write """> <span>格式:1 表示1小时</span>" & vbcrlf
	Response.Write " </dd>" & vbcrlf
	
	Response.Write " <dd id='s2'>" & vbcrlf
	Response.Write " <div class='firstd'>服务器路径：</div>" & vbcrlf
	Response.Write " <input type=""text"" class=""textbox"" name=""DownloadPath"" id=""DownloadPath"" size=""60"" value="""
	Response.Write Rs_e("DownloadPath")
	Response.Write """>" & vbcrlf
	Response.Write " </dd>" & vbcrlf

	Response.Write " <dd id='s3'>" & vbcrlf
	Response.Write " <div class='firstd'>是否直接显示下载地址：</div>" & vbcrlf
	...
	Response.Write "> 是"
	Response.Write " </dd>" & vbcrlf
	Response.Write " <dd id='s4'>" & vbcrlf
	Response.Write " <div class='firstd'>是否外部连接：</div>" & vbcrlf
	...
	Response.Write "<font id='unionarea' style='display:none'>"
	Else
	Response.Write "<font id='unionarea'>"
	End If
	Response.Write "联盟ID:<input type='text' class=""textbox"" name='unionid' value='" & Rs_e("UnionID") &"' size=12>"
	Response.Write "<font color=""red"">如果还没有联盟ID”，"
	Response.Write "请先注册<a href=""http://union.xunlei.com/"" target=""_blank""><font color=""blue"">迅雷联盟</font></a>|<a href=""http://union.flashget.com/"" target=""_blank""><font color=""blue"">快车联盟</font></a>"
	Response.Write "</font>"
	Response.Write "</dd>" & vbcrlf
...
Sub savenew()
	...
	Rs("Down_s") = KS.ChkClng(KS.G("Down_s"))
	Rs("Down_day") = KS.ChkClng(KS.G("Down_day"))
	Rs("AllDownHits") = 0
	Rs("DayDownHits") = 0
	Rs("HitsTime") = Now()
	Rs.Update
	...
	End If
	call KS.Alert("服务器添加成功！","download/KS.DownServer.asp?ChannelID=" & ChannelID)
	Set Rs = Nothing
End Sub
'================================================
'过程名：saveedit
...
Sub saveedit()
	...
	If KS.ChkClng(ParentID) <> KS.ChkClng(Request("servers")) And Not (iparentid = 0 And CInt(Request("servers")) = 0) Then
		...
		End If
	End If
	Call KS.Alert("服务器修改成功！","download/KS.DownServer.asp?ChannelID=" & ChannelID)
	Set Rs = Nothing
	Set mrs = Nothing
	Set trs = Nothing
End Sub
...
Sub DelDownPath()
	...
	End If
	Set Rs = Nothing
	Call KS.Alert("服务器删除成功！","download/KS.DownServer.asp?ChannelID=" & ChannelID)
End Sub
'================================================
'过程名：isLock
'作  用：锁定服务器
...
Sub serverorders()
	...
	Response.Write "<div class='pageCont2'><table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" align=center>" & vbcrlf
	Response.Write " <tr class='sort'>" & vbcrlf
	...
	Else
		Do While Not Rs.EOF
			Response.Write "<form action=KS.DownServer.asp?action=updateorders method=post><tr height='23' class='list' onmouseout=""this.className='list'"" onmouseover=""this.className='listmouseover'""><td width=""50%"" class='splittd'>"
			If Rs("depth") = 1 Then Response.Write "&nbsp;&nbsp;<font color=""#666666"">├</font>"
			If Rs("depth") > 1 Then
				For i = 2 To Rs("depth")
					Response.Write "&nbsp;&nbsp;<font color=""#666666"">│</font>"
			...
			If Rs("child") > 0 Then Response.Write "(" & Rs("child") & ")"
			Response.Write "</td><td width=""50%"" class='splittd'>"
			If Rs("ParentID") > 0 Then
