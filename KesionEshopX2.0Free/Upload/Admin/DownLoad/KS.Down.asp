...
Class Admin_Down
		...
		Private FolderID, TemplateID,WapTemplateID,Action,FileName,oTid,Oid,RelatedID
	...
	sub folderlist(uploadDir)
	  ...
	  Set FileObj = FolderObj.Files
		...
		ks.echo "<script src='../../KS_Inc/common.js'></script>" & vbCrlf
		ks.echo "<script src='../../KS_Inc/Jquery.js'></script>" & vbCrlf
	   ks.echo "<table border='0' width='100%'>"
	   if FolderObj.subfolders.count>0 then
			 if ParentPath<>"" then
			   ks.echo "<tr><td><=<a href='KS.Down.asp?action=SelectFolder&channelid=" & channelid & "&path=" & ParentPath & "/'>回上一级</a></td></tr>"
			 end if
		  For Each FsoItem In FolderObj.subfolders
			ks.echo "<tr><td><img src='../images/folder/hr.gif' align='absmiddle' /><a href='KS.Down.asp?action=SelectFolder&channelid=" & channelid & "&path=" & uploadDir & fsoitem.name & "/'>" & FsoItem.Name & "</a></td><td><label><input type='checkbox' onclick=""top.frames['MainFrame'].returnFolder('" &uploadDir & fsoItem.Name & "/')"">选择</label></td></tr>"
		  Next
	   else
	     if ParentPath<>"" then
	       ks.echo "<tr><td><=<a href='KS.Down.asp?action=SelectFolder&channelid=" & channelid & "&path=" & ParentPath & "/'>没有子目录了,点此返回</a></td></tr>"
		 end if
	   end if
	   ks.echo "</table>"
	end sub
	  ...
	  'On Error Resume Next
		...
		ElseIf Action = "Edit" Or Action="Verify" Then
		   ...
		   If RS.EOF And RS.BOF Then
			Call KS.Alert("参数传递出错!", "download/KS.Down.asp")
			Set KS = Nothing
			Response.End
			Exit Sub
		   End If
			...
			Set RS = Nothing
			 ...
			 Else
			  .Write ("<script>$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=" & server.URLEncode(KS.C_S(ChannelID,1)&" >> <font color=red>搜索" & ks.c_s(channelid,3) &"结果</font>") & "&ButtonSymbol=DownSearch';</script>")
			 ...
			 Exit Sub
		   End If
		   If Action="Verify" And KS.C("Role")="1" Then           '审核前台会员投稿的下载
			  .Write ("<script>$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=ViewFolder&FolderID=" & FolderID & "&channelid= "& channelid & "';</script>")
			  Call KS.ReturnErr(1, "KS.Down.asp?channelid=" & channelid & "&Page=" & Page & "&ID=" & FolderID)
		   End If
		...
		.Write "<script language=""javascript"" src=""../../KS_Inc/DatePicker/WdatePicker.js""></script>" & vbCrlf
		...
		{ 
		 ...
		 if ($("input[name=addtype]:checked").val()==1){
		   if ($('#AddressUrls').val()==''){
			 top.$.dialog.alert('请输入批量地址列表',function(){
			 $('#AddressUrls').focus();
			 });
			 return;
		   }
			...
			if($('#DownUrls').val()=='')
			{
			 top.$.dialog.alert('请输入下载地址',function(){
			 $('#DownAddress1').focus();
			 });
			 return;
			}
		...
		.Write "    <form  action='?Method=Save&ChannelID=" & ChannelID & "' method='post' id='myform' name='myform' >"
...
IF IsObject(Application(KS.SiteSN & "_FieldGroupXml")) Then
  For Each TypeNode In Application(KS.SiteSN & "_FieldGroupXml").DocumentElement.SelectNodes("row[@channelid=" & ChannelID &"]")
	 ...
	 .Write " <dl class='dtable'>"
			...
			TTN=TTN+1
			If KS.G("ID")="" and TTN=1 Then
		%>
		  <script type="text/javascript">
		    function selectFolder(){
		        top.openWin('选择批量添加的目录','DownLoad/KS.Down.asp?action=SelectFolder&channelid=<%=channelid%>',false,450,400);
			}
		...
		.Write "              <dd>"
		.Write "                <div>添加方式:</div>"
		.Write "                <label><input name='addtype' type='radio' onclick='setAddMode()' checked value='0'>直接添加</label> <label><input name='addtype' type='radio' onclick='setAddMode()' value='1'>批量添加</label>"
		.Write "</dd>"
		End If
	...
	For Each FNode In FieldXML.DocumentElement.SelectNodes("fielditem[showonform=1&&fieldtype!=13&&@groupid=" & TypeNode.SelectSingleNode("@id").text & "]")
		...
		Else
	     ...
	     Select Case lcase(FNode.SelectSingleNode("@fieldname").text)
	       case "title"
				...
				.Write "  <input name='title' type='text'  id='title' class='rule' value=""" & Title & """ size=50><font color='#FF0000'>*</font> "
				.Write "<input class='button' type='button' value='重名检测' onclick=""if($('#title').val()==''){ top.$.dialog.alert('请输入" & KS.C_S(ChannelID,3) & "标题!');}else top.openWin('" & KS.C_S(ChannelID,3) & "重名检测','DownLoad/KS.Down.asp?ChannelID=" & ChannelID & "&Action=CheckTitle&title='+escape($('#title').val()),false,360,370);"">"
		  ...
		  case "tid"
                .Write " <dd>"
				.Write " <div>" & Replace(XTitle,"栏目",KS.GetClassName(ChannelID)) & ":</div><input type='hidden' name='OldClassID' value='" & FolderID & "'> "
					If Action<>"Edit" Then
					...
					var box=''
					function sel(){
					top.openWin(false,'download/KS.Down.asp?channelID=<%=ChannelID%>&FolderID='+$("#tidtb").val()+'&action=SelectClass',false,400,420);
					}
					</script>
					<%
		...
		case "nature" 
				.Write "  <dd>"
				.Write "  <div>" & XTitle & ":</div>类别:<select name='DownLB'>"
				.Write DownLBList
		...
		case "platform"
				.Write " <dd>"
				.Write "    <div>" & XTitle & ":</div><input type='text' class='textbox' size=79 name='DownPT' id='DownPT' value='" & DownPT & "'><br>"
		...
		case "uploadphoto"
			   If CBool(UpPowerFlag) = True Then
			    .Write "<dd><div></div>"
				.Write "<table width=""90%""><tr><td><iframe id='UpPhotoFrame' name='UpPhotoFrame' src='../System/KS.UpFileForm.asp?ChannelID=" & ChannelID & "&UpType=Pic' frameborder=0 scrolling=no width='100%' height='35'></iframe></td></tr></table>"
				.Write " </dd>"& vbcrlf
				End If
       case "keywords"
		...
		case "address"
		  %>
		      <script language="javascript">
				...
				End If
				%>
				 str+='<table><tr><td><input type="text" name="DownName'+i+'" class="textbox" value="下载地址'+i+'" size="8">-'+'<select style="width:140" name="serverid'+i+'"><%=SelDownServer(0)%></select><input class="textbox" type="text" name="DownAddress'+i+'" id="DownAddress'+i+'" size="40" value="">&nbsp;<input type="button" class="button" value="选择下载地址..." name="button1" onClick="OpenThenSetValue(\'Include/SelectPic.asp?ChannelID=<%=ChannelID%>&CurrPath=<%=CurrPath%>/DownUrl\',550,290,window,document.myform.DownAddress'+i+');"></td></tr></table>';
				 $('#upid').html(str);
			 }
			 </script>
				<span id="add0">
				<dd>
					  <div>地址数:</div>
					  <input type="text" class="textbox" name="no"value="1" size=2>&nbsp;&nbsp;<input type="button" name="Button" class="button" onClick="setid();" value="添加下载地址数"> <span><font color=blue>下载服务器路径 + 下载文件名称 = 完整下载地址</font>,<font color=red>删除某个地址,请在地址里输入 "del"或留空。</font></span>
				</dd>
				<dd>
				  <div><%=XTitle%>:</div>
					<input type="hidden" name="DownUrls" id="DownUrls">
			     <%	
			   If Action="Edit" Or Action="Verify" Then
				...
				For I=1 To Ubound(DownUrlsArr)+1
					...
					Response.Write "<input name=""DownName" & I & """ class=""textbox"" type=""text"" size=""8"" value=""" & UrlsParam(1) &""">-"
					Response.Write "<select name=""serverid" & I & """ size=""1"" style=""width:140"">"
					Response.Write SelDownServer(UrlsParam(0))
					Response.Write "</select>"
					Response.Write "<input name=""DownAddress" & I & """ id=""DownAddress" & I & """ value=""" & UrlsParam(2) & """ class=""textbox"" type=""text"" size=""40"">&nbsp;"
			  ...
			  End If
				.Write "<table cellpadding=""0"" cellspacing=""0"" border=""0""><tr><td  style=""padding:0px;"" id=""upid""></td></tr></table>"
				.Write "</dd>"
				
				 If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='uploadsoft']/showonform").text="1" Then
					If CBool(UpPowerFlag) = True Then
					.Write "<dd><div></div><iframe id='UpPhotoFrame' name='UpPhotoFrame' src='../System/KS.UpFileForm.asp?UpType=UpByBar&ChannelID=" & ChannelID & "' frameborder=0 scrolling=no width='300' height='30'></iframe></dd>"
					End If
				 End If
				.Write "</span>"
				
				.Write "<span id='add1' style='display:none'>"
				.Write "<dd><div>批量地址列表:</div>"
				.Write "<table><tr><td>输入目录<input type='text' class='textbox' name='folderpath' id='folderpath' size='26' value='/uploadfiles/" & year(now) & "-" & right("0" & month(now),2) & "/'> <input type='button' onclick='selectFolder()' value='选择...' class='button' /> <label><input type='checkbox' name='subfolder' id='subfolder' value='1' checked>包含子栏目</label>  扩展名限定<input type='text' class='textbox' name='ext' id='ext' value='.rar|.exe|.jpg' size=16> <input type='button' class='button' onclick='getAddress()' value='自动获取'><br/>地址标题:<input type='text' class='textbox' name='AddressName' value='下载地址1' /> "
				.Write "选择服务器：<select name=""serverids"" size=""1"" style=""width:140px"">"
				.Write SelDownServer(0)
				.Write "</select>"

				.Write "<br/><textarea name='AddressUrls' id='AddressUrls' class=""textbox"" style='width:530px;height:150px'></textarea><span class=""block"">格式:一行对应一个软件地址</span></td></tr></table></dd>"
				.Write "</span>"	

		case "content"
				.Write " <dd>"
				.Write "  <div>" & XTitle & ":<font>(<input name='BeyondSavePic' type='checkbox' value='1'>自动下载简介里的图片)</font></div>"
				.Write EchoEditor("Content",DownContent,"Basic","90%","200px")
				.Write "</dd>"
		case "ysdz"
				.Write "<dd>"
				.Write " <div>" & XTitle & ":</div>"
				.Write " <input name='YSDZ' type='text' id='YSDZ' value='" & YSDZ & "' size='50' class='textbox'>"
				.Write " </dd>" &vbcrlf
		case "zcdz"
				.Write " <dd>"
				.Write " <div>" & XTitle & ":</div>"
				.Write " <input name='ZCDZ' type='text' id='ZCDZ' value='" & ZCDZ & "' size='50' class='textbox'>"
				.Write " </dd>"&vbcrlf
		case "jymm"
				.Write " <dd>"
				.Write "  <div>" & XTitle & ":</div>"
				.Write "  <input name='JYMM' type='text' id='JYMM' value='" & JYMM & "' size='50' class='textbox'>"
				.Write " </dd>" &vbcrlf
	  End Select
	End If
Next
...
END IF
	...
	If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='attroption']/showonform").text="1" Then	
		...
		If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='rank']/showonform").text="1" Then
			.Write "              <dd>"
			.Write "                <div>" & FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='rank']/title").text & ":</strong></div><select name='rank'>"
...
End If
		...
		Sub DownSave()
         ' On Error Resume Next
		  Dim SelectInfoList,HasInRelativeID,Relateda,Related_s,ii
          With Response
			Page        = KS.G("Page")
				...
				dim RelatedArray,n:n=0
					if KS.ChkClng(KS.G("EditNewtb"))=1 then
						if KS.ChkClng(KS.G("RelatedID"))=0 or KS.ChkClng(KS.G("RelatedID"))=-11 then
							RelatedArray=KSCls.GetRelatedArray( KS.C_S(ChannelID,2), DownID ,11)'同步文章
						else
							RelatedArray=KSCls.GetRelatedArray( KS.C_S(ChannelID,2), KS.ChkClng(KS.G("RelatedID")) ,22)'同步文章
						end if 
					else
						RelatedArray=Array(DownID)	
					end if
					...
					for ii=0 to UBound(RelatedArray)
							...
							RS("Strip")       = Strip
							RS("IsTop")       = IsTop
							if DownID=KS.ChkClng(RTrim(Trim(RelatedArray(ii)))) then
							RS("Tid")       = KS.G("Tid")
							RS("oTid")      = KS.G("oTid")
					   ...
					   If TID<>Request.Form("OldClassID") Then
							 Call KSCls.DelInfoFile(ChannelID,Request.Form("OldClassID"), "",RS("Fname"),RS("ID"),RS("AddDate"))
					   End If
					   if ii=0 then
				 ...
				 Next
				  RS.Close:Set RS = Nothing
					IF (Action="Verify" or (Not KS.IsNul(KS.S("oldverific")) AND KS.S("oldverific")="0" And Verific=1)) And Inputer<>KS.C("AdminName")  Then     '如果是审核投稿下载，对用户，进行加积分等，并返回签收下载管理
							  '对用户进行增值，及发送通知操作
