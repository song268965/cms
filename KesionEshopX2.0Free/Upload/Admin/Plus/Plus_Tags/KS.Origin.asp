...
Class Admin_Origin
		...
		function OriginAdd()
		{
		 box=top.$.dialog.open('Plus/plus_tags/KS.Origin.asp?Action=Add',{title:'新增来源',width:650,height:410});
		}
		function EditOrigin(id)
		{ box=top.$.dialog.open('Plus/plus_tags/KS.Origin.asp?action=Edit&ID='+id,{title:'编辑来源',width:650,height:410});
		}
		...
		<%
		 ...
		 .echo "<ul id='menu_top'>"
		 .echo "<li class='parent' onClick=""OriginAdd();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>新增来源</span></li>"
		 .echo "<li class='parent' onclick='OriginControl(1);'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>修改来源</span></li>"
		 .echo "<li class='parent' onclick='OriginControl(2);'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除来源</span></li>"
		 .echo "</ul>"
		 .echo "<div class='pageCont2'>"
		 .echo "<div class='tabTitle'>来源管理</div>"
		...
		With KS
		Do While Not RS.EOF
		  .echo "<tr  onDblClick=""EditOrigin(" & RS("ID") & ")"" title=""双击编辑"" onmouseout=""this.className='list'"" onmouseover=""this.className='listmouseover'"" id='u" & RS("ID") & "' onclick=""chk_iddiv('" & RS("ID") & "')"">"
		  .echo "    <td class='splittd' align=center><input name='id'  onclick=""chk_iddiv('" &RS("ID") & "')"" type='checkbox' id='c"& RS("ID") & "' value='" &RS("ID") & "'></td>"
		  .echo "    <td class='splittd' height='19'><span OriginID='" & RS("ID") & "'>"
   ...
   End If
		 ...
		 .echo "<li class='parent' onclick=""top.frames['MainFrame'].box.close();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><img src='../../images/ico/back.gif' border='0' align='absmiddle'>关闭取消</span></li>"
		 .echo "</div>"
		...
		End Sub
		 ...
		 If RS.EOF And RS.BOF Then
		  ...
		  RS("AddDate") = Now()
		  RS.Update
		  KS.Echo ("<Script> if (confirm('来源增加成功,继续添加吗?')) { location.href='KS.Origin.asp?Action=Add';} else{top.frames[""MainFrame""].location.reload();top.box.close();}</script>")
		 Else
		 ...
		 Set RS = Server.CreateObject("ADODB.RECORDSET")
		  ...
		  RS.Update
		  RS.Close
		  KS.Echo ("<Script> alert('来源修改成功!');top.frames[""MainFrame""].location.reload();top.box.close();</script>")
		 End With
		 End Sub
