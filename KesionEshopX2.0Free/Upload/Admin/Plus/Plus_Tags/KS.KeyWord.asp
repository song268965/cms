...
Class Admin_KeyWord
		...
		Sub KeyWordList()
			...
			<script>
				...
				function KeyWordAdd()
				{
				  top.openWin('新增关键字Tags','Plus/plus_tags/KS.KeyWord.asp?Action=Add',true,500,140);
				}
				function EditKeyWord(id)
				{	
				 top.openWin('修改关键字Tags','Plus/plus_tags/KS.KeyWord.asp?Page='+Page+'&Action=Edit&ID='+id,true,500,140);
				}
		...
		If IsSearch="1" Then
			.echo "<div class='topSift'><strong>搜索关键词维护</strong> <a href='?issearch=1&order=1'>按搜索次数最多查看</a> | <a href='?issearch=1&order=2'>按搜索次数最少查看</a></div>"
		Else
			.echo "<li class='parent' onClick=""KeyWordAdd();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加关键字</span></li>"
			.echo "<li class='parent' onClick=""KeyWordControl(1);""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>编辑关键字</span></li>"
			.echo "<li class='parent' onClick=""KeyWordControl(2);""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除关键字</span></li>"
		End If
			.echo "</ul>"
			.echo "<div class='menu_top_fixed_height'></div>"
			.echo "<div class='pageCont2'>"
			.echo "<div class='tabTitle'>关键字管理</div>"
		...
		Else
			...
			.echo "          <td class='sort' align='center'>操作</td>"
		End If
			.echo "  </tr>"
			  ...
			  Set RS = Server.CreateObject("ADODB.RecordSet")
					 ...
					 If RS.EOF And RS.BOF Then
					     .echo "<tr><td class='splittd' colspan='10' style='text-align:center'>找不到记录！</td></tr>"
					 Else
	        ...
	        .echo ("</td>")
	        .echo ("<td class='operatingBox'><select  onchange='set(this.value)' name='setattribute'><option value=0>快速选项...</option><option value='1'>编辑关键字</option><option value='2'>执行删除</option></select></td>")
			...
			if IsSearch=1 Then
		      ...
		      .echo ("<br /> <strong>删除范围：</strong><br/><label><input onclick=""$('#s1').show();$('#s2').hide();"" name=""deltype"" type=""radio"" value=""1"" checked=""checked""/> 按搜索次数</label><label><input onclick=""$('#s2').show();$('#s1').hide();"" name=""deltype"" type=""radio"" value=""2"" /> 按日期</label> <div id='s1'>搜索次数小于<input class='textbox' name=""searchnum"" size='4' value=10 style='text-align:center'>次</div><div id='s2' style='display:none'>最近<input type='text' class='textbox' name='days' value='100' size='4' style='text-align:center'>天内没有被重新搜索</div><input onclick=""$(parent.frames['FrameTop'].document).find('#ajaxmsg').toggle();"" type=""submit""  class=""button"" value=""执行删除"">")
		     .echo ("</div>")
			 .echo ("</form>")
			End If
			...
			Sub showContent()
			   With KS
					Do While Not RS.EOF
			          .echo "<tr title=""双击编辑"" ondblclick=""EditKeyWord(" & RS("ID") & ")"" onmouseout=""this.className='list'"" onmouseover=""this.className='listmouseover'"" id='u" & RS("ID") & "' onclick=""chk_iddiv('" & RS("ID") & "')"">"
			          .echo "<td class='splittd' align=center><input name='id'  onclick=""chk_iddiv('" &RS("ID") & "')"" type='checkbox' id='c"& RS("ID") & "' value='" &RS("ID") & "'></td>"
					  .echo "<td class='splittd' height='20'><span KeyWordID='" & RS("ID") & "'><img src='../../Images/key.gif' align='absmiddle'>"
					  ...
					  If IsSearch<>"1" Then
					  .echo "  <td class='splittd' align='center'><a href=""" & KS.TagsUrl(RS("ID"),0,0,1) & """ target=""_blank"">预览</a></td>"
					  End If
					  .echo "</tr>"
					  I = I + 1
			...
			With KS
			 Dim Action, KeyWordText, ID, PageRS, KeySql,Page,TemplateID
			  ID = KS.ChkClng(KS.G("ID"))
			  Page = KS.G("Page")
			  TemplateID="{@TemplateDir}/common/tagsList.html"
			  If ID<>0 Then
				 Set RS = Server.CreateObject("ADODB.RECORDSET")
				 KeySql = "Select * From [KS_KeyWords] Where ID=" & ID
				 RS.Open KeySql, conn, 1, 1
				 If Not RS.EOF Then KeyWordText = RS("KeyText"):TemplateID=RS("TemplateID")
			...
			.echo "    </tr>" & vbCrLf
			.echo "    <tr>" & vbCrLf
			.echo "      <td width='23%' align='right' nowrap>Tags列表模板：</td>" & vbCrLf
			.echo "      <td width='20' height='30' align='right' nowrap> <div align='center'></div></td>" & vbCrLf
			.echo "      <td width='74%' height='30' nowrap>" & vbCrLf
			.echo "        <input name='TemplateID' class='textbox' type='text' id='TemplateID' value='" & TemplateID & "'>&nbsp;"
			
			Dim CurrPath:CurrPath=KS.Setting(3)&KS.Setting(90)
		   If Right(CurrPath,1)="/" Then CurrPath=Left(CurrPath,Len(CurrPath)-1)
			
			.echo "<input type='button' name=""Submit"" class=""button"" value=""选择模板..."" onClick=""OpenModalDialog('../../System/KS.Template.asp?Action=SelectTemplate&CurrPath=" &CurrPath & "',550,360,window,$('#TemplateID')[0]);"">"& vbCrLf
			.echo "        </td>" & vbCrLf
			.echo "    </tr>" & vbCrLf
			...
			Sub DoSave()
				...
				If KeyWordText = "" Then
				 ...
				 End If
				  ...
				  RS.Update
				 RS.Close:Set RS=Nothing
				 Application(KS.SiteSN&"_ClassTags")=""
				 If ID=0 Then
				  KS.Echo ("<Script> if (confirm('关键字增加成功,继续添加吗?')) { location.href='?Action=Add';} else{top.frames[""MainFrame""].location.reload();top.box.close();}</script>")
				 Else
				  KS.Echo ("<Script> alert('关键字修改成功!');top.frames[""MainFrame""].location.reload();top.box.close();</script>")
				 End If
			End Sub
