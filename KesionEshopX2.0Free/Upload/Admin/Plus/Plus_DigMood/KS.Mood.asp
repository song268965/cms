...
Class Admin_MoodCls
		...
		Public Sub Kesion()
		  With KS
		   ...
		   End If
			...
			.echo"<ul id='menu_top'>"
			.echo"<li class='parent' onclick=""location.href='KS.Mood.asp?action=Add&TypeFlag=" &TypeFlag&"';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Go&OpStr=" & ItemStr & " >> <font color=red>添加" & ItemStr  &"项目</font>';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加" & ItemStr  &"项目</span></li>"
			.echo"<li class='parent' onclick='location.href=""KS.Mood.asp?TypeFlag=" & TypeFlag &"&action=GetCode""'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon move'></i>调用代码</span></li>"
             If KS.G("Action")="" Then
			.echo"<li class='parent' disabled"
		     Else
			.echo"<li class='parent'"
			 End If
			.echo" onclick='location.href=""KS.Mood.asp?typeflag=" & typeflag & """;'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon mainer'></i>管理首页</span></li>"
			.echo"</ul>"
		...
		Sub Main()
		   With KS
			...
			.echo"});</script>"
			.echo"<div class='tabTitle'>" & ItemStr  &"管理</div>"&_
			.echo("<div style=""height:94%; width:100%"">")
		  ...
		  Do While Not RS.Eof 
			...
			if typeflag=1 then
				.echo"<a href='#' onClick=""window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=点评管理 >> <font color=red>查看点评数据</font>&ButtonSymbol=Disabled'; location.href='KS.Comment.asp?projectid=" & rs("ID") & "';"" class='setA'>查看管理用户点评</a>｜"
			else
			.echo"<a href='#' onClick=""window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=子系统 >> <font color=red>查看详情</font>&ButtonSymbol=Disabled';location.href='KS.Mood.asp?typeflag=" & typeflag&"&MoodID=" & rs("ID") & "&action=Show';"" class='setA'>查看详情</a>｜"
			end if
			...
			.echo"<a href='?typeflag=" & typeflag &"&action=Edit&ID=" & rs("ID") & "' onclick=""$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=GoSave&OpStr=子系统 >> <font color=red>" & ItemStr & "</font>';"" class='setA'>修改</a>｜"
			 .echo"<a href='?typeflag=" & typeflag &"&action=Del&ID=" & rs("ID") & "' onclick='return(confirm(""此操作不可逆，确定删除吗？""))' class='setA'>删除</a>｜"
			 			 
			 If RS("Status")="1" Then .echo"<a href='?typeflag=" & typeflag&"&Action=SetFormParam&Flag=FormOpenOrClose&ID=" & RS("ID") & "' class='setA'>锁定</a>" Else .echo"<a href='?typeflag=" & typeflag &"&Action=SetFormParam&Flag=FormOpenOrClose&ID=" & RS("ID") & "' class='setA'>开启</a>"
		...
		Sub GetCode()
		  ...
		  Do While Not RS.Eof
			...
			.echo "</td><td></td><td></td>"
			Else
			.echo "<textarea style='width:500px;height:50px;margin-top:10px;margin-bottom:10px;' name='s" & rs(0) & "'>&lt;script language=&quot;javascript&quot; type=&quot;text/javascript&quot; src=&quot;" & KS.Setting(2) & "/plus/digmood/mood.asp?id=" & rs("id") & "&c_id={$InfoID}&M_id={$ChannelID}&quot;&gt;&lt;/script&gt;</textarea>"
	   ...
	   If KS.G("Action")="Edit" Then
		...
		"	<div class='tabTitle'>" & ItemStr  &"管理</div>"&_
		
		"<div class=tab-page id=Formpanel>"& _
		"<form name=""myform"" method=""post"" action=""KS.Mood.asp?Action=EditSave&ID=" & ID & """ >" & _
		...
		.echo"    <tr class='tdbg'>"
		.echo"      <td> <div><strong>项目状态：</strong></div></td>"
...
%>
		...
		<tr valign="middle" class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">      
		...
		<tr valign="middle" class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">      
			...
			<%
			 ...
			 .echo " 图片地址 <input type=""text"" class=""textbox"" name=""Pic" & I & """ value=""" & PicValue & """></span><br><br>"
			 Next
			 %>
		...
		<tr ID="BeginDate" style="display:none" valign="middle" class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">     
		...
		<tr ID="EndDate" style="display:none" valign="middle" class="tdbg" onMouseOver="this.className='tdbg'" onMouseOut="this.className='tdbg'">      
		...
		Sub FormSave()
			...
			End If
				...
				if err<>0 then
					...
					Conn.CommitTrans
					If ID=0 Then
					 KS.Alert OpName & ItemStr & "项目添加成功!","plus/plus_digmood/KS.Mood.asp?typeflag=" & typeflag
					Else
					 KS.Alert OpName & ItemStr & "项目修改成功!","plus/plus_digmood/KS.Mood.asp?typeflag=" & typeflag
					End If
				end if
		End Sub
			...
			End Select
			 ...
			 Else
							...
							Do While Not RS.Eof
								...
								.echo"<td align='center' class='splittd'><input type='checkbox' name='id' value='" & RS("ID") & "'></td>"
								.echo"<td class='splittd'><a href='../../../item/show.asp?m=" & rs("channelid") & "&d=" & RS("InfoID") & "' target='_blank'>" & RS("Title") & "</a></td>"
								If TypeFlag=1 Then
								 .echo "<td align='center' class='splittd'>" &RS("Score") & " 分</td><td align='center'>" & RS("HitsNum") & " 人</td><td  align='center'>" & round(Formatnumber(RS("AvgScore"),2,-1,0,-1),2) & "</td>"			
								End If
								
								For i=0 to Ubound(VoteTitle)
								 If VoteTitle(i)<>"" Then
								 .echo"<td class='splittd' align=""center"">" & rs("m"&i) & "</td>"
								 End If
								Next
								.echo"</tr>"
								.echo""
