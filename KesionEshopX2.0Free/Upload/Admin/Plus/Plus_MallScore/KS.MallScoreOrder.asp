...
Class Admin_MallScoreOrder
		...
		Public Sub Kesion()
		 With Response
					If Not KS.ReturnPowerResult(0, "KSMS200101") Then          '检查是权限
					 Call KS.ReturnErr(1, "")
					 Response.End
					 End If
			  ...
			  .Write "<li class='parent' onclick=""window.parent.frames['BottomFrame'].location.href='../../Post.Asp?OpStr='+escape('积分兑换系统 >> <font color=red>添加礼品</font>')+'&ButtonSymbol=GOSave';location.href='KS.MallScore.asp?action=Add';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加礼品</span></li>"
			  .Write "<li class='parent' onclick=""location.href='KS.MallScoreOrder.asp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon num'></i>处理兑换订单</span></li>"
			  .Write "<li><strong>查看方式:</strong><a href=""KS.MallScoreOrder.asp"">所有订单</a> <a href=""KS.MallScoreOrder.asp?flag=1"">已审核</a>  <a href=""KS.MallScoreOrder.asp?flag=-1"">未审核</a> <a href=""KS.MallScoreOrder.asp?flag=2"">配货中</a> <a href=""KS.MallScoreOrder.asp?flag=3"">已发货</a> <a href=""KS.MallScoreOrder.asp?flag=4"">已完成</a></li>"
...
If KS.S("ProductID")<>"" Then
 %>
  <div style="height:45px;font-size:14px;line-height:45px;text-align:left;font-weight:bold">查看礼品 <font color=red>[<%=LFCls.GetSingleFieldValue("Select ProductName From KS_MallScore Where ID=" & KS.ChkClng(KS.G("ProductID")))%>]</font> 的兑换记录</div>
 <%end If%>
 <div class="Table">
...
<%
	...
	If Rs.bof And Rs.EOF Then
		Response.Write "<tr><td height=""25"" align=center bgcolor=""#ffffff"" colspan=10 class='operatingBox'>对不起,找不到订单！</td></tr>"
	Else
...
<tr height="25" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'">
	...
	%></td>
	<td align="center" class="splittd"><a href="?action=Edit&ID=<%=RS("ID")%>"  onclick="window.parent.frames['BottomFrame'].location.href='../../Post.Asp?OpStr='+escape('积分兑换系统 >> <font color=red>修改礼品信息</font>')+'&ButtonSymbol=GOSave';">查看/修改</a> <a href="?Action=Del&ID=<%=rs("id")%>" onclick="return(confirm('此操作不可逆,确定删除该订单吗？'));">删除</a> 
...
Dim ProductName,ActiveDate,AddDate,DeliveryType,Amount,Score,Telphone,RealName,ZipCode,Protection,BuyFlow,Notes,Tel,Status,Address,Email,Remark,UserName,chargeType
Dim ID:ID=KS.ChkClng(KS.G("ID"))
Dim RS:Set RS=server.createobject("adodb.recordset")
If KS.G("Action")="Edit" Then
	RS.Open "Select top 1 a.*,b.productname,score,chargeType From KS_MallScoreOrder a Left Join KS_MallScore b on a.productid=b.id Where a.ID=" & ID,conn,1,1
...
function CheckForm()
{
	if ($('#RealName').val()=='')
	{
	 top.$.dialog.alert('请输入收货人!',function(){
	 $("#RealName").focus();
	 });
	 return false;
	}
...
<table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" class="ctable">
	  ...
	  <%if Status<>5 then%>
        ...
        无效订单并退回<%IF chargeType=0 Then
		 response.write "积分"
		 Else
		 response.write KS.Setting(45)
		 End If
		 %></label>
	<%else%>
 ...
 <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
      <td  width='170' height='30' align='right' class='clefttitle'><strong>兑换礼品：</strong></td>
...
自取 </td>
    ...
    <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
      ...
      <td height='30'>&nbsp;
          <input type='text' name='RealName' class="textbox" id='RealName' value='<%=RealName%>' size="20" />
          <font color=red>*</font></td>
    </tr>
    <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
      <td  width='170' height='30' align='right' class='clefttitle'><strong>联系电话：</strong></td>
      <td height='30'>&nbsp;
        <input type='text' name='Tel' class="textbox" value='<%=Tel%>' size="20" />
        <font color="red">*</font></td>
    </tr>
    <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
      <td  width='170' height='30' align='right' class='clefttitle'><strong>收货地址：</strong></td>
      <td height='30'>&nbsp;
          <input type='text' name='Address' class="textbox" value='<%=Address%>' size="35" />        
        <font color=red>*</font></td>
    </tr>
    <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
      <td  width='170' height='30' align='right' class='clefttitle'><strong>邮政编码：</strong></td>
      <td height='30'>&nbsp;
          <input name='ZipCode' value="<%=ZipCode%>" class="textbox" size="10" /></td>
    </tr>
    <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
      <td  width='170' height='30' align='right' class='clefttitle'><strong>电子邮箱：</strong></td>
      <td height='30'>&nbsp;
        <input type='text' name='Email' value='<%=Email%>' class="textbox" size="25" /></td>
    </tr>
    <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
      <td height='30' align='right' class='clefttitle'><strong>备注说明：</strong></td>
      <td height='30'>&nbsp;
        <textarea name='Remark' style="width:400px;height:80px" class="textbox"><%=Remark%></textarea></td>
    </tr>
</table>
  </form>
<%
...
Sub DoSave()
	   ...
	   Dim chargeType:chargeType=KS.ChkClng(KS.G("chargeType"))
            ...
            Dim RS: Set RS=Server.CreateObject("Adodb.RecordSet")
			  RS.Open "Select top 1 * From KS_MallScoreOrder Where ID=" & ID,Conn,1,3
				...
				if Status=5 then
				  ...
				  Else
				   Call KS.PointInOrOut(0,0,RS("UserName"),1,KS.ChkClng(KS.G("Score"))*RS("Amount"),"系统","礼品兑换失败，返回兑换订单号<font color=red>" & RS("OrderID") & "</font>的礼品积分!",0)   
  ...
  Response.Write "<script>alert('兑换订单修改成功！');parent.frames['BottomFrame'].location.href='../../Post.Asp?ButtonSymbol=Disabled&OpStr=" & Server.URLEncode("积分兑换系统 >> <font color=red>订单管理</font>") & "';location.href='"& ComeUrl & "';</script>"
