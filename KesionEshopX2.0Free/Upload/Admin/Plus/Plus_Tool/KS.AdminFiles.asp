...
Class AdminUploadFileCls
	...
	Private Sub ShowChildFolder()
		...
		If fso.FolderExists(strFolderPath) Then
			...
			For Each DirFolder In fsoFile.SubFolders
				Response.Write "<a href=""?UploadDir=" &Request("UploadDir") & "/"& DirFolder.Name& "&ThisDir=" & DirFolder.Name & """><i class=""icon folder"" width=20 height=20 border=0 alt=""修改时间：" & DirFolder.DateLastModified & """ align=absMiddle></i> "
	...
	Private Function showpage(ByVal CurrentPage, ByVal TotalNumber, ByVal maxperpage, ByVal TotleSize)
		...
		strTemp = strTemp & "&nbsp;转到："
		strTemp = strTemp & "<input name=page size=3 value='" & CurrentPage & "' class='textbox'> <input type=submit name=Submit value='转到' class='button'>"
	...
	Private Sub ShowUploadMain()
		...
		Response.Write "<ul id='mt'> <div style='font-weight:bold;'><a href='?'>上传文件管理</a> | <a href='?action=clear'>清理无用文件</a></div></ul>"
		Response.Write "<div class='pageCont2'>"
		Response.Write "<div class='tabTitle'>上传文件管理</div>"
		Response.Write "<table border=0 align=center cellpadding=3 cellspacing=1 width='99%' class='ctable'>"
		Response.Write "<tr>"
	...
	Private Sub ClearUploadFile()
		...
		Response.Write "<body topmargin='0' leftmargin='0'>"
		Response.Write "<div class='tabTitle mt20'>"
		...
		Response.Write "<input type=hidden name=UploadDir value='" & Request("UploadDir") & "'>" & vbNewLine
		Response.Write "<tr><td class=clefttitle style='text-align:left'>" & vbNewLine
		Response.Write "<br>&nbsp;&nbsp;①、你的网站在使用一段时间后，就会产生大量无用垃圾文件。如果怕浪费您的空间，可以定期使用本功能进行清理；<br>"
		Response.Write "<br>&nbsp;&nbsp;②、此功能仅对上传目录（如UpLoadFiles）执行清理功能，使用此功能前请先备份上传目录，如果因使用此功能导致有用的文件被删除，官方不承担任何责任；<br>"
		Response.Write "<br>&nbsp;&nbsp;③、如果上传文件很多，或者数据库的信息量较多，执行本操作需要耗费相当长的时间，请在访问量少时执行本操作。<br>"
		Response.Write "<br></td></tr>" & vbNewLine
		Response.Write "<tr align=center><td  class=clefttitle style='text-align:left'>请选择要清理的目录："
		Call ShowFolderPath
	...
	Public Function CheckFileExists(ByVal str)
	   ...
	   if instr(filename,"user/avatar")<>0 then
