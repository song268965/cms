...
Class DB_BackUp
		...
		if KS.G("Action")<>"ExecSql" then
		  .echo ("<body>")
		  .echo "<ul id='menu_top'>"
		  .echo "<li class='parent' onclick=""location.href='?Action=BackUp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon merge'></i>备份数据库</span></li>"
		  .echo "<li class='parent' onclick=""location.href='?Action=Restore';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon back'></i>恢复数据库</span></li>"
		  .echo "<li class='parent' onclick=""location.href='?Action=Compact';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon folder'></i>"
	    ...
	    elseif ks.g("flag")<>"Result" then
		  .echo ("<body>")
		  .echo "<div class=""tabTitle mt20"">在线执行SQL语句</div>"
	  ...
	  Sub Db_BackUp()
		...
		if DataBaseType=0 then
				  ...
				  .echo "                </tr>"
				  .echo "                <tr> "
				  .echo "                  <td height=""22"" align=""center""><input class='textbox' type=text size=50 name=DBpath value=""" &server.mappath(DBPath) & """ readonly></td>"
				  .echo "                </tr>"
				  .echo "                <tr> "
				  ...
				  .echo "                </tr>"
				  .echo "                <tr> "
				  .echo "                  <td height=""22"" align=""center""><input class='textbox' type=text size=50 name=bkDBname value=""Data(" & year(now)&month(now)&day(now) & ").bak""></td>"
				  .echo "                </tr>"
				  .echo "              </table>"
				  .echo "			  </fieldset>"
		...
		Else
				  ...
				  .echo "                </tr>"
				  .echo "                <tr> "
				  .echo "                  <td height=""22"" align=""center""><input class='textbox' type=text size=50 name=DBpath value=""" & DataBaseName & """ readonly></td>"
				  .echo "                </tr>"
				  .echo "                <tr> "
				  .echo "                  <td height=""22""></td>"
				  .echo "                </tr>"
				  ...
				  .echo "                </tr>"
				  .echo "                <tr> "
				  .echo "                  <td height=""22"" align=""center""><input class='textbox' type=text size=50 name=bkDBname value=""/KS_Data/SQL(" & year(now)&month(now)&day(now) & ").bak""></td>"
				  .echo "                </tr>"
				  .echo "              </table>"
				  .echo "			  </fieldset>"
				  .echo "			  <table width=""100%"" border=""0"">"
		  ...
		  .echo "                </tr>"
		  .echo "                <tr> "
		  .echo "                  <td height=""22"" align=""center""><input class='textbox' type=text size=50 name=DBpath value=""" &server.mappath(CollectDBPath) & """ readonly></td>"
		  .echo "                </tr>"
		  .echo "                <tr> "
		  .echo "                  <td height=""22""></td>"
		  .echo "                </tr>"
		  ...
		  .echo "                </tr>"
		  .echo "                <tr> "
		  .echo "                  <td height=""22"" align=""center""><input class='textbox' type=text size=50 name=bkDBname value=""Collect(" & year(now)&month(now)&day(now) & ").bak""></td>"
		  .echo "                </tr>"
		  .echo "              </table>"
		  .echo "			  </fieldset>"
		  .echo "			  <table width=""100%"" border=""0"">"
		...
		Sub Db_Restore()
		  With KS
			...
			if havebackfile=false then   .echo " disabled" 
			  ...
			  .echo "             <br><table width=""560"" style=""margin:0 auto"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""2"">"
			  .echo "                <tr> "
	   ...
	   Sub Db_Compact()
			 ...
			 conn.execute("DBCC SHRINKDATABASE([" & DataBaseName & "])")
			  ...
			  If   err   Then   
			       'sql 2008
					...
					conn.execute("ALTER DATABASE " & DataBaseName & " SET RECOVERY SIMPLE WITH NO_WAIT")
					conn.execute("ALTER DATABASE " & DataBaseName & " SET RECOVERY SIMPLE")
				    conn.execute("USE [" & DataBaseName & "]")
					conn.execute("DBCC SHRINKFILE (N'" & DataBaseName & "_Log' , 11, TRUNCATEONLY)")
					conn.execute("USE [master]")
					conn.execute("ALTER DATABASE " & DataBaseName & " SET RECOVERY FULL WITH NO_WAIT")
					conn.execute("ALTER DATABASE " & DataBaseName & " SET RECOVERY FULL ")
					if err then
				   .echo ("<font color=red>操作失败!</font>")
				   else
		...
		if DataBaseType=0 then
		  ...
		  .echo "                  <td width=""77%"" style='text-align:left'><font color=#ff6600>" & server.mappath(DBPath) & "</font></td>"
		  .echo "                </tr>"
		  .echo "                <tr> "
		  .echo "                  <td height=""22"" align=""right""><strong>压缩前大小：</strong></td>"
		  .echo "                  <td height=""22"" style='text-align:left'>" & FormatNumber(filesize, 0, False, False, True) & " 字节</td>"
		  .echo "                </tr>"
		  .echo "                <tr> "
		  .echo "                  <td height=""22"" align=""right""><strong>压缩后大小：</strong></td>"
		  .echo "                  <td height=""22"" style='text-align:left'>" & FormatNumber(filesize - ReclaimedSpace, 0, False, False, True) & " 字节 (总计可以减少" & FormatNumber(ReclaimedSpace, 0, True, False, True)& " 字节)</td>"
		  .echo "                </tr>"
		  .echo "                <tr> "
		  .echo "                  <td height=""22"" align=""right""><strong>地区标识符：</strong></td>"
		  .echo "                  <td height=""22"" style='text-align:left'>" & GetLocaleName(LocaleIdentifier) & "</td>"
		  .echo "                </tr>"
	   ...
	   RS.Close:Set RS=Nothing
		if err then KS.AlertHintScript "对不起,您的服务器不支持此操作!"
		  ...
		  .echo "                <tr> "
		  .echo "                  <td width=""23%"" height=""22"" align=""right""><strong>数据库路径：</strong></td>"
		  .echo "                  <td width=""77%"" style='text-align:left'><font color=#ff6600>" & server.mappath(CollectDBPath) & "</font></td>"
		  .echo "                </tr>"
		  .echo "                <tr> "
		  .echo "                  <td height=""22"" align=""right""><strong>压缩前大小：</strong></td>"
		  .echo "                  <td height=""22"" style='text-align:left'>" & FormatNumber(filesize, 0, False, False, True) & " 字节</td>"
    ...
    %>
	 ...
	 if ($('textarea[name=Sql]').val()=='')
	  ...
	  $(document).ready(function(){
	    $("#ExeSQLFrame").height($(window).height()-170)
	  });
	</script>
    <div class="pageCont2">
	...
	<tr height="25">
	 <td align="center" class="operatingBox">
	  ...
	  <tr> 
		<td valign="top" class="operatingBox"><iframe id="ExeSQLFrame" scrolling="auto" src="KS.Database.asp?Action=ExecSql&Flag=Result" style="width:100%;height:93%" frameborder=1></iframe></td>
	  </tr>
	</table>
    </div>
	<% End iF%>
