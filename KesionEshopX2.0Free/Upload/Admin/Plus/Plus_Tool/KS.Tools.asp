...
Class Tools
        Private KS,KSCls,ChannelID,n,SQL
...
Function CleanLabel()
		   ...
		   RS.Open "select ID,LabelName,LabelContent from KS_Label",conn,1,1
		   if NOT RS.Eof Then
		       SQL=RS.GetRows(-1)
			   dim i,total:total=rs.recordcount
			   response.write "<strong>共有标签<font color=green>" & total & "</font>个</strong><br/>"
			   For  i=0 to Ubound(SQL,2)
			       response.write "<script>sss.innerHTML='正在检查标签[<font color=#ff6600>" & SQL(1,I) &"</font>]...';</script>"
				   response.flush
				   Call InnerJS(i,Total,"个")
				   dim labelname:labelname=lcase(replace(replace(trim(SQL(1,I)&""),"{",""),"}",""))
				   dim check:check=CheckFile(Server.MapPath(KS.Setting(3) &KS.Setting(90)),labelname)
				   if Cbool(check)=false then
					n=n+1
				    Conn.Execute("delete from KS_Label Where ID='" & SQL(0,I) &"'")
					response.write "<font color=#999999>标签:" & SQL(1,I) & "没用，已删除！</font><Br/>"
				   end if
			  Next
			   RS.Close
			   Set RS=Nothing
		 End If
		  response.write "<script>$('#txt2').html(100);sss.innerHTML='<strong>恭喜，执行完毕！<a href=""KS.Tools.asp"" style=""color:#ff6600"">点此返回</a></strong>';</script>"
		   response.write "共有标签<font color=green>" & total & "</font>个，删除了<font color=red>" & n & "</font>个没用的标签。"
		  response.write "</div>"
...
End Function
'检查标签是否存在，存在为true 不存在为false
Function CheckFile(strFolderPath,LabelName)

  Dim KSClSs:Set KSCLSs=New Refresh
			...
			If fso.FolderExists(strFolderPath) Then
				...
				For Each DirFiles In fsoFile.SubFolders
				 CheckFile=CheckFile(ParentPath & "\" & DirFiles.Name,LabelName)
				 if checkfile=true then exit function
				Next
				
				For Each DirFiles In fsoFile.Files
					strFileName = DirFiles.Name
					strFilePath = strFolderPath & "\" & DirFiles.Name
					Dim Txt:Txt=Lcase(KSClss.LoadTemplate(replace(strFilePath,server.MapPath(ks.setting(3)),""))&"")
					'response.write strFilePath &"<br/>"
					if instr(txt,labelname)<>0 then
					 ' ks.die strFilePath & "===" & labelname
					  CheckFile=true
					  exit function
					end if
				Next
			...
			End If
			Set fso = Nothing
			CheckFile=CheckInLabel(labelname)
	End Function
	
	'从数据库中检查是否存在
	Function CheckInLabel(labelname)
	  Dim I
	  If IsArray(SQL) Then
	   For  i=0 to Ubound(SQL,2)
	     if instr(Lcase(SQL(2,I)&""),labelname)<>0 then
		   CheckInLabel=true
		   Exit Function
		 end if
	   Next
	...
	End Function
		...
		Sub Kesion()
		 With KS
			...
			select case KS.S("Action")
			  ...
			  case "CleanLabel" CleanLabel:response.end
			end select
			...
			.echo "      <div class='tabTitle'> 一键相关管理工具</div>"
			...
			.echo " <table width=""99%"" border=""0"" align=""center"" cellspacing=""1"" bgcolor=""#f5f5f5"">"
			.echo "    <tr>"
			...
			.echo " <table width=""99%"" style=""margin-top:5px"" border=""0"" align=""center"" cellspacing=""1"" bgcolor=""#f5f5f5"">"
			.echo "    <tr>"
		...
		If KS.Setting(56)="1" Then
			.echo " <a name=""club""></a><table width=""99%"" style=""margin-top:5px"" border=""0"" align=""center"" cellspacing=""1"" bgcolor=""#f5f5f5"">"
			.echo "    <tr>"
			...
			.echo " <table width=""99%"" style=""margin-top:5px"" border=""0"" align=""center"" cellspacing=""1"" bgcolor=""#f5f5f5"">"
			.echo "    <tr>"
			.echo "      <td style='text-align:left' height=""30"" class='clefttitle'>&nbsp;<font color=""#000080""><b>一键删除无用的标签</b></font></td>"
			.echo "    </tr>"
			.echo "    <tr>"
			.echo "      <td width=""25%"" class='tdbg' style='padding:10px'><strong>功能说明：</strong>"
			.echo "       如果您的网站已在正式运行，并且数据库里可能存在很多没有用的标签，建议使用此功能清理一次（此功能仅检测标签是否被" & KS.Setting(3) & KS.Setting(90) & "目录下的模板调用），以提高系统性能。使用此功能前，强烈建议您先备份数据库，由于此工具导致的有用标签被误删除，官方不承担任何责任。<br/>"
			.echo "<form action=""KS.Tools.asp?action=CleanLabel"" method=""post"" name=""boardform"" id=""boardform"">"
			.echo "      <table border='0' width='98%' align'center'>"
			.echo "       <tr><td><input type='submit' class='button' onclick=""return(confirm('此操作不可逆，确定执行清理无用标签操作吗？'))"" value='一键删除无用的标签'></td></tr>"
			.echo "      </table>"
			.echo "        </td></form>"
			.echo "    </tr>"
			.echo "  </table>"
...
Function DocContent()
 ...
 If DataBaseType=1 Then
  Param=" Where convert(nvarchar(255),articlecontent)=''"
 Else
...
Sub UpGuestBoard()
	...
	If Not (RS.Eof or rs.bof) Then
	  ...
	  Do While Not RS.Eof
	     NowNum=NowNum+1
		 TopicNum=Conn.Execute("Select count(id) From KS_GuestBook Where deltf=0 and BoardID=" & RS(0))(0)
...
Sub Main()
		  ...
		  '.echo ("<body oncontextmenu=""return false;"" scroll=no style='background-color:transparent'>")
		  		.echo "      <div class='tabTitle mt20'> 一键相关管理工具</div>"
		...
		Sub InnerJS(NowNum,TotalNum,itemname)
		  With KS
				...
				.echo "try{img2.width=" & Fix((NowNum / TotalNum) * 400) & ";txt2.innerHTML=p;$('#t1').html(" & TotalNum &");$('#t2').html(" & NowNum & ");t0.innerHTML=t00.innerHTML='" & ItemName &"';img2.title=""(" & NowNum & ")"";}catch(e){}" & vbCrLf
