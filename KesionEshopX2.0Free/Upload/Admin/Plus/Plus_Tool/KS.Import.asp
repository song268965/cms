...
Class Import
	   ...
	   Sub Step1()
		 With KS
			...
			.echo "      <td><select id='channelid' name='channelid' style=""height:100px"" size=""8"">"
            ...
            .echo "      <td><textarea name='datasourcestr' id='datasourcestr' cols='70' rows='6'>Provider=Microsoft.Jet.OLEDB.4.0;Data Source=/数据库.mdb</textarea>"
            ...
            .echo "      <td><input type='text' class='textbox ml0'  name='tablename' id='tablename' value='Table1' /></td>"
			.echo "    </tr>"
			.echo "  </table>"
		...
		Sub OpenImporIConn()
				   if not isobject(IConn) then
					...
					If Err Then 
					  ...
					  Response.Write "<script>KesionJS.Alert('数据源连接失败,请检查数据库连接!原因：" & replace(err.description,"'","\'") &"','history.back();');</script>"
					  Err.Clear
					  response.end
					end if
				   end if		
       ...
       '**************************************************
		...
		Sub Step2()
		   ...
		   With KS
			...
			If KS.C_S(ChanneliD,6)=1 Then
			 If FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='fulltitle']/showonform").text="1" Then
			.echo "<tr class='tdbg'><td height='25' align='right' class='clefttitle'>"
			...
			.Echo "<div id='picpath1'>图片路径<input class='textbox' type='text' name='mypicpath1'  value='" & KS.Setting(3) & "images/" & Year(Now) & "-" & Month(Now) & "/'> <br/><font color=#999999>请先将原tbi图片复制到以上文件夹下,执行导入的同时系统会自动重命名为jpg格式</font></div>"
			...
			.Echo "<div id='picpath'>图片路径<input class='textbox' type='text' name='mypicpath'  value='" & KS.Setting(3) & "images/" & Year(Now) & "-" & Month(Now) & "/'> <br/><font color=#999999>请先将原tbi图片复制到以上文件夹下,执行导入的同时系统会自动重命名为jpg格式</font></div>"
			.Echo "</td></tr>"
			End If
		...
		If KS.C_S(ChanneliD,6)<>5 Then
		 if FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='keywords']/showonform").text="1" then
		   ...
		   if FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='author']/showonform").text="1" then
			.echo "<tr class='tdbg'><td height='25' align='right' class='clefttitle'>"
		...
		If KS.C_S(ChanneliD,6)<>5 Then
		  if FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='origin']/showonform").text="1" then
			.echo "<tr class='tdbg'><td height='25' align='right' class='clefttitle'>"
		...
		If KS.C_S(ChannelID,6)=3 Then
		  ...
		  ElseIf KS.C_S(ChannelID,6)=1 Then
		   if FieldXML.DocumentElement.selectsinglenode("fielditem[@fieldname='intro']/showonform").text="1" then
			.echo "<tr class='tdbg'><td height='25' align='right' class='clefttitle'>"
