...
if trim(request("type1"))="GetChird" then
	Call ShowChird()
		...
		Response.Write "<li class='parent' onclick=""location.href='KS.DataReplace.asp';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon num'></i>普通模式</span></li>"
		Response.Write "<li class='parent' onclick=""location.href='?Action=Main2';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon merge'></i>高级模式</span></li>"
		Response.Write "<li class='parent' onclick=""location.href='?Action=Main3';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add'></i>赋值模式</span></li>"
		Response.Write "<li class='parent' onclick=""location.href='?Action=Main4';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon photo'></i>所有图片地址替换</span></li>"
		Response.Write "</ul><div class='pageCont2'><div class='tabTitle'>数据库字段值替换</div>"
...
function changedb(){
   $("#tablechird").attr("src","KS.DataReplace.asp?type1=GetChird&type="+document.myform.TableName.value);
}
function CheckForm(){
...
<form method="post" name="myform" onSubmit="return CheckForm();" action="KS.DataReplace.asp" target="_self">
	...
	<td height="30"><input name="ColumnName" type="hidden" value="" size="40"><iframe style="top:2px;" ID="tablechird" src="KS.DataReplace.asp?type1=GetChird" frameborder=0 scrolling=no width="100%" height="40"></iframe></td></tr>
	<tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
...
<form method="post" name="myform" action="?" target="_self">
	...
	<tr class="tdbg" onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
	<td height="30" align="right" class="clefttitle"><strong>替 换 成：</strong></td>
	<td height="30"><textarea name="strNew" cols="60" rows="4"></textarea>&nbsp;如 http://www.kesion.com</td></tr>
...
Sub DoReplaceData(TableName,ColumnName,strOld,strNew)	
		...
		on error resume next
		do while not rs.eof
			if instr(rs(ColumnName),strOld)<>0 and instr(lcase(rs(ColumnName)),"demo.kesion.com")=0 then
				rs(ColumnName)=replace(rs(ColumnName)&"",strOld&"",strNew&"",1,-1,1)
				i=i+1
				'if err then ks.die err.description&"==" & rs(ColumnName)
				'Response.Write "."
			end if
			rs.update
			rs.movenext
...
Sub ShowChird()
	...
	if trim(request("type"))<>"" then	
		OpenConn : Set rs = Conn.OpenSchema(4)	
		Do Until rs.EOF or lcase(rs("Table_name")) = lcase(trim(request("type")))
			rs.MoveNext
		Loop
		Do Until rs.EOF or lcase(rs("Table_name")) <> lcase(trim(request("type")))
