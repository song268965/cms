...
Response.Write "<!DOCTYPE html><html><head><title>wss设置</title>" & vbCrLf
...
Select Case Trim(Action)
	Case "save"
		Call savewss
	Case "show"
		If Not KS.ReturnPowerResult(0, "KSO100031") Then                  '栏目权限检查
				Call KS.ReturnErr(1, "")   
				Response.End()
		End iF
	    Call show
	Case Else
	    If Not KS.ReturnPowerResult(0, "KSO10003") Then                  '栏目权限检查
				Call KS.ReturnErr(1, "")   
				Response.End()
		End iF
...
<div class="tabTitle mt20">WSS流量统计设置</div>
<div class="pageCont2">
...
<tr class="tdbg">
	...
	<td width="80%"><input type="text" class="textbox" name="Wss_Domain" size="35" value="<%=Wss_Domain%>"> 
		<font color="red">* 如：www.kesion.com,不能加http://</font>
	</td>
</tr>
<tr class="tdbg">
	<td class="clefttitle" align="right"><u>WSS统计站点ID</u>：</td>
	<td><input type="text" class="textbox" name="Wss_SiteID" size="35" value="<%=Wss_SiteID%>"> 
		<font color="red">* 如果你已经注册过WSS请输入你的站点ID</font>
	</td>
</tr>
<tr class="tdbg">
	<td class="clefttitle" align="right"><u>WSS统计登录密码</u>：</td>
	<td><input type="text" class="textbox" name="Wss_PassWord" size="35" value="<%=Wss_PassWord%>"> 
...
<tr class="tdbg">
	...
	<td class="clefttitle" style="text-align:left"><input type="checkbox" name="apply" value="1"/> 
		<font color="red">* 如果你是第一次申请请选择</font>
	</td>
</tr>
</form>
<tr>
	<td class="clefttitle" style="text-align:left" colspan="2"><b>说明</b><br/>&nbsp;&nbsp;<a href="http://intf.cnzz.com/" target="_blank">WSS</a> 一直致力于精确时实的网站流量统计分析，并且通过不断的努力为贵网站提供更快速、更直观、更准确的统计服务。<br/><br/>
	<b>申请失败情况下错误代码：</b><br/>
...
<tr>
	<td align="right"><u>统计代码</u>：</td>
	<td class="clefttitle" style="text-align:left">
	<textarea name="wsscode" class="textbox" rows="3" cols="70">&lt;script src='http://pw.cnzz.com/c.php?id=<%=Wss_SiteID%>&l=2' language='JavaScript' charset='utf-8'&gt;&lt;/script&gt;</textarea>
	...
	If XmlDoc.Load(Xml_Files) Then
		...
		Else
			If Len(Request.Form("wss_domain")) > 3 Then
				...
				strDomain = Replace(KS.G("wss_domain"),"http://","")
				...
				Else
					Response.Write "<script>alert('申请WSS失败!错误代码：" & replace(strWssData,vbcrlf,"\n") & strKey &"');history.back();</script>"
					Exit Sub
				End If
			End If
		End If
