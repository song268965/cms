...
Class Collect_ItemCollectFast
		...
		'项目变量
		Private ItemID, ItemName, ChannelID, strChannelDir, ClassID, SpecialID, ProvinceCity,LoginType, LoginUrl, LoginPostUrl, LoginUser, LoginPass, LoginFalse
		...
		ErrMsg = ""
		 ...
		 '取得同义词库
		TYCK=KS.ReadFromFile(KS.Setting(3)& KS.Setting(89) & "plus/Plus_collect/seo/tyc.txt")
		If Not KS.IsNul(TYCK) Then TYCKArr=Split(TYCK,vbcrlf)
		   ...
		   For Arr_i = 0 To UBound(NewsArray)
				...
				If KS.C_S(ChannelID,6)="2" Then  '图片模型
					...
					Set regEx = New RegExp
						...
						If Tp_is<>"" and Tp_Io<>"" Then
							regEx.Pattern = Tp_is&"(.+?)"&Tp_io
							Dim IntroStr:IntroStr=""
							Set Matches = regEx.Execute(Tp_AddressList_Code)
							For Each Match In Matches
							   dim iInstro:iInstro=replace(replace(Match.Value,Tp_is,""),Tp_io,"")
							   if IntroStr="" then
							   IntroStr=iInstro
							   else
							   IntroStr=IntroStr &"###" & iInstro
							   end if
							Next
							IntroStr=split(IntroStr&"","###")
						End If
						
						regEx.Pattern = Tp_srcs&"(.+?)"&Tp_srco
						Set Matches = regEx.Execute(Tp_AddressList_Code)
						dim ii:ii=0
						For Each Match In Matches
						    tp_url=replace(replace(Match.Value,Tp_srcs,""),Tp_srco,"")
							...
							End If
							if isarray(IntroStr) then
							 if ubound(IntroStr)>=ii then
							    iInstro=IntroStr(ii)
							 else
							    iInstro=""
							 end if
							...
							if tp_str="" then
							 tp_str=Replace(iInstro,"|","")&"|" & Tp_Url & "|" & Tp_Url
							else
							 tp_str=tp_str&"|||"&Replace(iInstro,"|","")&"|" & Tp_Url & "|" & Tp_Url
							end if
							If UploadFiles="" Then
							  UploadFiles=Tp_Url
							Else
											...
											Else
											 PageNum = PageNum + 1
												...
												If Tp_is<>"" and Tp_Io<>"" Then
													...
													Set Matches = regEx.Execute(Tp_AddressList_Code)
													For Each Match In Matches
													   iInstro=replace(replace(Match.Value,Tp_is,""),Tp_io,"")
													   if IntroStr="" then
													   IntroStr=iInstro
													   else
													   IntroStr=IntroStr &"###" & iInstro
			  ...
			  If FoundErr <> True Then
				 ...
				 If UploadFiles <> "" Then
					...
					End If
					If BeyondSavePic <> 1 Then  UploadFiles = ""
				 Else
					ImagesNum = 0
					DefaultPicUrl = ""
					IncludePic = 0
			  ...
			  End If
			   if DefaultPicUrl<>"" AND left(DefaultPicUrl,4)<>"http" then DefaultPicUrl=KS.Setting(2) & DefaultPicUrl
		...
		Response.Write "<div class='pageCont2 mt20'><div class=""tabTitle"">采集系统采集管理</div>"
		End Sub
		...
		Response.Write "<table width=""98%"" border=""0"" style=""text=align:center"" cellpadding=""0"" cellspacing=""0"">"
		 Response.Write "   <tr>"
		...
		Response.Write "window.setInterval('ShowPromptInfo()',200);</script>"
			...
			Response.Write "<br><br><br><table id=""BarShowArea"" width=""400"" border=""0"" align=""center"" cellspacing=""0"" cellpadding=""0"">"
			Response.Write "<tr> "
			...
			Response.Write "<table width=""550"" border=""0"" align=""center"" cellspacing=""0"" cellpadding=""0""><tr> "
		...
		Sub SetItems()
			  ...
			  ProvinceCity= Arr_Item(7, ItemNumTemp)     '归属地
		...
		Sub SaveArticle()
		  ...
		  Select Case  KS.C_S(ChannelID,6)
		   ...
		   Rs("TitleFontType") = ""
		   If config(1)="1" Then
		   Rs("ArticleContent") = TYCKTF(Content)
		   Else
		   ...
		   If IntoBase <>0 Then  '直接写入数据库
		     if not ks.isnul(provincecity) then
			   Rs("province")=Split(provincecity&"|||","|")(0)
			   Rs("city")=Split(provincecity&"|||","|")(1)
			   Rs("county")=Split(provincecity&"|||","|")(2)
			 end if
		   End If
		  ...
		  Case 2
		   ...
		   Rs("Origin") = KS.GotTopic(CopyFrom,50)
		   If config(1)="1" Then
		   Rs("PictureContent") = TYCKTF(Content)
		   Else
		  ...
		  End Select
		   ...
		   Rs("WapTemplateID")=KS.C_C(ClassID,22) '3g模板
		   ...
		   If IntoBase = 1 or IntoBase=2 Then          '当直接插入数据库,同时往主数据库写数据
		      ...
		      if not ks.isnul(specialid) then  '专题
			     dim sids:sids=KS.FilterIds(specialid)
				 if not ks.isnul(sids) then
				     dim sidsArr:sidsArr=split(sids,",")
					 dim iii
					 For iii=0 To Ubound(sidsArr)
							Conn.Execute("Insert Into KS_SpecialR(SpecialID,InfoID,ChannelID) values(" &sidsArr(iii) & "," & RS("ID") & "," & ChannelID & ")")
					 Next
				 end if
			  end if
		...
		'同义词替换
		function TYCKTF(byval content)
		  if isarray(TYCKArr) then
			dim tempstr,key,yllen
			dim iPosCur,iPosLast
			iPosCur=0
			iPosLast    = 1
			tempstr=""
			Do While True
				  iPosCur=findKey(content,key,iPosLast,yllen)
				  if iPosCur>0 Then
				   tempstr    = tempstr &  mid(content,iPosLast,iposcur-iPosLast)
				   '加上被替换的词
				   'tempstr    = tempstr &  "<font color=red>" & key & "</font>"
				   tempstr    = tempstr &  key 
				   iPosLast=iPosCur+yllen
				  Else
					tempstr    = tempstr &  mid(content,iPosLast)
					Exit Do
				  End if
			Loop		  
		    tycktf=tempstr
		...
		'找出最小位置,被替换词,原词长度
		'参数: content 原文,key 被找出的词 iPosLast 原文开始找的位置 yllen 原词的长度
		function findKey(content,ByRef Key,ByVal iPosLast,ByRef yllen)
		  dim i,cur,posCur,a,b
		  cur=0
		  for i=0 to ubound(TYCKArr)
		    IF instr(tyckarr(i),"=")<>0 THEN
			   a=split(tyckarr(i),"=")(0)   '被替换词
			   b=split(tyckarr(i),"=")(1)   '替换词
			   posCur=instr(iPosLast,content,a)    '正序查找
			   if posCur<>0 and Cur=0 Then 
				 cur=posCur
				 key=b
				 yllen=len(a)
				 if cur<=1 then Exit For   '说明已经是最小了,可以退出
			   elseif posCur<>0 and PosCur<cur Then
				 cur=posCur
				 key=b
				 yllen=len(a)
				 if cur<=1 then Exit For   '说明已经是最小了,可以退出
			   End If
			  If config(2)="1" Then
				   posCur=Instr(iPosLast,content,b)  '反向查找
				   if posCur<>0 And Cur=0 Then
					 cur=PosCur
					 key=a
					 yllen=len(b)
