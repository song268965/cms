...
Class Collect_IntoDatabase
		...
		Response.Write "<script src=""../../../ks_inc/jquery.js""></script>"
			If Not KS.ReturnPowerResult(0, "M0100083") Then                  '权限检查
				Call KS.ReturnErr(1, "")   
				Response.End()
			End if
		...
		Response.Write "<li class='parent' onclick='location.href=""Collect_ItemHistory.asp?ChannelID=" & ChannelID & """;'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon merge'></i>历史记录</span></li>"
		...
		Response.Write "<div class='tabTitle'>文档审核入库</div>"
		If Request("Key")<>"" Then
		...
		Response.Write "<table border=""0"" cellspacing=""0"" width=""100%"" cellpadding=""0"" class=""mainTable"">"
		Response.Write "<tr><td colspan=6 height='25' class='operatingBox2'>"
				Response.Write "<span style='float:right'>请按模型入库<select id='channelid' name='channelid' onchange=""if (this.value!=0){location.href='?channelid='+this.value;}"">"
		...
		Response.Write "&nbsp;&nbsp;<strong>选择：</strong><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a>&nbsp;&nbsp;<input type='submit' value='批量删除' class='button' onclick=""this.form.action='Collect_IntoDataBase.asp?ChannelID=" & ChannelID & "&Action=del&page=" & currentpage&"'"">&nbsp;<input type='submit' onclick=""this.form.action='?Action=Into&page=" & currentpage& "&channelid=" & channelid & "&inflag=1';"" value='批量入库' class='button'>&nbsp;<input type='button' onclick=""AllIntoDataBase();"" value='全部入库' class='button'>&nbsp;<input type='button' onclick=""DelAllRecords();"" value='全部删除' class='button'>&nbsp;"
		

		
		Response.Write "    </td></tr><tr style=""padding: 0px 2px;"">"
		Response.Write "      <td height=""22"" align=""center"" class=sort>选择</td>"
		Response.Write "      <td align=""center"" class=sort>标题</td>"
		Response.Write "      <td align=""center"" class=sort>文章来源</td>"
		Response.Write "      <td align=""center"" class=sort>栏目</td>"
		Response.Write "      <td align=""center"" class=sort>时间</td>"
		Response.Write "      <td align=""center"" class=sort>管理操作</td>"
		Response.Write "    </tr>"
		...
		Sub showContent()
		 ...
		 Do While Not RSObj.EOF
			 Response.Write "  <tr height=""23"" class='list' id='u" & RSObj("ID") &"' onclick=""chk_iddiv('" & RSObj("ID") &"')"" onMouseOver=""this.className='listmouseover'"" onMouseOut=""this.className='list'"">"
		     Response.Write "<td class='splittd' style='text-align:center'><input name=""id"" type=""checkbox""  onclick=""chk_iddiv('" & RSObj("ID") &"')"" id='c" & RSObj("ID") &"'  value='" & RSObj("ID") &"'></td>"
			 Response.Write (" <td class='splittd' width=""435"" height=""18"">")
			 Response.Write "<a href='javascript:ViewRecords(" & RSObj("ID") & ");' NewsID='" & RSObj("ID") & "'><img border='0' src='../../Images/folder/TheSmallWordNews1.gif'  align='absmiddle'>"
			  Response.Write "" & KS.GotTopic(RSObj("Title"), 38) & "</a>"
			  'if rsobj("verific")=0 then response.write " <font color=red>未审</font>" else response.write "<font color=#999999>已审</font>"
			  Response.Write ("</td> ")
		...
		RSObj.Close
		Set RSObj = Nothing
		   Response.Write "<tr><td colspan=7 height='25'>&nbsp;&nbsp;<strong>选择：</strong><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a> <input type='submit' value='批量删除' class='button' onclick=""this.form.action='Collect_IntoDataBase.asp?ChannelID=" & ChannelID & "&Action=del&page=" & currentpage&"'"">&nbsp;<input type='submit' onclick=""this.form.action='?Action=Into&page=" & currentpage& "&channelid=" & channelid & "&inflag=1';"" value='批量入库' class='button'>&nbsp;<input type='button' onclick=""AllIntoDataBase();"" value='全部入库' class='button'>&nbsp;<input type='button' onclick=""DelAllRecords();"" value='全部删除' class='button'>&nbsp;</td></tr>"
		   Response.Write "</form>"
		Response.Write ("<tr> ")
