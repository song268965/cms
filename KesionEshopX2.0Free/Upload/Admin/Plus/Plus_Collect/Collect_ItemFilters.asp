...
Class Collect_ItemFilters
		...
		End Sub
		Sub Kesion()
			If Not KS.ReturnPowerResult(0, "M0100081") Then                  '权限检查
				Call KS.ReturnErr(1, "")   
				Response.End()
			End if
		...
		function CreateFilters()
		{
			...
			$(parent.document).find('#BottomFrame')[0].src='<%=KS.Setting(3) & KS.Setting(89)%>Post.Asp?OpStr=信息采集管理 >> 过滤设置 >> <font color=red>添加过滤</font>&ButtonSymbol=FiltersAdd';
		}
		function EditFilters(id)
		{
		  if (id!=null&&id!='')
		  {
			...
			$(parent.document).find('#BottomFrame')[0].src='<%=KS.Setting(3) & KS.Setting(89)%>Post.Asp?OpStr=信息采集管理 >> 过滤设置 >> <font color=red>编辑过滤</font>&ButtonSymbol=FiltersEdit';
           return ;
		  }
		 else
		  alert('请选择要编辑的过滤!');
		...
		Response.Write "<li class='parent' onclick='location.href=""Collect_ItemModify.asp?channelid=" & ChannelID & """;'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add'></i>新建项目</span></li>"
		...
		Response.Write "<li class='parent' onclick='location.href=""Collect_main.asp?ChannelID=" & ChannelID & """;'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon back'></i>回上一级</span></li>"
		Response.Write ("</ul>")

		Response.Write "<div class='pageCont2'>"
		Response.Write "<div class='tabTitle'>采集过滤设置</div>"
		Response.Write "<table border=""0"" cellspacing=""0"" width=""100%"" cellpadding=""0"">"
		Response.Write "        <TR class=""sort"">"
		Response.Write "          <TD>选择</TD>"
		Response.Write "          <TD>过滤名称</TD>"
		Response.Write "          <TD>所属项目</TD>"
		 Response.Write "         <TD>过滤对象</TD>"
		 Response.Write "         <TD>过滤类型</TD>"
		  Response.Write "        <TD>状态</TD>"
		  Response.Write "        <TD>管理操作</TD>"
		...
		If Not RSObj.EOF Then
			...
			End If
		 RSObj.Close
		   Response.Write "<tr><td height='30' class='operatingBox' colspan=7>&nbsp;&nbsp;<strong>选择：</strong><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a> <input type='submit' value=' 批量删除 ' onclick=""return(confirm('确定删除吗？'));"" class='button'>&nbsp;<input type='button' onclick=""CreateFilters();"" value=' 添加过滤 ' class='button'></td></tr>"
		   Response.Write "</form>"
		...
		Sub SetFlag()
		   ...
		   If FilterID="" Then FilterID=KS.S("ID")
		   If FilterID <> "" Then
		...
		Sub showContent()
		 iItem = 0
		  ...
		  Response.Write "  <tr height=""23"" class='list' id='u" & FilterID &"' onclick=""chk_iddiv('" & FilterID &"')"" onMouseOver=""this.className='listmouseover'"" onMouseOut=""this.className='list'"">"
		  Response.Write "<td class='splittd' style='text-align:center'><input name=""id"" type=""checkbox""  onclick=""chk_iddiv('" & FilterID &"')"" id='c" & FilterID &"'  value='" & FilterID &"'></td>"
		...
		Response.Write "<br>"
		Response.Write "<table width=""100%"" border=""0"" align=""center"" cellpadding=""2"" cellspacing=""1"" class=""ctable"" >"
		Response.Write "    <tr class='tdbg'>"
		Response.Write "      <td width=""100"" height=""25"" align=""center"" class='clefttitle'> 过滤名称：</td>"
		Response.Write "      <td><input name=""FilterName"" class='textbox' type=""text"" id=""FilterName"" value=""" & FilterName & """ size=""25"" maxlength=""30"">"
		Response.Write "        &nbsp;</td>"
		Response.Write "    </tr>"
		Response.Write "    <tr class='tdbg'>"
	  ...
	  Sub FilterAddSave()
		...
		If FoundErr <> True Then
		   ...
		   If FilterID <> "" Then
			 ...
			 RsItem.Update
			 RsItem.Close
			 Response.Write ("<script src='../../../ks_inc/jquery.js'></script><script>alert('过滤修改成功!');location.href='Collect_ItemFilters.asp?channelid=" & channelid & "&Page=" & Request("Page") & "';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=信息采集管理 >> <font color=red>过滤设置</font>&ButtonSymbol=Disabled';</script>")
		   Else
		   ...
		   RsItem.Update
		   RsItem.Close
		   Response.Write ("<script src='../../../ks_inc/jquery.js'></script><script>if (confirm('过滤添加成功，继续添加吗?')){location.href='?action=Add&ChannelID=" & ChannelID& "';}else{location.href='Collect_ItemFilters.asp?channelid=" & channelid & "';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=信息采集管理 >> <font color=red>过滤设置</font>&ButtonSymbol=Disabled'}</script>")
		   End If
		   Set RsItem = Nothing
		Else
