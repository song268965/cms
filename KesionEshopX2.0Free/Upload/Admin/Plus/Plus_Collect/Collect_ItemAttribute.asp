...
Class Collect_ItemAttribute
		...
		Private ItemID, ItemName, ChannelID, ClassID, SpecialID,WebUrl
		...
		Sub Kesion()
			...
			Response.Write "<div class='pageCont2'><table align='center' width=""100%"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""1"" class=""ctable"">"
			Response.Write "<form method=""post"" action=""Collect_ItemSuccess.asp"" name=""myform"">"
			 ...
			 Response.Write "   </tr>"
			  ...
			  Response.Write "    <td>"
			  Response.Write "<select name='SpecialID' id='SpecialID' multiple style='height:100px;width:380px;'>" &KS.ReturnSpecial(KS.FilterIds(SpecialID)) & "</select>"
			  Response.write"   <br/><span class='tips'>提示：需要选择直接入库才有效。</span>  </td>"
			  Response.Write "   </tr>"
			  Response.Write "  <tr class='tdbg'>"
			  Response.Write "    <td height=""20"" width=""20%"" align=""right"" class='clefttitle'> 选择地区：</td>"
			  Response.Write "    <td>"
			   dim province,city,county
			   if Not KS.IsNuL(WebUrl) Then
			      province=split(WebUrl&"|||","|")(0)
			      city=split(WebUrl&"|||","|")(1)
			      county=split(WebUrl&"|||","|")(2)
			   End If
			  	Response.Write "<script type='text/javascript'>"
				Response.write "try{setCookie(""pid"",'" & province & "');setCookie(""cid"",'" & city & "');}catch(e){}" & vbcrlf
				Response.write "</script>"
				Response.Write "   <script src=""../../../plus/area.asp"" type=""text/javascript""></script> <span class='tips'>提示：需要选择直接入库才有效。</span>" &vbcrlf
				Response.Write "<script type='text/javascript'>"
				if Province<>"" then
				  Response.Write "$('#Province').val('" & province & "');"
				end if
				if City<>"" Then
				  Response.Write "$('#City').val('" & City & "');"
				end if
				if County<>"" Then
				  Response.Write "$('#County').val('" & County & "');"
				end if
		...
		If Right(CurrPath,1)="/" Then CurrPath=Left(CurrPath,Len(CurrPath)-1)
			  ...
			  Response.Write "    <td><input type='text' size='25' name='TemplateID' id='TemplateID' value='" & templateid & "'>"
			  Response.Write "  </td>"
			  Response.Write "  </tr>"
			  Response.Write "  <tr class='tdbg'>"
