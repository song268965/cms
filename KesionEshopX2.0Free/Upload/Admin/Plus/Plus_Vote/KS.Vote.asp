...
Class Admin_Vote
	  ...
	  Sub MainList()
			...
			function VoteAdd()
			{
				...
				window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=调查主题管理中心 >> <font color=red>添加新调查主题</font>&ButtonSymbol=VoteAddSave';
			}
			function EditVote(id)
			{
			   ...
			   if (id==''){
				...
				window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=调查主题管理中心 >> <font color=red>编辑调查主题</font>&ButtonSymbol=VoteEdit';
				}else{
				alert('一次只能编辑一个调查主题!');
				}
			}
			...
			.Write "<li class='parent' onclick=""VoteAdd();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加调查</span></li>"
			.Write "<li class='parent' onclick=""EditVote('');""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>编辑调查</span></li>"
			.Write "<li class='parent' onclick=""DelVote('');""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除调查</span></li>"
			.Write "</ul>"
			.Write "<div class='pageCont2'>"
			.Write "<div class='tabTitle'>站点调查管理</div>"
			...
			.Write "          <td  height=""25"" class=""sort"">ID</td>"
			.Write "          <td height=""25"" class=""sort""align=""center"">调查主题</td>"
			.Write "          <td class=""sort"" align=""center"">论坛发起</td>"
			.Write "          <td class=""sort"" align=""center"">发起人</td>"
			.Write "          <td align=""center"" class=""sort"">时间</td>"
			.Write "          <td class=""sort"" align=""center"">是否最新</td>"
			...
			Sub showContent()
			  ...
			  With Response
					Do While Not RSObj.EOF
					   ...
					   .Write "<td class='splittd' align='center'>" & ID & "</td>"
					  ...
					  End If
					   .Write "  <td class='splittd' align='center'><a href=""javascript:EditVote('"&Id&"');"" class='setA'>修改</a>|<a href=""javascript:DelVote('"&Id&"');"" class='setA'>删除</a>|<a href=""../../../index.asp?do=vote&id=" & id & """ target=""_blank"" class='setA'>查看</a></td>"
					  .Write "</tr>"
			...
			Sub VoteAdd()
	           ...
	           If KS.G("Action")="Edit" Then
				.Write "  <div class=""topdashed sort"">修 改 调 查 主 题</div>"
			   Else
				.Write "  <div class=""topdashed sort"">添 加 调 查 主 题</div>"
			   End If
	           
			   dim timelimit,Title,AddDate,NewestTF,rs,sql,voteid,ItemArr,VoteNumArr,i,XMLStr,ShowVerifyCode
			   Dim VoteType,timebegin,timeend,nmtp,AllowGroupID,ipnum,ipnumS,templateid,Status,editnum
			   timelimit=0:nmtp=0:Status=1:editnum=0:timebegin=now:timeend=dateadd("m",1,now):ShowVerifyCode=0
			   templateid="{@TemplateDir}/投票页.html"
				...
				if Title<>"" then
					...
					RS("ShowVerifyCode")=KS.ChkClng(request("ShowVerifyCode"))
					...
					if ks.chkclng(request("voteid"))=0 then
					 ks.die "<script>top.$.dialog.confirm('恭喜，投票项目添加成功，继续添加吗？',function(){ location.href='plus/plus_vote/KS.Vote.asp?action=Add';},function(){location.href='plus/plus_vote/KS.Vote.asp';});</script>"
					else
					 ks.die "<script>top.$.dialog.alert('恭喜，投票项目修改成功!',function(){ location.href='plus/plus_vote/KS.Vote.asp'; });</script>"
					end if
				end if
				 End With
			...
			if KS.ChkClng(request("voteid"))<>0 Then
			  ...
			  If Not RS.Eof Then
				...
				ShowVerifyCode=KS.ChkClng(RS("ShowVerifyCode"))
				AddDate  = RS("AddDate")
				status=rs("status")
			  End If
			End If	 
               ...
               <div class="pageCont2">
				...
				<input type="hidden" name="voteid" value="<%=request("voteid")%>">
						<table width="99%" border="0" align="center" cellpadding="0" cellspacing="1" class="ctable">
						  <tr class="tdbg"> 
							<td width="101" height="30" align="right" class="clefttitle"><strong>主题名称：</strong></td>
							<td colspan="3" bgcolor="#EEF8FE">
							<input name="Title" type="text" class="textbox" size="50" value="<%=title%>" maxlength="50">
							<span class="tips">如：你对本站的哪些栏目较感兴趣!</span></td>
						  </tr>
                          <tr class="tdbg"> 
								 ...
								 if VoteXML.readystate=4 and VoteXML.parseError.errorCode=0 Then
									editnum=VoteXml.DocumentElement.SelectNodes("voteitem").length
									 For Each Node In VoteXml.DocumentElement.SelectNodes("voteitem")
									  tempstr=tempstr & "<tr><td width=9% height=20> <div align=center><input type=hidden name=id value=" & Node.getAttribute("id") & ">" & Node.getAttribute("id") & "</div></td><td width='65%'> <div align=center><input class=""textbox"" type=text name=item size=40 value='" & trim(Node.childNodes(0).text) & "'></div></td><td width='26%'> <div align=center><input class=""textbox"" type=text name=votenum style=text-align:center value='" & Node.childNodes(1).text & "' size=6></div></td></tr>"
									 Next
								 end if
							    end if
    ...
    {
    j=i+oldi;
    str=str+"<tr><td width=9% height=20> <div align=center><input type=hidden name=id value=0>"+j+"</div></td><td width=65%> <div align=center><input class=textbox type=text name=item size=40></div></td><td width=26%> <div align=center><input class=textbox type=text name=votenum style='text-align:center' value=0 size=6></div></td></tr>";
    }
	...
	<%end if%>
						  ...
						  <tr class="tdbg"> 
							<td width="101" height="30" align="right" class="clefttitle"><strong>启用验证码：</strong></td>
							<td colspan="3" bgcolor="#EEF8FE">
							<label><input type='radio' name='ShowVerifyCode'  value='0'<%IF ShowVerifyCode="0" Then Response.Write " checked"%>>不启用</albe>
							<label><input type='radio' name='ShowVerifyCode' value='1'<%IF ShowVerifyCode="1" Then Response.Write " checked"%>>启用</label>
							</td>
						  </tr>
						  <tr class="tdbg"> 
						  ...
						  <tr class="tdbg"> 
							...
							<td colspan="3" bgcolor="#EEF8FE">
							有效期 从<input onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" type='text' class="textbox" name='timebegin' value='<%=timebegin%>'>到
							<input type='text' onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" class="textbox" name='timeend' value='<%=timeend%>'> 时间格式为:YYYY-MM-DD HH:mm:ss
							</td>
						  </tr>
						  </tbody>
						  <tr class="tdbg"> 
						  ...
						  <tr class="tdbg"> 
							...
							<td colspan="3" bgcolor="#EEF8FE">
							一天内最多可以投<input type="text" class="textbox" name='ipnum' value='<%=IPNUM%>' size='3' style='text-align:center'>次 ,总共可以投<input type="text" name='ipnums' value='<%=IPNums%>' size='3' class="textbox" style='text-align:center'>次。tips:不限制请输入0
							</td>
						  </tr>
						  <tr class="tdbg"> 
							<td width="101" height="30" align="right" class="clefttitle"><strong>投票页模板：</strong></td>
							<td colspan="3" bgcolor="#EEF8FE">
							 <input type="text" class="textbox" name="templateid" value="<%=templateid%>" size="50" id="TemplateID">
						  ...
						  <tr class="tdbg"> 
							...
							<tr class="tdbg"> 
							<td width="101" height="30" align="right" class="clefttitle"><strong>添加时间：</strong></td>
							<td colspan="3" bgcolor="#EEF8FE">
							<input onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" size="50" type='text' class="textbox" name='adddate' value='<%=adddate%>'> <span class="tips">格式：<%=now%></span>
							</td>
						  </tr>		
	...
	<script>
	 ...
	 { var form=document.voteform;
	  if (form.Title.value=='')
	   {
		  top.$.dialog.alert('请输入调查主题!',function(){
		  form.Title.focus();
		  });
		 return false;
	   }
