...
Class Main
		...
		Public Sub Kesion()
			If Not KS.ReturnPowerResult(0, "KSMS200141") Then
			  Call KS.ReturnErr(1, "")
			  exit sub
			End If
		...
		Sub MainList()
			...
			.Write "<body topmargin=""0"" leftmargin=""0"">"
			  .Write "<div class='tabTitle mt20'>"
			  .Write "网友PK观点管理   <input type='button' value='返回' class='button' onclick=""location.href='KS.PKZT.asp'""/>"
			  .Write "</div>"
			...
			.Write "  </tr>"
			 ...
			 Set RSObj = Server.CreateObject("ADODB.RecordSet")
					 ...
					 If RSObj.EOF And RSObj.BOF Then
					   response.write "<tr><td colspan=10 style='height:30px;text-align:center'>对不起，找不到记录！</td>"
					 Else
			...
			Sub showContent()
			  ...
			  With Response
					Do While Not RSObj.EOF
					  ...
					  .Write "  <td class='splittd' align='center'>"
					  if rsobj("status")="1" then
					  .Write "<a href='?action=verify&v=0&id=" & rsobj("id") &"' title='取消审核' class='setA'>取审</a>|"
					  else
					  .Write "<a href='?action=verify&v=1&id=" & rsobj("id") &"' title='审核意见' class='setA'>审核</a>|"
					  end if
					  .Write"<a href='?action=del&id=" & rsobj("id") & "' onclick=""return(confirm('确定删除吗?'))"" class='setA'>删除</a></td>"
					  .Write "</tr>"
					 I = I + 1
					...
					Loop
					  ...
					  %>
						  <tr>
						   <td colspan=6 class="operatingBox">
						   <div><b>选择：</b><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a>
