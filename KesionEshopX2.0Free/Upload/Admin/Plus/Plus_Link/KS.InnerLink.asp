...
Class Inner_Main
	...
	%>
	   <script language="javascript">
		...
		function InnerAdd()
		{
		 top.openWin('添加新站内链接','plus/plus_link/KS.InnerLink.asp?Action=Add',false,700,280);
		}
		function EditInner(id)
		{
		top.openWin('修改站内链接','plus/plus_link/KS.InnerLink.asp?Action=Edit&Flag=Edit&InnerID='+id,false,700,280);
		}
	...
	<%
		 ...
		 .echo "<ul id='menu_top'>"
		 .echo "<li class='parent' onclick=""InnerAdd();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加站内链接</span></li>"
		 .echo "<li class='parent' onclick=""InnerControl(1);""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>编辑站内链接</span></li>"
		 .echo "<li class='parent' onclick=""InnerControl(2);""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>删除站内链接</span></li>"
		 .echo "</ul>"
		 .echo "<div class='pageCont2'>"
		 .echo "<div class='tabTitle'>站内链接管理</div>"
		 ...
		 .echo "  </tr>"
		  ...
		  SqlStr = "SELECT * FROM KS_InnerLink order by AddDate desc"
		   RSObj.Open SqlStr, Conn, 1, 1
				 ...
				 .echo "<tr><td class='splittd' colspan=10 style='text-align:center;height:30px'>没有添加任何链接！</td></tr>"
				 Else
		   ...
		   CloseConn
	        ...
	        .echo ("</td>")
	        .echo ("<td class='operatingBox'><input type='button' value='删 除' onclick=""InnerControl(2)"" class='button'></td>")
		...
		Sub InnerAdd()
			   With KS
					...
					End If
					 ...
					 .echo "  <br/><br/><form name=InnerForm method=post action=""KS.InnerLink.asp?Action=AddSave"">"
					 ...
					 .echo "        <td><input name=""Title"" type=""text"" id=""Title"" value=""" & server.HTMLEncode(Title) & """ class=""textbox"" style=""width:50%""> 如KesionIExam,KesionCMS等</td>"
					 .echo "    </tr>"
					 .echo "    <tr class=""tdbg"">"
					...
					End If
					 ...
					 .echo "    </tr>"
					 .echo "   <tr class=""tdbg"">"
					 .echo "            <td height=""28"" align=""right"" class=""clefttitle""><strong>查找位置:</strong></td><td><input type=""text"" name=""Start"" class=""textbox"" value=""" & Start & """>缺省值是 1,表示从第一个位置开始查找.   </td>"
					 .echo "    </tr>"
					 .echo "   <tr class=""tdbg"">"
					 .echo "            <td height=""28"" align=""right"" class=""clefttitle""><strong>替换次数:</strong></td><td><input type=""text"" name=""times""  class=""textbox"" value=""" & times & """>替换的次数。如果忽略，缺省值是 -1,表示全部替换.   </td>"
					 .echo "    </tr>"
			 ...
			 Sub InnerAddSave()
				...
				If Flag = "Add" Then
				  ...
				  End If
				   Set RSObj = Nothing
				    KS.Echo ("<script> if (confirm('站内链接添加成功!继续添加吗?')) {location.href='KS.InnerLink.asp?Action=Add';}else{top.frames[""MainFrame""].location.reload();top.box.close()}</script>")
				ElseIf Flag = "Edit" Then
				  ...
				  Else
				   ...
				   Set RSObj = Nothing
				  End If
				  Response.Write ("<script>alert('站内链接修改成功!');top.frames[""MainFrame""].location.reload();top.box.close();</script>")
				End If
			 End Sub
