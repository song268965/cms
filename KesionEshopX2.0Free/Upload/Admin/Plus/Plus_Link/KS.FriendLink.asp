...
Class FriendLink
		...
		SearchParam = "KeyWord=" & KeyWord & "&SearchType=" & SearchType
			...
			Select Case Action
			 ...
			 Case "AddFolder","EditFolder" '添加修改类别
			   If Not KS.ReturnPowerResult(0, "KSMS200011") Then                '检查友情链接操作(增和改)的权限检查
				  Call KS.ReturnErr(1, "")
				  Exit Sub
			   End If
			   Call Link_AddFolder()
		...
		{
		  top.openWin('添加友情链接类别','plus/plus_link/KS.FriendLink.asp?Action=AddFolder',false,700,280);
		}
		function EditFolder(FolderID)
		{ 
		  top.openWin('编辑友情链接类别','plus/plus_link/KS.FriendLink.asp?Action=EditFolder&FolderID='+FolderID,true,700,280);
		}
		function CreateLink()
		{
		   location.href='KS.FriendLink.asp?Action=AddLink';
           $(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape("链接管理 >> 添加友情链接")+'&ButtonSymbol=AddInfo';
		...
		{var ids=get_Ids(document.myform);
		 ...
		 if (linkID!=''){
			 if (linkID.indexOf(',')==-1){ 
			   location.href='KS.FriendLink.asp?Action=EditLink&Flag=EditLink&LinkID='+linkID;
               $(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr='+escape("链接管理 >> 修改友情链接")+'&ButtonSymbol=AddInfo';
		...
		function Verific()
		{
		 $(parent.document).find('#BottomFrame')[0].src='Post.Asp?Opstr=友情链接管理 >> <font color=red>审核友情链接</font>&ButtonSymbol=Disabled';
		...
		<%
		 ...
		 .echo "<ul id='menu_top'>"
			  If KeyWord = "" Then
			   .echo "<li class='parent' onclick=""location.href='?action=ClassManage';""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon  folder'></i>分类管理</span></li>"
			   .echo "<li class='parent' onclick=""CreateFolder();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加类别</span></li>"
			    .echo "<li class='parent' onclick=""CreateLink()""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加链接</span></li>"
			   .echo "<li class='parent' onclick=""EditLink('')""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>编辑链接</span></li>"
			   .echo "<li class='parent' onclick=""Delete('');""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除链接</span></li>"
			   .echo "<li class='parent' onclick=""Verific();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon audit'></i>审核用户申请</span></li>"
		
			  Else
				 .echo ("<i class='icon mainer'></i><span style='cursor:pointer' onclick=""location.href='KS.FriendLink.asp';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr=常规管理 >> <font color=red>友情链接管理首页</font>';"">链接首页</span>")
			    .echo (">>> 搜索结果: ")
		 ...
		 '============分类显示,带记忆功能=======================================
		    ...
		    .echo "<div style='padding-top:2px;cursor:pointer;text-align:center;position:absolute; z-index:1; right: 0px; top: 2px;'  onclick=""ClassToggle(" & ExtStatus &")""> <img src='../../images/mk_del.png' align='absmiddle'></div>"
			 For Each Node In ClassXML.DocumentElement.SelectNodes("row")
			   .echo "<li><i class='icon folder'></i><a href='?folderid=" & Node.SelectSingleNode("@folderid").text & "' title='" & Node.SelectSingleNode("@foldername").text & "'>" & KS.Gottopic(Node.SelectSingleNode("@foldername").text,10) & "</a></li>"
			 Next
			 .echo "</ul></div>"
			End If
			 '=============================================================
			  ...
			  .echo "<div class='tabTitle'>友情链接管理</div>"
		...
		Sub ClassManage()
		  ...
		  With KS
	       Call  Head()
			  ...
			  .echo "<div class='tabTitle'>链接分类管理</div>"
		   	  .echo (" <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"">")
			  ...
			  If IsObject(ClassXml) Then
			    For Each Node In ClassXml.DocumentElement.SelectNodes("row")
				  ...
				  .echo ("<td class='splittd'><i class='icon folder'></i><a href='?folderid=" & FolderID & "'>" & Node.SelectSingleNode("@foldername").text & "</a></td>")
				  ...
				  .echo ("<td class='splittd' align=center><a href='javascript:EditFolder("& FolderID & ")' class='setA'>修改</a>|<a href='KS.FriendLink.asp?Action=DelFolder&FolderID=" & FolderID & "' onclick=""return(confirm('删除分类将同时删除该分类下的链接,确定删除分类吗?'))"" class='setA'>删除</td>")
				  .echo ("</tr>")
				Next
			  End If
              .echo ("</table>")
		...
		Sub showContent()
		 ...
		 Do While Not RSObj.EOF
				...
				LinkID=RSObj("LinkID")
				  ...
				  .echo ("<td class='splittd' height=25>&nbsp;<span ondblclick=""EditLink('" & LinkID & "');"" LinkID=""" & LinkID & """><span style=""cursor:default""><i class='icon sorting'></i>" & RSObj("SiteName") &LockStr & "</span></span></td>")
			  ...
			  End If
			   ...
			   .echo ("<td class='splittd' align=""center""><a href=""javascript:EditLink('" & LinkID & "');"" class='setA'>修改</a>|<a href=""javascript:Delete(" & LinkID & ")"" class='setA'>删除</a></td>")
			   .echo ("</tr>")
			  RSObj.MoveNext
			 I = I + 1
		...
		Sub Link_AddFolder()
			...
			End If
			 ...
			 .echo "<body leftmargin=""0"" topmargin=""0"" marginwidth=""0"" marginheight=""0"" style='background:#fff;'>"
			 ...
			 .echo "          <tr class=""tdbg"">"
			 .echo "            <td width=""179"" class=""clefttitle"" height=""35""> <div align=""right"">类别名称</div></td>"
			 .echo "            <td style=""font-size:12px;""> <input name=""FolderName"" value=""" & FolderName & """ type=""text"" id=""FolderName"" size=""30"" class=""textbox"">概况性说明文字 </td>"
			 .echo "          </tr>"
			 .echo "          <tr class=""tdbg"">"
			 .echo "            <td height=""35"" class=""clefttitle""> <div align=""right"">简要说明</div></td>"
			 .echo "            <td><textarea name=""Description"" rows=""8"" id=""Description"" style=""width:80%;height:100px;"" class=""textbox"">" & Descript & "</textarea></td>"
			 .echo "          </tr>"
			 .echo "          <tr class=""tdbg"">"
			 .echo "            <td width=""179"" height=""35"" class=""clefttitle""> <div align=""right"">排列序号</div></td>"
			 .echo "            <td width=""542"" style=""font-size:12px;""> <input name=""OrderID"" value=""" & OrderID & """ type=""text""  size=""8"" class=""textbox"">数字越小排在越前面 </td>"
			 .echo "          </tr>"
			 .echo "        </table>"
			 .echo "  </form>"
		...
		OrderID    = KS.ChkClng(KS.G("OrderID"))
			 ...
			 Set TempObj = Conn.Execute("Select FolderName from [KS_LinkFolder] where FolderID<>" & FolderID & " and FolderName='" & FolderName & "'")
			  ...
			  LinkSql = "select top 1 * from [KS_LinkFolder] Where FolderID=" & FolderID
			  ...
			  Set LinkRS = Nothing
			  If FolderID=0 Then
			   KS.echo ("<script>if (confirm('添加友情链接类别成功,继续添加吗?')) {location.href='KS.FriendLink.asp?Action=AddFolder';} else { top.frames[""MainFrame""].location.reload();top.box.close();}</script>")
		      Else
			  KS.echo ("<script>alert('修改友情链接类别成功!');top.frames[""MainFrame""].location.reload();top.box.close();</script>")
		      End If
		End With
		End Sub
		...
		End If
		 ...
		 If Flag = "AddLink" Then
		 .echo "<div class=""topdashed sort"">添加友情链接</div>"
		 Else
		 .echo "<div class=""topdashed sort"">修改友情链接</div>"
		 End If
		 .echo "<div class='pageCont2'>"
		...
		End If
		 ...
		 .echo "            <td height=""25"">"
		 .echo "              <textarea name=""Description"" rows=""3"" id=""Description"" class='textbox' style=""width:80%;border-style: solid; border-width: 1"">" & Descript & "</textarea></td>"
		 .echo "          </tr>"
		 .echo "        </table>"
		 ...
		 .echo "}" & vbCrLf
		 .echo "function SubmitFun()" & vbCrLf
		 .echo "{ var form=document.LinkForm;" & vbCrLf
		...
		With KS
				...
				If SiteName <> "" Then
				 ...
				 End If
						...
						'End If
					  Set LinkRS = Server.CreateObject("adodb.recordset")
					  LinkSql = "select top 1 * from [KS_Link] Where LinkID=" & LinkID
					  LinkRS.Open LinkSql, Conn, 1, 3
					  If LinkRS.Eof Then 
					   LinkRS.AddNew
					   LinkRS("AddDate") = Now
					  ...
					  Set LinkRS = Nothing
					 If LinkID=0 Then
					   .echo ("<script>if (confirm('添加友情链接成功,继续添加吗?')) {location.href='KS.FriendLink.asp?Action=AddLink&FolderID=" & FolderID & "';} else {location.href='KS.FriendLink.asp?FolderID=" & FolderID & "';}</script>")
					Else
					  .echo ("<script>alert('修改友情链接成功!');location.href='KS.FriendLink.asp';</script>")
					End If
...
var ids=get_Ids(document.myform);
		 ...
		 if (linkID!=''){
			 if (linkID.indexOf(',')==-1){ 
			 top.openWin('编辑友情链接','plus/plus_link/KS.FriendLink.asp?Action=EditLink&Flag=EditLink&LinkID='+linkID,true)
			 }else{alert('一次只能够编辑一个友情链接站点!'); }
		 }else{
		  alert('请选择要编辑的链接站点!');
		 }
...
<%
		 ...
		 .echo "<ul id='menu_top'>"
		 .echo "<li class='parent' onclick=""VerificLink();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon audit'></i>批量审核</span></li>"
		 .echo "<li class='parent' onclick=""Delete('');""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除站点</span></li>"
		 .echo "<li class='parent' onclick=""location.href='KS.FriendLink.asp';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?Opstr=常规管理 >> <font color=red>友情链接管理</font>&ButtonSymbol=Disabled'""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon back'></i>返回首页</span></li>"
		  .echo "</ul>"
		  .echo "<div class='pageCont2'>"
		...
		Sub showLinkContent()
		 ...
		 Do While Not RSObj.EOF
			...
			.echo "<td class='splittd' align=center><input name='id'  onclick=""chk_iddiv('" & LinkID & "')"" type='checkbox' id='c"& LinkID & "' value='" & LinkID & "'></td>"
				 .echo ("<td class='splittd' height=25>&nbsp;<span ondblclick=""ViewLink(this.LinkID);"" LinkID=""" & RSObj("LinkID") & """><i class='icon sorting'></i><span style=""cursor:default"">" & RSObj("SiteName") & "</span></span></td>")
				 .echo ("<td  class='splittd' align=""center"">" & RSObj("FolderName") & "</td>")
			  If RSObj("LinkType") = 0 Then
			   .echo ("<td class='splittd' align=""center"">文字链接</td>")
			  Else
