...
Class Announce_Main
		...
		Sub MainList()
			...
			function AnnounceAdd()
			{
				...
				window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=网站公告 >> <font color=red>添加新公告</font>&ButtonSymbol=GO';
			}
			...
			{ 
				...
				window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=网站公告 >> <font color=red>编辑公告</font>&ButtonSymbol=GoSave';
				}else{
				alert('一次只能编辑一条公告!');
				}
			}
			...
			.echo "<ul id='menu_top'>"
			.echo "<li class='parent' onclick=""AnnounceAdd();""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加公告</span></li>"
			.echo "<li class='parent' onclick=""EditAnnounce('');""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>编辑公告</span></li>"
			.echo "<li class='parent' onclick=""DelAnnounce('');""><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除公告</span></li>"
			.echo "</ul>"
			.echo "<div class=""pageCont2""><table width=""100%""  border=""0"" cellpadding=""0"" cellspacing=""0"">"
			...
			.echo "  </tr>"
			 ...
			 Set RSObj = Server.CreateObject("ADODB.RecordSet")
					 ...
					 If RSObj.EOF And RSObj.BOF Then
					   .echo "<tr><td class=""splittd"" colspan=""10"" style=""text-align:center"">还没有添加公告！</td></tr>"
					 Else
			...
			.echo "  </tr>"
             .echo " <tr>"
			 .echo " <td colspan='2' class='pt10'><div style='margin:5px'><b>选择：</b><a href='javascript:void(0)' onclick='Select(0)'>全选</a> -  <a href='javascript:void(0)' onclick='Select(1)'>反选</a> - <a href='javascript:void(0)' onclick='Select(2)'>不选</a> <input type='submit' class='button' value='删 除' onclick=""return(confirm('确定删除选中的公告吗?'))""></td></form>"
			  ...
			  With KS
			   If IsObject(AnnounceXML) Then
			    For Each Node In AnnounceXML.DocumentElement.SelectNodes("row")
					  ...
					  End If
					   .echo "  <Td class='splittd' align='center'><a href=""../../../plus/announce/index.asp?" & ID &""" target=""_blank"">预览</a> |  <a href=""javascript:EditAnnounce('');"">修改</a> |  <a href=""javascript:DelAnnounce(" & ID &")"">删除</a> </td>"
					  .echo "</tr>"
					Next
				End If
			 End With
		  ...
		  Sub AnnounceAddOrEdit()
		        If Not KS.ReturnPowerResult(0, "KSMS200021") Then
				  Call KS.ReturnErr(1, "")
				  exit sub
				End If
				...
				.echo "<body>"
				.echo "<div class=""pageCont2 mt20""><table width='100%' cellspacing='0' cellpadding='0' border='0'>"
				.echo " <tr><td class='topdashed sort'>"
				If Flag = "Edit" Then
				 .echo "修改公告"
				Else
				 .echo "添加公告"
				...
				.echo "            <td>"
				.echo "              <input name=""AddDate"" onclick=""WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"" type=""text"" id=""AddDate"" value="""
				 If Flag <> "Edit" Then
				 .echo (Now)
				 Else
				 .echo (AddDate)
				 End If
				.echo """ Readonly class=""textbox Wdate"" style=""width:200px"">"
				.echo "              </td>"
				.echo "          </tr>"
				.echo "          <tr>"
				...
				.echo "      <td valign=""top"">"
				.echo  EchoEditor("Content",Content,"NewsTool","96%","320px")
		  ...
		  Sub AnnounceSave()
			...
			If Flag = "Add" Then
			  ...
			  End If
			   ...
			   KS.Echo ("<script> if (confirm('公告添加成功!继续添加吗?')) {location.href='KS.Announce.asp?Action=Add';}else{location.href='KS.Announce.asp';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr=常规管理 >> <font color=red>网站公告</font>';}</script>")
			ElseIf Flag = "Edit" Then
			  ...
			  End If
			  KS.Echo "<script src='../../../ks_inc/jquery.js'></script>"
			  KS.Echo ("<script>alert('公告修改成功!');location.href='KS.Announce.asp?Page=" & Page & "';$(parent.document).find('#BottomFrame')[0].src='Post.Asp?ButtonSymbol=Disabled&OpStr=常规管理 >> <font color=red>网站公告</font>';</script>")
			End If
		  End Sub
