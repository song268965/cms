...
Class Admin_Field
		...
		function FieldAdd()
		{
		   ...
		   window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=自定义表单 >> 表单项管理 >> <font color=red>新增'+ItemName+'表单项</font>&ButtonSymbol=Go';
		}
		function EditField(id)
		{
		  location="KS.FormField.asp?ItemID="+ItemID+"&Page="+Page+"&Action=Edit&ID="+id;
		  window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=自定义表单 >> 表单项管理 >> <font color=red>编辑'+ItemName+'表单项</font>&ButtonSymbol=GoSave';
		}
		...
		.Write "<li class='parent' onclick='FieldAdd();'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>新增表单字段</span></li>"
		.Write "<li class='parent' onclick='FieldControl(1);'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon write'></i>修改表单字段</span></li>"
		.Write "<li class='parent' onclick='FieldControl(2)'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon delete'></i>删除表单字段</span></li>"
		.Write "<li class='parent' onclick='location.href=""KS.Form.asp"";'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon back'></i>回上一级</span></li>"
		.Write "</ul>"
		
        .Write "<div class='pageCont2'>"
		.Write "<form action='KS.FormField.asp?action=order&ItemID=" & ItemID & "&page=" & Page & "' name='myform' method='post'>"
		.Write "<table width='100%' border='0' cellpadding='0' cellspacing='0'>"
		.Write "        <tr class='sort'>"
		.Write "         <td align='center'>选择</td>"
		...
		.Write "        </tr>"
			...
			End If
		 .Write " <tr>"
		 .Write "   <td colspan='15' height='30' class='operatingBox'>&nbsp;&nbsp;<strong>选择：</strong><a href='javascript:Select(0)'><font color=#999999>全选</font></a> - <a href='javascript:Select(1)'><font color=#999999>反选</font></a> - <a href='javascript:Select(2)'><font color=#999999>不选</font></a> &nbsp;&nbsp;<input type='button' value='删除表单项' class='button' onclick=""FieldControl(2)""/> <input type='button' value='表单预览' onclick=""window.$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=自定义表单 >> <font color=red>表单预览</font>&ButtonSymbol=Disabled';location.href='KS.Form.asp?ItemID=" & ItemID & "&action=view';"" class='button'>&nbsp;<input type='submit' class='button' value='批量保存设置'> <font color=blue>越小排在越前面</font></td></form>"
		 .Write "  </tr><tr> <td height='35' colspan='16' align='right'>"
		...
		With Response
		Do While Not RS.EOF
		.Write "  <tr height=""23"" class='list' id='u" & RS("FieldID") &"' onclick=""chk_iddiv('" & RS("FieldID") &"')"" onMouseOver=""this.className='listmouseover'"" onMouseOut=""this.className='list'"">"
		 .Write "<td class='splittd' style='text-align:center'><input name=""id"" type=""checkbox""  onclick=""chk_iddiv('" & RS("FieldID") &"')"" id='c" & RS("FieldID") &"'  value='" & RS("FieldID") &"'></td>"
		 .Write "<td class='splittd' style='text-align:center'><input  class='textbox' type='text' name='OrderID' style='width:45px;text-align:center' value='" & RS("OrderID") &"'><input type='hidden' name='FieldID' value='" & RS("FieldID") & "'></td>"
		.Write "    <td class='splittd'>"
		.Write "    <span FieldID='" & RS("FieldID") & "' onDblClick=""EditField(" & RS("FieldID") & ")"">"
...
.Write "    <tr id=""ldArea"" style='display:none' class='tdbg'>"
		...
		.Write "      <td align='right' width='180' class='clefttitle'><strong>列表选项：</strong><br><font color=blue>每一行为一个列表选项</font>如果值和显示项不同可以用<font color=red>|</font>隔开<br/>正确格式如：男 或 0<font color=red>|</font>男</td>"
		...
		End Sub
		 ...
		 Else
			Conn.CommitTrans
		 End IF
		 Response.Write ("<Script> if (confirm('表单项增加成功,继续添加吗?')) { location.href='KS.FormField.asp?ItemID=" & ItemID& "&Action=Add';} else{location.href='KS.FormField.asp?ItemID=" & ItemID&"&Page='"&Page &";$(parent.document).find('#BottomFrame')[0].src='Post.Asp?OpStr=" & server.URLEncode("自定义表单 >> <font color=#ff0000>表单字段管理</font>") &"&ButtonSymbol=Disabled';}</script>")
		 Else
		 ...
		 .Write ("<input type=""hidden"" name=""OpStr"" value=""自定义表单 >> <font color=red>表单字段管理</font>"">")
		 ...
		 Call KS.Alert("表单项修改成功!", "plus/Plus_Form/KS.FormField.asp?ItemID=" & ItemID&"&Page=" & Page)
		 End With
		 End Sub
