...
<%
	...
	Response.Write "<li onclick=""parent.frames['BottomFrame'].location.href='../Post.Asp?OpStr='+escape('问答系统 >> <font color=red>添加问答分类</font>')+'&ButtonSymbol=Go';location.href='?action=add';"" class='parent'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon add3'></i>添加分类</span></li>"
	Response.Write "<li onclick='location.href=""?action=orders""' class='parent' onclick='MoveClassInfo();'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon sorting'></i>一级分类排序</span></li>"
	Response.Write "<li onclick='location.href=""?action=total""' class='parent' onclick='MoveClassInfo();'><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon sorting'></i>更新分类统计</span></li>"
	Response.Write "<li class='parent' onclick=""location.href='?';"""
	if KS.G("Action")="" Then Response.Write " disabled"
	Response.Write"><span class=child onmouseover=""this.parentNode.className='parent_border'"" onMouseOut=""this.parentNode.className='parent'""><i class='icon back'></i>回上一级</span></li>"
	Response.Write "</ul>"
...
Sub showmain()
	Dim Rs,SQL,i
	Dim tdstyle
	Response.Write "<div class='pageCont2'><table border='0' cellpadding='0' cellspacing='0'  width='100%' align='center'>"
	...
	Do While Not Rs.EOF
		...
		If Rs("depth") > 1 Then
			...
			Response.Write "&nbsp;&nbsp;<font color=""#666666"">├</font> "
		End If
		If Rs("parentid") = 0 Then Response.Write ("<i class='icon manage'></i><b>")
		Response.Write Rs("ClassName")
		If Rs("child") > 0 Then Response.Write "(" & Rs("child") & ")"
		Response.Write " </td>" & vbNewLine
		Response.Write " <td class='splittd' align=""center"">"
		Response.Write "<a onclick=""parent.frames['BottomFrame'].location.href='../Post.Asp?OpStr='+escape('问答系统 >> <font color=red>添加问答分类</font>')+'&ButtonSymbol=Go';"" href=""?action=add&editid="
		Response.Write Rs("classid")
		Response.Write """ class='setA'>添加分类</a>"
		Response.Write "|<a onclick=""parent.frames['BottomFrame'].location.href='../Post.Asp?OpStr='+escape('问答系统 >> <font color=red>问答分类设置</font>')+'&ButtonSymbol=GoSave';"" href=""?action=edit&editid="
		Response.Write Rs("classid")
		Response.Write """ class='setA'>分类设置</a>"
		Response.Write "|"
		Response.Write " "
		If Rs("child") < 1 Then
			Response.Write " <a href=""?action=del&ChannelID="&ChannelID&"&editid="
			Response.Write Rs("classid")
			Response.Write """ onclick=""{if(confirm('删除将包括该分类的所有信息，确定删除吗?')){return true;}return false;}"" class='setA'>删除分类</a>"
		Else
			Response.Write " <a href=""#"" onclick=""{if(confirm('该分类含有下属分类，必须先删除其下属分类方能删除本分类！')){return true;}return false;}"" class='setA'>"
...
<div class="pageCont2">
<div class="tabTitle mt20">添加问答分类</div>
<dl class="dtable">
...
%>
	...
	<dd>
        ...
        <span class="block">可留空</span>
	</dd>
	<dd>
		<div>SEO关键字：</div>
		<textarea name="SeoKeyWord" cols="50" rows="5"></textarea>
		<br/><span class="tips">问答列表页模板请用"{@class.seokeyword}" 进行调用</span>
		</td>
	</dd>
	<dd>
		<div>SEO描述：</div>
		<textarea name="SeoDescript" cols="50" rows="5"></textarea>
		<br/>
		<span class="tips">问答列表页模板请用"{@class.seodescription}" 进行调用</span>
		</td>
	</dd>	</form>
</dl>
</div>
...
<div class="pageCont2">
<div class="tabTitle mt20">编辑问答分类</div>
<dl class="dtable">
...
<%
	...
	dim arr:arr=split(rs("c_setting")&"№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№","№§№")
%>
	</dd>
	<dd>
		<div>分类名称：</div>
		<input type="text" class="textbox" name="ClassName" id="ClassName" size="35" value="<% = Rs("ClassName")%>">
	</dd>
	<dd>
		<div>分类简要说明：</div>
	...
	<dd>
		<div>SEO关键字：</div>
		<textarea name="SeoKeyWord" cols="50" rows="5"><%=Server.HTMLEncode(arr(0)&"")%></textarea>
		<br/><span class="tips">问答列表页模板请用"{@class.seokeyword}" 进行调用</span>
		</td>
	</dd>
	<dd>
		<div>SEO描述：</div>
		<textarea name="SeoDescript" cols="50" rows="5"><%=Server.HTMLEncode(arr(1)&"")%></textarea>
		<br/>
		<span class="tips">问答列表页模板请用"{@class.seodescription}" 进行调用</span>
		</td>
	</dd>
	<dd>
		<div>分类统计：</div>
		未解决：<input type="text" name="AskPendNum" class="textbox" size="10" value="<%=Rs("AskPendNum")%>">
		已解决：<input type="text" name="AskDoneNum" class="textbox" size="10" value="<%=Rs("AskDoneNum")%>">
...
Sub savenew()
	...
	For i = 0 To UBound(m_arrClassname)
		...
		If strClassname <> "" Then
			...
			Rs("Askmaster") = ""
			Rs("c_setting") = request("SeoKeyWord")&"№§№"&request("SeoDescript")&"№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№№§№"
			Rs("AskPendNum") = 0
			Rs("AskDoneNum") = 0
			Rs("AskVoteNum") = 0
			Rs("AskshareNum") = 0
	...
	Call KS.Confirm("恭喜您！添加新的分类成功,继续添加吗?","Ask/KS.AskClass.asp?action=add","Ask/KS.AskClass.asp")
End Sub
...
Sub savedit()
	...
	CheckAndFixClass 0,1
	Call KS.Alert("恭喜您！分类修改成功!","Ask/KS.AskClass.asp")
End Sub
...
Sub delCategory()
	...
	CheckAndFixClass 0,1
	UpdateClassTotal
	Call KS.Alert("恭喜您！分类删除成功。","Ask/KS.AskClass.asp")
End Sub
