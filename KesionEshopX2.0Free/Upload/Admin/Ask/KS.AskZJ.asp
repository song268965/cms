...
Class Admin_AskZJ
	   ...
	   End Sub
		...
		%>
		<div class="pageCont2">
		<div style="height:25px;line-height:25px" class="pt10 pd10">
		...
		<tr align="center" onMouseOut="this.className='list'" onMouseOver="this.className='listmouseover'" id='u<%=rs("id")%>'>
			...
			<td class="splittd"><%if rs("istop")="1" then
			 response.write "<a href=""?action=istop&id=" & rs("id") & "&v=0""><font color=blue>√</font></a>"
			 else
			  response.write "<a href=""?action=istop&id=" & rs("id") & "&v=1""><font color=red>X</font></a>"
			 end if%>
			</td>
			...
			%></td>
			<td class="splittd" noWrap="noWrap"><a href="?action=modify&ID=<%=rs("id")%>" class="setA">查看编辑</a>|<a href="?action=del&ID=<%=rs("id")%>" onClick="return confirm('删除后将不能恢复，您确定要删除吗?')" class="setA">删除</a></td>
		</tr>
		<%   i=i+1
		...
		<tr>
			<td  align="right" colspan="13" id="NextPageText">
			<%
		...
		Sub IsTop()
		  ...
		  If Id="" Then
		   KS.AlertHintScript "对不起，您没有选择要设置的记录!"
		  End If
		  Conn.Execute("Update KS_AskZJ Set istop=" & KS.ChkClng(KS.G("V")) & " Where ID in(" & ID & ")")
		  KS.AlertHintScript "恭喜，设置成功!"
		End Sub
		Sub unverifyZJ()
		  Dim ID:ID=KS.FilterIds(KS.S("ID"))
					  ...
					  <form action="?Action=modifysave" method="post" name="myform" id="myform">
					    <input type="hidden" value="<%=id%>" name="id"/>
						 ...
						 <tr class="tdbg">
							...
							Response.write "try{setCookie(""pid"",'" & rs("Province") & "');setCookie(""cid"",'" &  rs("City") & "');}catch(e){}" & vbcrlf
		...
		Sub ModifySave()
			 ...
			 Dim County:County=KS.DelSql(KS.G("County"))
