...
Class ClubCls
        Private KS, KSR,ListStr,Node,BSetting,KSUser,GuestTitle,Master,MasterArr,F_C,TopicID
		...
		Public Sub Kesion()
			...
			GetClubPopLogin F_C
			F_C=KSR.ReplaceGeneralLabelContent(F_C)
			F_C=Replace(Replace(F_C,"｛#","{"),"#｝","}")  '标签替换回来
			F_C=RexHtml_IF(F_C)
			F_C=Replace(F_C,"{#ExecutTime}","页面执行" & FormatNumber((timer()-startime),5,-1,0,-1) & "秒 powered by <a href='http://www.kesion.com' target='_blank'>KesionCMS 9.0</a>")
		...
		'加载首页模板
		Sub LoadTemplate()
				Application(KS.SiteSn &"Club3GIndexUpdateTime")=Now
				F_C = KSR.LoadTemplate(KS.Setting(3) & KS.Setting(90) & TemplatePath & "/club/index.html")
				InitialCommon
				F_C=KSR.ReplaceAllLabel(F_C)
				F_C=KSR.ReplaceLableFlag(F_C)
				Application(KS.SiteSN&"Club3GIndex")=F_C
		End Sub
		'主页
		Sub LoadClubIndex()
			FCls.RefreshFolderID = 0
			If CachePage=false Or KS.ChkCLng(CacheTime)=0 Or KS.IsNUL(Application(KS.SiteSN&"Club3GIndex")) Or Not isDate(Application(KS.SiteSn &"Club3GIndexUpdateTime")) Then
			   LoadTemplate
			ElseIf  isDate(Application(KS.SiteSn &"Club3GIndexUpdateTime") And  DateDiff("n",Application(KS.SiteSn &"Club3GIndexUpdateTime"),Now)>=KS.ChkCLng(CacheTime)) Then
			    LoadTemplate()
			Else
			    F_C = Application(KS.SiteSN&"Club3GIndex")
			End If
			...
			F_C=Replace(F_C,"{$TodayNum}",doc.documentElement.attributes.getNamedItem("todaynum").text)
			F_C=Replace(F_C,"{$YesterDayNum}",doc.documentElement.attributes.getNamedItem("yesterdaynum").text)
			F_C=Replace(F_C,"{$MaxDayNum}",doc.documentElement.attributes.getNamedItem("maxdaynum").text)
			F_C=Replace(F_C,"{$TopicNum}",doc.documentElement.attributes.getNamedItem("topicnum").text)
			F_C=Replace(F_C,"{$ReplayNum}",doc.documentElement.attributes.getNamedItem("postnum").text)
			F_C=Replace(F_C,"{$UserNum}",doc.documentElement.attributes.getNamedItem("totalusernum").text)
			F_C=Replace(F_C,"{$NewUser}",doc.documentElement.attributes.getNamedItem("newreguser").text)
			F_C=Replace(F_C,"{$MaxOnline}",doc.documentElement.attributes.getNamedItem("maxonline").text)
			F_C=Replace(F_C,"{$MaxOnlineDate}",doc.documentElement.attributes.getNamedItem("maxonlinedate").text)
			PostBtnStr="<a href=""javascript:Posted()""><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/button_post.png"" align=""absmiddle"" alt=""发帖""></a>"
			F_C=Replace(F_C,"{$PostButtonAction}",PostBtnStr)
			F_C=Replace(F_C,"{$GuestTitle}",KS.Setting(61))
			F_C=Replace(F_C,"{$GetGuestList}",ListTemplate)
		End Sub
	    '版面
		Sub LoadClubBoardList()
		   If Not KS.IsNul(KS.Setting(69)) and Request.QueryString<>"" Then
			...
			If Node Is Nothing Then LoadClubIndex() : Exit Sub  '没有找到版本参数时，转到首页
		     ...
		     F_C = KSR.LoadTemplate(KS.Setting(3) & KS.Setting(90) & TemplatePath & "/club/list.html")
			InitialCommon
		   
			
			BSetting=Node.SelectSingleNode("@settings").text
			ParentId=KS.ChkClng(Node.SelectSingleNode("@parentid").text)
			F_C=Replace(F_C,"{$BoardName}",Node.SelectSingleNode("@boardname").text)
			F_C=Replace(F_C,"{$GetBoardUrl}","bbs.asp?boardid=" & boardid)
			...
			End IF
				...
				F_C=RexHtml_IF(F_C) '列表页先过滤其它标签,减少标签解释
				F_C=Replace(F_C,"{$GuestTitle}",GuestTitle)
				F_C=Replace(F_C,"{$GetGuestList}",ListTemplate)
				F_C=Replace(Replace(Replace(Replace(Replace(F_C,"{$ShowManageCheckBox}",""),"{$Img}",""),"{$PageList}",""),"{$Jing}",""),"{$Status}","") '替换掉无用标签,加快解释
				F_C=KSR.ReplaceAllLabel(F_C)
				F_C=KSR.ReplaceLableFlag(F_C)
                F_C=Replace(F_C,"{$BoardID}",boardid)
		End Sub	
		...
		Sub ShowBoardList()
		     ...
		     if boardid<>0  Then
				...
				PostBtnStr="<span style=""position:relative;"" onmouseover=""$('#postlist').show()"" onmouseout=""$('#postlist').hide()""><a href=""post.asp?bid=" & boardid & """><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/button_post.png""></a><div id=""postlist"" class=""submenu noli"">"
				PostBtnStr=PostBtnStr&"<dl><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/new_post.gif"" align=""absmiddle""/> <a href=""post.asp?bid=" & boardid & """>发帖</a></dl>"
				If KS.ChkClng(bsetting(64))>0 Then
				PostBtnStr=PostBtnStr &"<dl><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/vote.gif"" align=""absmiddle""> <a href=""post.asp?bid=" & BoardID&"&posttype=1"">投票</a></dl>"
				End If
				...
				F_C=Replace(F_C,"{$PostButtonAction}",PostBtnStr)						   
				F_C=Replace(F_C,"{$GuestTitle}",GuestTitle)
				F_C=RexHtml_IF(F_C) '先过滤无用的标签,减少标签解释
		...
		Function Parse(sTemplate, iPosBegin)
			...
			Select Case lcase(sValue)
					...
					Else
					 ...
					 If Not KS.IsNul(MyMaster) Then
						 ...
						 For I=0 To Ubound(MasterArr)
						   If I=0 Then echo "" & Split(MasterArr(i),"|")(1) & "" Else echo "," & "" & Split(MasterArr(i),"|")(1) & ""
						 Next
					 End If
					End If
			   ...
			   case "executtime" echo "页面执行" & FormatNumber((timer()-startime),5,-1,0,-1) & "秒 powered by <a href='http://www.kesion.com' target='_blank'>KesionCMS 9.0</a>"
			   case "showpage"
				   echo ShowPage(TotalPut,MaxPerPage,"",CurrentPage,false,false)
				Case Else
					    ...
					    case "item"
						  select case lcase(sTokenName)
							...
							case "subjectlist"
							  ...
							  echo " href=""display.asp?id=" & topicid & """ title=""" & KS.LoseHtml(replace(replace(TopicNode.SelectSingleNode("@subject").text,"｛#","{"),"#｝","}")) & """>" & replace(replace(TopicNode.SelectSingleNode("@subject").text,"｛#","{"),"#｝","}") & "</a>"
							  ...
							  If TotalReplay<>0 Then
							   ...
							   If Pages>1 Then
										...
										if pages>5 then
										   echo " <a href='display.asp?id=" & Topicid &"'>1</a><a href='display.asp?id=" & Topicid &"&page=2'>2</a>... <a href='display.asp?id=" & Topicid &"&page=" & pages-3 & "'>" & pages-3 &"</a> <a href='display.asp?id=" & Topicid &"&page=" & pages-2 & "'>" & pages-2 &"</a> <a href='display.asp?id=" & Topicid &"&page=" & pages-1 & "'>" & pages-1 &"</a> <a href='display.asp?id=" & Topicid &"&page=" & pages & "'>" & pages &"</a>"
										Else
										   For k=1 to Pages
											 echo " <a href='display.asp?id=" & Topicid &"&page=" & k & "'>"&k&"</a>"
										   Next
										End If
								   echo "</span>"
								End if
		...
		Sub GetBoardList()
		  ...
		  If IsObject(xml) Then
		       ...
		       LoopTemplate=LFCls.GetConfigFromXML("GuestBook","/guestbook/template","board3g")
			   For Each Pnode In Xml.DocumentElement.SelectNodes("row[@" & BParam & "]")
					 LC=PLoopTemplate
					 GuestTitle=PNode.SelectSingleNode("@boardname").text
					 LC=Replace(LC,"{$BoardUrl}","bbs.asp?boardid=" & PNode.SelectSingleNode("@id").text)
				   ...
				   For Each Node In Xml.DocumentElement.SelectNodes("row[@parentid=" & Pnode.SelectSingleNode("@id").text & "]")
				     str=LoopTemplate
					 str=Replace(str,"{$BoardUrl}","bbs.asp?boardid=" & Node.SelectSingleNode("@id").text)
		...
		Sub GetLoopList()
		    ...
		    If KSUser.GetUserInfo("ClubSpecialPower")="1" or KSUser.GetUserInfo("ClubSpecialPower")="2" Or (KSUser.GetUserInfo("ClubSpecialPower")="3" and KS.FoundInArr(Master,KSUser.UserName,",")=true) Then 
			 Param=" Where deltf=0"	
			 if KS.S("A")="m" then
			   F_C=Replace(F_C,"{$ShowManageButton}","<a href=""" & KS.GetClubListUrl(boardid) & """><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/button_manage.png""></a>")
			 else
			   F_C=Replace(F_C,"{$ShowManageButton}","<a href=""" & ListUrl & "?page=" & currentpage &"&a=m&boardid=" & BoardID & """><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/button_manage.png""></a>")
			 end if
			Else  
		    ...
		    F_C=Replace(F_C,"{$ListType}",ListType)
			...
			If BSetting(23)="1" And BSetting(26)="1" Then
			  ...
			  End If
		      F_C=Replace(F_C,"{$BoardCategory}",CategoryStr)
...
Class ClubDisplayCls
		...
		Private ListTemplate,LoopTemplate,LoopList,F_C,RST,master,PostType,CheckIsMaster
		...
		Function Parse(sTemplate, iPosBegin)
			...
			Select Case lcase(sValue)
				...
				case "boardid" echo boardid
				case "boardurl" Echo "bbs.asp?boardid=" & BoardID
						  ...
						  select case lcase(sTokenName)
							 ...
							 case "topicmanagemenu"
							   If CheckIsMaster Then
							     ...
							     End If
							     If N=1 Then
							      Echo "<a href='post.asp?action=edit&bid=" & boardid&"&id=" & ReplayID & "&topicid=" & TopicID & "&istopic=1'>编辑主题</a> | <a href=""javascript:void(0)"" onclick=""delsubject("&TopicID &",'" & KS.Setting(66) & "'," &boardid&")"">删除主题</a>"
							     Else
							      echo "<a href='post.asp?action=edit&bid=" & boardid&"&id=" & ReplayID & "&topicid=" & TopicID & "&istopic=0&page=" & CurrentPage & "'>编辑</a> | <a onclick=""delreply('" & KS.Setting(66) &"'," & topicid & "," & replayid & "," & boardid & "," & N & ")"" href='javascript:void(0);'>删除</a>"
							     End If
							  
							  ElseIf KS.ChkClng(BSetting(29))=1 And KSUser.UserName= PostUserName Then
								 If N=1 Then
								  echo "<img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/edit.gif"" align=""absmiddle""/><a href='post.asp?action=edit&bid=" & boardid&"&id=" & ReplayID & "&topicid=" & TopicID & "&istopic=1'>编辑主题</a>"
								  Else
								  echo "<img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/edit.gif"" align=""absmiddle""/><a href='post.asp?action=edit&bid=" & boardid&"&id=" & replayID & "&topicid=" & TopicID & "&istopic=0&page=" & CurrentPage & "'>编辑</a> "
								  End If
							  End If
							   ...
							   If Not Un Is Nothing Then UserIsLock=KS.ChkClng(UN.SelectSingleNode("@lockonclub").text) Else UserIsLock=0
								...
								If UserIsLock=1 Then
									if CheckIsMaster=true or KSUser.GetUserInfo("ClubSpecialPower")="1" or KSUser.GetUserInfo("ClubSpecialPower")="2" then
									 MyContent=GetModelContent(Mstr): Content="<div class=""nopurview"">该用户所发的帖已全被锁定,由于您是版主/管理员所以可以看到此信息.</div>" & Mstr & MyContent
									else
									 Content="<div class=""nopurview"">对不起，该用户所发的帖已全被锁定!</div>"
									end if
								ElseIf TopicNode.SelectSingleNode("@verific").text="2" then
									if CheckIsMaster=true or KSUser.GetUserInfo("ClubSpecialPower")="1" or KSUser.GetUserInfo("ClubSpecialPower")="2" then
									 MyContent=GetModelContent(Mstr): Content="<div class=""nopurview"">该信息已屏蔽,由于您是版主/管理员所以可以看到此信息.</div>" & Mstr & MyContent
									else
									 Content="<div class=""nopurview"">对不起，该信息已屏蔽!</div>"
									end if
								ElseIf TopicNode.SelectSingleNode("@verific").text="0" then
									if CheckIsMaster=true or KSUser.GetUserInfo("ClubSpecialPower")="1" or KSUser.GetUserInfo("ClubSpecialPower")="2" then
									 MyContent=GetModelContent(Mstr): Content="<div class=""nopurview"">该信息未审核,由于您是版主/管理员所以可以看到此信息.</div>" &  Mstr & MyContent
									else
									 Content="<div class=""nopurview"">对不起，该信息未审核!</div>"
									end if
								ElseIf N=1 Then  '主题
									  Content=GetModelContent(Mstr)
												  ...
												  Else
													  ...
													  ElseIf KS.S("ShowByScore")="true" Then
														...
														Select Case KS.ChkClng(BSetting(56))
														 ...
														 Call KS.ScoreInOrOut(KSUser.UserName,1,KS.ChkClng(ShowScore),"系统","在论坛查看主题[" & Subject & "]消费!",9994,TopicID)
														End Select
		...
		Function GetModelContent(ByRef MStr)
		 If KS.ChkClng(ChannelID)<>0 And KS.ChkClng(InfoID)<>0 And N=1 Then  '绑定模型
				 ...
				 If Not MRS.Eof Then
					...
					Next
						 MStr=MStr & "</table>"
						 GetModelContent=MRS("articlecontent")
					End If
					Set MRS=Nothing
				End If
			 Else
				 GetModelContent=TopicNode.SelectSingleNode("@content").text
			 End If
		End Function
		...
		Public Sub Kesion()
			...
			End If
			If CurrentPage<=0 Then CurrentPage=1
				F_C = KSR.LoadTemplate(KS.Setting(3) & KS.Setting(90) & TemplatePath & "/club/show.html")
				InitialCommon
				   ...
				   End If
					...
					F_C=Replace(F_C,"{$GetSiteUrl}",KS.GetDomain)
					F_C=Replace(F_C,"{$GetClubInstallDir}",KS.Setting(66))
		...
		Sub ShowTopic()
		   F_C=RexHtml_IF(F_C) '先过滤无用的标签,减少标签解释
			 Dim PostBtnStr:PostBtnStr="<span style=""position:relative;z-index:1000"" onmouseover=""$('#postlist').show()"" onmouseout=""$('#postlist').hide()""><a href=""post.asp?bid=" & boardid & """><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/button_post.png""></a><div id=""postlist"" class=""submenu noli"">"
			 PostBtnStr=PostBtnStr&"<dl><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/new_post.gif"" align=""absmiddle""/> <a href=""post.asp?bid=" & boardid & """>发帖</a></dl>"
			 If KS.ChkClng(bsetting(64))>0 Then
			 PostBtnStr=PostBtnStr &"<dl><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/vote.gif"" align=""absmiddle""> <a href=""post.asp?bid=" & BoardID&"&posttype=1"">投票</a></dl>"
			 End If
			 PostBtnStr=PostBtnStr &"</div></span>"
					   ...
					   F_C=Replace(F_C,"{$PostButtonAction}",PostBtnStr)
					   F_C=Replace(F_C,"{$BoardID}",BoardID)
					   F_C=Replace(F_C,"{$TopicID}",TopicID)
					   F_C=Replace(F_C,"{$PostTable}",PostTable)
					   F_C=Replace(F_C,"{$IsTop}",IsTop)
					   F_C=Replace(F_C,"{$Page}",currentpage)
					   AllowShow=true
					   F_C=Replace(F_C,"{$GuestTitle}","{@topic.subjectnohtml}")
					   F_C=KSR.KSLabelReplaceAll(F_C)
		...
		Sub GetReplayList()	
		 ...
		 If DataBaseType=1 Then
			    Dim Cmd : Set Cmd = Server.CreateObject("ADODB.Command")
				Set Cmd.ActiveConnection=conn
				Cmd.CommandText="KS_ClubDisplay"
				Cmd.CommandType=4	
				CMD.Prepared = true 	
	...
	Sub GetTopicList(Xml)
			 ...
			 If DataBaseType=1 Then
			    Dim Cmd : Set Cmd = Server.CreateObject("ADODB.Command")
				Set Cmd.ActiveConnection=conn
				Cmd.CommandText="KS_ClubUserList"
				Cmd.CommandType=4	
				CMD.Prepared = true 	
