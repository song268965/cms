...
Class SiteIndex
		...
		Public Sub Kesion()
			...
			UserName=KS.S("UserName")
				   ...
				   Dim TpDir:TpDir=KS.SSetting(60)
				   If KS.IsNUL(TpDir) Then TpDir=KS.Setting(3) & KS.Setting(90) & "空间模板/企业通用/company_show.html"
		...
		Sub GetCompany()
		 ...
		 rs.open "select top 1 a.*,b.logo from ks_enterprise a inner join KS_Blog B ON A.UserName=B.UserName where a.username='" & KS.S("UserName") & "'",conn,1,1
		 ...
		 Else
			...
			template=replace(template,"{$ShowLogo}",KS.HtmlCode(rs("logo")))
			...
			if instr(template,"{$ShowMarkerList}")<>0 then
			   dim i,marstr,MarkerArr,MapMarker:MapMarker=rs("mapmarker")
			   if Not KS.IsNul(MapMarker) Then
			     MarkerArr=Split(MapMarker,"|")
				   For i=0 to Ubound(MarkerArr)
				     marstr=marstr&"point = new BMap.Point(" & MarkerArr(i) & "); " & vbcrlf
				     marstr=marstr&"addMarker(point, " & i & ");" &vbcrlf
				   Next
			   end if
			   template=replace(template,"{$ShowMarkerList}",marstr)
			end if
		...
		Sub GetSupply()
		 ...
		 url=KS.GetItemUrl(8,RS("TID"),RS("id"),RS("Fname"),RS("AddDate"))
         n=n+1
		 if n mod 2=0 then
