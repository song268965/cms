...
{ 
	box=$.dialog({title:"<img src='../images/user/mail.gif' align='absmiddle'>发送消息",content:"对方登录后可以看到您的消息(可输入255个字符)<br /><textarea name='message' id='s_message' style='width:340px;height:80px'></textarea><div style='text-align:center;margin:10px'><input type='button' onclick='return(ksblog.checkmsg())' value=' 确 定 ' class='button'><input type='hidden' value="+username+" name='username' id='s_username'> <input type='button' value=' 取 消 ' onclick='box.close()' class='button'></div>"});
    ksblog.checkIsLogin();
}
...
ksblog.ShowLogin=function(){ 
  box=$.dialog({title:'会员登录',content:'url:../user/userlogin.asp?Action=Poplogin',width:397,height:184});
}

ksblog.addF=function(ev,username){ 
	box=$.dialog({title:"<img src='../images/user/log/106.gif'>添加好友",content:"通过对方验证才能成为好友(可输入255个字符)<br /><textarea name='message' id='f_message' style='width:340px;height:80px'></textarea><div style='text-align:center;margin:10px'><input type='button' onclick='return(ksblog.checkAddF())' value=' 确 定 ' class='button'><input type='hidden' value="+username+" name='username' id='f_username'> <input type='button' value=' 取 消 ' onclick='box.close()' class='button'></div>"});
...
function ShowLogin(){box=$.dialog({title:'会员登录',content:'url:../user/userlogin.asp?Action=Poplogin',width:397,height:184});}