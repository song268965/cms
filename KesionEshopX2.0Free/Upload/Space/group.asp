...
Class Group
        Private KS,KSBCls,KSUser,KSR,ViewTf
		  ...
		  case "showtopic"
				  ...
				  If ViewTf<>"0" Then
				   IF Cbool(KSUser.UserLoginChecked)=false Then 
					 KS.Die "<script>alert('对不起，浏览圈子帖子，请先登录！');location.href='../user/login/';</script>"
				   End If
				   If ViewTf="1" Then
					 If Conn.Execute("Select top 1 * From KS_TeamUsers Where UserName='" & KSUser.UserName & "' and TeamID=" & ID & " and (status=1 or status=3)").eof then
					  KS.Die "<script>alert('对不起，只有本圈成员才有权限浏览！');history.back();</script>"
					 end if
				   End If
			  End If
		...
		showtopic=EchoUeditorHead()
		showtopic=showtopic &"<div id=""form_comment""><a name=""add_comment""></a>"
		showtopic=showtopic &"<script type=""text/javascript"">function checkform(){if (editor.hasContents()==false){alert('请输入回复内容!');editor.focus();return false;}return true;}</script>"
		...
		showtopic=showtopic &"		<td bgcolor=""#FFFFFF""><input type=""text"" readonly value=""Re:" & conn.execute("select top 1 title from ks_teamtopic where id="& tid )(0) & """ size=""50"" name=""title"">"
        showtopic=showtopic &"        </td>"
		showtopic=showtopic &"	</tr>"
		showtopic=showtopic &"	<tr>"
		showtopic=showtopic &"		<td colspan='2' bgcolor=""#FFFFFF"">"
		
		showtopic=showtopic & "<script id=""Content"" name=""Content"" type=""text/plain"" style=""width:90%;height:250px;""></script>"
	    showtopic=showtopic & "<script>setTimeout(""editor = " & GetEditorTag() &".getEditor('Content',{toolbars:[" & Replace(GetEditorToolBar("Basic"),"'source',","") &"],wordCount:false,autoHeightEnabled:false,minFrameHeight:250,elementPathEnabled:false });"",10);</script>"
		...
		dim content:content=Request.form("content"):if content="" then call KS.alert("请输入回复内容!",""):exit sub
		...
		Function showjoin()
		 ...
		 if not conn.execute("select username from ks_teamusers where username='" & ksuser.username & "' and teamid=" & id).eof then
		  ...
		  showjoin=showjoin & "	</tr>"
		  showjoin=showjoin & "	<tr class=""tabName"">"
		  showjoin=showjoin & "	  <td width=""100"" align=""right"">申 请 人：</td>"
		  showjoin=showjoin & "	  <td><input name=""username"" type=""textbox"" value=""" & ksuser.username & """ readonly size=10></td>"
		  showjoin=showjoin & "	</tr>"
		  showjoin=showjoin & "	<tr class=""tabName"">"
		  showjoin=showjoin & "	  <td align=""right"">加入理由：</td>"
		  showjoin=showjoin & "	  <td><textarea name=""reason"" cols=""50"" rows=""6""></textarea></td>"
		  showjoin=showjoin & "	</tr>"
		  showjoin=showjoin & "	<tr>"
		  showjoin=showjoin & "	  <td colspan=2 align=""center""><input type=""submit"" value=""提交申请"" class=""quanBtn""></td>"
		  showjoin=showjoin & "	</tr>"
		  showjoin=showjoin & "	</form>"
		  showjoin=showjoin & "</table>"
		  showjoin=showjoin & "<div class=""xz""><b>申请须知：</b>"
		  showjoin=showjoin & RS("Note")
		  showjoin=showjoin & "</div>"
		End Function
		...
		function showpost()
		 ...
		 if conn.execute("select username from ks_teamusers where username='"& ksuser.username & "' and teamid=" & id).eof then
		  showpost= "<p class=""sorry"">对不起，你不是该圈子的成员，没有权利发表话题！</p>"
		  exit function
        ...
        showpost=EchoUeditorHead()
		showpost=showpost & "<script>"
		showpost=showpost & "function checktopicform()"
		showpost=showpost & " {"
		...
		showpost=showpost & "  }"
		showpost=showpost & "  if (editor.hasContents()==false)"
		showpost=showpost & "  {"
		showpost=showpost & "   alert('请输入讨论内容!');"
		showpost=showpost & "   editor.focus();"
		showpost=showpost & "   return false;"
		showpost=showpost & "  }"
		...
		showpost=showpost & "<form action='group.asp?action=topicsave&id=" & id & "' onSubmit=""return(checktopicform())"" method='post' name='myform' id='myform'>"
		...
		showpost=showpost & "<script id=""Content"" name=""Content"" type=""text/plain"" style=""width:90%;height:250px;""></script>"
	    showpost=showpost & "<script>setTimeout(""editor = " & GetEditorTag() &".getEditor('Content',{toolbars:[" & Replace(GetEditorToolBar("Basic"),"'source',","") &"],wordCount:false,autoHeightEnabled:false,minFrameHeight:250 ,elementPathEnabled:false});"",10);</script>"
		...
		Sub topicsave()
		 ...
		 dim content:content=KS.HTMLEncode(Request.Form("content"))
		 dim rs:set rs=server.createobject("adodb.recordset")
		 rs.open "select top 1 * from ks_teamtopic",conn,1,3
		 rs.addnew
		  rs("title")=topic
