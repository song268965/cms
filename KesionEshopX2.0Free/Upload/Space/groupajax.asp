...
Class AjaxCls
      Private KS,KSUser
	  Private Action,Template,id,groupadmin,ViewTf
     ...
     Sub Kesion()
	  ...
	  if id=0 then response.End()
	   Dim RS:Set RS=Conn.Execute("select top 1 username,viewTf From KS_Team Where ID=" & ID)
   ...
   Sub CheckViewPower()
      If ViewTf<>"0" Then
	       IF Cbool(KSUser.UserLoginChecked)=false Then 
		     KS.Die "<div class=""message"">对不起，浏览圈子帖子，请先<a href=""../user/login/"" target=""_blank"">登录</a>。</div>"
		   End If
		   If ViewTf="1" Then
		     ...
		     KS.Die "<div class=""message"">对不起，只有本圈成员才有权限浏览。</div>"
			 end if
		   End If
	  End If
   End Sub
