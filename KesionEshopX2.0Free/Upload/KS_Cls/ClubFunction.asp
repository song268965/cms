...
Sub GetClubPopLogin(ByRef FileContent)
 ...
 If KS.IsNul(KS.C("UserName")) And KS.IsNul(KS.C("PassWord")) Then
   ...
   If XslDoc.Load(request.ServerVariables("APPL_PHYSICAL_PATH") &"api/api.config") Then
	  ...
	  If QQEnable Or AlipayEnable Or SinaEnable Then
		...
		'If QQEnable Then Str=Str & "<a title=""使用qq账号登录"" onclick=""$.dialog({width:'520px',height:'370px',title:'绑定QQ登录',content:'url:" & KS.GetDomain & "api/qq/redirect_to_login.asp'});"" href=""javascript:;""><img src=""" &KS.GetDomain & "images/default/icon_qq.png"" align=""absmiddle""/></a>&nbsp;&nbsp;"
 ...
 Else
   ...
   If DataBaseType=1 Then
				...
				Cmd.CommandText="KS_CountMessage"
				Cmd.CommandType=4	
				CMD.Prepared = true 
...
Sub MustReLoadTopTopic()
	  ...
	  Else
	    Application(KS.SiteSN &"TopXML")=""
	  End If
	 RS.Close:Set RS=Nothing
End Sub


Sub LoadNewTopic(boardid,BSetting)
    Dim TopNum:TopNum=0
	Dim CachePage:CachePage=KS.ChkClng(BSetting(69))
	If CachePage<=0 Then Exit Sub
    Dim ListTopicFields:ListTopicFields="ID,UserName,UserID,Subject,AddTime,Verific,LastReplayUser,LastReplayUserID,LastReplayTime,TotalReplay,BoardID,Hits,IsPic,IsTop,IsBest,PostType,AnnexExt,CategoryId,ShowScore" rem 主题列表用到的字段
	TopNum=CachePage *KS.ChkClng(BSetting(20))
	If TopNum=0 Then TopNum=100
    Dim RSS:Set RSS=Conn.Execute("Select top " & TopNum & " " & ListTopicFields & " From KS_GuestBook Where deltf=0 and boardid=" & boardid & " and IsTop=0 Order BY LastReplayTime Desc")
	If Not RSS.Eof Then
		Set Application(KS.SiteSN &"TopicXML"& boardid)=KS.RsToXml(RSS,"row","")
	End If
...
Function InsertPost(BoardID,PostType,UserName,UserID,Subject,Content,Pic,AnnexExt,Purview,ShowIP,ShowSign,ShowScore,CategoryId,Hits,IsTop,IsBest,IsSlide,O_LastPost,verific,ByRef TableName)
		    ...
		    End If
		    SqlStr = "Insert Into KS_GuestBook(BoardID,Subject,PostTable,UserName,UserID,LastReplayUser,LastReplayUserID,face,IsPic,GuestIP,Verific,AddTime,LastReplayTime,Purview,ShowIP,ShowSign,isReplytips,ShowScore,CategoryId,TotalReplay,Hits,IsTop,IsBest,IsSlide,DelTF,AnnexExt,posttype) " & _
			 "values(" & BoardID &",'" & Replace(Subject,"'","''") & "','" & TableName & "','" & UserName & "'," & UserID & ",'" & UserName & "'," & UserID & ",'" & pic & "'," & IsPic &",'" & KS.GetIP & "'," & Verific &"," & SQLNowString &"," & SQLNowString &"," & Purview & "," & ShowIP & "," & ShowSign &"," & KS.ChkClng(Request("isReplytips")) &"," &ShowScore&"," & CategoryId &",0," & Hits & "," & IsTop & "," & IsBest &"," & IsSlide &",0,'" & Replace(AnnexExt,"'","''") & "'," & posttype &")"
			If DataBaseType=1 Then
			   Set RSObj=Conn.Execute(SQLStr & " SELECT @@IDENTITY AS Id").NextRecordSet
			   TopicID=RSObj(0)
			Else
			   Conn.Execute(SQLStr)
				'得到帖子ID号
   ...
   votestr=votestr & ",共有" &VoteNums &"人参与投票, <a href=""javascript:void(0)"" onclick=""showVoteUser('"& KS.Setting(66) &"'," & VoteID& ")"">查看参与用户</a></td></tr>"
						  ...
						  For Each VNode In Xml.DocumentElement.SelectNodes("voteitem")
						   ...
						   dim perVote,pstr,votebg
							...
							if VoteN>=10 then 
						   votestr=votestr & "<tr><td><div class=""vbg""><div style=""width:" & votebg & "px;background:" & VoteColorArr(voten-10) &""">&nbsp;</div></div></td><td align=""left"">" &pstr&"<em style=""color:" & VoteColorArr(voten-10) &""">(" & VNode.childNodes(1).text &")</em></td></tr>"
						   else
