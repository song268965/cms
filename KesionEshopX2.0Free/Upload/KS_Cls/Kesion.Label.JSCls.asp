...
Class JSCommonCls
		...
		Sub Run(sTemp,ByRef Templates)
		 ...
		 select case Lcase(sTemp)
		   ...
		   case "js_collection": echo "<a href=""#"" onclick=""javascript:AddFavorite('http://'+location.hostname+(location.port!=''?':':'')+location.port,'" & KS.Setting(0) &"',document.title);"">加入收藏</a>"
		   case "js_homepage" : echo "<a onclick=""setHomepage('http://'+location.hostname+(location.port!=''?':':'')+location.port);"" href=""#"">设为首页</a>"
