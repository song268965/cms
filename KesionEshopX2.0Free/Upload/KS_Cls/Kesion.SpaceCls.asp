...
'******************************************************************
Class BlogCls
      Public KS,UserName,UserID,Domain,Node,Title,PreviewTemplateID,MetaKeyWords,MetaDescript
	...
	'日志链接
	 Function GetLogUrl(RS)
	  GetLogUrl=GetCurrLogUrl(userid,RS("ID"))
	 End Function
	 Function GetCurrLogUrl(UserID,ID)
	  If KS.SSetting(21)="1" Then
	  GetCurrLogUrl=GetRewriteDir & "list-" & userid & "-" & id&KS.SSetting(22)
	  Else
	 ...
	 Function ReplaceBlogLabel(Template)
	  ...
	  Template=Replace(Template,"{$ShowHits}",Node.SelectSingleNode("@hits").text)
	  if Node.SelectSingleNode("@recommend").text="1" then
	   Template=Replace(Template,"{$ShowRecommend}","<img title=""VIP企业"" src=""" & KS.GetDomain &"images/default/vip.png""/>")
	  else
	  Template=Replace(Template,"{$ShowRecommend}","")
	  end if
	  ...
	  Template=Replace(Template,"{$GetMetaKeyWords}",MetaKeyWords&"")
	  Template=Replace(Template,"{$GetMetaDescript}",MetaDescript&"")
	  Template=Replace(Template,"{$ShowLogo}",ReplaceLogo(Node.SelectSingleNode("@logo").text))
	  Template=Replace(Template,"{$ShowLogoSrc}",Node.SelectSingleNode("@logo").text)
	  ...
	  Template=Replace(Template,"{$ShowUserLogin}","<iframe width=""220"" height=""122"" id=""login"" name=""login"" src=""../user/userlogin.asp"" frameBorder=""0"" scrolling=""no"" allowtransparency=""true""></iframe>")
	 ...
	 Function GetRewriteDir()
	   GetRewriteDir=KS.GetDomain & KS.SSetting(42) &"/"
	 End Function
	 ...
	 Function ReplaceMenu() 
	   ...
	   If KS.SSetting(21)="1" Then
	    If Not KS.IsNul(Domain) Then
			...
			End If
		Else
		 HomeUrl =GetRewriteDir & userid
		End If
		BlogUrl   = GetRewriteDir &"blog-" & userid
		ClubUrl   = GetRewriteDir &"club-" & userid
		MessageUrl= GetRewriteDir &"message-"&userid
		ProductUrl= GetRewriteDir &"product-"&userid
		IntroUrl  = GetRewriteDir &"intro-" &userid
		NewsUrl   = GetRewriteDir &"news-" & userid
		JobUrl    = GetRewriteDir &"job-" & userid
		RyzsUrl   = GetRewriteDir &"ryzs-" &userid
		AlbumUrl  = GetRewriteDir &"album-" & userid
		GroupUrl  = GetRewriteDir &"group-" & userid
		FriendUrl = GetRewriteDir &"friend-" & userid
		XXUrl     = GetRewriteDir &"xx-" & userid
		InfoUrl   = GetRewriteDir &"info-" & userid
	   Else
	 ...
	 Function UserInfo(ByVal Template)
		...
		If KS.IsNul(KS.SSetting(57)) Then
		 Str=ReplaceUserInfoContent(Template,1)
		Else
		 
		 If KS.IsNul(KS.C("AdminName")) And Not KS.IsNul(KS.SSetting(57)) And (KS.IsNul(KS.C("GroupID")) or KS.FoundInArr(KS.SSetting(57),KS.C("GroupID"),",")=false) Then
		  Str="<div class=""info"">对不起，您没有权限查看！</div>"
		 Else
		  Str=ReplaceUserInfoContent(Template,1)
		 End If
		End If
	 ...
	 Function ReplaceUserInfoContent(ByVal Content,Flag)
	   ...
	   If Flag=1 Then
			If not Conn.Execute("select top 1 id from KS_Enterprise Where UserName='" & username &"'").eof Then 
			 Content=LFCls.GetConfigFromXML("space","/labeltemplate/label","companyinfo")
			 Content=ReplaceEnterpriseInfo(Content,RS("UserName"))
			Else
			 Content=LFCls.GetConfigFromXML("space","/labeltemplate/label","userinfo")
			End If
	   ElseIf not Conn.Execute("select top 1 id from KS_Enterprise Where UserName='" & username &"'").eof Then 
	     Content=ReplaceEnterpriseInfo(Content,RS("UserName"))
	   End If
...
on error resume next
		 For Each Node in Application(KS.SiteSN&"_userfiledlist"&channelid).documentElement.SelectNodes("userfiledlist"&channelid)
			 ...
			 If Left(Lcase(FieldName),3)="ks_" Then
				...
				If Not IsNull(RS(FieldName)) Then
				  F_C=Replace(F_C,"{$" & FieldName & "}",KS.CheckXSS(RS(FieldName)),1,-1,1)
				Else
	...
	Function GetNewFresh()
		...
		Else
		   Dim KSR:Set KSR=New refresh
		   GetNewFresh="<strong><a href='" & KS.GetSpaceUrl(userid) & "'>" & UserName & "</a>说：</strong>" & Ubbcode(KSR.ReplaceEmot(rs("note")),1) & " <a href='" & KS.Setting(3)& "user/weibo.asp?userid=" & userid & "'>转播(<span style='color:#ff6600'>" & RS("transnum") & "</span>)</a>" 
		   Set KSR=Nothing
		End If
        RS.Close :Set RS=Nothing
	End Function
	...
	End Function
     ...
     Function GetNewXX()
	    Dim SQLStr:SQLStr="Select top 5 ChannelID,InfoID,Title,Tid,Fname,AddDate,Intro from KS_ItemInfo Where Inputer='" & UserName & "' and deltf=0 and verific=1 Order By AddDate Desc,id desc"
		...
		Str="  <table border=""0"" align=""center"" width=""100%"">" & vbcrlf
		 Dim RS:Set RS=conn.execute(SQLStr)
				 ...
				 Else 
				   I=0
					do while not rs.eof 
					      url=KS.GetItemUrl(rs("channelid"),RS("tid"),RS("infoid"),RS("fname"),RS("AddDate"))
					      if i=0 then
	 ...
	 End Function
	 Function GetSupply()
	   Dim RS:Set RS=Conn.Execute("Select top 10 ID,Title,AddDate,TypeID,Tid,Fname,AddDate From KS_GQ where verific=1 and inputer='" & UserName & "' order by id desc")
	   ...
	   For I=0 To Ubound(SQL,2)
	    GetSupply =GetSupply & "<tr><td height='22'><img src='../images/default/arrow_r.gif' align='absmiddle'>"& KS.GetGQTypeName(SQL(3,I)) & "<a href='" & KS.GetItemUrl(8,SQL(4,I),SQL(0,I),SQL(5,I),SQL(6,I)) & "' target='_blank'>" & SQL(1,I) &  "(" & SQL(2,I) & ")</a></td></tr>"
	   Next
	...
	End Function
	 ...
	 Function GetUserInfo()
	  ...
	  if not rs.eof then
	    ...
	    if left(userfacesrc,1)<>"/" and lcase(left(userfacesrc,4))<>"http" then userfacesrc="../" & userfacesrc
		 ...
		 &"<br /><br />"_
		 &"<div class=""userinfomenu""><li><a href=""../space/?" & UserID & "/message""><img border=""0"" src=""" & KS.GetDomain & "space/images/yes.gif"" align=""absmiddle""> 给我留言</a></li><li><a href=""javascript:void(0)"" onclick=""ksblog.addF(event,'" & UserName & "');""><img src=""" & KS.GetDomain & "space/images/adfriend.gif"" border=""0"" align=""absmiddle""> 加为好友</a></li><li> <a href=""javascript:void(0)"" onclick=""ksblog.sendMsg(event,'" & username & "')""><img src=""" & KS.GetDomain & "space/images/sendmsg.gif"" border=""0"" align=""absmiddle""> 发小纸条</a></li><li>"
		...
		' str=str &"<li><a target=blank href=tencent://message/?uin=" & rs(2) &"&Site=" & KS.Setting(2) & "&Menu=yes><img SRC=http://wpa.qq.com/pa?p=1:" & rs(2) & ":5 alt=""点击这里给我发消息"" border=""0""></a>"
		 'else
		 str=str &"<a href=""../space/?" & UserID & "/info""><img border=""0"" src=""" & KS.GetDomain & "space/images/card.gif"" align=""absmiddle""> 小档案</a>"
		' end if
		 str=str & "</li></div></div>"
	  end if
	  rs.close:set rs=nothing
	 ...
	 Function Getcalendar()
	  ...
	  call CalCls.calendar(Getcalendar,userid)
	  Set CalCls=Nothing
	 End Function
	 '搜索
	 Function GetSearch()
	 ...
	 Function GetBlogTotal()
	   GetBlogTotal="<li><a>日志<span>"&conn.execute("select count(id) from ks_bloginfo where username='" & UserName &"' and status=0")(0) & " 篇</span></a></li>"_
	   & "<li><a>相片<span>"&conn.execute("select count(id) from KS_Photozp where username='" & UserName &"'")(0) & " 张</span></a></li>"_
	   & "<li><a>回复<span>"&conn.execute("select count(id) from ks_blogcomment where username='" & UserName &"'")(0) & " 条</span></a></li>"_
	   & "<li><a>留言<span>"&conn.execute("select count(id) from ks_blogmessage where status=1 and username='" & UserName &"'")(0) & " 条</span></a></li>"_
	   & "<li><a>日志阅读<span>"&conn.execute("select Sum(hits) from ks_blogInfo where username='" & UserName &"' and status=0")(0) & " 人</span></a></li>"_
	   &"<li><a>访问量<span>" & conn.execute("select top 1 hits from ks_blog where username='" & username & "'")(0) & " 人</span></a></li>"
	 ...
	 Function GetMessage()
	  ...
	  If IsObject(Xml) Then
	    ...
	    For Each Node In Xml.DocumentElement.SelectNodes("row")
		 ...
		 If Not KS.IsNul(Node.SelectSingleNode("@replay").text) Then
			GetMessage=GetMessage&"<div style=""margin : 5px 20px; border : 1px solid #efefef; padding : 5px; line-height : normal;""><b>以下为space主人的回复:</b><br>" & KS.LoseHtml(Node.SelectSingleNode("@replay").text) & "<br><div align=right>时间:" & Node.SelectSingleNode("@replaydate").text &"</div></div>"
		 End If
	  ...
	  End If
		 ...
		 Else
		 GetMessage=GetMessage & "<div style=""clear:both""></div><iframe src=""about:blank"" name=""siframe"" width=""0"" height=""0""></iframe><a name=""write""></a><table width=""98%"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" class=""comment_write_table"">"
		 GetMessage = GetMessage & "<form name=""msgform"" target=""siframe"" action=""" & KS.GetDomain &"plus/ajaxs.asp?action=MessageSave"" method=""post"">"
		...
		GetMessage = GetMessage & "   <input name=""AnounName"" maxlength=""100"" type=""text"" value=""" & KS.C("UserName") & """ id=""AnounName"" class=""inputname""/>&nbsp;<font color=red>*</font> <span>验证码 </span><script>writeVerifyCode("""&KS.GetDomain&""",1)</script></td>"
		GetMessage = GetMessage & "</tr>"
		GetMessage = GetMessage & "  <tr>"
		
		GetMessage = GetMessage & "<td height=""25""><textarea   class=""inputcontent""  rows=""4"" id=""Content"" name=""Content"" onfocus=""if (this.value=='既然来了，就顺便留句话儿吧...') this.value='';"" onblur=""if (this.value=='') this.value='既然来了，就顺便留句话儿吧...';"">既然来了，就顺便留句话儿吧...</textarea></td>"
		
		GetMessage = GetMessage & "  </tr>"
		GetMessage = GetMessage & "  <tr>"
		GetMessage = GetMessage & "   <td height=""30""><input type=""submit"" onclick=""return(CheckPostMsg());""  name=""SubmitComment"" value=""OK了，提交留言"" class=""btn""/>"
