<%
 '================================文章模型开始================================
        case "getspecialname"  '内容页显示所属专题
		    Dim RSSpecial:Set RSSpecial=Conn.Execute("select top 10 SpecialName,A.SpecialID,A.FsoSpecialIndex,a.SpecialEname From KS_Special A inner JOin KS_SpecialR B ON A.SpecialID=B.SpecialID Where B.InfoID=" & ItemID &" and B.ChannelID=" & ModelID)
			Dim SpecialStr
        ...
        case "getarticlepagelist"    '分页导航
		 ...
		 if Not KS.IsNul(PageTitle) Then
		   ...
		   echo " <ul>" &vbcrlf
			...
			Dim ShowUrl:ShowUrl=KS.LoadInfoUrl(ModelID,Tid,"",ItemId,Node.SelectSingleNode("@adddate").text)
	    ...
	    case "getfav"
		          echo "<iframe src=""about:blank"" name=""favhidframe"" style=""display:none""></iframe>" &vbcrlf
			      echo "<a target=""favhidframe"" href=""" & DomainStr & "User/User_Favorite.asp?Action=Add&from3g=" & Fcls.CallFrom3g & "&ChannelID=" & ModelID & "&InfoID=" & ItemID & """>加入收藏</a>" &vbcrlf
		case "getarticleaction"
		          echo "<iframe src=""about:blank"" name=""favframe"" style=""display:none""></iframe>" &vbcrlf
			      echo "【<A href=""" & DomainStr & "plus/digmood/Comment.asp?ChannelID=" & ModelID & "&InfoID=" & ItemID & """ target=""_blank"">发表评论</A>】【<A href=""" & DomainStr & "item/SendMail.asp?m="&ModelID &"&ID=" & ItemID & """ target=""_blank"">告诉好友</A>】【<A href=""" & DomainStr & "item/Print.asp?m=" & ModelID &"&ID=" & ItemID & """ target=""_blank"">打印此文</A>】【<a href=""" & DomainStr & "User/User_Favorite.asp?Action=Add&from3g=" & Fcls.CallFrom3g & "&ChannelID=" & ModelID & "&InfoID=" & ItemID & """ target=""favframe"">收藏此文</a>】【<A href=""javascript:window.close();"">关闭窗口</A>】"
		...
		case "getarticleurl"   echo KS.GetItemURL(ModelID,GetNodeText("tid"),ItemID,GetNodeText("fname"),GetNodeText("adddate"))
		...
		case "getdateformat" echo KS.GetTimeFormat(GetNodeText("adddate"))
	
		case "getyear" echo year(GetNodeText("adddate"))
		case "getmonth" echo month(GetNodeText("adddate"))
		case "getday" echo day(GetNodeText("adddate"))
		case "gethour" echo hour(GetNodeText("adddate"))
		case "getminute" echo minute(GetNodeText("adddate"))
		case "getsecond" echo second(GetNodeText("adddate"))
		...
		case "getpictureaction" echo "【<A href=""" & DomainStr & "plus/digmood/Comment.asp?ChannelID=" & ModelID & "&InfoID=" & ItemID & """ target=""_blank"">我来评论</A>】【<A href=""" & DomainStr & "User/User_Favorite.asp?Action=Add&ChannelID=" & ModelID & "&InfoID=" & ItemID & """ target=""_blank"">我要收藏</A>】【<A href=""javascript:window.close();"">关闭窗口</A>】"
