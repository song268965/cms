...
sub initialOpenId()
   ...
   If XslDoc.Load(request.ServerVariables("APPL_PHYSICAL_PATH") &"api/api.config") Then
     ...
     API_WeiXinEnable = Cbool(XslDoc.documentElement.selectSingleNode("rs:data/z:row").getAttribute("api_weixinenable"))
...
function ShowSynchronizedOption(ByRef CheckJS)
    ...
    dim str:str="<script type=""text/javascript"">" & vbcrlf
			...
			str=str & "   $.dialog.open('" & KS.GetDomain & "api/qq/redirect_to_login.asp',{width:'750px',height:'390px',title:'绑定QQ登录'});},function(){});" &vbcrlf
...
'同步微博 回复一条评论到新浪
'参数 weiboid 需要评论的微博ID,cid需要回复的评论ID，content 内容
function add_sina_replycomment(weiboid,cid,content)
    initialOpenId
	if cbool(API_SinaEnable)=false Then Exit function
	Dim str
	str="id=" & weiboid&"&cid=" & cid & "&comment=" & content &"&comment_ori=1&access_token="&getuserinfo("sinatoken")
	add_sina_replycomment = do_post("https://api.weibo.com/2/comments/reply.json",str,false)     '纯文本
end function
'转发一条微博
'参数 weiboid 需要转发的微博ID,status 添加的转发文本
function add_sina_transweibo(weiboid,status)
    initialOpenId
	if cbool(API_SinaEnable)=false Then Exit function
	Dim str
	str="id=" & weiboid&"&status=" & status &"&access_token="&getuserinfo("sinatoken")
	add_sina_transweibo = do_post("https://api.weibo.com/2/statuses/repost.json",str,false)     '纯文本
end function
