...
Class KesionStaticCls
		...
		Private DomainStr,ID,UserLoginTF,PayTF,UserName,UrlsTF
		...
		Public Sub Run()
		   ...
		   Else
			   ...
			   If Ubound(G_P_Arr)<1 Then 
				 Response.Redirect(KS.Setting(3) & "index.asp")
				 Response.End()
			   End If
			   ...
			   If ThreadType=PreContentTag Then
				   ...
				   If ChannelID=0 Then  Response.Redirect(KS.Setting(3) & "index.asp"): Response.End()
		...
		End If
		 IF RSObj.Eof And RSObj.Bof Then  RSObj.Close:Set RSObj=Nothing:Call KS.Alert("非法参数!",""):Exit Sub
		  ...
		  End If
			 ...
			 If KS.ChkClng(KS.C_S(ChannelID,21))=0 Then   Call KS.ShowTips("error","对不起，本频道已关闭!"):Response.End
			...
			End If
			if Instr(FileContent,"[KS:PageStyle]")<>0 Then
		  	FileContent=Replace(FileContent,"[KS:PageStyle]",KS.ReplacePage(FCls.PageStyle,ChannelID,id,CurrPage,FCls.TotalPut,FCls.PerPageNum))
			End If
			FileContent=Replace(FileContent,"[#CurrPage]",CurrPage)
		...
		Sub StaticContent()
		  ...
		  If instr(request.servervariables("http_user_agent"),"Mobile")>0 Then '手机访问,自动跳到手机版
                  If KS.WSetting(0)=1 Then
				    Response.Redirect(KS.Setting(3) & KS.WSetting(4)&"/show.asp?m=" & ChannelID&"&d=" & ID)
				    Exit Sub
				  End If
			End If
		...
		Sub CheckCharge(KSR)
		 With KSR
			 ...
			 If InfoPurview=2 or ReadPoint>0 Then
			   ...
			   Else
					 IF KS.FoundInArr(.Node.SelectSingleNode("@arrgroupid").text,KSUser.GroupID,",")=false and readpoint=0 and KSUser.GroupID<>1 Then
				  ...
				  Else     
					 ...
					 If ClassPurview=2 Then
						 IF KS.FoundInArr(.Node.SelectSingleNode("@defaultarrgroupid").text,KSUser.GroupID,",")=false Then
							...
							Call KS.Alert("对不起，你所在的用户级没有权限浏览!",KS.Setting(3)):Response.End
						 Else
							Call PayPointProcess()
						 End If
					Else    
		...
		Sub StaticArticleContent()
		 ...
		 End IF
		  ...
		  With KSR 
			 ...
			 End If
			 Call FCls.SetContentInfo(ChannelID,.Tid,.oTid,ID,.Node.SelectSingleNode("@title").text)
		   ...
		   If .Node.SelectSingleNode("@verific").text=1 Then
			   If .Node.SelectSingleNode("@refreshtf").text="1" and Cint(KS.C_S(ChannelID,7))<>0 and not (readpoint>0) and not (infopurview=2) and not (InfoPurview=0 And ClassPurview>0) Then
				 Response.Redirect KS.GetItemUrl(ChannelID,.Tid,ID,.Node.SelectSingleNode("@fname").text,.Node.SelectSingleNode("@adddate").text)
			   End If
		   End If
		  
		   '附属文档
		   IF KS.ChkClng(.Node.SelectSingleNode("@oid").text)<>0 Then
		      If KS.ChkClng(KS.C_C(.Node.SelectSingleNode("@otid").text,12))>0 Then
		        Dim RSO:Set RSO=Conn.Execute("select top 1 * From "& KS.C_S(KS.C_C(.Node.SelectSingleNode("@otid").text,12),2) & " Where ID=" & KS.ChkClng(.Node.SelectSingleNode("@oid").text))
				If Not RSO.Eof Then
				   Dim DocOXML:Set DocOXML=KS.RsToXml(RSO,"row","root")
				   Set .ONode=DocOXML.DocumentElement.SelectSingleNode("row")
				End If
				RSO.Close
				Set RSO=Nothing
			  End If
		...
		Dim articlecontent:articlecontent=(.Node.SelectSingleNode("@articlecontent").text)
		If KS.IsNul(articlecontent) Then articlecontent=" "
		 If .Node.SelectSingleNode("@postid").text<>"0" Then
		  ContentArr=Split(.UbbCode(articlecontent&"",1),"[NextPage]")
		 Else
		  ContentArr=Split(articlecontent&"","[NextPage]")
		 End If
	   ...
	   Sub StaticPhotoContent()
		  ...
		  Call CheckCharge(KSR)
		   ...
		   End If
			...
			FileContent = KSR.LoadTemplate(.Node.SelectSingleNode("@templateid").text)
			 ...
			 If InStr(FileContent,"[KS_Charge]")=0 Then
			   FileContent = Replace(FileContent,"{$ShowPictures}","[KS_Charge]{$ShowPictures}[/KS_Charge]")
			 End If
			 If Cbool(UrlsTF)=false  Then
			   Dim ChargeContent:ChargeContent=KS.CutFixContent(FileContent, "[KS_Charge]", "[/KS_Charge]", 0)
			   FileContent=Replace(FileContent,"[KS_Charge]" & ChargeContent &"[/KS_Charge]",Content)
			 Else
			   FileContent=Replace(Replace(FileContent,"[KS_Charge]",""),"[/KS_Charge]","")
		...
		'	 If Cbool(UrlsTF)=true Then
			      dim picurls:picurls=KSR.Node.SelectSingleNode("@picurls").text
				  ...
				  Select Case ShowStyle
				   ...
				   Case 2
						...
						For r=1 to pagenum
							  ...
							  thumbsphoto=thumbsphoto&"<li><a href=""" & Split(PicUrlsArr(n), "|")(1) & """  class=""swipebox"" title=""" & KS.LoseHtml(Split(PicUrlsArr(n), "|")(0)) & """><img alt='" & KS.LoseHtml(Split(PicUrlsArr(n), "|")(0)) & "' src='" & Split(PicUrlsArr(n), "|")(2)  & "' border='0'/></a><div style='text-align:center'>" & KS.Gottopic(Split(PicUrlsArr(n), "|")(0),18) & "</div></li>"
                              else 
							   exit for
							  end if
							  n=n+1
				 ...
				 Case 3
					   ...
					   For r=1 to pagenum
							  if n<=ubound(PicUrlsArr) Then
							    ...
							    ThumbList=ThumbList & "<img title=""" &Split(PicUrlsArr(n), "|")(0) & """ href=""" & BigPic &""" class=""scrollLoading swipebox"" alt='" & Split(PicUrlsArr(n), "|")(0) & "' style=""cursor:pointer;background:url(" & KS.GetDomain &"images/default/loading.gif) no-repeat center;""  data-url=""" & BigPic &""" src=""" & KS.GetDomain &"images/default/pixel.gif"" border='0'><div class=""imgtitle"">" & Split(PicUrlsArr(n), "|")(0) & "</div>"
							  Else 
							    Exit For
							  End If
							  n=n+1
	   ...
	   Sub StaticProductContent()
	       SQLStr="Select top 1 * From " & KS.C_S(ChannelID,2) & "  Where deltf=0 and verific=1 And ID=" & ID
	       If DataBaseType=1 Then
			    Dim Cmd : Set Cmd = Server.CreateObject("ADODB.Command")
				Set Cmd.ActiveConnection=conn
				Cmd.CommandText="KS_TSql"
		 ...
		 ElseIF KS.ChkCLng(RS("Changes"))=1 Then 
		   Dim changesUrl:changesUrl=RS("changesUrl")
		   RS.Close:Set RS=Nothing
		   Response.Redirect changesUrl
		 End IF
		 Set DocXML=KS.RsToXml(RS,"row","root") : RS.Close:Set RS=Nothing
		 With KSR 
			Set .Node=DocXml.DocumentElement.SelectSingleNode("row")
	   ...
	   Sub StaticMovieContent()
	       SQLStr="Select top 1 * From KS_Movie Where deltf=0 and verific=1 And ID=" & ID
	       If DataBaseType=1 Then
			    Dim Cmd : Set Cmd = Server.CreateObject("ADODB.Command")
				Set Cmd.ActiveConnection=conn
				Cmd.CommandText="KS_TSql"
		 ...
		 IF RS.Eof And RS.Bof Then
		  RS.Close :Set RS=Nothing
		  KS.ShowTips "error","您要观看的" & KS.C_S(7,3) & "已删除。或是没有通过审核!"
		 End IF
		 If RS("Verific")<>1 And KS.C("UserName")<>RS("Inputer") And KS.C("AdminName")="" Then
		   RS.Close :Set RS=Nothing
		   KS.ShowTips "error","对不起，该" & KS.C_S(7,3) & "还没有通过审核!"
		 End If
		 ...
		 With KSR 
			Set .Node=DocXml.DocumentElement.SelectSingleNode("row")
		      .Tid=.Node.SelectSingleNode("@tid").text
			   .oTid=.Node.SelectSingleNode("@otid").text
			  Call FCls.SetContentInfo(7,.Tid,.Otid,ID,.Node.SelectSingleNode("@title").text)
			   '附属文档
			   IF KS.ChkClng(.Node.SelectSingleNode("@oid").text)<>0 Then
				  If KS.ChkClng(KS.C_C(.Node.SelectSingleNode("@otid").text,12))>0 Then
					Dim RSO:Set RSO=Conn.Execute("select top 1 * From "& KS.C_S(KS.C_C(.Node.SelectSingleNode("@otid").text,12),2) & " Where ID=" & KS.ChkClng(.Node.SelectSingleNode("@oid").text))
	   ...
	   Sub StaticSupplyContent()
		 ...
		 Else
		 SQLStr="Select top 1 b.TemplateID,b.classpurview,b.defaultarrgroupid,a.* From KS_GQ a inner join KS_Class b on a.Tid=b.ID where a.deltf=0 and a.verific=1 and a.ID=" & ID
		 End If
		...
		End If
		 ...
		 With KSR 
			Set .Node=DocXml.DocumentElement.SelectSingleNode("row")
		      .Tid=.Node.SelectSingleNode("@tid").text
			   .oTid=.Node.SelectSingleNode("@otid").text
			  Call FCls.SetContentInfo(8,.Tid,.Otid,ID,.Node.SelectSingleNode("@title").text)
			   '附属文档
			   IF KS.ChkClng(.Node.SelectSingleNode("@oid").text)<>0 Then
				  If KS.ChkClng(KS.C_C(.Node.SelectSingleNode("@otid").text,12))>0 Then
					Dim RSO:Set RSO=Conn.Execute("select top 1 * From "& KS.C_S(KS.C_C(.Node.SelectSingleNode("@otid").text,12),2) & " Where ID=" & KS.ChkClng(.Node.SelectSingleNode("@oid").text))
			  ...
			  If Instr(FileContent,"[KS_Charge]")<>0 Then
				 ...
				 FileContent=Replace(FileContent,ChargeContent,LFCls.GetConfigFromXML("supply","/labeltemplate/label","divajax"))
				End If
			  FileContent = .KSLabelReplaceAll(FileContent)
		 End With
		 KS.Echo FileContent
