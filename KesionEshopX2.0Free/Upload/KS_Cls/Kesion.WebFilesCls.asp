...
Class WebFilesCls
		...
		<%end if%>
				...
				if instr(currentdir,".")<> 0 or instr(topdir,".")<>0 or instr(topdir,KS.Setting(91))=0 then
				...
				{
				<%if(ks.s("fieldid")<>"") then%>
				    $('#<%=ks.s("fieldid")%>', parent.document).val(v)
				 <%end if%> 
				 <%if (request("groupbuy")="ok") then 
				 	if ks.g("Photo_id")="PhotoUrl" then
						%>$(window.top.document).find("#pic").attr("src",v);<%
					end if
				 %>
				<%end if%> 
					top.box.close();
			...
			//   alert($('#Title', parent.document).val());
				...
				<table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
					...
					For Each DirFolder in FsoFile.SubFolders%>
					 <tr bgcolor="#ffffff" class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'"> 
					  ...
					  <td>&nbsp;<a href="?from=<%=Request("from")%>&ChannelID=<%=ChannelID%>&topdir=<%=topdir%>&action=Main&OpTypeStr=<%=OpTypeStr%>&CurrentDir=<%=CurrentDir & DirFolder.name%>"><img src="<%=WebDir%>Editor/ksplus/FileIcon/folder.gif" border=0 width="16" height="16" align="absmiddle"></a>&nbsp;<a href="?from=<%=Request("from")%>&ChannelID=<%=ChannelID%>&topdir=<%=topdir%>&action=Main&fieldid=<%=ks.s("fieldid")%>&OpTypeStr=<%=OpTypeStr%>&CurrentDir=<%=CurrentDir & DirFolder.name%>"><%=DirFolder.name%></a></td>
					  <td width="197" align="center"><%=GetSize(DirFolder.size,"b")%></td>
					  <td align="center" nowrap>&nbsp;<%=DirFolder.DateLastModified%></td>
					  <td width="198" align="center"><a href="?from=<%=Request("from")%>&ChannelID=<%=ChannelID%>&topdir=<%=topdir%>&action=Main&OpTypeStr=<%=OpTypeStr%>&CurrentDir=<%=CurrentDir & DirFolder.name%>">打开</a></td>
					 </tr>
					...
					%>
					 <tr class='list' onMouseOver="this.className='listmouseover'" onMouseOut="this.className='list'"> 
					  ...
					  <a href="javascript:;" title="文件名：<%=TopDir & CurrentDir & DirFiles.name%>" onClick="setReturn('<%=TopDir & CurrentDir & DirFiles.name%>')">
					  <%else%>
					  ...
					  <td align="center"><%=GetSize(DirFiles.size,"b")%></td>
				...
				Sub DelAll()
					...
					if instr(FileId,"./")<>0 or instr(FolderId,"./")<>0 then
					 KS.Die("<script>$.dialog.alert('非法路径',function(){location.href='" & ComeUrl & "';});</script>")
					end if
					...
					If Ubound(FolderId) > -1 then
						...
						Next
					End If
					Response.Write("<script>$.dialog.alert('成功删除 "& FileNum &" 个文件<br/>成功删除 "& FolderNum &" 个文件夹',function(){location.href='" & ComeUrl & "';});</script>")
				End Sub
				Sub Rname()
