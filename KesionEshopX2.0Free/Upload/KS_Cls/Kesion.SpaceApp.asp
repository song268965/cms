...
'******************************************************************
Class SpaceApp
        Public  Domain,FoundSpace,Param,PreviewTemplateID,BgUrl,BgRepeat,bgposition,BlogName,Descript
        Private KS,UserName,UserID,KSR,Action,ID,Node,CurrPage,TotalPut,MaxPerPage,PageNum
		Private Template,TemplateSub,SubStr,KSBCls
		...
		QueryStrings=replace(QueryStrings,"/","↓")
		...
		'============================结束记录============================================================
		 ...
		 select case Lcase(action)
		   ...
		   case "info"
		     KSBCls.Title="资料"
			 substr=substr & KSBcls.Location("首页 >> 联系方式")
			 SubStr=SubStr & "<div class=""contactus"">" & KSBCls.UserInfo(Template) &"</div>"
		   case "message"
		     KSBCls.Title="留言"
		     Call ShowMessage
		   case "intro"
			 ...
			 if Not Irs.Eof Then
		       KSBCls.MetaDescript=KS.Gottopic(KS.LoseHtml(Irs(0)),200)
			   SubStr=SubStr & "<div class=""intro"">" & Irs(0) &"</div>"
		     Else
		       Irs.Close: Set Irs=Nothing
		       KS.AlertHintScript "对不起，该用户不是企业用户！"
			 End If
			 Irs.Close:Set IrS=Nothing
		   case "news" KSBCls.Title="促销信息" : GetNews
		  ...
		  If KS.IsNul(KSBCls.MetaKeyWords) Then KSBCls.MetaKeyWords=BlogName
		  If KS.IsNul(KSBCls.MetaDescript) Then KSBCls.MetaDescript=Descript

		  
		  template=KSBCls.ReplaceBlogLabel(Template)
		  KS.Echo KSBCls.LoadSpaceHead
		  KS.Echo Replace(Template,"｛#","{")
		  If Not KS.IsNul(BgUrl) Then 
		    dim repeatstr,bgpositionstr
		    select case ks.chkclng(bgrepeat)
			  case 0 repeatstr="no-repeat"
			  case 1 repeatstr="repeat-x"
			  case 2 repeatstr="repeat-y"
			end select 
			select case KS.ChkClng(bgposition)
			  case 1 bgpositionstr=" top left"
			  case 2 bgpositionstr=" top center"
			  case 3 bgpositionstr=" top right"
			  case 4 bgpositionstr=" center left"
			  case 5 bgpositionstr=" center center"
			  case 6 bgpositionstr=" center right"
			  case 7 bgpositionstr=" bottom left"
			  case 8 bgpositionstr=" bottom center"
			  case 9 bgpositionstr=" bottom right"
			end select
		  %>
		   <script>
			 $(document).ready(function(){
			  $("body").attr("style","background:url(<%=bgurl%>) <%=repeatstr%><%=bgpositionstr%>;");
			 });
			</script>
		  <%
		  End If
		...
		Sub BlogLog()
		  ...
		  End If
		  if rs("istalk")=1 then
		   KSBCls.Title="查看" & UserName & "发表的微博"
		  else
		   KSBCls.Title=rs("title")
		  end if
		  
		  KSBCls.MetaKeyWords=rs("Tags") : If KS.IsNul(KSBCls.MetaKeyWords) Then KSBCls.MetaKeyWords=RS("Title")
		  KSBCls.MetaDescript=KS.Gottopic(KS.LoseHtml(Ubbcode(RS("Content")&"",0)),200)
		  ...
		  Else
			  SubStr=substr & LFCls.GetConfigFromXML("space","/labeltemplate/label","log")
			   ...
			   End If
				...
				Dim JFStr:If RS("Best")="1" then JFStr="  <img src=""../images/default/jh.jpg"" align=""absmiddle"">" else JFStr=""
			   ...
			   SubStr=Replace(SubStr,"{$ShowLogText}",replace(KS.ReplaceInnerLink(Ubbcode(ContentStr,i)),chr(13),"<br/>"))
	...
	substr=substr & "<div id=""blogmain"">loading...</div>"&vbcrlf
	substr=substr & "<div id=""kspage"" style=""text-align:right""></div>"&vbcrlf
	substr=substr &"<script>$(document).ready(function(){Page(1,'logcmt&id=" & ID & "','" & username & "')});</script>"
...
Function ReplacePrevNextArticle(NowID,TypeStr)
		         ...
		         If RSObj.EOF and RSObj.Bof  Then
						...
						Else
						  If KeyWord="" And ClassID=0 Then
							substr=substr & "<div class=""writeblog""  colspan=3><p>您还没有写博文,<a href=""" & KS.GetDomain & "user/user_blog.asp?action=Add"" style=""color:#0082ca"">点此开始写博文</a>！</p></div>"
	...
	End Sub
	 ...
	 Sub FriendList()
	     ...
	     substr=substr & "           <table  align=""center"" class=""border"" width=""100%"" >" & vbcrlf
		 Dim RSObj:Set RSObj=Server.CreateObject("ADODB.RECORDSET")
		 RSObj.Open "select friend,u.userface,u.username,u.userid from ks_friend f inner join ks_user u on f.friend=u.username where f.username='" & username & "' and f.accepted=1",Conn,1,1
		                 If RSObj.EOF and RSObj.Bof  Then
						  substr=substr & "<tr><td class=""friend"" colspan=3>没有加好友！</td></tr>"
						 Else
	 ...
	 Sub GroupList()
		 ...
		 RSObj.Open "select * from KS_team where username='" & username & "' and verific=1 order by id desc",Conn,1,1
		   If RSObj.EOF and RSObj.Bof  Then
			substr=substr & "<tr><td class=""circle""colspan=3>没有创建圈子！</td></tr>"
		   Else
				totalPut = RSObj.RecordCount
				If CurrPage >1 and (CurrPage - 1) * MaxPerPage < totalPut Then
						RSObj.Move (CurrPage - 1) * MaxPerPage
	 ...
	 Sub AlbumList()
		 ...
		 RSObj.Open "Select * from KS_Photoxc Where username='" & username & "' and status=1 order by id desc",Conn,1,1
		  If RSObj.EOF and RSObj.Bof  Then
			 substr=substr & "<div  class=""setpicture""  >没有创建相册！</div>"
		  Else
							totalPut = RSObj.RecordCount
	...
	'查看相片
	 Sub ShowAlbum()
	  ...
	  KSBCls.MetaKeyWords=rsxc("xcname")
	   KSBCls.MetaDescript=KS.Gottopic(KS.LoseHtml(RSxc("Descript")),200)
	 ...
	 End Sub
	  Sub GetAlbumBody(xcname)
				 ...
				 Else
						...
						substr=substr & "<div id=""blogmain"">loading...</div>"&vbcrlf
						substr=substr & "<div id=""kspage"" style=""text-align:right""></div>"&vbcrlf
						substr=substr &"<script>$(document).ready(function(){Page(1,'photocmt&id=" & rs("id") & "','" & username & "')});</script>"
				        SubStr=SubStr & "<script src=""writecomment.asp?xcid=" & id & "&tag=photo&UserID=" & UserID &"&ID=" & rs("ID") & "&UserName=" & UserName & "&Title=" & rs("Title") & """></script>"
		...
		Sub ShowMessage()
		 ...
		 SubStr=substr & "<div class=""message"">" &  GetWriteMessage() 
		 MaxPerPage =8
		 SubStr=SubStr &"  <table border=""0"" align=""center""  width=""98%"">" & vbcrlf
		 Dim RSObj:Set RSObj=Server.CreateObject("ADODB.RECORDSET")
		 RSObj.Open "Select * from KS_BlogMessage Where UserName='" & UserName & "' and status=1 Order By AddDate Desc",Conn,1,1
		         If RSObj.EOF and RSObj.Bof  Then
				  SubStr=SubStr &"<tr style=""background:#DEF1F4; color:#4293A9""><td class=""messageme"" >还没有人给主人留言哦^_^</td></tr></table>"
				 Else
		 ...
		 substr=substr &  "</table>" & vbcrlf 
		 if totalPut>0 then substr=substr& ShowPage
		 substr=substr &"</div>" &vbcrlf
		 RSObj.Close:Set RSObj=Nothing
		...
		Sub ShowGuest(rs)
		 Dim I,CommentStr,n
		  ...
		  Do While Not RS.Eof 
		   ...
		   CommentStr=CommentStr & KS.CheckXSS(RS("Title")&"") & "<br/>"
		   End If
		   CommentStr=CommentStr & Replace(KS.CheckXSS(RS("Content"))&"",chr(13)&chr(10),"<br/>")
		...
		GetWriteMessage = GetWriteMessage & " maxlength=""100"" type=""text"" value=""" & KS.C("UserName") & """ id=""AnounName"" style=""background:#FBFBFB;color:#999;border:1px solid #ccc;width:100px;height:20px;line-height:20px""/>&nbsp;<font color=red>*</font> <span>验证码 </span><script>writeVerifyCode("""&KS.GetDomain&""",1)</script><br/><input type=""submit"" onclick=""return(CheckPostMsg());""  name=""SubmitComment"" value=""OK了，提交留言"" class=""btn""/>"
		...
		Sub GetNews()
		 Dim SQL,i,param
		 SubStr=KSBcls.Location("首页 >> 促销信息")
		 SubStr=SubStr & "<div class=""newslist"">" &vbcrlf
		...
		Sub ShowNews()
		 ...
		 SubStr=KSBcls.Location("首页 >> 促销信息 >> 查看促销")
		 SubStr=SubStr & "<table border='0' width='98%' align='center' cellspacing='2' cellpadding='2'>"
		 Set RS=Server.CreateObject("ADODB.RECORDSET")
		 ...
		 Else
		   KSBCls.Title=rs("Title")
		   KSBCls.MetaKeyWords=rs("Title")
		   KSBCls.MetaDescript=KS.Gottopic(KS.LoseHtml(rs("content")),200)
		   ...
		   SubStr=SubStr &"<tr><td><div style='text-align:center'><a href='" & Url & "'>[返回促销信息]</a></div></td></tr>"
		 End If
		 SubStr=SubStr &"</table>"   
         RS.Close:Set RS=Nothing
		End Sub
		...
		Function ProductList()
		 ...
		 If ID<>0 Then Param=Param & " and classid=" & id
		 SubStr=SubStr & "<div class=""productlist""><ul>" &vbcrlf
		 Set RS=Server.CreateObject("ADODB.RECORDSET")
		 RS.Open "Select ID,Title,PhotoUrl From KS_Product " & Param &" order by adddate desc",conn,1,1
		 If RS.EOF and RS.Bof  Then
			SubStr=SubStr & "<p class=""empty"">没有发布产品展示,请<a href='../user/user_myshop.asp?Action=Add' target='_blank'><font color=red>点此发布</font></a>！</p>"
		Else
			  totalPut = RS.RecordCount
				If CurrPage> 1 and (CurrPage - 1) * MaxPerPage < totalPut Then
						RS.Move (CurrPage - 1) * MaxPerPage
				...
				Dim K,N,Total,PhotoUrl,Url
				 ...
				 For I=0 To Total
				   SubStr=SubStr &"<li>" &vbcrlf
					...
					SubStr=SubStr & "<a href='" & Url & "' target='_blank'><img border='0' src='" & PhotoUrl & "' alt='" & SQL(1,N) & "' /></a>" &vbcrlf & "<div  class=""productname""><a href='" & Url & "'>" & KS.Gottopic(SQL(1,N),20) & "</a></div>" &vbcrlf
					N=N+1
					 SubStr=SubStr &"</li>" &vbcrlf
		...
		Function ShowProduct()
		 ...
		 SubStr=SubStr & "<table border='0'style='width:600px' align='left' cellspacing='2' cellpadding='2'>"
		 Set RS=Server.CreateObject("ADODB.RECORDSET")
		 ...
		 Else
		   KSBCls.Title=RS("title")
		   KSBCls.MetaKeyWords=rs("Title")
		   KSBCls.MetaDescript=KS.Gottopic(KS.LoseHtml(RS("proIntro")),200)

		   photourl=RS("BigPhoto")
		   If PhotoUrl="" Or IsNull(photourl) Then photourl="../images/nopic.gif"
		   SubStr=SubStr &"<tr><td align='center' style='color:#ff6600;font-weight:bold;font-size:14px'>" & rs("Title") & "</td></tr>"
		   SubStr=SubStr & "<tr><td align='left'><img  style='max-width:600px;width:expression(document.body.clientWidth>600?""600px"":""auto"");overflow:hidden;' src='" & photourl &"' border='0'></td></tr>"
		   ...
		   SubStr=SubStr & "<tr><td>在线购买：<a target='_blank' href=""" & KS.GetItemURL(5,rs("Tid"),rs("ID"),rs("Fname"),rs("AddDate"))   & """><img src='" & KS.GetDomain & "images/ProductBuy.gif' align='absmiddle' border='0'/></a></td></tr>"
		   End If
	...
	End Function
		...
		Function ShowPhoto()
		 ...
		 Else
		   KSBCls.Title = rs("title")
		   KSBCls.MetaKeyWords=rs("KeyWords")
		   KSBCls.MetaDescript=KS.Gottopic(KS.LoseHtml(RS("PictureContent")),200)
		   ...
		   substr=substr & "<tr><td><span class=""writecomment""><Script Language=""Javascript"" Src=""../plus/digmood/Comment.asp?Action=Write&ChannelID=2&InfoID=" &id & """></Script></span></td></tr>"
		...
		Dim SQL,K,OPStr,RSC:Set RSC=Conn.Execute("Select ChannelID,itemName From KS_Channel Where ChannelStatus=1 and basictype<10 and channelid<>6  And ChannelID<>9 And ChannelID<>10 order by channelid")
		SQL=RSc.GetRows(-1)
		...
		Substr=Substr& "<div class=""xxcategory"">信息分类<select name='channelid' onchange=""location.href=this.value"">" & opstr & "</select></div>"
		 ...
		 RSObj.Open SqlStr,Conn,1,1
		         If RSObj.EOF and RSObj.Bof  Then
					substr=substr & "<tr><td class=""textmessage"" colspan=3><p>找不到您要的信息！</p></td></tr>"
				 Else
							totalPut = RSObj.RecordCount
							 If CurrPage>1 and (CurrPage - 1) * MaxPerPage < totalPut Then
										RSObj.Move (CurrPage - 1) * MaxPerPage
	...
	Sub showxx(rs,channelid)
		...
		Else
			 Dim K,SQL
			 Do While Not RS.Eof
				substr=substr & "<tr><td style=""border-bottom: #efefef 1px dotted;height:22""><img src=""../images/default/arrow_r.gif"" align=""absmiddle""> [" & KS.GetClassNP(RS(2)) & "] <a href='" & KS.GetItemUrl(channelid,RS(2),RS(0),RS(5),RS(7)) & "' target='_blank'>" & RS(1) & "</a>&nbsp;&nbsp;(" & RS(7) & ")</td></tr>"
				K=K+1
	...
	'===========9-30========================
		Function GetUserPhoto(RS,totalPut,ChannelID)
			...
			Dim PerLineNum:PerLineNum=4   '每行显示作品数
			  ...
			  Else
			   Url=KS.GetItemUrl(channelid,RS(2),RS(0),RS(5),RS(7))
			  End If
