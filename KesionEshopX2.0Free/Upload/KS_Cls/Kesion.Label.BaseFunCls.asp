...
Class LabelBaseFunCls
				  ...
				  If DiyNode.Length>0 Then
						For Each Node In DiyNode
							...
							If FieldType=10 And Node.SelectSingleNode("mustfilltf").text="1" Then
								 if flag=1 then
								 Response.Write "if (" &GetEditorContent(fieldName) & "==false){$.dialog.alert('" & Xtitle & "必须输入内容！',function(){ " &GetEditorFocus(fieldName) & " }); return false; }" &vbcrlf
								 else
								 Response.Write "if (editor" & fieldName& ".hasContents()==false){$.dialog.alert('" & Xtitle & "必须输入内容！',function(){editor" & fieldName& ".focus();}); return false;}" &vbcrlf
								 end if
		...
		Sub ModifyItemInfo(ChannelID,InfoID,Title,Tid,Intro,KeyWords,PhotoUrl,Verific)
		 Gcls.Execute("Update [KS_ItemInfo] Set Title='" & replace(Title,"'","''") & "',Tid='" & Tid & "',oTid='" & KS.S("Tid") & "',oid=" & KS.ChkClng(KS.S("Oid")) &",Intro='" & Replace(left(KS.LoseHtml(Intro),255),"'","''")  & "',KeyWords='" & Replace(KeyWords,"'","''") & "',PhotoUrl='" & PhotoUrl & "',AddDate=" & SQLNowString &",ModifyDate=" & SQLNowString &",Verific=" & Verific & " Where  ChannelID=" & ChannelID & " and InfoID=" & InfoID)
		End Sub	
		
		 '取评论存放数据表
		Function GetCommentTable()
			Dim Nodes,Doc,CommentTable
			set Doc = KS.InitialObject("msxml2.FreeThreadedDOMDocument"& MsxmlVersion)
			Doc.async = false
			Doc.setProperty "ServerHTTPRequest", true 
			Doc.load(Server.MapPath(KS.Setting(3)&"Config/commenttable.xml"))
			Set Nodes=Doc.DocumentElement.SelectSingleNode("item[@isdefault='1']")
			CommentTable=nodes.selectsinglenode("tablename").text
			Set Doc=Nothing
			GetCommentTable=CommentTable
		End Function	
		
		'返回评论数
		Function GetCmtNum(PostTable,ChannelID,InfoID)
		 If KS.IsNul(PostTable) Then PostTable="KS_Comment"
		 if instr(lcase(PostTable),"ks_comment")=0 Then
		  GetCmtNum=Conn.Execute("Select top 1 TotalReplay From KS_GuestBook Where ChannelID=" & ChannelID &" and InfoID=" & InfoID)(0)
		 Else
		  GetCmtNum=Conn.Execute("Select Count(1) From " & PostTable &" Where projectID=0 and verific=1 and ChannelID=" & ChannelID &" and InfoID=" & InfoID)(0)
		 End If
		End Function
        ...
        Sub AddItemInfo(ByVal ChannelID,InfoID,Title,Tid,Intro,KeyWords,PhotoUrl,AddDate,Inputer,Hits,HitsByDay,HitsByWeek,HitsByMonth,Recommend,Rolls,Strip,Popular,Slide,IsTop,Comment,Verific,Fname)
		 ...
		 Dim OrderID:OrderID=KS.ChkClng(Conn.Execute("Select Max(OrderID) From KS_ItemInfo Where Tid='" & Tid &"'")(0))+1
		 Gcls.Execute("Insert Into [KS_ItemInfo](ChannelID,InfoID,Title,Tid,Intro,KeyWords,PhotoUrl,AddDate,ModifyDate,Inputer,Hits,HitsByDay,HitsByWeek,HitsByMonth,Recommend,Rolls,Strip,Popular,Slide,IsTop,Comment,Verific,DelTF,Fname,Otid,oid,OrderID) values(" & Channelid & "," & InfoID & ","& KS.WithKorean() &"'" & replace(Title,"'","''") & "','" & Tid & "' ,"& KS.WithKorean() &"'" & left(Replace(KS.CheckScript(KS.LoseHtml(Intro)),"'","''"),200) & "',"& KS.WithKorean() &"'" & Replace(KeyWords,"'","''") & "' ,'" & PhotoUrl & "' ,'" & AddDate & "','" & AddDate&"',"& KS.WithKorean() &"'" & Inputer & "' ," & KS.ChkClng(Hits) & "," & KS.ChkClng(HitsByDay) & ", " & KS.ChkClng(HitsByWeek) & "," & KS.ChkClng(HitsByMonth) & "," & KS.ChkClng(Recommend) & "," & KS.ChkClng(Rolls) & "," & KS.ChkClng(Strip) & "," & KS.ChkClng(Popular) & "," & KS.ChkClng(Slide) & "," & KS.ChkClng(IsTop) & "," & KS.ChkClng(Comment) & "," & KS.ChkClng(Verific)& ",0,'" & Fname & "','" & KS.G("Otid") &"'," & KS.ChkClng(KS.S("Oid")) &"," & OrderID&")")
		End Sub
		'后台修改数据表数据
		Sub UpdateItemInfo(ChannelID,InfoID,Title,Tid,Intro,KeyWords,PhotoUrl,AddDate,Hits,HitsByDay,HitsByWeek,HitsByMonth,Recommend,Rolls,Strip,Popular,Slide,IsTop,Comment,Verific)
		 Dim SqlStr:SqlStr="Update [KS_ItemInfo] Set Title="& KS.WithKorean() &"'" & replace(Title,"'","''") & "',Tid='" & Tid & "',Intro="& KS.WithKorean() &"'" & Replace(left(KS.LoseHtml(Intro),255),"'","''")  & "',KeyWords="& KS.WithKorean() &"'" & Replace(KeyWords,"'","''") & "',PhotoUrl='" & PhotoUrl & "',AddDate='" & AddDate & "',ModifyDate=" & SQLNowString & ",Hits=" & KS.ChkClng(Hits) & ",HitsByDay=" & KS.ChkClng(HitsByDay) & ",HitsByWeek=" & KS.ChkClng(HitsByWeek) & ",HitsByMonth=" & KS.ChkClng(HitsByMonth) & ",Recommend=" & KS.ChkClng(Recommend) & ",Rolls=" & KS.ChkClng(Rolls) & ",Strip=" & KS.CHkClng(Strip) & ",Popular=" & KS.ChkClng(Popular) & ",Slide=" & KS.ChkClng(Slide) & ",IsTop=" & KS.ChkClng(IsTop)  &",Comment=" & KS.ChkClng(Comment) &",Otid='" & KS.G("Otid") &"',oid=" & KS.ChkClng(KS.S("Oid")) &""
		 if Verific<>100 Then SqlStr=SqlStr &",Verific=" & KS.CHkClng(Verific)
		 SqlStr=SqlStr & " Where ChannelID=" & KS.ChkClng(ChannelID) & " and InfoID=" & KS.ChkClng(InfoID)
		 Gcls.Execute(SqlStr)
		End Sub		
		...
		Function GetPrevNextURL(ChannelID,NowID, Tid, TypeStr,ByRef Title)
		     ...
		     Dim SqlStr,LinkUrl
		     SqlStr="SELECT Top 1 ID,Title,Tid,Fname,AddDate From " & KS.C_S(ChannelID,2) & " Where Tid='" & Tid & "' And ID" & TypeStr & NowID & " And Verific=1 and  DelTF=0 Order By ID"
			 ...
			 Else
			  If Fcls.CallFrom3g="true" Then
			   LinkUrl = KS.Get3GItemURL(ChannelID,RS(2),RS(0),RS(0)&KS.Wsetting(9))
			  Else
			   LinkUrl = KS.GetItemURL(ChannelID,RS(2),RS(0),RS(3),RS(4))
			  End If
