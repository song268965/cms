...
Class RefreshFunction
		...
		Private Templates,AjaxOut,ModelID,ClassID,IncludeSubClass,oTid,IncludeoTid,SpecialID,N  rem 定义本类全局变量 其中 ModelID 模型ID ClassID 栏目ID templates临时变量 N行号
		...
		Function Parse(sTemplate, iPosBegin)
			...
			select case Lcase(sTemp)
			 ...
			 case "target" if Not KS.IsNul(Node.SelectSingleNode("@target").text) Then echo " target=""" & Node.SelectSingleNode("@target").text&""""
			 ...
			 case "photo","photourl","userface","bigphoto","logo" 
			  ...
			  If Left(Lcase(PhotoUrl),4)<>"http" and left(PhotoUrl,1)<>"/" Then PhotoUrl=KS.Setting(3) & PhotoUrl
			  If Left(Lcase(PhotoUrl),4)<>"http" Then PhotoUrl=KS.Setting(2) & PhotoUrl
			  echo PhotoUrl
			 case "groupbuyurl" 
			   If Node.SelectSingleNode("@changes").text="1" Then
			     Echo Node.SelectSingleNode("@changesurl").text
			   Else
			    If KS.ChkClng(KS.Setting(179))=1 Then echo KS.GetDomain & "groupbuy/show-" & Node.SelectSingleNode("@id").text & ".html" Else Echo KS.GetDomain & "shop/groupbuyshow.asp?id=" & Node.SelectSingleNode("@id").text
			   End If
			 case "groupbuycarturl" 
			  If Node.SelectSingleNode("@changes").text="1" Then
			     Echo Node.SelectSingleNode("@changesurl").text
			   Else
			   If KS.ChkClng(KS.Setting(179))=1 Then echo KS.GetDomain & "groupbuy/cart-" & Node.SelectSingleNode("@id").text & ".html" Else Echo KS.GetDomain & "shop/GroupBuycart.asp?id=" & Node.SelectSingleNode("@id").text
			   End If
			 ...
			 case "sjurl" 
			  ...
			  Else
			  echo DomainStr & KS.C_S(9,8) & "sj/" & Node.SelectSingleNode("@id").text & ".htm"
			  End If
			 case "sjxdurl" echo DomainStr & "mnkc/exam/xd.asp?id=" & Node.SelectSingleNode("@sjid").text
			 ...
			 case "class3gurl" 
			  if Node.SelectSingleNode("@classid")=null Then
			    echo KS.Setting(3) & KS.WSetting(4) &"/list.asp?id="&KS.C_C(Node.SelectSingleNode("@tid").text,9)
			  Else
			   echo KS.Setting(3) & KS.WSetting(4) &"/list.asp?id="&Node.SelectSingleNode("@classid").text
			  End If
			 ...
			 case "currclass"
			    If FCls.RefreshFolderID=Node.SelectSingleNode("@tid").text Then
					 echo  " class=""currclass"""
				 ElseIf  Split(KS.C_C(Fcls.RefreshFolderID,8)&",",",")(0)=Node.SelectSingleNode("@tid").text then
					echo  " class=""currclass"""
				 Else
					 echo  ""
				 End If
			 ...
			 case "oclassname" echo KS.C_C(Node.SelectSingleNode("@otid").text,1)
			 case "oclassurl" echo KS.GetFolderPath(Node.SelectSingleNode("@otid").text)
			 case "otopclassname" echo KS.C_C(split(KS.C_C(Node.SelectSingleNode("@otid").text,8),",")(0),1)
			 case "otopclassurl" echo KS.GetFolderPath(split(KS.C_C(Node.SelectSingleNode("@otid").text,8),",")(0))
			 ...
			 case "vipprice","price_member","price_market","limitbuyprice" echo KS.GetPrice(Node.SelectSingleNode("@" & lcase(sTemp)).text)
			 case "price" if isnumeric(Node.SelectSingleNode("@price").text) then echo KS.GetPrice(Node.SelectSingleNode("@price").text) else echo Node.SelectSingleNode("@price").text
			 ...
			 case "keywordsbylink"
			      dim kn,keyarr:keyarr=split(Node.SelectSingleNode("@keywords").text,",")
				  for kn=0 to ubound(keyarr)
				     if kn=0 then
				     echo "<a href=""" & KS.TagsUrl(keyarr(kn),0,0,0) &""" target=""_blank"" title=""" &keyarr(kn) & """>" & trim(keyarr(kn)) &"</a>"
					 else
				     echo ",<a href=""" & KS.TagsUrl(keyarr(kn),0,0,0) &""" target=""_blank"" title=""" &keyarr(kn) & """>" & trim(keyarr(kn)) &"</a>"
					 end if
				  next
			 ...
			 case "title","xcname","specialname"
			  Dim temptitle
			 If KS.C_S(ModelID,6)="1" and Lcase(sTemp)="title" Then
			 ...
			 case "link3gurl" '3g版内容链接URL
			     If ModelID=0 Then
				    echo KS.Get3GItemURL(Node.SelectSingleNode("@channelid").text,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@id").text & KS.WSetting(9))
				 Else
					echo KS.Get3GItemURL(ModelID,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@id").text & KS.WSetting(9))
				 End If
			 case "linkurl" 
			     If ModelID=0 Then
					echo KS.GetItemURL(Node.SelectSingleNode("@channelid").text,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@fname").text,Node.SelectSingleNode("@adddate").text)
				 Else
				    If ParamNode.getAttribute("callbyspace")="true" And (ModelID=2 Or ModelID=5) And Not KS.IsNul(Session("SpaceUserID")) Then
					 ...
					 End If
					Else
				    echo KS.GetItemURL(ModelID,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@fname").text,Node.SelectSingleNode("@adddate").text)
					End If
				 End If
			 case "url":
			    If LabelFunName="GetLinkList" Then
			      If ParamNode.getAttribute("show")="0" Then  echo Node.SelectSingleNode("@url").text Else echo DomainStr & "plus/link/To?" & Node.SelectSingleNode("@linkid").text
				Else
				   echo Node.SelectSingleNode("@url").text
				End If
              case "downaddress"
				 Dim DUrlArr,DI,DN,DTotalNum, DAUrl
				 DUrlArr = Split(Node.SelectSingleNode("@downurls").text, "|||")
				 DTotalNum = UBound(DUrlArr)
				 For DI = 0 To DTotalNum
				    DN=DN+1: DAUrl = Split(DUrlArr(DI),"|")
						If DAUrl(0)=0 Then
						  echo "<img src="""& DomainStr & "Images/Default/down.gif"" border=""0"" alt="""" align=""absmiddle"" /><a href=""" & DomainStr & "item/downLoad.asp?m=" & FCls.ChannelID & "&id=" & Node.SelectSingleNode("@id").text & "&downid=" & DN & """ target=""_blank"">" & DAUrl(1) & "</a>"      
						  If DI<>DTotalNum Then echo " "
					  Else
						  Dim RS_DS:Set RS_DS=Conn.Execute("Select DownloadName,IsDisp,DownloadPath,DownID,SelFont From KS_DownSer Where ParentID=" & DAUrl(0))
						  If RS_DS.Eof Then
						    If DTotalNum=0 Then echo " 暂不提供下载地址 "
						  Else
						    DO While Not RS_DS.Eof
								  IF RS_DS(1)=1 Then
							      echo "<img src="""& DomainStr & "Images/Default/down.gif"" border=""0"" align=""absmiddle""><a href=""" & RS_DS(2) & DAurl(2) & """ " & RS_DS(4)&" target=""_blank"">" & RS_DS(0) & "</a>"          
								  Else
							      echo "<img src="""& DomainStr & "Images/Default/down.gif"" border=""0"" align=""absmiddle""><a href=""" & DomainStr & "item/DownLoad.asp?m=" & FCls.ChannelID & "&id=" & Node.SelectSingleNode("@id").text & "&DownID=" & DN & "&Sid=" & RS_DS(3) & """ " & RS_DS(4)&" target=""_blank"">" & RS_DS(0) & "</a>"        
								  End If
								  RS_DS.MoveNext
								  IF Not RS_DS.Eof Or DI<>DTotalNum Then echo " "
							  Loop
						  End If
						  Set RS_DS=Nothing
						End If
				 Next			 
			case else 
			   Dim TArr,FieldName,FieldParamArr,Length,FieldType,FieldValue,BFieldName,BChannelID
			   If Instr(sTemp,"(")<>0 Then
				   Tarr=Split(sTemp,"(")
				   FieldName=lcase(Tarr(0))
				   If Instr(FieldName,"->")<>0 Then  '绑定模型的字段
					 BFieldName=split(FieldName,"->")(1)
				     FieldName=split(FieldName,"->")(0)
					 BChannelID=KS.ChkClng(Node.SelectSingleNode("@" & FieldName&"_channelid").Text)
				   End If
				   
				   Set MyNode=Node.SelectSingleNode("@" & FieldName)
				   If Not MyNode Is Nothing Then 
				       FieldValue=MyNode.Text
				       If Not KS.IsNul(BFieldName) and BChannelID>0 and KS.ChkClng(FieldValue)>0 Then
					         Dim RSB:Set RSB=Conn.Execute("select top 1 * From " & KS.C_S(BChannelID,2) & " Where ID=" & KS.ChkClng(FieldValue))
							  If Not RSB.Eof Then
								if BFieldName="url" then
								 FieldValue= KS.GetItemURL(BChannelID,RSB("Tid"),KS.ChkClng(FieldValue),RSB("Fname"),RSB("AddDate"))
								Else
								 FieldValue= RSB(BFieldName)
								End If
							  End If
		...
		'加载标签必要参数
		Sub LoadLabelParam()
		     LabelParamStr=replace(replace(LabelParamStr,"<","&lt;"),">","&gt;")
			 ...
			 oTid            = ParamNode.getAttribute("otid")
			 IncludeoTid     = ParamNode.getAttribute("includeotid")
		...
		Sub LoadSQLParam()
			...
			If ClassID="-2" Then ClassID=Split(KS.C_C(FCls.RefreshFolderID,8),",")(0)
			...
			If oTid <> "0" Then  
			  If oTid="-1" Then
			    Param= Param & " And I.oTid In (" & KS.GetFolderTid(FCls.RefreshFolderID) & ")" 
			  ElseIf oTid="-2" Then
			    Param= Param & " And I.oTid In (" & KS.GetFolderTid(FCls.RefreshCurrOtid) & ")" 
			  ElseIf oTid="-3" Then
			    Param= Param & " And I.oid =" & KS.ChkClng(FCls.RefreshInfoID)
			  ElseIf Cbool(IncludeoTid)=True Then
			    Param= Param & " And I.oTid In (" & KS.GetFolderTid(oTid) & ")" 
			  Else
			   Param= Param & " And I.oTid='" & oTid & "'"
			  End If
			End If
		...
		Public Sub LoadField(ByVal ModelID,ByVal PrintType,ByVal PicStyle,ByVal ShowPicFlag,ByRef FieldStr,ByRef TableName,ByRef Param)
		    If ModelID="0" Then 
			 ...
			 FieldStr  = "I.ChannelID,I.InfoID as ID,I.Title,I.Tid,I.OTid,I.Intro,I.PhotoUrl,I.AddDate,I.Inputer,I.Popular,I.Fname,I.Hits,0 as CmtNum,I.KeyWords"
		   Else 
			 TableName=KS.C_S(ModelID,2)
			 Select Case KS.C_S(ModelID,6) 
			  Case 1 
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits,I.CmtNum,I.IsVideo"
			   FieldStr=FieldStr & ",I.TitleType,I.TitleFontColor,I.TitleFontType"
			   If PrintType>=2 Then  FieldStr=FieldStr & ",I.fulltitle as fulltitles,I.PhotoUrl,I.Intro,I.Origin,I.Author,I.KeyWords" 
			   If PrintType>=3 Then  FieldStr=FieldStr & ",I.ReadPoint"
			  Case 2  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits,I.CmtNum,I.Score"
			   If PrintType>=2 Then FieldStr=FieldStr & ",I.PhotoUrl,I.PictureContent As Intro,I.Origin,I.Author,I.PicNum,I.KeyWords"
			   If PrintType>=3 Then FieldStr=FieldStr & ",I.ReadPoint"
			  Case 3  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits,I.CmtNum"
			   If PrintType>=2 Then FieldStr=FieldStr & ",I.PhotoUrl,I.DownContent As Intro,I.DownSize,I.Rank,I.Origin,I.Author,I.DownUrls"
			   If PrintType>=3 Then FieldStr=FieldStr & ",I.ReadPoint"
			  Case 4  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits,I.CmtNum"
			   If PrintType>=2 Then FieldStr=FieldStr & ",I.PhotoUrl,I.FlashContent As Intro,I.Origin,I.Author,I.Rank,I.KeyWords"
			   If PrintType>=3 Then FieldStr=FieldStr & ",I.ReadPoint"
			  Case 5  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits,I.CmtNum,I.unit"
			   If PrintType>=2 Then FieldStr=FieldStr & ",I.PhotoUrl,I.ProIntro As Intro,I.BigPhoto,I.Price_member,I.Price_Member as Price,I.Price as Price_Market,I.Promodel,I.BrandID,I.ProID,I.LimitBuyPrice,I.VipPrice,I.IsLimitBuy,I.KeyWords"
			  Case 7  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits,I.CmtNum"
			   If PrintType>=2 Then FieldStr=FieldStr & ",I.PhotoUrl,I.MovieContent As Intro,I.KeyWords"
			  ...
			  Case 8  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits,I.CmtNum,I.TypeID,I.Price"
			   If PrintType>=2 Then FieldStr=FieldStr & ",I.PhotoUrl,I.GQContent As Intro,I.KeyWords"
			   If PrintType>=3 Or PicStyle=16 Or PicStyle=17 Then FieldStr=FieldStr & ",I.ValidDate,I.ContactMan,I.Tel,I.Address,I.Province,I.City,I.County,I.CompanyName"
			   If KS.ChkClng(ParamNode.getAttribute("typeid"))<>0 Then Param =Param & " And I.TypeID="&KS.ChkClng(ParamNode.getAttribute("typeid"))
			  Case Else  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.OTid,I.AvgScore,I.PhotoUrl,I.AddDate,I.Inputer,I.Popular,I.Fname,I.Hits,I.CmtNum,I.KeyWords"
			 ...
			 For K=0 To TotalNum-1
				 ...
				 For I = 1 To Col
						 ...
						 If Cbool(HotTF)=True And Node.SelectSingleNode("@popular").text=1 Then HotImgStr="<img src=""" & DomainStr & "images/hot.gif"" border=""0""/>" Else HotImgStr=""
						  ...
						  LinkUrl=KS.GetItemURL(CurrModelID,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@fname").text,Node.SelectSingleNode("@adddate").text)
			...
			if FieldXML.readystate=4 and FieldXML.parseError.errorCode=0 Then 
						  ...
						  If DiyNode.Length>0 Then
								For Each N In DiyNode
			                        ...
			                        If N.SelectSingleNode("fieldtype").text="14" then Tstr=Tstr &",I." & N.SelectSingleNode("@fieldname").text&"_channelid"
								Next
								Set N=nothing
						  End If
			End If
				 ...
				 For I = 1 To Col
				      Set Node=DocNode.Item(n)
					   ...
					   ElseIf ModelID=8 And (PrintType>=3 Or PicStyle=16 Or PicStyle=17) Then
					     Dim ProCity,Province,City,County
						 Province = Node.SelectSingleNode("@province").text
						 City     = Node.SelectSingleNode("@city").text
						 County     = Node.SelectSingleNode("@county").text
						 IF Not KS.IsNul(Province) Then 
						   ProCity=Province & "/" & City 
						   If Not KS.IsNul(County) Then ProCity=Province & "/" & City  &"/" & County
						 Else 
						   ProCity="地区不限"
						 End If
					   End If
					  ...
					  select case PicStyle
					   ...
					   case 13,14,15
						   ...
						   echoln "   <div class=""btn""><a href=""" & DomainStr & "movie/play/?" & Node.SelectSingleNode("@id").text & "|0"" target=""_blank""><img src=""" & DomainStr & "images/guankan.gif"" border=""0"" alt=""观看"" /></a> <a href=""" & LinkUrl & """ target=""_blank""><img src=""" & DomainStr & "images/xianqin.gif"" border=""0"" alt=""详情"" /></a></div>"
		...
		Function GetPriceStr(PriceType,Price_Market,Price_Member)
		      If Price_Market=0 Then Price_Market="—" Else Price_Market="￥"&KS.GetPrice(Price_Market)
			  If Price_Member=0 Then Price_Member="—" Else Price_Member="￥"&KS.GetPrice(Price_Member)
	  ...
	  End Function
		...
		Function GetSlide(LabelStyle)
			 ...
			 Dim SqlStr,TableName,FieldStr,ClassID
			 If ModelID="0" Then 
			  TableName = "[KS_ItemInfo]" : FieldStr="I.InfoID as ID,I.Title,I.ChannelID,I.Tid,I.PhotoUrl,I.Fname,I.Intro,I.AddDate"
			  Param = Param & " and PhotoUrl<>''"
			 ElseIf ModelID="-1000" Then
			  ClassID=ParamNode.getAttribute("classid")
			  TableName = "[KS_GuestBook]" : FieldStr="id,Subject as title,-1000 as channelid,0 as  tid,face as photourl,id as fname,subject as intro,addtime as adddate" : Param=" Where verific=1 and isslide=1 and deltf=0"
			  if classid<>0 then
			    if instr(classid,",")=0 then
				 Param=Param & " and bordid=" & classid
				else
				 Param=Param & " and boardid in(" & KS.FilterIds(classid) &")"
				end if
			  end if
			 ElseIf ModelID="-1001" Then
			  TableName = "[KS_BlogInfo]" : FieldStr="id,Title,-1001 as channelid,0 as tid,photourl,userid,title as intro,adddate" : Param=" Where status=0 and photourl<>'' and isslide=1"
			  ClassID=ParamNode.getAttribute("classid")
			  if classid<>0 then
			    if instr(classid,",")=0 then
				 Param=Param & " and typeid=" & classid
				else
				 Param=Param & " and typeid in(" & KS.FilterIds(classid) &")"
				end if
			  end if
			 ElseIf ModelID="-1002" Then
			  TableName="[KS_Special]" :FieldStr="SpecialID as id,SpecialName AS Title,-1002 as channelid,0 as tid,photourl,SpecialNote as intro,SpecialEname,FsoSpecialIndex,SpecialAddDate as adddate" : Param=" Where slide=1 and photourl<>''"
			  ClassID=ParamNode.getAttribute("classid")
			  if classid<>0 then
			    if instr(classid,",")=0 then
				 Param=Param & " and ClassID=" & classid
				else
				 Param=Param & " and ClassID in(" & KS.FilterIds(classid) &")"
				end if
			  end if
			 ElseIf ModelID="-1003" Then
			  TableName="[KS_Advertise]" :FieldStr="id,sitename AS Title,-1003 as channelid,0 as tid,gif_url as photourl,intro,url,[time] as adddate" : Param=" Where 1=1"
			  ClassID=ParamNode.getAttribute("classid")
			  if classid<>0 then
			    if instr(classid,",")=0 then
				 Param=Param & " and place=" & classid
				else
				 Param=Param & " and place in(" & KS.FilterIds(classid) &")"
				end if
			  end if
			 Else
			  TableName = KS.C_S(ModelID,2) : FieldStr="I.ID,I.Title,I.Tid,I.PhotoUrl,I.Fname,I.AddDate"
			  Select Case  KS.C_S(ModelID,6)
			   case 1  
			     FieldStr=FieldStr & ",I.Intro"
			     Param=Param & " and I.PicNews=1"
			   case 2
			     FieldStr=FieldStr & ",I.PictureContent as intro"
			   case 3
			     FieldStr=FieldStr & ",I.DownContent as intro"
			   case 4
			     FieldStr=FieldStr & ",I.FlashContent as intro"
			   case 5
			     FieldStr="I.ID,I.Title,I.Tid,I.BigPhoto as PhotoUrl,I.Fname,I.ProIntro as intro"
			   case 8
			     FieldStr=FieldStr & ",I.GQContent as intro"
			   case 7
			     FieldStr=FieldStr & ",I.MovieContent as intro"
			   case Else
			     FieldStr=FieldStr & ",I.title as intro"
			  End Select
			 End If
			 SqlStr = "SELECT TOP " & Num & " " & FieldStr & " FROM " & TableName & " I " & Param 
			 if modelid<0 or KS.IsNul(ParamNode.getAttribute("orderstr")) then
			   SqlStr=SqlStr & " ORDER BY I.ID Desc"
			 else
			   SqlStr=SqlStr & " ORDER BY " & GetOrderParam() 
			 end if
			 Dim RS:Set RS=Conn.Execute(SqlStr)
			 If Not RS.Eof Then Set XMLSql=KS.RsToXml(RS,"row","root") Else XMLSql=Empty
			 RS.Close:Set RS=Nothing
		...
		Function ExplainSlideLabelBody()
		     Dim SlideType,T_Len,Intro_Len,Width,Height,ShowTitle,delay,O_T_S,T_CssStr,txtHeight,LabelID
		     Dim DocNode,TotalNum,K,Title,TempTitle,PhotoUrl,LinkUrl,ArrLength,trigger,auto,wrap,loadIMGTimeout
			 Set DocNode=XMLSql.DocumentElement.SelectNodes("row")
		     TotalNum=DocNode.length
			 SlideType = ParamNode.getAttribute("slidetype") : If KS.IsNul(SlideType) or isnumeric(SlideType) Then SlideType="mF_sohusports"
			 T_Len     = KS.ChkClng(ParamNode.getAttribute("titlelen")) 
			 Intro_Len = KS.ChkClng(ParamNode.getAttribute("introlen")) : If Intro_Len=0 Then Intro_Len=200
			 ...
			 trigger   = ParamNode.getAttribute("trigger") : If KS.IsNUL(trigger) Then trigger="click"
			 wrap      = ParamNode.getAttribute("wrap") : If KS.IsNUL(wrap) Then wrap="true"
			 auto      = ParamNode.getAttribute("auto") : If KS.IsNUL(auto) Then auto="true"
			 loadIMGTimeout= ParamNode.getAttribute("loadimgtimeout")
			 trigger   = ParamNode.getAttribute("trigger") : If KS.IsNUL(trigger) Then trigger="click"
			 delay= ParamNode.getAttribute("delay")
			 O_T_S     = KS.G_O_T_S(ParamNode.getAttribute("opentype"))
			 txtHeight = ParamNode.getAttribute("txtheight")
			 LabelID   = ParamNode.getAttribute("labelid")
			 Templates = ""
			 
					 Dim ImgArrStr,LinkArrStr,TextArrStr,IntroArrStr
					 N=0
					 For K=0 To TotalNum-1
					      Set Node=DocNode.Item(n)
					      Title         = Node.SelectSingleNode("@title").text
						  ...
						  ElseIf Cint(ModelID)=-1001 Then
						      LinkUrl=GetLogUrl()
						  ElseIf Cint(ModelID)=-1002 Then
						      LinkUrl=KS.GetSpecialPath(Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@specialename").text,Node.SelectSingleNode("@fsospecialindex").text)
						  ElseIf Cint(ModelID)=-1003 Then
						  ...
						  TempTitle     = KS.Gottopic(Title,T_Len)
                          if N=0 Then
						    ImgArrStr=PhotoUrl : LinkArrStr=LinkUrl :  TextArrStr=TempTitle :IntroArrStr=KS.Gottopic(KS.LoseHtml(Node.SelectSingleNode("@intro").text),Intro_Len)
					      Else
						   ImgArrStr=ImgArrStr & "@@@" & PhotoUrl
						    LinkArrStr=LinkArrStr & "@@@" & LinkUrl
						   TextArrStr=TextArrStr & "@@@" & TempTitle
						   IntroArrStr=IntroArrStr &"@@@" & KS.Gottopic(KS.LoseHtml(Node.SelectSingleNode("@intro").text&"")&"",Intro_Len)
						  End if
						  N=N+1
                    Next
					 ...
					 Dim IntroArr:IntroArr=Split(IntroArrStr&"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@","@@@")

					 echoln "<script>!window.myFocus && document.write('<script src="""&DomainStr&"ks_inc/myFocus/myfocus-2.0.4.min.js""><\/script>');</script>"	
					 echoln "<script type=""text/javascript"">"
					 echoln "myFocus.set({"
					 echoln "	id:'myFocus" & LabelID &"',//ID"
					 echoln "	pattern:'" & SlideType & "',//风格"
					 echoln "   trigger:'" & trigger &"',"
					 echoln "   wrap:'" & wrap &"',"
					 echoln "   auto:'" & auto &"',"
					 if delay<>"" then echoln "delay:" & delay &","
					 if loadIMGTimeout<>"" then echoln " loadIMGTimeout:" & loadIMGTimeout &","
					 if txtHeight<>"" then echoln "	txtHeight:" & txtHeight &","
					 echoln "	width:" & Width & ","
					 echoln "	height:" & Height 
					 echoln "});"
					 echoln "</script>"
	
					 echoln "<div id=""myFocus" & LabelID &""">"
					 echoln "<div class=""loading""></div>"
					 echoln " <div class=""pic""><!--图片列表-->"
					 echoln "	<ul>"
					   Dim i
					    For I=0 To Ubound(ImgArr)
							echoln "<li><a href=""" & LinkArr(i) & """" & O_T_S&" title=""" & textArr(i) & """><img onerror=""this.src='" & KS.GetDomain&"images/nopic.gif';"" style=""width:" & width &"px;height:" & height &"px"" src=""" & ImgArr(i) &""" thumb="""" alt=""" & textArr(i) & """ text=""" & introArr(i) & """ /></a></li>"
						Next
					 ...
					 if wrap="false3" then
					  echoln "<script>"
					  'echoln "$(document).ready(function(){jQuery(""." &SlideType &"_wrap"").removeClass(""" &SlideType &"_wrap"");})"
					 ' echoln "$(document).ready(function(){jQuery(""#myFocus" & LabelID &""").parent().removeClass(""" &SlideType &"_wrap"");})"
					  echoln "</script>"
					 end if
		...
		Function GetRolls(LabelStyle)
			...
			If ModelID="0" Then 
			 TableName = "[KS_ItemInfo]" 
			 FieldStr  = "I.ChannelID,I.InfoID as id,I.Title,I.Tid,I.PhotoUrl,I.Fname,I.AddDate"
			 Param = Param & " and I.PhotoUrl<>''"
			Else 
			 TableName=KS.C_S(ModelID,2)
			 FieldStr  = "I.ID,I.Title,I.Tid,I.PhotoUrl,I.Fname,I.AddDate"
	   ...
	   Function ExplainRollsLabelBody()
	         Dim TempPicStr, T_CssStr,Title,T_Css,TempTitleStr,T_Len, O_T_S,M_Dir,LinkUrl,LinkAndPicStr,Marqueebgcolor,ShowTitle
		     ...
		     If MoreLink <> "" And ClassID <> "0" And C_F_T = False Then M_L_S = KS.GetMoreLink(1,1, R_H, MoreType, MoreLink, KS.GetFolderPath(ClassID), O_T_S)
			   ...
			   End If
				...
				For K=0 To TotalNum-1
					...
					If ModelID=0 Then CurrModelID=Cint(Node.SelectSingleNode("@channelid").text) Else CurrModelID=ModelID
					LinkStr=T_CssStr & " href=""" & KS.GetItemURL(CurrModelID,Node.SelectSingleNode("@tid").text,Node.SelectSingleNode("@id").text,Node.SelectSingleNode("@fname").text,Node.SelectSingleNode("@adddate").text) & """" & O_T_S & " title=""" & Title & """"
		...
		Function GetCirClassList(LabelStyle)
		     ...
		     LabelID   = ParamNode.getAttribute("labelid")
             ClassID   = ParamNode.getAttribute("classid")
			 ModelID   = KS.ChkClng(ParamNode.getAttribute("modelid"))
			 ...
			 if modelid>0 then ClassParam=ClassParam & " and channelid=" & modelid
			 ...
			 SQLStr="Select Top 100 ID From KS_Class " & ClassParam & " Order By root,folderorder"
			   ...
			   If ClassPrintType=2 Then
				   ...
				   For Each ClassNode In classXml.DocumentElement.SelectNodes("row")
					 ...
					 If Matches.count > 0 Then LabelParamStr=Replace(LabelParamStr,Matches.item(0),"classid=""" & ID & """")
						 ...
						 LoopClassStyle = Replace(LoopClassStyle,"{@parentid}",KS.C_C(ID,13))
						 LoopClassStyle = Replace(LoopClassStyle,"{@parentclassid}",KS.C_C(KS.C_C(ID,13),9))
						 ...
						 LoopClassStyle = Replace(LoopClassStyle,"{@class3gurl}",KS.Setting(3) & KS.WSetting(4) &"/list.asp?id="&KS.C_C(ID,9))
						 LoopClassStyle = Replace(LoopClassStyle,"{@parentclassurl}",KS.GetFolderPath(KS.C_C(ID,13)))
		...
		Function GetPageList(LabelStyle)
		 If FCls.RefreshType = "Folder" Or FCls.RefreshType="Special" Then
				  ...
				  If (FCls.RefreshType = "Folder" And (Fcls.CallFrom3g="true" or KS.C_S(FCls.ChannelID,7)="0" or KS.C_S(FCls.ChannelID,7)="2" Or KS.C_C(FCls.RefreshFolderID,3)="2")) Or (FCls.RefreshType = "Special" And KS.Setting(78)="0") Then
				   Application(KS.SiteSN&"PageParam")=LabelParamStr:Application("LabelStyle")=LabelStyle : GetPageList="{Tag:Page}": Exit Function
				  End If
					...
					If KS.ChkClng(FCls.FsoListNum)<>0 And KS.ChkClng(FCls.FsoListNum)<PageNum Then EndPageNum=KS.ChkClng(FCls.FsoListNum)				
					  ...
					  For CurrPage = 1 To EndPageNum
						 ...
						 If FCls.RefreshType="Folder" And EndPageNum>5 And CurrPage Mod 2=0 Then
							KS.Echo "<script>input.innerHTML=('正在生成栏目""<font color=red>" & KS.C_C(FolderID,1) & "</font>"",本栏目共有<font color=red>" & EndPageNum & "</font>个分页需要生成,正在获取第<font color=red>" & CurrPage & "</font>个分页数据...');</script>" &vbcrlf
							Response.Flush()
						 End If
						 
					   If RS.Eof Then Exit For
					 Next
					 If FCls.RefreshType="Folder" And EndPageNum>5 Then KS.Echo "<script>input.innerHTML=('获取分页数据完毕,分页生成中...');</script>"
					 RS.Close:Set RS = Nothing
		...
		Function GetSpecialList(ByVal LabelStyle)
			 ...
			 Dim Slide:Slide= ParamNode.getAttribute("slide"): if KS.IsNul(Slide) Then Slide=false
		  ...
		  End If
		    Param=" Where 1=1"
			If ClassID<>0 Then Param=Param & " and S.ClassID=" & ClassID
			If Cbool(Recommend)=true Then Param=Param & " and Recommend=1"
			If Cbool(Slide)=true Then Param=Param & " and slide=1"
		...
		Function GetLastSpecialList(LabelStyle)
			 ...
			 If FCls.FromAspPage=True Then 
			  	   Application(KS.SiteSN&"PageParam")=LabelParamStr
			 ...
			 If FCls.RefreshType = "ChannelSpecial" Then 
				  ...
				  PerPageNum=KS.ChkClng(PerPageNum)
				   Dim N,PageNum, CurrPage,TempStr
				    if (TotalPut mod PerPageNum)=0 then
							PageNum = TotalPut \ PerPageNum
				    else
					...
					Dim EndPageNum:EndPageNum=PageNum
					  For CurrPage = 1 To EndPageNum
						 RS.Move (CurrPage - 1) * PerPageNum,1
						 Set XMLSQL=KS.ArrayToXml(RS.GetRows(PerPageNum),rs,"row","root")
					     TempStr = TempStr & ExplainSpecialListLabelBody(LabelStyle) & "{KS:PageList}" '加上分页符
					   If RS.Eof Then Exit For
					 Next
					 RS.Close:Set RS = Nothing
					 FCls.PageList=TempStr
		   ...
		   Else
		    GetLastSpecialList="此标签只能放在专题分类页模板!"
		   End If
		End Function
		...
		Function GetNavigation(LabelStyle)
			 ...
			 Else 
				 Set ParamNode=XMLDoc.DocumentElement.SelectSingleNode("param")
			 End If
             Dim ChannelID,SQLStr,NavType,Nav,SplitPic,Col, OpenType,O_P_T,T_Css,P_T,DivID,DivCss,UlID,ULCss,LiID,LICss,FieldStr,CallFrom
			 ...
			 CallFrom    = KS.ChkClng(ParamNode.getAttribute("callfrom"))
			 If Cint(P_T)=3 Then
			  FieldStr="ClassID,ID,ID AS Tid, FolderName,TS,ClassBasicInfo,folder,Target"
			 Else
			  FieldStr="ClassID,ID,FolderName,TS,folder,Target"
			 End If
			 Dim ShowTopField:ShowTopField="TopFlag"
			 If CallFrom=1 Then ShowTopField="A.WapSwitch"
			 select case channelid
			   case "0" SqlStr = "Select " & FieldStr & ",TN,FolderOrder From KS_Class A Inner Join KS_Channel B On A.ChannelID=B.ChannelID Where B.ChannelStatus=1 and TN='0' AND " & ShowTopField & "=1 And DelTF=0 Order By root,FolderOrder"
			   case "9999" 
			       if FCls.RefreshFolderID="0" then
					   SqlStr = "Select " & FieldStr & ",TN,FolderOrder From KS_Class A inner join KS_Channel b on a.channelid=b.channelid Where B.ChannelStatus=1 and TN='0' AND " & ShowTopField & "=1 And DelTF=0 Order By root,FolderOrder"
				   else
					    SqlStr = "Select " & FieldStr & " From KS_Class A Inner Join KS_Channel B On A.ChannelID=B.ChannelID Where B.ChannelStatus=1 And TN='" & FCls.RefreshFolderID & "' And " & ShowTopField & "=1 And DelTF=0  Order BY root,FolderOrder"
					end if
			   case "9998"
					  Dim Rst ,ParentID
					  ...
					  SqlStr = "Select " & FieldStr & " From KS_Class A Inner Join KS_Channel B On A.ChannelID=B.ChannelID Where B.ChannelStatus=1 "
					  If KS.IsNul(ParentID) and Fcls.CallFrom3g="true" Then
					   SqlStr=SqlStr & " And tj=2 and a.classtype=1 and A.ChannelID=" & KS.ChkCLng(FCls.ChannelID)
					  Else
					   SqlStr=SqlStr & " And TN='" & ParentID & "'"
					  End If
					  SqlStr=SqlStr & " And " & ShowTopField & "=1 and DelTF=0  Order BY root,FolderOrder"
			    ...
			    case else
			        If Len(ChannelID)<=3 Then
						SqlStr = "Select " & FieldStr & " From KS_Class A where TN='0' And ChannelID=" & ChannelID & " AND " & ShowTopField & "=1 And DelTF=0  Order BY root,FolderOrder"
					Else
					    SqlStr = "Select " & FieldStr & " From KS_Class A Inner Join KS_Channel B On A.ChannelID=B.ChannelID Where B.ChannelStatus=1 And " & ShowTopField & "=1 And TN='" & ChannelID & "' And DelTF=0 Order BY root,FolderOrder"
			 ...
			 If IsObject(XML) Then
                     ...
                     ElseIf Cint(P_T)=2 Then
						  ...
						  For Each Node In XML.DocumentElement.SelectNodes("row")
							 ...
							 If O_P_T="" And Not KS.IsNul(Node.SelectSingleNode("@target").text) Then O_P_T=" target=""" & Node.SelectSingleNode("@target").text&""""
		...
		Function GetAskZJList(LabelStype)
			 ...
			 SqlStr="Select TOP " & num & " UserID,UserName,RealName,QQ,AddDate,AskDoneNum,UserFace,Tel,Province,City,County,Intro, '' as [Domain],BigClassID,SmallClassID From KS_AskZJ " & Param
			If DataBaseType=1 Then
		...
		Function GetSJxdList(LabelStyle)
			 ...
			 DateRule  = ParamNode.getAttribute("daterule")
			 Num       = ParamNode.getAttribute("num")
			 OrderStr  =ParamNode.getAttribute("orderstr") : If OrderStr="" Then OrderStr="a.id desc"
			 Param="Where a.verific=1"
			 If Instr(lcase(orderStr),"hits")<>0 Then Param=Param & " order by " & OrderStr & ",a.id desc" Else Param=Param &" order by " & OrderStr
			 If LabelID<>"ajax" and Cbool(ParamNode.getAttribute("ajaxout"))=true Then 
			  GetSJList="<span id=""ks" & LabelID & "_0_0_0_0""></span>":Exit Function
			 End If
			 Dim SqlStr
			 
			 SqlStr="Select TOP " & num & " b.id,a.SJID,a.SJname,a.username,a.Content,a.adddate,b.title,a.userip From KS_sjxd a inner join ks_sj b on a.sjid=b.id " & Param
			...
			If IsObject(XMLSql) Then
			 Set DocNode=XMLSql.DocumentElement.SelectNodes("row")
			 GetSJxdList=ExplainDiyStyle(LabelStyle,DocNode.length)
			End If 
			Set Node=Nothing
		End Function
		...
		Function GetSpaceList(LabelStyle)
			...
			Dim ShowType:ShowType = KS.ChkClng(ParamNode.getAttribute("showtype"))
			Dim recommend:recommend=ParamNode.getAttribute("recommend"):If KS.IsNul(recommend) Then recommend=false
			...
			ElseIf ShowType=1 Then
			 Param=" inner join KS_User U on B.UserName=U.UserName Where B.Status=1 And U.UserType=0" 
			Else
		...
		Function GetBlogInfoList(LabelStyle)
			 ...
			 Dim UserName,TypeID,OrderStr,isbest,ShowFlag,recommend,isslide
			 ...
			 isslide   = ParamNode.getAttribute("isslide") : If isslide="" Then isslide=false
			 recommend = ParamNode.getAttribute("recommend") : If recommend="" Then recommend=false
			 ...
			 If cbool(IsSlide)=true Then Param=Param & " And IsSlide=1"
			 If cbool(Recommend)=true Then Param=Param & " And Recommend=1"
	   ...
	   Function GetLogUrl()
	    If KS.SSetting(21)="1" Then
		  GetLogUrl = DomainStr & KS.SSetting(42) & "/list-" & Node.SelectSingleNode("@userid").text & "-" & Node.SelectSingleNode("@id").text & KS.SSetting(22)
		Else
			   ...
			   If Not RS.EOF Then
				 ...
				 Do While Not RS.EOF
				   ...
				   For I = 1 To Col
					  ...
					  TempTitle = "<a " & KS.GetCss(T_Css) & " href=""" & KS.GetItemUrl(channelid,rs("tid"),rs("id"),rs("fname"),rs("adddate")) & """" & KS.G_O_T_S(OpenType) & " title=""" & RS("Title") & """>" & TempTitle & "</a>"
					  R_H = KS.G_R_H(R_H)
					 ...
					 Case "D"
						   ...
						   TempStr = TempStr & "<td><table width=""100%"" height=""100%""><tr><td><span " & KS.GetCss(ContentCss) & ">&nbsp;&nbsp;&nbsp;&nbsp;" & KS.GotTopic(Replace(Replace(Replace(KS.LoseHtml(ArticleContent), vbCrLf, ""), "[NextPage]", ""), "&nbsp;", ""), C_Len) & "……</span></tr>" & KS.GetMoreLink(1,1, R_H, MoreType, MoreLink, KS.GetItemUrl(channelid,rs("tid"),rs("id"),rs("fname"),rs("adddate")), KS.G_O_T_S(OpenType)) & "</table></td></tr>"
						   TempStr = TempStr & KS.GetSplitPic(SplitPic, ColSpanNum)
						   TempStr = TempStr & "</table>"
					 Case "E"
						   TempStr = TempStr & "<table width=""100%"" height=""100%"" cellpadding=""0"" cellspacing=""0"" border=""0"">"
						   TempStr = TempStr & "<tr><td><table width=""100%"" height=""100%""><tr><td><span " & KS.GetCss(ContentCss) & ">&nbsp;&nbsp;&nbsp;&nbsp;" & KS.GotTopic(Replace(Replace(Replace(KS.LoseHtml(ArticleContent), vbCrLf, ""), "[NextPage]", ""), "&nbsp;", ""), C_Len) & "……</span></tr>" & KS.GetMoreLink(1,1, R_H, MoreType, MoreLink, KS.GetItemUrl(channelid,rs("tid"),rs("id"),rs("fname"),rs("adddate")), KS.G_O_T_S(OpenType)) & "</table></td>"
