...
Class ClubCls
		...
		Public Sub Kesion()
		    startime=Timer()
		    If KS.Setting(56)="0" Then KS.Die "<script>alert('本频道已关闭!');location.href='../index.asp';</script>"
			...
			FileContent=Replace(FileContent,"{#ExecutTime}","页面执行" & FormatNumber((timer()-startime),5,-1,0,-1) & "秒 powered by <a href='http://www.kesion.com' target='_blank'>KesionCMS X1.5</a>")
		...
		Sub LoadClubBoardList()
		   ...
		   FileContent = KSR.LoadTemplate(KS.Setting(172))
			...
			totalPut=Node.SelectSingleNode("@topicnum").text
		...
		Sub ShowBoardList()
		     ...
		     if boardid<>0  Then
				...
				PostBtnStr="<div style=""position:relative;float:left;"" onmouseover=""$('#postlist').show()"" onmouseout=""$('#postlist').hide()""><a href=""" & KS.Setting(3) & KS.Setting(66) & "/post.asp?bid=" & boardid & """><img src=""" & KS.Setting(3) & KS.Setting(66) & "/images/button_post.png""></a><div id=""postlist"" class=""submenu noli"">"
		...
		Function Parse(sTemplate, iPosBegin)
			...
			Select Case lcase(sValue)
				...
				case "seotitle" 
				    If KS.IsNul(BSetting(70)) Then
					 echo GuestTitle
					Else
				     echo BSetting(70)
					End If
				case "seokeywords" echo BSetting(71)
				case "seodescription" echo BSetting(72)
			   ...
			   case "executtime" echo "页面执行" & FormatNumber((timer()-startime),5,-1,0,-1) & "秒 powered by <a href='http://www.kesion.com' target='_blank'>KesionCMS X1.5</a>"
			   case "showpage"
			...
			Select Case sTokenName
			  ...
			  Case "loop"
			    If IsObject(TopicXML) Then
					  If KS.ChkClng(BSetting(69))<>0 And CurrentPage<=KS.ChkClng(BSetting(69)) And Request("istop")="" And Request("isbest")="" And Request("d")="" And Request("o")="" And Request("c")="" Then
					  Dim DocNode:Set DocNode=TopicXML.DocumentElement.SelectNodes("row")
					  Dim NLen:NLen=MaxPerPage-1
					  If NLen>DocNode.Length-1 Then NLen=DocNode.Length-1
					  For I = 0 To NLen
						 Set TopicNode=DocNode.Item((currentpage-1)*MaxPerPage+i)
						 TopicID=TopicNode.SelectSingleNode("@id").text
						 scan sTemplate
					  Next
					 Else
						  ...
						  select case lcase(sTokenName)
							...
							case "subjectlist"
							   ...
							   If KS.ChkClng(BSetting(25))>0 and isobject(Application(KS.SiteSN&"_ClubBoardCategory")) Then
								...
								categoryname=CategoryNode.SelectSingleNode("@categoryname").text 
								categoryIco=CategoryNode.SelectSingleNode("@ico").text
									...
									Else
									If Instr(categoryname,"[")=0 and categoryname<>"" Then categoryname="<span class=""scategory"">[" & categoryname & "]</span>"
		...
		Sub GetLoopList()
		 ...
		 ' Application(KS.SiteSN &"TopicXML" & boardid)=""
		  If KS.ChkClng(BSetting(69))<>0 And Not IsObject(Application(KS.SiteSN &"TopicXML" & boardid)) Then
		   Call LoadNewTopic(boardid,BSetting)
		   'Application(KS.SiteSN &"TopicXML" & boardid).save(server.MapPath(boardid&".xml"))
		  End If
		  

        If KS.ChkClng(BSetting(69))<>0 And CurrentPage<=KS.ChkClng(BSetting(69)) And Request("istop")="" And Request("isbest")="" And Request("d")="" And Request("o")="" And Request("c")="" and isobject(Application(KS.SiteSN &"TopicXML" & boardid)) Then
           set TopicXML=Application(KS.SiteSN &"TopicXML" & boardid)
		   If IsObject(Application(KS.SiteSN &"TopXML")) Then 
				TotalPut=TotalPut-KS.ChkClng(Application(KS.SiteSN &"TopXML").DocumentElement.SelectNodes("row[@boardid=" & BoardID &"]").length)
           End If
		Else
		  Param=Param & " and istop=0"
		  If DataBaseType=1 Then
				...
				Set Rs=Cmd.Execute
				Set Cmd=Nothing
				If Request("istop")="" And Request("isbest")="" And Request("d")="" And Request("o")="" And Request("c")="" And IsObject(Application(KS.SiteSN &"TopXML")) Then '直接从缓存里取
				TotalPut=TotalPut-KS.ChkClng(Application(KS.SiteSN &"TopXML").DocumentElement.SelectNodes("row[@boardid=" & BoardID &"]").length)
                Else
...
Class ClubDisplayCls
		...
		Private MaxPerPage, TotalPut , CurrentPage, TotalPage, i, j, Loopno,ShowScore,IsBest,IsTop,IsReplyTips,DelTF,Verific,Subject,Hits
	    Private SqlStr,GuestTitle,AllowShow,CategoryID,CategoryNode,categoryname,startime,IsClose
		Public UserFields,PostUserName,PostUserID,BSetting,N,KSUser,LoginTF,TopicID,BoardID,AddTime
		...
		Sub ParseArea(sTokenName, sTemplate)
					Select Case sTokenName
						...
						case "replay"
							...
							if  KS.ChkClng(BSetting(91))<>0 then
							 	if datediff("d",CDate(AddTime),now())>KS.ChkClng(BSetting(91)) then
										Exit Sub
								end if
							end if
							...
							If LoginTF=true Then
							   dim nowdays
							   if KS.ChkClng(KS.U_S(KS.ChkClng(KSUser.GroupID),28))<>0 then
									nowdays=conn.execute("select count(ID) from KS_GuestReply where UserName='"& ksuser.username &"' and year(ReplayTime)=" & Year(now()) & " and month(ReplayTime)=" & month(now()) &" and day(ReplayTime)=" & day(now())  & "")(0)
									if KS.ChkClng(nowdays) > KS.ChkClng(KS.U_S(KS.ChkClng(KSUser.GroupID),28)) then
										Exit Sub
									end if
								end if
							End If
					...
					Select Case lcase(sTokenType)
					    case "item"
						  select case lcase(sTokenName)
							 ...
							 case "showbshare" 
							 if N=1 Then
							  echo "<tr><td class=""topicleft"" style=""border-bottom:none"">&nbsp;</td><td style=""padding-left:10px;height:20px""><div class=""bshare-custom icon-medium""><div class=""bsPromo bsPromo1""></div><a title=""分享到"" href=""http://www.bShare.cn/"" id=""bshare-shareto"" class=""bshare-more"">分享到</a><a title=""分享到微信"" class=""bshare-weixin"" href=""javascript:void(0);""></a><a title=""分享到QQ空间"" class=""bshare-qzone""></a><a title=""分享到新浪微博"" class=""bshare-sinaminiblog""></a><a title=""分享到天涯"" class=""bshare-tianya"" href=""javascript:void(0);""></a><a title=""分享到开心网"" class=""bshare-kaixin001"" href=""javascript:void(0);""></a><a title=""分享到Facebook"" class=""bshare-facebook"" href=""javascript:void(0);""></a><a title=""分享到Twitter"" class=""bshare-twitter"" href=""javascript:void(0);""></a><a title=""分享到Pinterest"" class=""bshare-pinterest"" href=""javascript:void(0);""></a><a title=""更多平台"" class=""bshare-more bshare-more-icon more-style-addthis""></a><span class=""BSHARE_COUNT bshare-share-count"" style=""float: none;"">252</span></div><script type=""text/javascript"" charset=""utf-8"" src=""http://static.bshare.cn/b/button.js#style=-1&amp;uuid=8a5892db-a8f6-4b91-b5dd-93753bd581aa&amp;pophcol=2&amp;lang=zh""></script><a class=""bshareDiv"" onclick=""javascript:return false;""></a><script type=""text/javascript"" charset=""utf-8"" src=""http://static.bshare.cn/b/bshareC0.js""></script></td></tr>"
							  End If
						     ...
						     case "userinfo" 
                              ...
                              If KS.ChkClng(BSetting(66))=0 Then
								   ...
								   If UN.SelectSingleNode("@issfzrz").text="1" then
									echo "<a href='" & KS.GetDomain & "space/company/rz.asp?userid=" & UN.SelectSingleNode("@userid").text & "' target='_blank'><img src='"  & KS.GetDomain & "Images/default/medal_rz.gif' align='absmiddle' title='已实名认证'/></a><br/>"
								   Else
									echo "<a href='" & KS.GetDomain & "space/company/rz.asp?userid=" & UN.SelectSingleNode("@userid").text & "' target='_blank'><img src='"  & KS.GetDomain & "Images/default/medal_norz.gif' align='absmiddle' title='未实名认证'/></a><br/>"
								   End If
								   End If
						...
						case "replay" 
							...
							case "changecategory"
							  If BSetting(23)="1" and BSetting(68)="1" and (CheckIsMaster or ksuser.username=PostUserName) Then
								   ...
								   echo "</select> "
							  Else
							       echo "<input type=""hidden"" id=""categoryid"" name=""categoryid"" value=""-1"">"
							  End If
		...
		Sub ShowTopic()
		   FileContent=RexHtml_IF(FileContent) '先过滤无用的标签,减少标签解释
			 ...
			 PostBtnStr=PostBtnStr &"</div></span>"
					   ...
					   FileContent=Replace(FileContent,"{$IsReplyTips}",IsReplyTips)
					   FileContent=Replace(FileContent,"{$PostUserName}",PostUserName)
		...
		Sub GetSubject()
		  ...
		  Else
		    Param=" Where ID=" & ID
		  End If
		  Set RST=Conn.Execute("Select top 1 ID,Verific,IsBest,IsTop,CategoryID,Subject,UserName,Hits,PostTable,PostType,ShowScore,TotalReplay,BoardID,DelTF,ChannelID,InfoID,IsClose,AddTime ,IsReplyTips From KS_GuestBook" & Param)
		  If RST.Eof Then
		  ...
		  verific  = RST("Verific"):IsBest = KS.ChkClng(RST("IsBest")):IsTop = KS.ChkClng(RST("IsTop")) : IsClose=KS.ChkClng(RST("IsClose")) : CategoryID=KS.ChkClng(RST("CategoryID")):DelTF = KS.ChkClng(RST("DelTf")):PostUserName=RST("UserName") : IsReplyTips=KS.ChkClng(RST("IsReplyTips"))
		  ...
		  If IsObject(Application(KS.SiteSN &"TopicXML" & boardid)) Then
		     If Not Application(KS.SiteSN &"TopicXML" & boardid).DocumentElement.SelectSingleNode("row[@id=" & id&"]/@hits") is nothing Then 
		        Application(KS.SiteSN &"TopicXML" & boardid).DocumentElement.SelectSingleNode("row[@id=" & id&"]/@hits").text=hits
			 End If
		   End If
