...
Class CollectPublicCls
		...
		Function GetHttpPage(HttpUrl,CharsetCode)
		  ...
		  'If Http.Status<>200 then 
		   ...
		   GetHttpPage = KS.BytesToBstr(Http.ResponseBody, CharsetCode)
		   Set Http = Nothing
		...
		Function PostHttpPage(RefererUrl, PostUrl, PostData)
			...
			If Err.Number <> 0 Then
				...
				Exit Function
			End If
			PostHttpPage = KS.BytesToBstr(xmlHttp.ResponseBody, "utf-8")
			Set xmlHttp = Nothing
		End Function
		...
		Sub WriteCollectSucced(ErrMsg,ChannelID)
			...
			strErr = strErr & "  <tr  class='tdbg' align='center'><td><input type='button' onclick='location.href=""Collect_Main.asp?ChannelID=" & ChannelID & """;parent.frames[""BottomFrame""].location.href=""../../Post.Asp?OpStr=信息采集管理 >> <font color=red>数据采集</font>&ButtonSymbol=DataCollect"";' value='返回采集中心' class='button'></td></tr>" & vbCrLf
